...
import { getDataSourceById, getDsPrivileges, getEditDataSource, getEditHiddenFields, getFlatFormElements } from '../../utils'
import {
...
const FeatureFormComponent = (props: FeatureFormComponentProps) => {
  ...
  const [activeRecords, setActiveRecords] = React.useState<FeatureDataRecord[]>(null)
  ...
  const intl = useIntl()
  const countLabel = activeRecords?.length > 1 ? ` (${intl.formatNumber(activeRecords.length)})` : ''
  ...
  const handleClickItem = React.useCallback((dsId: string, record: FeatureDataRecord) => {
    setActiveId(dsId)
    setActiveRecords([record])
  }, [])
  const handleBatchEdit = React.useCallback((dsId: string, records: FeatureDataRecord[]) => {
    setActiveId(dsId)
  ...
  const getLayerInfo = React.useCallback(async (dsId: string) => {
    try {
      ...
      if (layerConfig.layerHonorMode === LayerHonorModeType.Webmap) {
        ...
        const fieldElements = getFlatFormElements(layer.formTemplate?.elements).filter(e => e.type === 'field')
        const elements = constructFormElements(layerConfig.groupedFields.asMutable({ deep: true }), hiddenFields, fieldElements)
        const expressionInfos = constructExpressionInfos(layer.formTemplate?.expressionInfos)
  ...
  React.useEffect(() => {
    if (previousBatchEditing !== batchEditing && !batchEditing && activeRecords?.length > 1) {
      setActiveId(null)
      setActiveRecords(null)
    }
    ...
    for (const dataSourceId of dataSourceIds) {
      ...
      if (editRecords.length > 0 && editRecords[0].getFieldValue(displayField) === undefined && fieldNames.includes(displayField)) {
        try{
          ...
          if (idField) outFields.push(idField)
          const uniqueIds = dataSource.getUniqueIdFields()?.asMutable() || []
          if (uniqueIds.length > 0) {
            outFields.push(...uniqueIds)
          }
          const queryResult = await dataSource.query({ outFields, objectIds: ids })
  ...
  return (
    <Paper shape='none' className='jimu-widget widget-edit esri-widget' css={[widgetStyle, calciteColorMapping]}>
      ...
      {delConfirm && <FeatureFormConfirm
        ...
        message={activeRecords?.length > 1 ? translate('deleteRecordsTips') : translate('deleteRecordTips')}
      ...
      <EditListDataSource
        ...
        unsavedChange={formChange === FormChangeType.Normal && activeRecords?.length > 0}
