...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & ExtraProps, State> {
  ...
  componentDidMount () {
    ...
    const configEnableShort = this.props.config.enableShortLink ?? true
    this._cached.enableShortUrl = configEnableShort
    this.updateUrls({ enableShortUrl: configEnableShort }) // init sharedUrl/shortUrl, otherwise, it will be slower to open when using it.
  }
  ...
  componentDidUpdate (prevProps: AllWidgetProps<IMConfig>, prevState: State) {
    ...
    // admin config for short link changed: update cached state and re-fetch URL
    const prevEnableShortLink = prevProps.config.enableShortLink ?? true
    const currEnableShortLink = this.props.config.enableShortLink ?? true
    if (prevEnableShortLink !== currEnableShortLink) {
      this.updateUrls({ enableShortUrl: currEnableShortLink })
    }
      ...
      {(uiMode === UiMode.Popup) &&
        <div className={'popup-mode-content ' + (isHideMainContentInPopupMode ? 'd-none ' : '')}>
          <ShareLink
            ...
            enableShortLink={this.props.config.enableShortLink ?? true}
            isShortLinkEnabled={this._cached.enableShortUrl}
            isShowing={true}
          />
