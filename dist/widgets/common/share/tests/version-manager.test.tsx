...
describe('Test allow users to reorder the media list, #6473', () => {
  beforeAll(() => {
    ...
    // eslint-disable-next-line @typescript-eslint/prefer-find
    upgrader121 = versionManager.versions?.filter(function (version) {
      return version.version === '1.21.0'
    })[0]?.upgrader
  })
  ...
  it('5. v1.21.0 migration adds enableShortLink: true for existing configs', () => {
    const oldConfig = Immutable({
      uiMode: UiMode.Popup,
      popup: {
        icon: '',
      ...
      inline: {
        items: [{ id: ItemsName.Facebook, enable: true }],
        design: {
          direction: 'HORIZONTAL',
          hideLabel: false,
          btnRad: 0,
          ...
          iconColor: '',
          size: 'default',
          labelColor: 'var(--ref-palette-neutral-1200)'
        }
      }
      // no enableShortLink field (pre-1.21.0 saved config)
    })

    const newConfig = upgrader121(oldConfig)
    expect(newConfig.enableShortLink).toStrictEqual(true)
  })
})
