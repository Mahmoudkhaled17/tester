import {
  ...
  WIDGET_PREFIX_FOR_A11Y_SKIP, type IMUser, LayoutItemType, type BrowserSizeMode, ContainerType
} from 'jimu-core'
...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & Props, State> {
  ...
  updateNeedTabIndex = () => {
    ...
    let nextNeedTabIndex: boolean

    if (controllerWidgetId) {
      nextNeedTabIndex = true
    } else {
      ...
      const isInWindow = rootContainerInfo?.type === ContainerType.Dialog
      nextNeedTabIndex = isInWindow
    }

    this.setState({ needTabIndex: nextNeedTabIndex })
  }
  ...
  componentDidUpdate (preProps, preStates) {
    ...
    const { appMode, config, id, theme, user, browserSizeMode, controllerWidgetId } = this.props
    const { embedCode, embedType, enableBlankMessage, blankMessage, autoRefresh, autoInterval, expression, honorThemeFont } = config
    const { config: preConfig, appMode: preAppMode, sectionNavInfos: preSectionNavInfos, theme: preTheme, user: preUser } = preProps
    const {
    ...
    const browserSizeModeChange = preProps.browserSizeMode !== browserSizeMode
    const controllerWidgetIdChange = preProps.controllerWidgetId !== controllerWidgetId
    const codeLimitExceeded = this.props?.stateProps?.codeLimitExceeded || false
    const isCodeType = embedType === EmbedType.Code

    if (browserSizeModeChange || controllerWidgetIdChange) {
      this.updateNeedTabIndex()
    }

    this.setState({ codeLimitExceeded })

    // Situations where reload is required
    // user credential update(login widget)
    const appModeChange = appMode !== preAppMode && appMode === AppMode.Design
    const credentialChange = user !== preUser
    const needReload = appModeChange || autoConfChange || credentialChange
    if (needReload) {
      this.reload()
    }
    // embedType change
        ...
        {({ isInView, isInCurrentView }: ViewVisibilityContextProps) => {
          ...
          return <Fragment>
            {embedLoad ?
              ...
              </Paper> : <div className='jimu-widget widget-embed'></div>
            }
          </Fragment>
        }}
      </ViewVisibilityContext.Consumer>
