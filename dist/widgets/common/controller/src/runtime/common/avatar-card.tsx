import { React, css, type IconResult, classNames, polished, ReactRedux, type IMState } from 'jimu-core'
...
import { WidgetInController } from './widget-in-controller'
...
const useStyle = (options: UseStyleOptions) => {
  const { size, buttonSize, iconSize, showLabel, showIndicator, shape, labelGrowth, inController } = options
  ...
  return React.useMemo(() => {
    ...
    return css`
      ...
      .avatar {
        ...
        .avatar-button svg, .avatar-button img {
          width: ${iconSize}px;
          height: ${iconSize}px;
        }
        .marker {
    ...
    `
  }, [size, buttonSize, showLabel, shape, labelGrowth, isClassicTheme, iconSize, showIndicator, inController])
}
...
export const AvatarCard = React.forwardRef((props: AvatarCardProps, ref: React.RefObject<HTMLDivElement>) => {
  ...
  const inController = ReactRedux.useSelector((state: IMState) => {
    const widgetsJson = state.appConfig.widgets
    return widgetid ? widgetsJson?.[widgetid]?.inControllerUx === 'inController' : false
  })
  ...
  const cssStyle = useStyle({ size, buttonSize, iconSize, showLabel, showIndicator, shape, labelGrowth, inController })
  ...
  let widgetButton = showTooltip ? <Tooltip title={label} style={{ pointerEvents: 'none' }}>{avatarButton}</Tooltip> : avatarButton

  if (inController) {
    widgetButton = <WidgetInController widgetId={widgetid} showTooltip={showTooltip} />
  }
