import { React, WidgetState, type IMRuntimeInfos, Immutable, css, ReactRedux, type IMState } from 'jimu-core'
...
export const OffPanelWidgetsLuncher = (props: OffPanelWidgetsLuncherProps) => {
  ...
  return <React.Fragment>
    {Object.entries(widgets).map(([id, runtimeInfo]) => {
      ...
      const ariaLabel = widgetsJson[id]?.label
      const ariaDescribedBy = widgetsJson[id]?.a11yLabel
      const ariaDescriptionId = ariaDescribedBy ? `${id}-offscreen-desc` : undefined
      ...
      return <Popper
        ...
        placement={placement}>
        {ariaDescribedBy && <span className='sr-only' id={ariaDescriptionId}>{ariaDescribedBy}</span>}
