...
export default function WidgetsLauncher (props: WidgetsLauncherProps) {
  ...
  const handleClickWidget = React.useCallback((evt: React.MouseEvent<HTMLDivElement>, widgetId: string) => {
    evt.stopPropagation()
    const isKeyboardActivation = evt.detail === 0 || evt.nativeEvent?.detail === 0
  ...
  // Keep only one opened widget in single mode when widgetState changed externally
  const openingWidgets = widgetIds.filter((widgetId) => isWidgetOpening(widgets[widgetId]))
  const prevOpeningWidgets = hooks.usePrevious(openingWidgets)
  React.useEffect(() => {
    if (!config.behavior.onlyOpenOne || openingWidgets.length <= 1) return
    const newlyOpenedWidgets = openingWidgets.filter(widgetId => !prevOpeningWidgets || !prevOpeningWidgets.includes(widgetId))
    const newlyOpenedWidget = newlyOpenedWidgets[newlyOpenedWidgets.length - 1]
    if (!newlyOpenedWidget) return
    const widgetsToClose = openingWidgets.filter(widgetId => widgetId !== newlyOpenedWidget)
    if (widgetsToClose.length === 0) return
    getAppStore().dispatch(appActions.closeWidgets(widgetsToClose))
  }, [config.behavior.onlyOpenOne, openingWidgets, prevOpeningWidgets, widgets])
  ...
  const widgetIdsOffPanel = ReactRedux.useSelector<IMState, string[]>((state) => {
    ...
    return widgetIds.filter(widgetId => {
      if (mobile) {
        ...
        return widgetsJson[widgetId].inControllerUx === 'offPanel'
      }
    })
  }, ReactRedux.shallowEqual)

  const widgetIdsInController = ReactRedux.useSelector<IMState, string[]>((state) => {
    const widgetsJson = state.appConfig.widgets
    return widgetIds.filter(widgetId => widgetsJson[widgetId].inControllerUx === 'inController')
  }, ReactRedux.shallowEqual)

  const widgetsOffPanel = React.useMemo(() => {
    let widgetsObj = Immutable({}) as IMRuntimeInfos
    widgetIdsOffPanel.forEach(widgetId => {
      widgetsObj = widgetsObj.set(widgetId, widgets[widgetId])
    })
    return widgetsObj
  ...
  const widgetsInPanel = React.useMemo(() => {
    ...
    Object.keys(widgets).forEach(widgetId => {
      if (!widgetIdsOffPanel.includes(widgetId) && !widgetIdsInController.includes(widgetId)) {
        widgetsObj = widgetsObj.set(widgetId, widgets[widgetId])
      }
    })
    return widgetsObj
  }, [widgetIdsInController, widgetIdsOffPanel, widgets])
