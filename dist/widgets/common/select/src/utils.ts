...
import type {
  ...
  JimuKnowledgeGraphSublayerView
} from 'jimu-arcgis'
import { type DataSourceItem, type IMConfig, type IMInteractiveTools, InteractiveToolType } from './config'

export type SelectableJimuLayerView = JimuFeatureLayerView | JimuSceneLayerView | JimuBuildingComponentSublayerView | JimuImageryLayerView | JimuOrientedImageryLayerView | JimuSubtypeGroupLayerView | JimuSubtypeSublayerView | JimuKnowledgeGraphSublayerView
...
export function getDefaultInteractiveTool (interactiveToolsConfig: IMInteractiveTools): InteractiveToolType {
  if (interactiveToolsConfig) {
    const tools = interactiveToolsConfig.tools

    if (tools?.length > 0) {
      const defaultTool = interactiveToolsConfig.defaultTool

      if (defaultTool && tools.includes(defaultTool)) {
        return defaultTool
      } else {
        const mutableTools = tools.asMutable({ deep: true })
        const sortedTools = sortInteractiveToolTypes(mutableTools)
...
function sortInteractiveToolTypes (toolTypes: InteractiveToolType[]): InteractiveToolType[] {
  toolTypes = toolTypes?.slice() || []

  const selectToolItemInfoMapping = {
  ...
  toolTypes.sort((toolType1, toolType2) => {
