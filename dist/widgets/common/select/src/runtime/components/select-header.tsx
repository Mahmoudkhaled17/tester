...
import { getFinalEnableAttributeSelectionForMap, getDefaultInteractiveTool } from '../../utils'
...
export default function SelectHeader (props: SelectHeaderProps) {
  ...
  const isReadyToCreateSketchViewModelRef = React.useRef<boolean>(isReadyToCreateSketchViewModel)
  isReadyToCreateSketchViewModelRef.current = isReadyToCreateSketchViewModel
  ...
  // default selected tool type
  const defaultSelectedToolType = React.useMemo((): AllSelectToolType => {
    let result: AllSelectToolType = null

    // interactiveTools is only valid when useMap is true
    if (useMap && interactiveTools) {
      result = getDefaultInteractiveTool(interactiveTools)
    }

    if (!result) {
      // goes here if select widget doesn't connect to a map widget, the result maybe 'data' if isLocationEnabled is true
      result = allSelectToolTypes[0]
    }

    return result
  }, [useMap, interactiveTools, allSelectToolTypes])

  // selectedToolType maybe undefined
  const [selectedToolType, setSelectedToolType] = React.useState<AllSelectToolType>(defaultSelectedToolType)
  ...
  // auto activate selected tool button if activateByDefault is true
  const activateByDefaultRef = React.useRef<boolean>(false)
  const activateByDefault = !!(interactiveTools?.activateByDefault)
  activateByDefaultRef.current = activateByDefault

  // activate selected tool button if activateByDefault is true when isReadyToCreateSketchViewModel becomes true or widgetState changes from Closed to Opened
  // deactivate selected tool button when widgetState changed to Closed (close by widget controller)
  hooks.useEffectWithPreviousValues((preValues) => {
    const [preWidgetState, preIsReadyToCreateSketchViewModel] = preValues || []
    const isWidgetStateChanged = widgetState !== preWidgetState
    const isReadyToCreateSketchViewModelChanged = isReadyToCreateSketchViewModel !== preIsReadyToCreateSketchViewModel
    const currSelectedTool = selectedToolTypeRef.current

    // WidgetState.Opened and WidgetState.Closed are two states changed by widget controller.
    // WidgetState.Hidden is changed by anchored window, sidebar, etc.
    if (currSelectedTool && currSelectedTool !== SELECT_BY_DATA && (isWidgetStateChanged || isReadyToCreateSketchViewModelChanged)) {
      let shouldTryAutoActivate = false

      if (isWidgetStateChanged) {
        if (widgetState === WidgetState.Closed || widgetState === WidgetState.Hidden) {
          if (isSelectedBtnActiveRef.current) {
            setSelectedBtnActive(false)
            return
          }
        } else if (widgetState === WidgetState.Opened && preWidgetState === WidgetState.Closed) {
          // opened from closed by widget controller
          shouldTryAutoActivate = true
        } else if (!widgetState && preWidgetState === WidgetState.Hidden) {
          // opened from hidden by anchored window, sidebar, etc.
          shouldTryAutoActivate = true
        }
      } else if (isReadyToCreateSketchViewModelChanged) {
        if (isReadyToCreateSketchViewModel) {
          // ready to create sketchViewModel
          shouldTryAutoActivate = true
        }
      }

      if (shouldTryAutoActivate &&
         widgetState !== WidgetState.Closed &&
         widgetState !== WidgetState.Hidden &&
         isReadyToCreateSketchViewModel &&
         activateByDefaultRef.current &&
         !isSelectedBtnActiveRef.current
      ) {
        // activate the selected tool button by default
        setSelectedBtnActive(true)
      }
    }
  }, [widgetState, isReadyToCreateSketchViewModel]) // make sure widgetState and isReadyToCreateSketchViewModel are the only dependency

  // activate/deactivate selected tool button when activateByDefault changed (user changes the config in Select setting)
  React.useEffect(() => {
    if (isReadyToCreateSketchViewModelRef.current && selectedToolTypeRef.current && selectedToolTypeRef.current !== SELECT_BY_DATA) {
      if (activateByDefault) {
        ...
        if (isSelectedBtnActiveRef.current) {
          // deactivate the selected tool button
          setSelectedBtnActive(false)
        }
      }
    }
  }, [activateByDefault]) // make sure activateByDefault is the only dependency
  ...
  // set selectedToolType to defaultSelectedToolType when defaultSelectedToolType changed
  React.useEffect(() => {
    setSelectedToolType(defaultSelectedToolType)
    setSelectedBtnActive(false)
  }, [defaultSelectedToolType])
