...
import {
  ...
  SqlExpressionMode, moduleLoader, focusElementInKeyboardMode, isKeyboardMode
} from 'jimu-core'
...
export default class FilterItem extends React.PureComponent<Props, State> {
  pillButton: any
  virtualNodeAfterApplyButton: any
  lastVirtualNode: any
  ...
  onApplyButtonsClick = (checked) => {
    if (isKeyboardMode()) {
      const nextButton = checked ? this.virtualNodeAfterApplyButton : this.lastVirtualNode
      setTimeout(() => {
        nextButton.focus()
      }, 200)
    }
    ...
    return (
      <div className={'w-100 d-flex justify-content-end apply-cancel-group'}>
        ...
        <Button
          ...
          onClick={() => { this.onApplyButtonsClick(true) }}
        >
          {this.formatMessage('apply')}
        </Button>
        <div className='sr-only' tabIndex={-1} ref={ref => { this.virtualNodeAfterApplyButton = ref }}></div>
        <Button
          type='default' className='filter-cancel-button ml-2'
          disabled={this.isDataSourceLoadingOrInvalid() || !this.state.applied}
          onClick={() => { this.onApplyButtonsClick(false) }}
          >
  ...
  render () {
    ...
    const itemLabel = (id === 0 && a11yLabel) ? `${a11yLabel}. ${name}` : name
    return (
      <div
                                  ...
                                  <div className={classNames('filter-item filter-item-popper', { 'filter-item-custom': isCustomFilter })}>
                                    <Paper variant='outlined' shape='shape2' className='filter-item-inline'>
                                      {this.getFilterItem(hasEndUserClausesNum, arrangeType !== FilterArrangeType.Popper)}
                                      {
                                        ...
                                        triggerType === FilterTriggerType.Button && !omitInternalStyle && <Button className='sr-only' aria-label={this.formatMessage('pressTabToContinue')} ref={ref => { this.lastVirtualNode = ref }} />
                                      }
                                    </Paper>
                                  </div>
                                </div>
