...
import { getGroupSQLExpressionWithoutValues, getShownClauseNumberByExpression, getSQLExpressionWithoutValues, getTotalClauseNumberByExpression } from 'jimu-ui/basic/sql-expression-runtime'
...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & FilterURLParamsProps, State> {
  ...
  prevQueries: { [dsId: string]: any }
  clearedDsIds: string[]
  clearedDsIdsTimer: ReturnType<typeof setTimeout> | null
  ...
  componentWillUnmount () {
    this.__unmount = true
    if (this.clearedDsIdsTimer) {
      clearTimeout(this.clearedDsIdsTimer)
      this.clearedDsIdsTimer = null
    }
    ...
    if (dsIds.length > 0) {
      this.publishFilterMessage(dsIds, dsIds)
    }
  }
    ...
    Object.keys(dss).forEach(dsId => {
      ...
      if (ds) { // exclude invalid dataSources
        ...
        if (prevSqlForDs !== sqlResult.sql) {
          dsIds.push(ds.id)
          if (!sqlResult.sql) {
            clearFilterDsIds.push(ds.id)
          }
        }
      }
    })
    if (dsIds.length > 0) {
      this.publishFilterMessage(dsIds, clearFilterDsIds)
    }
  }
  ...
  publishFilterMessage = (dataSourceIds: string[], clearFilterDsIds: string[]) => {
    MessageManager.getInstance().publishMessage(new DataSourceFilterChangeMessage(this.props.id, dataSourceIds, clearFilterDsIds))
  }
  ...
  onIsDataSourceInfochanged = (dataSourceId: string, _dataSourceStatus) => {
    const dataSource = this.state.dataSources[dataSourceId]
    if (!dataSource) {
      return
    }

    const prevQuery = this.prevQueries[dataSourceId]
    const isCleared = dataSourceUtils.isDataSourceFilterJustCleared(dataSourceId, this.props.id, prevQuery)

    if (isCleared) {
      this.cacheClearedDsIds(dataSourceId)
      delete this.prevQueries[dataSourceId]
    } else {
      this.prevQueries[dataSourceId] = dataSource.getInfo()?.widgetQueries?.[this.props.id]
    }
  ...
  cacheClearedDsIds = (dataSourceId: string) => {
    if (!this.clearedDsIds.includes(dataSourceId)) {
      this.clearedDsIds.push(dataSourceId)
    }
    ...
    this.clearedDsIdsTimer = setTimeout(() => {
      this.clearedDsIdsTimer = null

      if (this.clearedDsIds.length > 0) {
        this.clearFilterItems(this.clearedDsIds)
        this.publishFilterMessage(this.clearedDsIds, this.clearedDsIds)
        this.clearedDsIds = []
      }
    }, 200)
  }
  ...
  /**
   * check all filter items for UI styles when ds ids are included
   * with activation, cancel apply state
   * without activation, clear selected value for all clauses including input and select styles.
   */
  clearFilterItems = (dataSourceIds: string[]) => {
    const { omitInternalStyle } = this.props.config

    const updatedItems = this.state.filterItems.map((item, index) => {
      const hit = item.useDataSources?.find(useDs => dataSourceIds.includes(useDs.dataSourceId) && (useDs as any).checked !== false) // checked ds is the current one for custom item
      if (!hit) {
        return item
      }

      const sqlOptions: any = { autoApplyWhenWidgetOpen: false }

      // clear all values for sqlExprObj/sqlExprObjForGroup when omit style & configured clauses
      if (omitInternalStyle && item.type !== FilterItemType.Custom) {
        if (item.type === FilterItemType.Single) {
          // only 1 clause configured, and it's visible for endUser.
          const singleShownClause = getTotalClauseNumberByExpression(item.sqlExprObj) === 1 && getShownClauseNumberByExpression(item.sqlExprObj) === 1
          sqlOptions.sqlExprObj = singleShownClause ? getSQLExpressionWithoutValues(item.sqlExprObj) : item.sqlExprObj
        } else if (item.sqlExprObjForGroup) { // group & has sqlExprObjForGroup
          sqlOptions.sqlExprObjForGroup = getGroupSQLExpressionWithoutValues(item.sqlExprObjForGroup)
        }
      }
      return {
        ...item,
    ...
    this.setState({
      filterItems: updatedItems as any
    })
  }
  ...
  render () {
    ...
    const isOffPanel = this.props.controllerWidgetId && this.props.inControllerUx === 'offPanel'
          ...
          this.props.useDataSources?.map(useDs => {
            return (
              <FilterItemDataSource
                ...
                onIsDataSourceInfochanged={this.onIsDataSourceInfochanged}
                ...
                >
                  <div className='p-2' style={{ width: FILTER_PANEL_WIDTH }}>
                    {this.getFilterItems(config, FilterArrangeType.Block, false, true)}
                    {
                      ...
                      !config.resetAll && config.triggerType === FilterTriggerType.Button && !config.omitInternalStyle &&
