  ...
  return <div className="widget-setting-navigator jimu-widget-setting" css={style}>
    <SettingSection>
      ...
      {section && <React.Fragment>
        ...
        {isNavType && <SettingRow label={translate('space')} aria-label={translate('space')} flow="no-wrap">
          <InputUnit className="w-50" min={0} value={gap} aria-label={translate('space')} onChange={(value) => { onSettingConfigChange(['display', 'standard', 'gap'], `${value.distance}${value.unit}`) }} />
        </SettingRow>}
    ...
    {section && <SettingSection>
      ...
      {advanced && <React.Fragment>
        {isTabStyle && <CollapsablePanel label={translate('navArrowColor')} aria-label={translate('navArrowColor')} defaultIsOpen={false} className='my-4 nav-arrow-color-collapse' wrapperClassName='pl-2'>
