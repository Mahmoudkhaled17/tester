...
const Widget = (props: NavigatorProps) => {
  ...
  const defaultView = useSelector((state: IMState) => {
    const sectionJson = state?.appConfig?.sections?.[section]
    return sectionJson?.defaultView || sectionJson?.views?.[0]
  })
  ...
  return <div className="widget-view-navigation jimu-widget" css={style} ref={nodeRef}>
    ...
    <ViewNavigation
      ...
      standard={standard}
      theme={theme}
      a11yLandmark={a11yLandmark}
      addToA11ySkip={addToA11ySkip}
      ariaLabel={ariaLabel}
      widgetId={id}
    />
  </div>
