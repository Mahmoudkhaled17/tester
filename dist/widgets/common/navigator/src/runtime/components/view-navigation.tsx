/** @jsx jsx */
import { React, classNames, jsx, css, type ThemeSliderVariant, type ImmutableObject, lodash, type ImmutableArray, hooks, type IMThemeVariables, WIDGET_PREFIX_FOR_A11Y_SKIP } from 'jimu-core'
import {
...
export const ViewNavigation = (props: ViewNavigationProps) => {
  ...
  const a11yProps = React.useMemo(() => {
    const props = {
      role: role || (type === 'nav' ? 'tablist' : undefined),
      'aria-label': ariaLabel
    }
    if (addToA11ySkip) {
      const id = `${WIDGET_PREFIX_FOR_A11Y_SKIP}${widgetId}`
      return type === 'slider' ? { ...props, id } : { ...props, id, tabIndex: -1 }
    }
    return props
  }, [addToA11ySkip, role, type, ariaLabel, widgetId])
  ...
  return <div className={classNames('navigation', className)} css={[style, advanceStyle, containerPaddingStyle, oldThemeStyle]} {...others}>
    {type === 'nav' && <Navigation
      ...
      {...a11yProps}
      textAlign={textAlign} />}
    {type === 'slider' && <Slider className="h-100" value={progress * MAGNIFICATION} hideThumb={hideThumb} onChange={handleSliderChange} formatter={sliderFormatter} {...a11yProps}></Slider>}
    {type === 'navButtonGroup' &&
      <NavButtonGroup
