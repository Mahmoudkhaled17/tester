import { Immutable, type ImmutableArray, type UseDataSource, dataSourceUtils, type IMArcadeContentConfigMap, arcadeContentUtils, getAppStore, appActions } from 'jimu-core'
...
export const getMainDataSourceIds = (useDataSources: ImmutableArray<UseDataSource> = Immutable([])): ImmutableArray<string> => {
  return useDataSources.map(ds => {
    ...
    return ds.mainDataSourceId || dataSourceUtils.getMainDataSourceId(ds.dataSourceId)
  }).filter(ds => ds != null)
}
...
export const getUseDataSourceIds = (html: string): string[] => {
  ...
  while ((matches = regexp.exec(html)) !== null) {
    let ids = matches[1]
    if (ids.indexOf(',') > 0) {
      ids = ids.split(',').filter(id => id.trim() !== '')
