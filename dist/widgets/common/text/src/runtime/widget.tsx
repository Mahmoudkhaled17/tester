...
const Widget = (props: AllWidgetProps<IMConfig>): React.ReactElement => {
  ...
  const onEditorComplete = (value: string, placeholder: string): void => {
    ...
    if (configTranslationUtils.isBuilderConfiguringTranslations()) return
  ...
  // In builder mode, highlight expression chips with invalid data sources in the displayer.
  const invalidDataSourceIds = React.useMemo(() => {
    if (!isInBuilder || !builderUtils) return null
    const dsIds = builderUtils.getInvalidDataSourceIds(text, useDataSources)
    return dsIds?.length ? dsIds : null
  }, [isInBuilder, builderUtils, text, useDataSources])
