import { React, polished, type IMExpression, ExpressionResolverComponent, expressionUtils, DynamicStyleResolverComponent, type IMDynamicStyleConfig, type IMDynamicStyle, jsx } from 'jimu-core'
import { DownDoubleOutlined } from 'jimu-icons/outlined/directional/down-double'
import { styled, useTheme, type CSSObject } from 'jimu-theme'
...
export type DisplayerProps = Omit<RichTextDisplayerProps, 'sanitize'> & {
  ...
  invalidDataSourceIds?: readonly string[]
}
...
const useInvalidStyle = (invalidDataSourceIds?: readonly string[]): CSSObject | undefined => {
  return React.useMemo(() => {
    if (!invalidDataSourceIds?.length) return undefined
    const styles: CSSObject = {}
    invalidDataSourceIds.forEach(dsId => {
      styles[`exp[data-dsid*="${dsId}"]`] = InvalidStyle
      styles[`arcade[data-dsid*="${dsId}"]`] = InvalidStyle
    })
    return styles
  }, [invalidDataSourceIds])
}
...
export function Displayer (props: DisplayerProps): React.ReactElement<any> {
  ...
  const invalidStyle = useInvalidStyle(invalidDataSourceIds)
  ...
  return (
    <Root styleState={{ wrap, fadeLength }} title={tooltipText} onMouseEnter={handleEnter} onMouseLeave={delayLeave} ref={rootRef} css={invalidStyle} {...others}>
