  ...
  return {
    ...
    DataSourceManager: {
      getInstance: jest.fn().mockReturnValue({
        getDataSource: (dsid) => {
          if (dsid === 'ds_without_instance') return null
          return {
            id: dsid
          }
        }
...
describe('utils test', function () {
  ...
  it('does not treat configured data sources as invalid before the data source instance is created', () => {
    const html = '<p><exp data-uniqueid="e3c" data-dsid="ds_without_instance" data-expression="{name: value}">{Rank}</exp></p>'
    const useDataSources: any = Immutable([{
      dataSourceId: 'ds_without_instance',
      mainDataSourceId: 'ds_without_instance',
      fields: ['Rank']
    }])

    expect(builderUtils.getInvalidDataSourceIds(html, useDataSources)).toEqual([])
  })
