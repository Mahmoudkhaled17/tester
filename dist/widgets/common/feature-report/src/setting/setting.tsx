...
import {
  React, Immutable, css, jsx, type UseDataSource, DataSourceComponent, type DataSource, type FeatureLayerDataSource,
  DataSourceManager, portalUrlUtils, CONSTANTS, type IMDataSourceJson, SupportedServerTypes, dataSourceUtils, ServiceManager
} from 'jimu-core'
...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<IMConfig>, any> {
  ...
  onDataSourceCreated = async (createdDataSource: DataSource): Promise<void> => {
    const dataSource = createdDataSource as unknown as FeatureLayerDataSource
    ...
    if (!portalUrlUtils.isAGOLDomain(this.props.portalUrl)) {
      const isHostedService = await (ServiceManager.getInstance()?.isHostedService(fsUrl) ?? Promise.resolve(false)).catch(() => false)
      if (!isHostedService) {
  ...
  onDataSourceChange = (useDataSources: UseDataSource[]) => {
    ...
    // cache the current report name setting
    this.setCache({
  ...
  getDefaultReportName = (surveyItemInfo, serviceItemInfo, dataSource) => {
    // feature report didn't support the layers with uniqueIds enabled.
  ...
  removeTemplate = (e: any, deleteTemplateId: string, deleteFromAllList?: boolean) => {
    ...
    if (deleteFromAllList) {
      ...
      if (idx >= 0) {
        ...
        this.setCache({templateItems: newItems})
      }
    }
  }
