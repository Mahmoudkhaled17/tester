import { DataSourceManager, DataSourceStatus, loadArcGISJSAPIModules, dataSourceUtils, MessageManager, getAppStore, DataRecordSetChangeMessage, RecordSetChangeType, urlUtils, utils, Immutable, UrlManager } from 'jimu-core'
import type { DataSource, DataSourceConstructorOptions, DataSourceJson, ImmutableArray, QueriableDataSource } from 'jimu-core'
import {
...
import { geometryUtils } from 'jimu-arcgis'
// only used as type
...
export interface UpdateQueryParamsOption {
  ...
  originalServiceList?: ServiceList
  searchText: string
...
export const getServiceItemDsId = (serviceItem): string => {
  if (!serviceItem) return undefined
  if (serviceItem?.searchServiceType === SearchServiceType.FeatureService) {
    return serviceItem?.useDataSource?.dataSourceId
  }
  return serviceItem?.outputDataSourceId
}
...
export const clearFilterOfDeletedDs = (serviceList: IMDatasourceListItem, widgetId: string, configId: string, enableFiltering: boolean) => {
  ...
  const localId = getLocalIdWithDsId({
    configId: configId,
    widgetId: widgetId,
    dsId: useDataSourceId,
    serviceList: serviceList?.asMutable({ deep: true }) as any,
...
interface LoadAllDsRecordOption {
  resultServiceList: ServiceList
  originalServiceList?: ServiceList
  resultMaxNumber: number
  id: string
  enableFiltering?: boolean
  isPublishRecordCreateAction?: boolean
}

export const loadAllDsRecord = (option: LoadAllDsRecordOption): Promise<RecordResultType[]> => {
  const { resultServiceList, resultMaxNumber, id, enableFiltering, originalServiceList, isPublishRecordCreateAction } = option
  const suggestionPromiseList = []
  for (const configId in resultServiceList) {
    const serviceItem = resultServiceList[configId]
    let suggestionPromise
    if (serviceItem.searchServiceType === SearchServiceType.FeatureService) {
      const localId = getLocalIdWithDsId({
      configId: configId,
      widgetId: id,
      dsId: serviceItem?.useDataSource?.dataSourceId,
      serviceList: originalServiceList,
      enableFiltering: enableFiltering
    })
      suggestionPromise = loadDsRecords(serviceItem, resultMaxNumber, localId)
    } else {
...
export function updateAllLayerServiceDsQueryParams (option: UpdateQueryParamsOption) {
  const { serviceList, searchText, id, originalServiceList, selectedSuggestionItemId } = option
  ...
  for (const configId in serviceList) {
    ...
    if (searchServiceType === SearchServiceType.FeatureService && useDataSource?.dataSourceId) {
      ...
      const isDsUsedOnlyOnce = checkIsDsUsedOnlyOnce(dsId, originalServiceList)
      if (!isDsUsedOnlyOnce) {
...
export function checkIsDsUsedOnlyOnce (dsId: string, serviceList: ServiceList): boolean {
  if (!dsId || !serviceList) return false
  const dsIdList = Object.keys(serviceList).map(configId => {
    const serviceItem = serviceList[configId]
    const { searchServiceType, useDataSource } = serviceItem
    if (searchServiceType === SearchServiceType.FeatureService && useDataSource?.dataSourceId) {
      return useDataSource?.dataSourceId
    } else {
      return null
    }
  })
  return dsIdList.filter(id => id === dsId)?.length === 1
}
...
export const publishRecordCreatedMessageAction = (options: PublishRecordCreatedMessageActionOptions) => {
  const { widgetId, recordsArr, recordSetChangeType = RecordSetChangeType.CreateUpdate, zoomScale, Graphic, scaleExtent } = options
  let extent = options.extent as __esri.Extent | ((jimuMapView: JimuMapView) => Promise<__esri.Extent | __esri.Graphic>)
  ...
  if (extent) {
    if (zoomScale && scaleExtent) {
      extent = (jimuMapView) => {
        const graphic = getGraphicForMessageAction({
          pointExtent: options?.extent,
          jimuMapView: jimuMapView,
          GraphicClass: Graphic,
          zoomScale: zoomScale,
          scaleExtentFn: scaleExtent
        })
...
interface GraphicForMessageItemOption {
  pointExtent: __esri.Extent,
  jimuMapView: JimuMapView,
  GraphicClass: typeof __esri.Graphic,
  scaleExtentFn: (extent: __esri.Extent, view?: any, scale?: number) => __esri.Extent,
  zoomScale: number
}

export async function getGraphicForMessageAction (options: GraphicForMessageItemOption) {
  const { pointExtent, jimuMapView, GraphicClass, scaleExtentFn, zoomScale } = options
  const projectedGeometries = await geometryUtils.projectToSpatialReference([pointExtent], jimuMapView?.view?.spatialReference)
  const newGeometry = projectedGeometries?.[0]
  const newExtent = scaleExtentFn(newGeometry.extent, jimuMapView?.view, zoomScale)
  const graphic = new GraphicClass({ geometry: newExtent })
  return graphic
}
...
interface GetLocalIdWithDsIdOptions {
  configId: string
  widgetId: string
  dsId: string
  serviceList: ServiceList
...
export const getLocalIdWithDsId = (option: GetLocalIdWithDsIdOptions): string => {
  //https://devtopia.esri.com/Beijing-R-D-Center/ExperienceBuilder-Web-Extensions/issues/27248#issuecomment-5900782
  //If a layer is added only once, our DataSourceComponent directly uses the main ds.
  //If the same layer is added multiple times, our DataSourceComponent uses the local ds and updates the main ds's filters usingds.updateQueryParams.
  const { configId, widgetId, dsId, serviceList, enableFiltering, selectedSuggestionItemId } = option
  const isDsUsedOnlyOnce = checkIsDsUsedOnlyOnce(dsId, serviceList)
  const useSelectedSuggestion = !!selectedSuggestionItemId
  const notSelectedSuggestionItem = useSelectedSuggestion && selectedSuggestionItemId !== configId
  const useDataSourceComponentToFilterDs = isDsUsedOnlyOnce && enableFiltering && !notSelectedSuggestionItem
  if (!enableFiltering || !useDataSourceComponentToFilterDs || notSelectedSuggestionItem) {
    return getLocalId(configId, widgetId)
  } else {
    return null
  }
}
...
export async function loadEsriModulesForZoomTo () {
  return loadArcGISJSAPIModules(['esri/geometry/support/geometryUtils', 'esri/Graphic'])
}

//Check whether all records are loaded after update ds query params
export const checkIsAllRecordLoaded = (serviceList: ServiceList, id: string, enableFiltering: boolean) => {
...
export const checkIsAllDsCreated = (serviceList: ServiceList, id: string, enableFiltering: boolean) => {
