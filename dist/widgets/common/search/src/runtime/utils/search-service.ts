...
import { getDatasource, checkIsDsCreated } from './utils'
...
export function getQueryByServiceListItem (options: GetQueryOptions): QueryParams {
  ...
  const { useDataSource, searchText } = serviceListItem
  const clearFilter = (!searchTextForConfirmSearch || !searchText) && enableFiltering
  const SQL = serviceListItem?.SQL
...
*/
export const loadDsRecords = (serviceListItem: DatasourceListItem, resultMaxNumber: number, localId?: string): Promise<RecordResultType> => {
  ...
  return Promise.resolve({
    ...
    dsId: dsId,
    localDsId: localDsId,
    layerDsIdActuallyUsed: localDsId || dsId,
