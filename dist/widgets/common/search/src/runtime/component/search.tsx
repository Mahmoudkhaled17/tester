...
import { useConfigIdsWithClearDsFilter } from '../utils/search-hooks'
import {
...
interface SearchSettingProps {
  ...
  handleServiceListChange: (serviceList: IMServiceList, selectedSuggestionItemId?: string) => void
...
const SearchInput = (props: SearchSettingProps) => {
  ...
  const isFocusSearchInputRef = useRef(false)
  ...
  const currentServiceList = serviceListRef.current || serviceList || {}
  const searchConfigIdsWithClearedDsFilter = useConfigIdsWithClearDsFilter({
    currentServiceList,
    dataSourcesInfo,
    widgetId: id,
    enableFiltering: !!config?.enableFiltering
  })
  const prevSearchConfigIdsWithClearedDsFilterRef = useRef<Set<string>>(null)
  ...
  // After "Clear data filters" message action runs, re-initialize search result services
  // by removing items whose ds widget query has been cleared.
  useEffect(() => {
    if (prevSearchConfigIdsWithClearedDsFilterRef.current === searchConfigIdsWithClearedDsFilter) {
      return
    }
    prevSearchConfigIdsWithClearedDsFilterRef.current = searchConfigIdsWithClearedDsFilter

    if (!config?.enableFiltering || !searchConfigIdsWithClearedDsFilter || searchConfigIdsWithClearedDsFilter.size === 0) {
      return
    }

    const currentResultServiceList = resultServiceListRef.current || resultServiceList || {}
    const nextResultServiceList = { ...currentResultServiceList }
    let nextServiceList = newServiceListRef.current || serviceListRef.current || serviceList
    searchConfigIdsWithClearedDsFilter.forEach(configId => {
      delete nextResultServiceList[configId]
      nextServiceList = nextServiceList
        .setIn([configId, 'SQL', 'sql'], '1=1')
        .setIn([configId, 'SuggestionSQL', 'sql'], '1=1')
    })
    resultServiceListRef.current = nextResultServiceList
    setResultServiceList(nextResultServiceList)
    serviceListRef.current = nextServiceList
    newServiceListRef.current = nextServiceList
    handleServiceListChange(nextServiceList)
  }, [config?.enableFiltering, resultServiceList, serviceList, handleServiceListChange, searchConfigIdsWithClearedDsFilter])
  ...
  const confirmSearchBySearchTextInUrlHashObject = hooks.useEventCallback((serviceList) => {
    ...
    confirmSearchByInUrlSearchTextTimeout.current = setTimeout(() => {
      if (checkIsAllDsCreated(serviceList?.asMutable({ deep: true }), id, config?.enableFiltering) && getCanUseDsLength() > 0) {
        updateSearchValue(searchText, status)
        confirmSearch(searchText, false, status)
      } else {
        confirmSearchBySearchTextInUrlHashObject(serviceList)
  ...
  */
  const onSearchInputFocus = (e) => {
    isFocusSearchInputRef.current = true
  ...
  */
  const confirmSearch = hooks.useEventCallback((searchText: string, isClearSearch: boolean = false, initResultServiceListOption?: InitResultServiceListOption) => {
    ...
    setIsOpenSuggestion(false)
    toggleLocationOrRecentSearches(!searchText)
    clearSelectRecordAndAction()

    handleSearchTextForConfirmSearchChange(searchText)

    const selectedSuggestionItemId = initResultServiceListOption?.isFromSuggestion ? Object.keys(resultServiceListRef.current)?.[0] : null
    handleServiceListChange(newServiceListRef.current, selectedSuggestionItemId)
    ...
    const updateParamsOption = {
      ...
      originalServiceList: newServiceListRef.current?.asMutable({ deep: true }),
    ...
    if (config?.enableFiltering) {
      ...
      publishDataFilterAction(searchText)
    }
  ...
  */
  ...
  const publishDataFilterAction = React.useCallback((searchText: string) => {
  ...
  */
  const loadRecordAndCheckIsToOtherWidget = () => {
    if (!checkIsAllRecordLoaded(resultServiceListRef.current, id, config?.enableFiltering)) {
    ...
    const serviceRecords = loadAllDsRecord({
      resultServiceList: resultServiceListRef.current,
      originalServiceList: newServiceListRef.current?.asMutable({ deep: true }),
  ...
  */
  const publishRecordCreateAction = async () => {
    ...
    const resultServiceListForGeocode = {}
    Object.keys(resultServiceListRef.current).forEach(key => {
      if (resultServiceListRef.current[key]?.searchServiceType === SearchServiceType.GeocodeService) {
        resultServiceListForGeocode[key] = resultServiceListRef.current[key]
      }
    })
    const geocodeRecords = loadAllDsRecord({
      resultServiceList: resultServiceListForGeocode,
      originalServiceList: newServiceListRef.current?.asMutable({ deep: true }),
      resultMaxNumber: maxRecordNumber,
      id: id,
      enableFiltering: config?.enableFiltering,
      isPublishRecordCreateAction: true
    })
    if (!esriGeometryUtilsRef.current || !GraphicClassRef.current) {
      const esriModules = await loadEsriModulesForZoomTo()
      esriGeometryUtilsRef.current = esriModules[0]
      GraphicClassRef.current = esriModules[1]
    }
  ...
  */
  const setQuerySQL = hooks.useEventCallback((searchText: string, initResultServiceListOption?: InitResultServiceListOption) => {
    ...
    const { configId: id, magicKey, isFromSuggestion } = initResultServiceListOption || {}
    const isOnlyFilterSuggestionItem = !!(magicKey || isFromSuggestion)
    ...
    for (const configId in currentServiceList) {
      if (isOnlyFilterSuggestionItem && configId !== id) {
        continue
      }
      ...
      const currentSearchText = trimmedSearchText
      ...
      const SQL = getSQL(currentSearchText, searchFields, ds, searchExact)
      const SuggestionSQL = getSQL(currentSearchText, searchFields, ds, false)
      newServiceList = newServiceList
        .setIn([configId, 'SQL'], SQL)
        .setIn([configId, 'SuggestionSQL'], SuggestionSQL)
        .setIn([configId, 'searchText'], currentSearchText)
      if (searchExtent) {
  ...
  */
  ...
  const renderLoading = (isShowLoading: boolean, locationLoading: boolean) => {
    ...
    return <div className={classNames('position-absolute loading-con', { 'hide-loading': !showLoading })}>
      <Loading useAriaLive className='w-100' type={LoadingType.Bar} />
    </div>
  }
  ...
  */
  ...
  const handleInputBlur = (e) => {
    isFocusSearchInputRef.current = false
    inputBlurTimeoutRef.current = setTimeout(() => {
      if (!isFocusSuggestion.current) {
        setIsOpenSuggestion(false)
      }
      if (!locationLoadingRef.current && !isFocusLocationAndRecentSearch.current && !isFocusSearchInputRef.current) {
  ...
  return (
    <div className={`h-100 align-items-center position-relative d-flex flex-grow-1 ${className || ''} arrangement-${config.arrangementStyle}`} css={STYLE}>
      <div className='h-100 flex-grow-1 search-input-con'>
        ...
        />}
        <ResultList
          isOpenResultPopper={isOpenResultPopper && !!searchValue}
          isToOtherWidget={isToOtherWidget}
          serviceList={Immutable(resultServiceList)}
          originalServiceList={Immutable(serviceList)}
          searchConfigIdsWithClearedDsFilter={searchConfigIdsWithClearedDsFilter}
          config={config}
          dsStatus={dsStatus}
          reference={reference}
          searchText={searchValue}
          ...
          clearSearch={() => { clearSearchValue(false) }}
        />
      </div>
