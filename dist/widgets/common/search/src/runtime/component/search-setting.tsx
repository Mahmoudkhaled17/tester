...
const SearchSetting = (props: SearchSettingProps) => {
  ...
  const renderFieldListElement = () => {
    return datasourceConfig?.map((configDsItem, index) => {
      ...
      return <DropdownItem role='menuitemcheckbox' active={configDsItem?.enable} tag='div' tabIndex={0} toggle={false} key={`${configDsItem?.label}${index}`} title={configDsItem?.label} disabled={disabled} onClick={() => { onDsConfigItemChange(!configDsItem?.enable, index) }} >
  ...
  const hasSelectedService = useMemo(() => {
    return datasourceConfig?.some(configDsItem => configDsItem?.enable) ?? false
  }, [datasourceConfig])

  return (
    <div className={`${className || ''} ds-setting-${config.arrangementStyle}`} onKeyDown={handleSearchSettingConKeydown} css={STYLE}>
      {showLoading && <Button icon className='setting-dropdown-button'>
        <Loading useAriaLive width={14} height={14} type={LoadingType.Donut} />
      </Button>}
      {!showLoading && <Dropdown className='w-100 h-100' menuItemCheckMode='multiCheck' toggle={toggleSearchSetting} isOpen={isOpen}>
        <DropdownButton aria-label={nls('searchIn', { value: '' })} aria-pressed={hasSelectedService} ref={dropdownButtonRef} className='setting-dropdown-button' arrow={false} icon title={nls('searchIn', { value: '' })}>
        ...
        <DropdownMenu css={STYLE_OF_MENU} trapFocus={false} autoFocus={false} style={{ maxHeight: 'auto' }}>
          <div ref={dropdownMenuRef}>
            <DropdownItem role='menuitemcheckbox' active={isCheckAll && !isCheckPart} tag='div' tabIndex={0} toggle={false} onClick={selectAll} title={nls('all')} ref={selectAllButtonRef}>
