...
const LocationAndRecentSearch = (props: SearchSettingProps) => {
  ...
  React.useEffect(() => {
    if (recentSearchesData && isOpen) {
      setPopperVersion(Math.random())
    }
  }, [recentSearchesData, isOpen])
  ...
  const ariaLabel = React.useMemo(() => {
    const showRecentSearches = config.isShowRecentSearches && recentSearchesData[0]?.suggestionItem?.length > 0
    const isUseCurrentLoation = config.isUseCurrentLoation
    if (isUseCurrentLoation && showRecentSearches) {
      return `${nls('useCurrentLocation')}, ${nls('clearRecentSearches')}`
    } else if (isUseCurrentLoation) {
      return nls('useCurrentLocation')
    } else if (showRecentSearches) {
      return nls('clearRecentSearches')
    }
  }, [config.isShowRecentSearches, config.isUseCurrentLoation, nls, recentSearchesData])
  ...
  return (
    <div>
      ...
      >
        ...
        {config.isShowRecentSearches && renderRecentSearchElement()}
