...
const CustomSearchDataSetting = (props: Props) => {
  ...
  */
  ...
  const handleWidgetJsonChange = (newWidgetJson, dsInWidgetJson, outputDsJsonList?: DataSourceJson[]) => {
    ...
    const useUtilities = getUseUtilities(newWidgetJson.config)
    if (useUtilities.length > 0) {
      newWidgetJson.useUtilities = useUtilities
    }
    ...
    if(config?.sourceType === SourceType.CustomSearchSources) {
      ...
      Object.keys(dataSourceConfigWithMapCentric).forEach(key => {
        ...
        if (dataSourceConfigItem && dataSourceConfigItem?.synchronizeSettings as any === false) {
          dataSourceConfigItem?.dataSourceConfig?.forEach(configItem => {
            const utilityAlreadyExists = useUtilities.some(u => u?.utilityId === configItem?.useUtility?.utilityId)
            if (configItem?.searchServiceType === SearchServiceType.GeocodeService && !utilityAlreadyExists) {
              useUtilities.push(configItem?.useUtility)
            }
          })
        }
  ...
  const handleSynchronizeSettingsChange = (viewId: string) => {
    ...
    if (!synchronizeSettings) {
      ...
      newConfig = newConfig.setIn(['dataSourceConfigWithMapCentric', viewId, 'dataSourceConfig'], null).set('isAutoSelectFirstResult', true)
      confirmChangeWidgetJson(newConfig)
      addNewShowOnMapAction()
    }
  }
    ...
    return (
      <div className='mt-2' key={viewId}>
        ...
        {showLoadingListRef.current?.[viewId] && <div className='position-relative mt-3 mb-3 loading-con'>
          <Loading useAriaLive type={LoadingType.Secondary}/>
        </div>}
      </div>
    )
  }
