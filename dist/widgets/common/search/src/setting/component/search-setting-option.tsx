...
const SearchOptions = (props: SearchOptionsProps) => {
  ...
  const [autoCollapseOnSelection, setAutoCollapseOnSelection] = React.useState(false)
  ...
  React.useEffect(() => {
    ...
    setAutoCollapseOnSelection(config?.autoCollapseOnSelection)
  ...
  const onConfigChange = React.useCallback((key: string[], value: any) => {
    onSettingChange({ id: id, config: config.setIn(key, value) })
  }, [config, id, onSettingChange])

  const handleAutoSelectChange = React.useCallback((evt) => {
    setIsAutoSelectFirstResult(!config?.isAutoSelectFirstResult)
    onConfigChange(['isAutoSelectFirstResult'], !config?.isAutoSelectFirstResult)
  }, [config?.isAutoSelectFirstResult, onConfigChange])

  const handleAutoCollapseChange = React.useCallback((evt) => {
    setAutoCollapseOnSelection(!config?.autoCollapseOnSelection)
    onConfigChange(['autoCollapseOnSelection'], !config?.autoCollapseOnSelection)
  }, [config?.autoCollapseOnSelection, onConfigChange])

  const handleResultMaxNumberAccept = React.useCallback(value => {
  ...
  }, [resultMaxNumber, onConfigChange])

  const handleResultMaxNumberChange = React.useCallback((e) => {
  ...
  const onSettingLinkConfirm = React.useCallback((linkResult: IMLinkParam) => {
    if (!linkResult) {
      return
    }
  ...
  const onResultViewChange = React.useCallback((isShowSearchResultView: boolean) => {
  ...
  const renderSearchResultSetting = React.useCallback(() => {
    return (
      <SettingRow role='group' aria-label={nls('searchResult')}>
        ...
        >
          ...
          {searchResultView === SearchResultView.ResultPanel && <SettingRow>
            <div className='cursor-pointer d-flex align-items-center' onClick={handleAutoCollapseChange} aria-label={nls('autoCollapseOnSelection')}>
              <Checkbox checked={autoCollapseOnSelection || false} title={nls('autoCollapseOnSelection')} className='mr-1'/>
              <span className='check-box-label flex-grow-1 ml-1'>{nls('autoCollapseOnSelection')}</span>
            </div>
          </SettingRow>}
  ...
  }, [autoCollapseOnSelection, config, handleAutoCollapseChange, handleAutoSelectChange, handleResultMaxNumberAccept, handleResultMaxNumberChange, id, isAutoSelectFirstResult, linkParam, newResultMaxNumber, nls, onResultViewChange, onSettingChange, onSettingLinkConfirm, openSearchResult, searchResultView, useDataSources])
