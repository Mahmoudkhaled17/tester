 ...
 */
  ...
  utilityWillRemove (appConfig: IMAppConfig, utilityId: string): IMAppConfig {
    appConfig.widgets && Object.keys(appConfig.widgets).forEach(widgetId => {
      const widgetJson = appConfig.widgets[widgetId]
      if (widgetJson.uri === 'widgets/common/search/') {
        const config = widgetJson.config as IMConfig
        const isHasRemovedUtility = widgetJson?.useUtilities?.some(u => u?.utilityId === utilityId)
        if (isHasRemovedUtility) {
          if (config?.sourceType === SourceType.CustomSearchSources) {
            const newDatasourceConfig = getNewDatasourceConfigAfterRemoveUtility(utilityId, config?.datasourceConfig )
            appConfig = appConfig.setIn(['widgets', widgetId, 'config', 'datasourceConfig'], newDatasourceConfig)
          } else if (config?.sourceType === SourceType.MapCentric && config?.dataSourceConfigWithMapCentric) {
            let dataSourceConfigWithMapCentric = config?.dataSourceConfigWithMapCentric
            Object.keys(dataSourceConfigWithMapCentric).forEach(key => {
              const dataSourceConfigItem = dataSourceConfigWithMapCentric[key]
              if (dataSourceConfigItem && dataSourceConfigItem?.synchronizeSettings as any === false) {
                const newDatasourceConfig = getNewDatasourceConfigAfterRemoveUtility(utilityId, dataSourceConfigWithMapCentric[key]?.dataSourceConfig)
                dataSourceConfigWithMapCentric = dataSourceConfigWithMapCentric.setIn([key, 'dataSourceConfig'], newDatasourceConfig)
              }
            })
            appConfig = appConfig.setIn(['widgets', widgetId, 'config', 'dataSourceConfigWithMapCentric'], dataSourceConfigWithMapCentric)
          }

          const newUseUtilities = widgetJson?.useUtilities && widgetJson?.useUtilities?.filter(u => u?.utilityId !== utilityId)
          appConfig = appConfig.setIn(['widgets', widgetId, 'useUtilities'], newUseUtilities)
        }
      }
    })
    return appConfig
...
function getNewDatasourceConfigAfterRemoveUtility (utilityId: string, datasourceConfig: ImmutableArray<SearchDataConfig>) {
  return datasourceConfig.filter(item => {
    if (item.searchServiceType === SearchServiceType.GeocodeService) {
      return item?.useUtility?.utilityId !== utilityId
    } else {
      return true
    }
  })
