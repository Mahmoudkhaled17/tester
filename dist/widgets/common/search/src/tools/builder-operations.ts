...
function getTranslationKeyWithDatasourceConfig (option: Options): extensionSpec.TranslationKey[] {
  const { datasourceConfigPath, datasourceConfig, translationKeys, widgetId } = option
  datasourceConfig.forEach((item, index) => {
    if (item?.label) {
      const groupKey = `widgets.${widgetId}.config.${datasourceConfigPath}[${index}]`
    ...
    if (item?.hint) {
      translationKeys.push({
        ...
        groupKey: `widgets.${widgetId}.config.${datasourceConfigPath}[${index}]`,
        label: {
          key: 'hint',
