...
import { AlignModeType, EditModeType, LayerHonorModeType, ResponsiveType, SelectionModeType, type LayersConfig } from '../config'
...
export const batchDsTypes = Immutable([
  ...
  DataSourceTypes.BuildingGroupSubLayer, DataSourceTypes.BuildingSceneLayer,
  DataSourceTypes.SubtypeGroupLayer, DataSourceTypes.KnowledgeGraphLayer
])
...
export const getFilteredUseFields = (schemaFields, definitionFields) => {
  ...
  if (definitionFields?.length >= schemaFieldsKeys?.length) {
    filteredUseFields = definitionFields.filter(item => schemaFieldsKeys.includes(item.name || item.alias)).map(field => {
      ...
      return {
        ...field,
        alias: schemaField?.alias || schemaField?.name || field.alias || field.name,
...
export const getHonorWebmapUsedFields = (dataSource: SupportedDataSource): HonorWebmapUsedFields => {
  ...
  const popupAllFieldInfos = popupElements?.length > 0 ? popupElements.find(ele => ele.type === 'fields')?.fieldInfos || [] : []
  ...
  const hasPopup = popupAllFieldInfos.length > 0
  ...
  return {
    ...
    layerDefaultFields: filteredUseFields
  }
}
