...
import { TextInput, defaultMessages as jimuUIMessages, LoadingType, Loading, Switch, AlertPopup } from 'jimu-ui'
...
const LayerConfig = (props: LayerConfigProps) => {
  const {
    ...
    handleBatchAdd, onConfigSaved, getLayerModeConfigId, filterDs, onChange
  } = props
  ...
  const onOverrideGeneralSettings = React.useCallback((checked: boolean) => {
    if (!checked) {
      ...
      const propertiesToInclude = [
        'enableRelatedRecords', 'enableAttachments', 'columnSetting', 'headerFontSetting', 'enableSelect', 'selectMode', 'editMode',
  ...
  return (
    <div className='w-100 h-100' css={getStyle()}>
      {tempUseDs && <Loading type={LoadingType.Secondary} useAriaLive/>}
