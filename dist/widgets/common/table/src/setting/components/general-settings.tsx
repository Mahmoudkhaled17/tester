...
import { AlignModeType, type LayersConfig, ResponsiveType, SelectionModeType, type IMConfig, EditModeType } from '../../config'
...
const GeneralSettings = (props: GeneralSettingsProps) => {
  ...
  const {
    columnSetting, headerFontSetting, enableSelect, selectMode, editMode
  ...
  const getSelectModeOptions = React.useCallback((): React.JSX.Element[] => {
    return [
      ...
      <option key={SelectionModeType.Multiple} value={SelectionModeType.Multiple}>
        {translate('multiple')}
      </option>
    ]
  }, [translate])

  const getEditModeOptions = React.useCallback((): React.JSX.Element[] => {
    return [
      <option key={EditModeType.Single} value={EditModeType.Single}>
        {translate('single')}
      </option>,
      <option key={EditModeType.Multiple} value={EditModeType.Multiple}>
        {translate('multiple')}
      </option>
    ]
  }, [translate])
  ...
  const editModeLabel = (
    <div className='w-100 d-flex tip-container'>
      <Label className='tip-text' for='embed-honor-theme-font' title={translate('editMode')}>{translate('editMode')}</Label>
      <Tooltip title={translate('editModeTips')} showArrow placement='bottom'>
        <Button icon type='tertiary' className='d-inline jimu-outline-inside' disableHoverEffect={true} disableRipple={true}>
          <InfoOutlined />
        </Button>
      </Tooltip>
    ...
    <div className='general-settings-collapsable-area' css={getCollapsableStyle(theme)}>
      ...
      >
        ...
        {columnOptionsArray.map((key, index) => {
          ...
          return (
            <SettingRow tag='label' label={translate(key)} key={key}>
              <Switch
      ...
      >
        ...
        >
          <div style={{ width: '40%' }}>
            <Select
              size='sm'
              value={editMode || EditModeType.Single}
              onChange={evt => { onPropertyChange('editMode', evt.target.value) }}
              aria-label={translate('editMode')}
            >
              {getEditModeOptions()}
            </Select>
          </div>
