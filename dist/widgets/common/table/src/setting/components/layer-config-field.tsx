import { React, Immutable, css, type IMFieldSchema, hooks, type IMThemeVariables, type ArcGISQueriableDataSource } from 'jimu-core'
...
type LayerConfigFieldProps = Pick<LayerConfigProps, 'layerConfig' | 'onChange' | 'layerDefinition' | 'isEditableDs' | 'isMapMode'>
...
const LayerConfigField = (props: LayerConfigFieldProps) => {
  const { layerConfig, layerDefinition, isEditableDs, isMapMode, onChange } = props
  const {
  ...
  const [honorType, setHonorType] = React.useState<HonorType>(layerHonorMode === LayerHonorModeType.Custom ? HonorType.Custom : HonorType.Webmap)
  ...
  // rootDs is needed to determine the source of the layer
  const currentDs = React.useMemo(() => {
    return getDataSourceById(useDataSource?.dataSourceId)
  }, [useDataSource])
  const rootDataSource = React.useMemo(() => {
    return currentDs?.getRootDataSource()
  }, [currentDs])
  ...
  const getFieldEditable = React.useCallback((layerDefinition, jimuName: string) => {
    const isArcadeField = currentDs?.isArcadeField?.(jimuName)
    if (isArcadeField) return false
  ...
  const constructTreeItem = React.useCallback((tableFields: TableFieldsSchema[]) => {
      ...
      mutableGroupedFields.forEach((item, index) => {
        if (index === freezeItemIndex) {
          ...
          treeItemArr.push({
            ...
            itemStateIcon: dataComponentsUtils.getIconFromFieldType(item.type, theme),
            itemStateDetailContent: item,
            itemStateDisabled: !checkFieldsExist(allFields, item, currentDs),
            itemStateCommands: [],
            isCheckboxDisabled: !getFieldEditable(layerDefinition, item.jimuName)
          })
        }
    ...
    enableEdit, theme, useDataSource?.dataSourceId, getFieldEditable,
    layerDefinition, isFreezeFields, frozenFields, freezeLocation, currentDs
  ])
  ...
  React.useEffect(() => {
    setHonorType(layerHonorMode === LayerHonorModeType.Custom ? HonorType.Custom : HonorType.Webmap)
  }, [layerConfig?.id, layerHonorMode])

  const handleHonorModeChang = React.useCallback((e) => {
    onChange(layerConfig.set('layerHonorMode', e.target.value))
  }, [layerConfig, onChange])

  const onRadioModeChange = React.useCallback((val) => {
    onChange(layerConfig.set('layerHonorMode', val))
  }, [layerConfig, onChange])

  const handleMapHonorModeChang = React.useCallback((e) => {
    const newValue = e.target.value
    if (honorType === newValue) return
    setHonorType(newValue)
    if (newValue === HonorType.Webmap) {
      onChange(layerConfig.set('layerHonorMode', LayerHonorModeType.Webmap))
    } else {
      onChange(layerConfig.set('layerHonorMode', LayerHonorModeType.Custom))
    }
  }, [honorType, layerConfig, onChange])
  ...
  const handleEnableEditChange = React.useCallback(evt => {
    ...
    const newTableFields = tableFields.map(item => {
      const isArcadeField = currentDs?.isArcadeField?.(item.jimuName)
      if (isArcadeField) {
        return { ...item, editable: false, editAuthority: false }
      }
      return { ...item, editAuthority: item.editable }
    })
    onChange(layerConfig.set('enableEdit', checked).set('tableFields', newTableFields))
  }, [layerConfig, tableFields, currentDs, onChange])
  ...
  const isMapModeOrFromMap = React.useMemo(() => isMapMode || (rootDataSource as any)?.map, [isMapMode, rootDataSource])
  const isFromScene = rootDataSource?.type === 'WEB_SCENE'
    ...
    {isFromScene
      ? <SettingRow>
        <Select size='sm' className='w-100' value={layerHonorMode} onChange={handleHonorModeChang}>
          <option value={LayerHonorModeType.Webmap}>{translate('usePopupSetting')}</option>
      ...
      : isMapModeOrFromMap
        ? <React.Fragment>
          <SettingRow>
            <Select size='sm' className='w-100' value={honorType} onChange={handleMapHonorModeChang}>
              <option value={HonorType.Webmap}>{translate('useWebMapSetting')}</option>
              <option value={HonorType.Custom}>{translate('layerCustomize')}</option>
            </Select>
          </SettingRow>
          {honorType === HonorType.Webmap &&
              ...
              <div role='radiogroup'>
                <Label className='d-flex align-items-center'>
                  <Radio
                    ...
                    name='layerHonorMode'
                    className='mr-2'
                    checked={layerHonorMode === LayerHonorModeType.Webmap}
                    onChange={() => { onRadioModeChange(LayerHonorModeType.Webmap) }}
                  />
                  {translate('usePopupSetting')}
                </Label>
                <Label className='d-flex align-items-center'>
                  <Radio
                    style={{ cursor: 'pointer' }}
                    name='layerHonorMode'
                    className='mr-2'
                    checked={layerHonorMode === LayerHonorModeType.MapTable}
                    onChange={() => { onRadioModeChange(LayerHonorModeType.MapTable) }}
                  />
                  {translate('useTableSetting')}
                </Label>
              </div>
            </SettingRow>
          }
...
const checkFieldsExist = (allFields: IMFieldSchema[], tableField: TableFieldsSchema, currentDs: ArcGISQueriableDataSource) => {
  const isArcadeField = currentDs?.isArcadeField?.(tableField.jimuName)
  if (isArcadeField) return true
  let exist = false
