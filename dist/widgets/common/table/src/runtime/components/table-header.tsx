import { React, css, esri, type QueriableDataSource, type IMState, ReactRedux, AppMode, WidgetState, hooks } from 'jimu-core'
...
const TableHeader = (props: Props) => {
  const {
    searchToolTowed, curLayerConfig, dataSource, tableLoaded, widgetState, toolListNode,
    resetFlag, searchText, onSearchTextChange, handleSubmit
  } = props
  ...
  }, [widgetState, appMode, onSearchTextChange])
  ...
  const previousResetFlag = hooks.usePrevious(resetFlag)
  React.useEffect(() => {
    if (resetFlag !== previousResetFlag) {
      onSearchTextChange('')
      setIsShowSuggestion(false)
    }
  }, [resetFlag, previousResetFlag, onSearchTextChange])
