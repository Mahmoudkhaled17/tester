import {
  ...
  Immutable, QueryScope, WidgetState, loadArcGISJSAPIModule, DataSourceTypes, DataSourceStatus, css
} from 'jimu-core'
import { TableArrangeType, TableModeType, type IMConfig, type LayersConfig, TableDataActionType, PagingType } from '../config'
...
interface MapExtentFlag {
  [layerConfigId: string]: boolean
}

interface SearchTextByTabId {
  [tabId: string]: string
}
...
const TableWidget = (props: AllWidgetProps<IMConfig>) => {
  ...
  const {
    ...
    enableSelect, selectMode, showCount, enableRefresh, enableDelete, enableShowHideColumn, editMode
  } = config
  ...
  const [mapExtentFlag, setMapExtentFlag] = React.useState<MapExtentFlag>({})
  const [searchTextByTabId, setSearchTextByTabId] = React.useState<SearchTextByTabId>({})
  const [haveColumnShow, setHaveColumnShow] = React.useState<boolean>(true)
  const [resetFlag, setResetFlag] = React.useState(0)

  const prevWidgetQueryRef = React.useRef(null)
  ...
  const currentWidgetQuery = ReactRedux.useSelector((state: IMState) => {
    const dsId = dataSource?.id
    if (!dsId || !props.id) return undefined
    return state?.dataSourcesInfo?.[dsId]?.widgetQueries?.[props.id]
  })
  // only for chart selected view in table
  // dsInfo will be undefined, this led to the emptyTable becoming true and the layer being destroyed
  const isActionDsReady = ReactRedux.useSelector((state: IMState) => {
    const dataSourcesInfo = state?.dataSourcesInfo
    if (curLayerConfig?.dataActionDataSource) {
      const dsStatus = dataSourcesInfo?.[curLayerConfig?.dataActionDataSource?.id]?.status
      return dsStatus === DataSourceStatus.Unloaded
    } else {
      return false
    }
  })
  ...
  const usedConfig = React.useMemo(() => {
    ...
    const usedKeys = [
      ...
      'showCount', 'enableRefresh', 'enableDelete', 'enableShowHideColumn', 'editMode'
    ]
    ...
    enableShowHideColumn, headerFontSetting, selectMode, showCount, editMode, curLayerConfig
  ])
  ...
  const [table, tableLoaded, usedDsId, attributeTableTemplate, tableQueryingOrSyncing] = useTable(tableOptions)
  ...
  const previousTableQueryingOrSyncing = hooks.usePrevious(tableQueryingOrSyncing)
  ...
  const handleSearchTextChange = React.useCallback((searchText: string) => {
    if (!activeTabId) return
    setSearchTextByTabId((prev) => {
      if ((prev[activeTabId] || '') === searchText) return prev
      return {
        ...prev,
  ...
  const handleSubmit = React.useCallback((searchText: string) => {
    ...
    const newParam = { ...dsParam, ...tableQuery }
    dataSource.updateQueryParams(newParam as QueryParams, id)
  ...
  const syncSelectedForTableHighlight = React.useCallback(() => {
    ...
    const tableHighlightIds = table.highlightIds?.toArray() || []
    const notChange = lodash.isDeepEqual(selectedRecordIds, tableHighlightIds)
    if (notChange) return
    ...
    table.highlightIds?.push && table.highlightIds.push(...recordIds)
  }, [dataSource, table])
  ...
  const updateTableGeometry = React.useCallback(async (geometry, distance, units) => {
    ...
    if (geometryType !== 'polygon' && distance && distance <= 0) {
      ...
      if (!lodash.isDeepEqual(orgGeometryJson, geometry?.toJSON())) {
  ...
  const updateGeometryAndSql = React.useCallback((onlyUpdateExpression?: boolean) => {
    ...
    // special case: 3D associated layer
    const isNotCurrentTable = (usedDsId !== dataSource.id) && (usedDsId !== (dataSource as any)?.associatedDataSource?.id)
    ...
    const dsOrderByFields = dsParam?.orderByFields
    const previousOrderByFields = previousDsOrder.current
    if (previousOrderByFields && !lodash.isDeepEqual(previousOrderByFields, dsOrderByFields)) {
      const newTemplate = constructTableTemplate(dataSource, curLayerConfig, usedConfig, tableShowColumns, null, true)
      table.tableTemplate = newTemplate
    }
    previousDsOrder.current = dsOrderByFields
    //#endregion
  ...
  // when config change, table need to reset show columns
  const previousUsedConfig = hooks.usePrevious(usedConfig)
  React.useEffect(() => {
    if (previousUsedConfig && !lodash.isDeepEqual(previousUsedConfig, usedConfig)) {
      setTableShowColumns(undefined)
    }
  }, [usedConfig, previousUsedConfig])

  const prevActiveTabId = hooks.usePrevious(activeTabId)
  React.useEffect(() => {
    const isEmptyObject = (value): boolean => {
      return !!value && typeof value === 'object' && Object.keys(value).length === 0
    }

    const hasChanged = prevWidgetQueryRef.current !== currentWidgetQuery
    const isQueryCleared = currentWidgetQuery == null || isEmptyObject(currentWidgetQuery)
    if (hasChanged && isQueryCleared && activeTabId && prevActiveTabId === activeTabId) {
      setMapFilterEnabled(false)
    }
    if (hasChanged && isQueryCleared) {
      setResetFlag(flag => flag + 1)
    }
    prevWidgetQueryRef.current = currentWidgetQuery
  }, [currentWidgetQuery, activeTabId, prevActiveTabId])
  ...
  React.useEffect(() => {
    ...
    if (table && dataSource && !isActive) {
      ...
      table.highlightIds?.removeAll && table.highlightIds.removeAll()
      const recordIds = useSelectedRecords.map(record => record.getId())
      // table.highlightIds.addMany(recordIds)
      table.highlightIds?.push && table.highlightIds.push(...recordIds)
    }
  ...
  React.useEffect(() => {
    // when use action to filter map:
    // tableQueryingOrSyncing & tableLoaded state change: false false,true true,false true
    const justFinishedQuerying = previousTableQueryingOrSyncing && !tableQueryingOrSyncing
    if (table && dataSource && tableLoaded && justFinishedQuerying) {
      syncSelectedForTableHighlight()
    }
  }, [
    widgetState,
    ...
    dataSource,
    tableLoaded,
    previousTableQueryingOrSyncing,
    tableQueryingOrSyncing,
    syncSelectedForTableHighlight
  ])

  // Sometimes, selectedRecords changed after sql/geometry update and tableLoaded
  React.useEffect(() => {
    if (table && dataSource && tableLoaded && selectedRecords) {
      syncSelectedForTableHighlight()
    }
  ...
  // reset geometry when map extent filter turned off then switch to a tab that has map extent filter on before
  React.useEffect(() => {
    if (!mapFilterEnabled && dataSource && curLayerConfig && table?.layer) {
      const dsParam: any = dataSource.getInfo()?.widgetQueries?.[id]
      let newParam
      if (mapExtentFlag[curLayerConfig.id]) {
        newParam = {
          ...dsParam
        }
        delete newParam.geometry
        dataSource.updateQueryParams(newParam, id)
        setMapExtentFlag(old => {
          const newFlag = old
          newFlag[curLayerConfig.id] = false
          return newFlag
        })
      }
    }
  }, [mapFilterEnabled, dataSource, id, curLayerConfig, table, mapExtentFlag])

  // watch table columns change
  React.useEffect(() => {
    const watchColumns = reactiveUtils.watch(() => table?.columns?.toArray()?.some(item => (item as any).hidden === false), (haveColumn) => {
      setHaveColumnShow(haveColumn)
  ...
  // reset activeView when change to layer mode
  React.useEffect(() => {
    if (!isMapMode) setActiveView(null)
  }, [isMapMode, activeView])
  ...
  const updateViewExtent = React.useCallback((extent?: __esri.Extent) => {
    ...
    if (mapExtentAction) {
      ...
      const haveCurDsAction = actions.find(action => {
        if (action?.actionName === 'filter data record action') {
    ...
    if (curLayerConfig) {
      setMapExtentFlag(old => {
        const newFlag = old
        if (extent) {
          newFlag[curLayerConfig.id] = true
        } else {
          newFlag[curLayerConfig.id] = false
        }
        return newFlag
  ...
  }, [dsManager, dataSource, id, messageConfigs, useMapWidgetIds, curLayerConfig])
  ...
  const onValueChangeFromRuntime = (valuePairs: ClauseValuePair[]) => {
    ...
    minusArray(tableColumns, valuePairs, 'value').forEach(item => {
      // Special case for attachment, some attachment column value is like this 'EsriFeatureTableAttachmentsColumn-14'
      // toggleColumnVisibility only works when param is 'EsriFeatureTableAttachmentsColumn'
      const columnValue = item.value?.toString()
      if (columnValue?.startsWith('EsriFeatureTableAttachmentsColumn')) {
        table.toggleColumnVisibility('EsriFeatureTableAttachmentsColumn')
      } else {
        table.toggleColumnVisibility(columnValue)
      }
    })
  ...
  const handleSelectionChange = React.useCallback((tableSelected: number) => {
  ...
  const onSourceVersionChange = async () => {
    if (dataSource && !dataSource.url) {
      // My Location case
      const locationDsTag = '_output_'
      if (dataSource.id?.includes(locationDsTag)) {
        const tagIndex = dataSource.id.indexOf(locationDsTag)
        const locationWidgetId = dataSource.id.substring(0, tagIndex)
        const locationOutputIds = appConfig?.widgets?.[locationWidgetId]?.outputDataSources || []
        if (locationOutputIds.includes(dataSource.id)) {
  ...
  // has map but no available layers
  const noAvailableLayers = isMapMode && useMapWidgetIds?.length > 0 && allLayersConfig.length === 0
  const noColumns = table?.tableTemplate?.columnTemplates?.length === 0
  const haveTableButNoCols = table && table?.columns && (noColumns || !haveColumnShow)
  const toolListNode =
    ...
    <React.Fragment>
      {showPlaceholder &&
        <WidgetPlaceholder
          ...
          name={noAvailableLayers ? translate('noLayersAvailable') : translate('_widgetLabel')}
            ...
            <div className={isHorizontalTab ? 'horizontal-render-con' : 'dropdown-render-con'}>
              {curLayerConfig && searchOn &&
                <TableHeader
                  ...
                  toolListNode={toolListNode}
                  resetFlag={resetFlag}
                  searchText={searchTextByTabId[activeTabId] || ''}
                  onSearchTextChange={handleSearchTextChange}
                  handleSubmit={handleSubmit}
                />
              }
              {(emptyTable || haveTableButNoCols) &&
                <EmptyTablePlaceholder
                  searchOn={searchOn}
                  notReady={notReady}
                  noColumns={haveTableButNoCols}
                  dataSourceLabel={dataSource?.getLabel()}
                  widgetLabel={widgetLabel}
                  isSelectionView={dataSource?.dataViewId === SELECTION_DATA_VIEW_ID}
                />
              }
              <div className='table-con' ref={tableContainer} css={!haveColumnShow && css`display: none`}/>
