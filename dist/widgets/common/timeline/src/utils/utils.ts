...
import { TimeDisplayStrategy, type timeSpanValue, type timeSettings, TimeSpeed, TimePlayPosition } from '../config'
...
export function getTimeSettingsFromHonoredWebMap (dataSources: { [dsId: string]: DataSource }, isRuntime = false): timeSettings {
  ...
  if (props) {
    const {
      ...
      endTime: _endTime,
      // currentTimeExtent: propCurrentTimeExtent,
      timeStopInterval,
      numberOfStops,
      thumbMovingRate,
      thumbCount
    ...
    // TODO: remove it when scene viewer supports playPosition
    let _currentTimeExtent = props.currentTimeExtent
    if (webMap.type === DataSourceTypes.WebScene) {
      _currentTimeExtent = null
    }

    const playPosition = _currentTimeExtent ? TimePlayPosition.Saved :TimePlayPosition.Start
    let currentTimeExtent = null // not save extent to config
    if (isRuntime) {
      const times = getTimeExtentByTzOffset(_startTime, _endTime, true)
      startTime = times.startTime
      endTime = times.endTime
      if (_currentTimeExtent) {
        const currentTimes = getTimeExtentByTzOffset(_currentTimeExtent[0], _currentTimeExtent[1], true)
        currentTimeExtent = [currentTimes.startTime, currentTimes.endTime]
      }
    }
    settings = {
      ...
      timeDisplayStrategy: getDisplayStrategyFromThumbCount(thumbCount, props.currentTimeExtent)
    }
    if (timeStopInterval) {
...
function getDisplayStrategyFromThumbCount (thumbCount = 2, currentTimeExtent: number[]): TimeDisplayStrategy {
  if (currentTimeExtent) {
    const start = currentTimeExtent[0] != null ? new Date(currentTimeExtent[0]) : null
    const end = currentTimeExtent[1] != null ? new Date(currentTimeExtent[1]) : null
    const isInstant = start != null && end != null && start.getTime() === end.getTime()
    if (isInstant) {
      return TimeDisplayStrategy.instant
    }
    if (thumbCount === 2) {
      return TimeDisplayStrategy.current
    }
    if (start == null || start.getTime() === 0) {
      return TimeDisplayStrategy.cumulatively
    }
    // return TimeDisplayStrategy.cumulativeFromEnd
  }

  return thumbCount === 2 ? TimeDisplayStrategy.current : TimeDisplayStrategy.cumulatively
}
...
export function getCalculatedTimeSettings (timeSettings, dataSources: { [dsId: string]: DataSource }, isRuntime = false) {
  ...
  // update to the latest playPosition from webmap
  if (!newTimeSettings.playPosition || newTimeSettings.playPosition === TimePlayPosition.Saved) {
    const webMap = dataSources[Object.keys(dataSources).find(id => isWebMapOrWebScene(dataSources[id]?.type))] // webmap ds might be null when it's inaccessible
    const props = (webMap as MapServiceDataSource)?.getItemData()?.widgets?.timeSlider?.properties
    if (props) {
      if (!newTimeSettings.playPosition) {
        newTimeSettings = newTimeSettings.set('playPosition', props.currentTimeExtent ? TimePlayPosition.Saved :TimePlayPosition.Start)
      }
      if (isRuntime) {
        if (newTimeSettings.playPosition === TimePlayPosition.Saved && props.currentTimeExtent) {
          const currentTimeExtent = getTimeExtentByTzOffset(props.currentTimeExtent[0], props.currentTimeExtent[1], true)
          newTimeSettings = newTimeSettings.set('currentTimeExtent', [currentTimeExtent.startTime, currentTimeExtent.endTime])
        } else { // reset to start
          newTimeSettings = newTimeSettings.set('playPosition', TimePlayPosition.Start)
        }
      }
    }
  }
