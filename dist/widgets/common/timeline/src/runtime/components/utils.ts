import { dateUtils, type IntlShape } from 'jimu-core'
...
function getLabelForYearTick (details, date, intl, isFirst = false) {
  ...
  if (details.day) {
    ...
    label = date.getFullYear()
    if (isFirst) {
      label = dateUtils.formatDateValueByIntlFormat(new Date(date.getFullYear(), date.getMonth()), intl, { year: 'numeric', month: 'numeric' } as any)
    }
  } else {
    label = date.getFullYear()
  }
...
function getLabelForMonthTick (details, date, intl) {
  ...
  if (details.day && (!details.hour || details.hour.value !== 1)) {
    label = dateUtils.formatDateValueByIntlFormat(new Date(2000, date.getMonth(), date.getDate()), intl, { month: 'numeric', day: 'numeric' } as any)
  } else if (month !== 1) {
    label = month
  }
  return label
