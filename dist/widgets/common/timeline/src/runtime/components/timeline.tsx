...
function TimeLine (props: TimelineProps) {
  const {
    ...
    startTime, endTime, currentTimeExtent,
    accuracy = 'year', stepLength, dividedCount, displayStrategy = TimeDisplayStrategy.current,
    enablePlayControl = false, speed: _speed = TimeSpeed.Medium, autoPlay: _autoPlay, dateTimePattern, updating = false,
    onTimeChanged, onApplyStateChanged
  } = props
  ...
  React.useEffect(() => {
    ...
    // reset time slider extent
    if (currentTimeExtent) {
      setStartTimeForStep(currentTimeExtent[0])
      setEndTimeForStep(currentTimeExtent[1])
      onTimeChanged(currentTimeExtent[0], currentTimeExtent[1])
    } else {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [_autoPlay, enablePlayControl, startTime, displayStrategy, endTime, accuracy, stepLength, dividedCount, currentTimeExtent])
  ...
  const InfoButton = React.useMemo(() => {
    const startLabel = dateUtils.formatDateValueByIntlFormat(startTime, intl, dateTimePattern)
    const endLabel = dateUtils.formatDateValueByIntlFormat(endTime, intl, dateTimePattern)
  ...
  // Labels
  const startLabelForStep = dateUtils.formatDateValueByIntlFormat(_startTimeForStep, intl, dateTimePattern)
  const endLabelForStep = dateUtils.formatDateValueByIntlFormat(_endTimeForStep, intl, dateTimePattern)
  // arrows
