...
import {
  ...
  dataSourceUtils, type UseDataSource, type ImmutableArray
} from 'jimu-core'
...
const Widget = (props: TimelineProps) => {
  const {
    useMapWidgetIds: _useMapWidgetIds, useDataSources, theme, id: widgetId, config, intl, autoWidth, autoHeight, controllerWidgetId, inControllerUx
  } = props
  const {
  ...
  const isOffPanel = controllerWidgetId && inControllerUx === 'offPanel'
  ...
  const isCurrentDataSourcesWebMap = React.useMemo((): boolean => {
    if (!dataSources) {
      return false
    }
    return Object.keys(dataSources).some(dsId => isWebMapOrWebScene(dataSources[dsId]?.type))
  }, [dataSources])

  // handle clear all actionion
  const currentUseDataSources = React.useMemo((): ImmutableArray<UseDataSource> => {
    let useDss = null
    if (isSingleLayer(dataSourceType)) { // single layers
      useDss = useDataSources
    } else if (useMapWidgetIds?.length > 0 && layerViews) { // map widget
      useDss = Object.keys(layerViews).map(id => ({ dataSourceId: layerViews[id].layerDataSourceId } as UseDataSource))
    } else if (isWebMapOrWebScene(dataSourceType) && isCurrentDataSourcesWebMap) { // webmap
      const layers = getInsideLayersFromWebmapOrWebScene(dataSources, config.timeSettings?.layerList)
      useDss = Object.keys(layers).map(lyId => ({ dataSourceId: lyId } as UseDataSource))
    }
    return useDss ? Immutable(useDss) : null
  }, [dataSourceType, useDataSources, useMapWidgetIds, layerViews, isCurrentDataSourcesWebMap, dataSources, config.timeSettings?.layerList])

  const { resetToken, clearFilterDataSourceIds } = dataSourceUtils.useClearDataSourceFilters(widgetId, currentUseDataSources)

  React.useEffect(() => {
    if (resetToken && clearFilterDataSourceIds.length > 0) {
      setApplied(false)
    }
  }, [resetToken, clearFilterDataSourceIds])
    ...
    if (isWebMapOrWebScene(dataSourceType)) {
      if (useMapWidgetIds?.length) { // map widget
        Object.keys(layerViews).forEach(id => {
          ...
          layer && updateLayerQueryParams(layer, queryParams, widgetId)
        })
      } else { // webMap/webScene ds
        const layers = getInsideLayersFromWebmapOrWebScene(dataSources, config.timeSettings?.layerList)
        Object.keys(layers).forEach(lyId => {
          updateLayerQueryParams(layers[lyId], queryParams, widgetId)
        })
      }
    } else {
      Object.keys(dataSources).forEach(dsId => {
        dataSources[dsId] && updateLayerQueryParams(dataSources[dsId], queryParams, widgetId)
      })
    }
  })
  ...
  const updateLayerQueryParams = (layerDs, queryParams, wId) => {
    if (layerDs.type === DataSourceTypes.MapService) {
      ...
      if (layerDs.supportTime?.()) {
        ...
        layerDs.changeTimeExtent?.(queryParams.time, wId)
      }
    } else if (isSingleLayer(layerDs.type)) {
      if (layerDs.supportTime?.()) {
        queryParams = getTimeOffsetQueryParams(layerDs, queryParams)
        layerDs.updateQueryParams?.(queryParams, wId)
      }
    }
  }
  ...
  const onCreateDataSourceCreatedOrFailed = (dataSourceId: string, dataSource: DataSource, unMount = false) => {
    ...
    setDataSources(dataSources => {
      ...
      if (
        ...
        // remove filters applied by current ds when it's removed
        if (newDataSources[dataSourceId]) {
          updateLayerQueryParams(newDataSources[dataSourceId], { time: null }, widgetId)
        }
        // remove current ds
    ...
    // reset applied state to settings when layer dss are changed (added or removed).
    setApplied(applyFilteringByDefault)
                  ...
                  }
                  {
                    dataSources === null || !areDssReady
                      ...
                      : timeSettingsForRuntime && width >= 0 && <TimeLine
                        ...
                        currentTimeExtent={timeSettingsForRuntime.currentTimeExtent}
