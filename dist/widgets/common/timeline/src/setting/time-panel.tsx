/** @jsx jsx */
import {
  React, jsx, type IMThemeVariables, Immutable, type IntlShape, type DataSource, type ImmutableObject, type UseDataSource,
  type ImmutableArray, dateUtils, DataSourceTypes, getAppStore, TimezoneConfig, EsriFieldType
} from 'jimu-core'
...
import { TimeDisplayStrategy, TimePlayPosition, type timeSettings, TimeSpeed } from '../config'
import { getRowLabelStyle, getStyleForTimePanel } from './style'
import { DATE_TIME_FORMAT } from './components/utils'
import TimeSpan from './components/time-span'
import {
  DIVIDED_COUNT, getCalculatedTimeSettings, getInsideLayersFromWebmapOrWebScene,
  getStepLengthByAccuracy, getTimeExtentByTzOffset, getTimesByVirtualDate, isWebMapOrWebScene, MAX_DATE_TIME, MIN_DATE_TIME, SecondsForDateUnit,
...
export default class TimePanel extends React.PureComponent<Props, State> {
  ...
  render () {
    ...
    const { startTime, endTime, accuracy, timeDisplayStrategy, stepLength, dividedCount, playPosition, speed } = config
      ...
      <div className='time-panel w-100 h-100' css={getStyleForTimePanel(theme)}>
        ...
        >
          ...
          </SettingRow>
          {
            ...
            <SettingRow className='mt-1 date-label'>{dateUtils.formatDateValueByEsriFormat(exactStartTime, DATE_TIME_FORMAT, EsriFieldType.Date, intl)}</SettingRow>
          }
          {
        ...
        >
          ...
          </SettingRow>
          {
            ...
            <SettingRow className='mt-1 date-label'>{dateUtils.formatDateValueByEsriFormat(exactEndTime, DATE_TIME_FORMAT, EsriFieldType.Date, intl)}</SettingRow>
          }
          {
        ...
        {
          dataSourceType === DataSourceTypes.WebMap && // TODO: use isWebMapOrWebScene(type) after sceneViewer supports playPosition
          <SettingSection
            role='group'
            title={i18nMessage('playbackPosition')}
            aria-label={i18nMessage('playbackPosition')}
          >
            <SettingRow>
              <label>
                <Radio
                  name='play-position-radio'
                  style={{ cursor: 'pointer' }}
                  className='mr-2 align-text-bottom'
                  checked={playPosition !== TimePlayPosition.Saved}
                  onChange={() => { this.onChange('playPosition', TimePlayPosition.Start) }}
                />
                {i18nMessage('playFromStartTime')}
              </label>
            </SettingRow>
            <SettingRow>
              <label>
                <Radio
                  ...
                  className='mr-2 align-text-bottom'
                  checked={playPosition === TimePlayPosition.Saved}
                  onChange={() => { this.onChange('playPosition', TimePlayPosition.Saved) }}
                />
                {i18nMessage('playFromPositionSavedWithMap')}
              </label>
            </SettingRow>
          </SettingSection>
        }
