...
class _Setting extends React.PureComponent<AllWidgetSettingProps<IMConfig> & ExtraProps & TestProps, State> {
  ...
  hideDs = (dsJson) => {
    ...
    if (isSingleLayer(ds.type)) {
      hide = !(ds as FeatureLayerDataSource | SubtypeGroupLayerDataSource).supportTime() || dataSourceUtils.findMapServiceDataSource(ds) !== null
      ...
      <SettingRow>
        {
          addSourceByData
            ? <DataSourceSelector
              ...
              hideDataView
              hideDs={this.hideDs}
              // for multidimensional time-enabled single raster dataset case: it might has timeInfo but without fields (not queriable).
              showQueriableDataSourceWithoutFields={true}
  ...
  getPlaceholderContent = (addSourceByData) => {
    ...
    return <div className={classNames('empty-placeholder w-100 flex-grow-1 text-center', { 'add-ds-by-data': addSourceByData })}>
