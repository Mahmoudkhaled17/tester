/** @jsx jsx */
import { React, jsx, css, lodash, type IntlShape, dateUtils, EsriFieldType, hooks } from 'jimu-core'
import { interact } from 'jimu-core/dnd'
import { Tooltip } from 'jimu-ui'
import { bindResizeHandler, DATE_TIME_FORMAT, getLineInfo, getRangeByPixel, type ResizeHandlerProps } from './utils'
...
const InputRange = function (props: TimeSpanProps) {
  ...
  const startLabel = React.useMemo(() => {
    return dateUtils.formatDateValueByEsriFormat(range[0], DATE_TIME_FORMAT, EsriFieldType.Date, intl)
  }, [intl, range])

  const endLabel = React.useMemo(() => {
    return dateUtils.formatDateValueByEsriFormat(range[1], DATE_TIME_FORMAT, EsriFieldType.Date, intl)
  }, [intl, range])
