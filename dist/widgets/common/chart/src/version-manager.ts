import { type IMFeatureLayerQueryParams, Immutable, type ImmutableArray, type ImmutableObject, type WidgetUpgradeInfo, WidgetVersionManager, dataSourceUtils } from 'jimu-core'
...
import type { IField } from '@esri/arcgis-rest-request/dist/esm/types/service'
...
class VersionManager extends WidgetVersionManager {
  versions = [{
    ...
    version: '1.20.0',
    description: 'Upgrade the version of webChart to 25.0.0. Replace schema fields format prop with fieldFormat in the output data source.',
    upgradeFullInfo: true,
    upgrader: (oldInfo: WidgetUpgradeInfo) => {
      let widgetJson = oldInfo.widgetJson
      let outputDataSourceJsons = oldInfo.outputDataSourceJsons
      const widgetOutputDsIds = oldInfo.widgetJson.outputDataSources || []
      if (widgetJson?.config?.webChart) {
        widgetJson = widgetJson.setIn(['config', 'webChart', 'version'], '25.0.0')
      }
      if (outputDataSourceJsons) {
        widgetOutputDsIds.forEach((outputDsId) => {
          if (!outputDataSourceJsons[outputDsId]) {
            outputDataSourceJsons = outputDataSourceJsons.without(outputDsId)
            return
          }
          const fields = outputDataSourceJsons[outputDsId]?.schema?.fields
          if (fields) {
            const newFields = fields.asMutable({ deep: true })
            Object.keys(fields).forEach((fieldName) => {
              if (fields[fieldName].format) {
                const newFieldObj = fields[fieldName].asMutable({ deep: true })
                newFieldObj.fieldFormat = dataSourceUtils.fieldFormatFromFieldInfoFormat({ name: newFieldObj.name, type: newFieldObj.esriType } as IField, newFieldObj.format)
                delete newFieldObj.format
                newFields[fieldName] = newFieldObj
              }
            })
            outputDataSourceJsons = outputDataSourceJsons.setIn([outputDsId, 'schema', 'fields'], newFields)
          }
        })
      }
      return { widgetJson, outputDataSourceJsons }
    }
  }, {
    version: '1.21.0',
    description: 'Replace `options.hideEmptySeries` with `options.showEmptySeries`.',
    upgrader: (oldConfig: IMConfig) => {
      if (!oldConfig?.webChart) return oldConfig
      const options = (oldConfig?.options as ImmutableObject<any>)
      if (options) {
        oldConfig = oldConfig.set('options', options.set('showEmptySeries', !options.hideEmptySeries).without('hideEmptySeries'))
      }
      return oldConfig
    }
  }]
