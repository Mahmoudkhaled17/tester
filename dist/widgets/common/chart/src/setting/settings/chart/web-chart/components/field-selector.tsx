...
import { useDebouncedCallback, hasUniqueIdFields } from '../../../utils'
  ...
  switch (type) {
    ...
    case 'category':
      if (hideDateField) {
        ...
        return Immutable([JimuFieldType.String, JimuFieldType.Date, JimuFieldType.Number, JimuFieldType.DateOnly, JimuFieldType.TimeOnly])
      }
  }
}
...
export const FieldSelector = (props: FieldSelectorProps): React.ReactElement => {
  ...
  const hiddenFields = React.useMemo(() => {
    ...
    // services doesn't support the objectId field if it has uniqueId fields
    // for this special case, we should hide the id field even if props.hideIdField is false
    let finalHideIdField = hideIdField

    if (!finalHideIdField) {
      finalHideIdField = hasUniqueIdFields(dataSourceId)
    }

    if (!dataSourceId || (!finalHideIdField && !hideNonIntNumberField)) return
