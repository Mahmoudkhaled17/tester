...
import { isComboChartTemplate } from './by-group/utils'
...
export const StatisticsDataSetting = (props: StatisticsDataSettingProps): React.ReactElement => {
  ...
  const isComboTemplate = isComboChartTemplate(template)
  const effectiveCategoryType = isComboTemplate && categoryType === CategoryType.ByField
    ? CategoryType.ByGroup
    : categoryType
  const availableCategoryTypes = React.useMemo(() => {
    return Object.values(CategoryType).filter((currentCategoryType) => {
      return !isComboTemplate || currentCategoryType !== CategoryType.ByField
    })
  }, [isComboTemplate])
  ...
  const showMaxCategories = effectiveCategoryType !== CategoryType.ByField && !binTemporalData
    ...
    <div className='chart-data-setting w-100' {...others}>
      <SettingRow level={2} label={translate('categoryType')} flow='wrap' className='mt-2'>
        <Select
          ...
          value={effectiveCategoryType}
          onChange={handleCategoryTypeChange}
        >
          {availableCategoryTypes.map((currentCategoryType, i) => (
            <option value={currentCategoryType} key={i} className='text-truncate'>
              {translate(CategoryTypes[currentCategoryType])}
            </option>
          ))}
        </Select>
      </SettingRow>
      <>
        {
          effectiveCategoryType === CategoryType.ByGroup && <ByGroupData
            type={type}
            isComboTemplate={isComboTemplate}
            series={series}
        ...
        {
          effectiveCategoryType === CategoryType.ByField && <ByFieldData
            series={series}
            orderOptions={orderOptions}
            chartDataSource={propChartDataSource}
            supportPercentile={supportPercentile}
