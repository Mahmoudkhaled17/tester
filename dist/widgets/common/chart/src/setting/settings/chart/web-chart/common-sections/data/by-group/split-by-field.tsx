...
interface Props {
  ...
  onChange?: (field: string, values: Array<number | string | null>, type?: JimuFieldType) => void
}
...
export const SplitByField = (props: Props): React.ReactElement => {
  ...
  const handleChange = async (fields: string[], types: JimuFieldType[]): Promise<void> => {
    ...
    if (splitByField) {
      try {
        setLoading(true)
        const queryCount = MaxInitialSplitBySeriesCount + (fetchNullValues ? 1 : 0)
        const values = await queryFieldUniqueValues(ds, splitByField, queryCount, QueryScope.InConfigView, fetchNullValues)
        if (!values.length) {
