...
interface SeriesLoaderProps {
  ...
  onChange?: (values: SplitByValue[]) => void
}

const NumberPerLoads = 10
const NumberMaxCount = 100

const loadSplitByValues = async (dataSource: QueriableDataSource, field: string, inputValues: Array<string | number>, fetchNullValues: boolean, count: number = NumberPerLoads): Promise<{ values: Array<string | number>, hasNull: boolean }> => {
...
export const SeriesLoader = (props: SeriesLoaderProps): React.ReactElement => {
  const { className, dataSourceId, splitByField, values: propValues = [], fetchNullValues = false, onChange } = props
  ...
  const handleLoadClick = async () => {
    try {
      ...
      const inputNonNullValues = inputValues.filter((value): value is string | number => value !== null)
      const { values: nextValues, hasNull } = await loadSplitByValues(dataSource, splitByField, inputNonNullValues, fetchNullValues)
      const hasExistingNull = inputValues.includes(null)
      const hasNewNull = hasNull && !hasExistingNull
      const state = getLoadState(inputNonNullValues, nextValues)
      let values: SplitByValue[] = inputNonNullValues.concat(nextValues)
