...
const Icons = {
  ...
  combo: require('../../../assets/icons/combo.svg'),
...
const Thumbnails = {
  ...
  combo: require('../../../assets/thumbnail/combo.svg'),
...
const getChartTypeByTemplate = (template: string): ChartType => {
  if (template === 'donut') {
    return 'pie'
  }
  if (!template.includes('-')) {
    return template as ChartType
  } else {
    const chartType = template.split('-')?.[1] ?? ''
    if (chartType.includes('100')) {
...
export const getTemplateIconByTemplate = (template: string) => {
  const type = getChartTypeByTemplate(template) ?? 'column'
  return Icons[type]
}

export const getMainTypeTranslation = (template: string) => {
  const templateType = getChartTypeByTemplate(template) ?? 'column'
