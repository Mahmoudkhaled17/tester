...
import Templates, { getMainTypeTranslation, getTemplateIconByTemplate } from './templates'
...
const ChartTypeSelector = (props: ChartTypeSelectorProps): React.ReactElement => {
  const { templateId, useDataSources, onChange } = props
  ...
  const [templateIcon, templateLabel] = React.useMemo(() => {
    ...
    const icon = getTemplateIconByTemplate(templateId)
    const label = translate(getMainTypeTranslation(templateId))
    return [icon, label]
