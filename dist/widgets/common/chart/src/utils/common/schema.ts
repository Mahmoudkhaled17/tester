import { EsriFieldType, type FieldSchema, JimuFieldType, type IMFeatureLayerQueryParams, type ImmutableArray, Immutable, type ImmutableObject, type IMFieldSchema, StatisticType, type DataSourceSchema, type DataSource, dataSourceUtils, type FieldFormatProperties, type NumberFieldFormatProperties } from 'jimu-core'
...
const getCleanFieldSchema = (field: string, type: JimuFieldType, alias?: string, originFields?: string[]): FieldSchema => {
  ...
  let fieldFormat: FieldFormatProperties = null
  if (type === JimuFieldType.Number) {
    esriType = EsriFieldType.Double
    fieldFormat = dataSourceUtils.fieldFormatFromFieldInfoFormat({ name, type: esriType } as IField, {
      digitSeparator: true,
      places: 3
    })
  }
  const schema: FieldSchema = { jimuName, type, esriType, name, alias, fieldFormat }
  if (originFields) {
...
const completeStatisticsFormat = (inputFormat: ImmutableObject<FieldFormatProperties>, statisticType) => {
  ...
  let fieldFormat = inputFormat as ImmutableObject<NumberFieldFormatProperties>
  if (statisticType === StatisticType.Count) {
    fieldFormat = fieldFormat || Immutable({ type: 'number' })
    fieldFormat = fieldFormat.set('minimumFractionDigits', 0)
    fieldFormat = fieldFormat.set('maximumFractionDigits', 0)
  } else if (statisticType === StatisticType.Avg) {
    if (!fieldFormat?.maximumFractionDigits) {
      fieldFormat = fieldFormat || Immutable({ type: 'number' })
      fieldFormat = fieldFormat.set('minimumFractionDigits', 3)
      fieldFormat = fieldFormat.set('maximumFractionDigits', 3)
    }
  }
  return fieldFormat
}
  ...
  if (valid) {
    ...
    fieldInfos.forEach(({ name, alias }, index) => {
      ...
      if (schema) {
        ...
        if (statisticType === StatisticType.Count) {
          schema = schema.set('esriType', EsriFieldType.Integer).set('type', JimuFieldType.Number)
        }
        const fieldFormat = completeStatisticsFormat(schema.fieldFormat, statisticType)
        if (fieldFormat) {
          schema = schema.set('fieldFormat', fieldFormat)
        }
      }
...
export const getHistogramFields = (query: IMFeatureLayerQueryParams, dataSourceId: string): ImmutableObject<{ [jimuName: string]: FieldSchema }> => {
  ...
  if (originField) {
    ...
    fieldInfos.forEach(({ name, alias }) => {
      ...
      const originFieldFormat = originFieldSchema?.fieldFormat as ImmutableObject<NumberFieldFormatProperties>
      const useGrouping = originFieldFormat?.useGrouping ?? 'always'
      const maximumFractionDigits = name === HistogramCountField ? 0 : 3
      const minimumFractionDigits = name === HistogramCountField ? 0 : 3
      const fieldFormat = { ...originFieldSchema?.fieldFormat, useGrouping, maximumFractionDigits, minimumFractionDigits }
      const fieldSchema = { jimuName, type, esriType, name, alias, originFields, fieldFormat }
...
const completeScatterPlotFieldSchema = (schema: IMFieldSchema): IMFieldSchema => {
  if (schema.type !== JimuFieldType.Number) return schema

  schema = schema.set('esriType', EsriFieldType.Double)
  let fieldFormat = schema.fieldFormat as ImmutableObject<NumberFieldFormatProperties>
  if (fieldFormat) {
    fieldFormat = fieldFormat.set('maximumFractionDigits', 3)
    fieldFormat = fieldFormat.set('minimumFractionDigits', 3)
    schema = schema.set('fieldFormat', fieldFormat)
...
export const getScatterPlotFields = (query: IMFeatureLayerQueryParams, dataSourceId: string) => {
  ...
  if (outFields) {
    outFields.forEach((outField) => {
      let schema = getJimuFieldSchema(outField, dataSourceId)
      if (schema) {
        schema = completeScatterPlotFieldSchema(schema)
...
export const getSerialSplitByFields = (query: IMFeatureLayerQueryParams, dataSourceId: string, series: ImmutableArray<WebChartSeries>, splitByValues?: Array<number | string | null>): ImmutableObject<{ [jimuName: string]: FieldSchema }> => {
  let fields = Immutable({}) as ImmutableObject<{ [jimuName: string]: FieldSchema }>
      ...
      if (originField) {
        ...
        if (schema) {
          ...
          if(statisticType === StatisticType.Count) {
            ...
            schema = schema.set('esriType', EsriFieldType.Double).set('type', JimuFieldType.Number)
          }
          const fieldFormat = completeStatisticsFormat(schema.fieldFormat, statisticType)
          if (fieldFormat) {
            schema = schema.set('fieldFormat', fieldFormat)
