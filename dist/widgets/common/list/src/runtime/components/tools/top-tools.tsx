/** @jsx jsx */
import { jsx, React, hooks, dataSourceUtils, DataSourceStatus, classNames, ReactRedux, LayoutItemType, MessageManager, DataSourceFilterChangeMessage, defaultMessages as jimuCoreDefaultMessage, AppMode } from 'jimu-core'
...
const TopTools = (props: Props) => {
  ...
  const memoizedFieldsRef = useRef([] as string[])
  ...
  const memoizedFields = React.useMemo((): string[] => {
    ...
    const nextFields = Array.from(new Set(useFields)).sort()
    const prevFields = memoizedFieldsRef.current
    if (prevFields.length === nextFields.length && prevFields.every((field, index) => field === nextFields[index])) {
      return prevFields
    }
    memoizedFieldsRef.current = nextFields
    return nextFields
  }, [appConfig, id, browserSizeMode])
  ...
  const dataSets = React.useMemo(() => {
    return [{ dataSource: dataSource, records: selectRecords || [], name: dataName, type: 'selected', fields: memoizedFields }] as DataRecordSet[]
  }, [dataName, dataSource, memoizedFields, selectRecords])
