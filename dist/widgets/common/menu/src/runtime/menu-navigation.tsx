/** @jsx jsx */
import { React, jsx, type ThemeNavType, css, type ThemePaper, type ImmutableObject, type IMThemeVariables, WIDGET_PREFIX_FOR_A11Y_SKIP } from 'jimu-core'
...
export const MenuNavigation = (props: MenuNavigationProps) => {
  ...
  const a11yProps = React.useMemo(() => {
    const props = {
      role: role || (type === 'nav' ? 'navigation' : undefined),
      'aria-label': ariaLabel
    }
    if (addToA11ySkip) {
      const id = `${WIDGET_PREFIX_FOR_A11Y_SKIP}${widgetId}`
      return type === 'nav' ? { ...props, id, tabIndex: -1 } : { ...props, id }
    }
    return props
  }, [addToA11ySkip, widgetId, ariaLabel, role, type])
