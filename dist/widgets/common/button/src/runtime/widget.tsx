...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & ExtraProps, State> {
  ...
  getLinkComponent = (isNewTheme: boolean) => {
    ...
    const { id, addToA11ySkip } = this.props
    ...
    // Prefer the user-configured a11yLabel; fall back to tooltip → text → icon name.
    let accessibilityLabel = this.props.a11yLabel || this.state.toolTip || text || iconNameWithoutExt
    ...
    if (target === '_blank') {
      accessibilityLabel = `${accessibilityLabel}, ${this.props.intl.formatMessage({ id: 'openInNewWindow', defaultMessage: jimuUiDefaultMessages.openInNewWindow }) })}`
    }
    ...
    const skipTargetProps = addToA11ySkip
