...
const PrintServiceSelect = (props: Props) => {
  ...
  const selectedUtilityIdRef = useRef<string>(null)
  ...
  const handleUtilityChange = (utilities: ImmutableArray<UseUtility>) => {
    ...
    if (!utility) {
      ...
      isRemoveServiceRef.current = false
      selectedUtilityIdRef.current = utility.utilityId
  ...
  const addUseUtility = async (utility: IMUseUtility) => {
    ...
    getNewTemplateInfo(utility, config).then(newConfig => {
      if (isRemoveServiceRef.current || (utility.utilityId !== selectedUtilityIdRef.current && selectedUtilityIdRef.current)) {
        return false
      }
      toggleLoading(false)
      onSettingChange({
