...
import { PrintExtentType, ModeType, type IMPrintTemplateProperties, WKID_LINK, DATUM_TRANSFORMATION_WKID_LINK } from '../../config'
...
const CommonTemplateSetting = (props: Props) => {
  ...
  const [datumTransformationWkid, setDatumTransformationWkid] = React.useState(printTemplateProperties?.datumTransformationWkid)
  ...
  const getDatumSRLabelDynamic = React.useCallback(async (wkid: number) => {
    if (!modulesLoadedRef.current) {
      return moduleLoader.loadModule<typeof jimuCoreWkid>('jimu-core/wkid').then(module => {
        wkidUtilsRef.current = module
        modulesLoadedRef.current = true
        const { getDatumSRLabel, getSRLabel } = wkidUtilsRef.current
        return Promise.resolve((getDatumSRLabel && getDatumSRLabel(wkid)) || getSRLabel(wkid))
      })
    } else {
      const { getDatumSRLabel, getSRLabel } = wkidUtilsRef.current
      return Promise.resolve((getDatumSRLabel && getDatumSRLabel(wkid)) || getSRLabel(wkid))
  ...
  const openSettingCollapse = async (settingCollapseType: SettingCollapseType) => {
    ...
    if (settingCollapseType === SettingCollapseType.DatumTransformation && !printTemplateProperties?.datumTransformationWkidLabel && printTemplateProperties?.datumTransformationWkid) {
      const datumTransformationWkidLabel = await getDatumTransformationWkidLabel(printTemplateProperties?.datumTransformationWkid as string, getDatumSRLabelDynamic)
      setDescriptionOfDatumTransformationWkid(datumTransformationWkidLabel)
    }
  ...
  const handleDatumTransformationWKIDAccept = async (value) => {
    ...
    const wkidLabel = value ? await getDatumTransformationWkidLabel(value, getDatumSRLabelDynamic) : ''
    setDescriptionOfDatumTransformationWkid(wkidLabel)
    const newPrintTemplateProperties = printTemplateProperties.set('datumTransformationWkid', value).set('datumTransformationWkidLabel', wkidLabel)
    handleTemplatePropertyChange(newPrintTemplateProperties)
  }
  ...
  const getDatumTransformationWKIDElement = () => {
    return sanitizer.sanitize(
      nls('wellKnownId', { wkid: `<a target="_blank" href="${DATUM_TRANSFORMATION_WKID_LINK}">WKID</a>` })
    )
  }
  ...
  React.useEffect(() => {
    ...
    setDatumTransformationWkid(printTemplateProperties?.datumTransformationWkid)
    ...
    oldDatumTransformationWkid.current = printTemplateProperties?.datumTransformationWkid
    ...
    if (printTemplateProperties?.datumTransformationWkidLabel) {
      setDescriptionOfDatumTransformationWkid(printTemplateProperties?.datumTransformationWkidLabel)
    } else {
      if (wkidUtilsRef.current && printTemplateProperties?.datumTransformationWkid) {
        getDatumTransformationWkidLabel(printTemplateProperties?.datumTransformationWkid as string, getDatumSRLabelDynamic).then(label => {
          setDescriptionOfDatumTransformationWkid(label)
        })
      }
    }
  }, [getDatumSRLabelDynamic, getSRLabelDynamic, printTemplateProperties])
  ...
  return (
    <div css={STYLE}>
      <div role='group' aria-label={nls('setDefaults')}>
        ...
        <CollapsablePanel
          label={nls('printDatumTransformation')}
          isOpen={settingCollapse === SettingCollapseType.DatumTransformation}
          onRequestOpen={() => { openSettingCollapse(SettingCollapseType.DatumTransformation) }}
          onRequestClose={closeSettingCollapse}
          role='group'
          type='primary'
          aria-label={nls('printDatumTransformation')}
          rightIcon={EditIcon}
          className={settingCollapse === SettingCollapseType.DatumTransformation && 'active-collapse'}
        >
          <SettingRow flow='wrap' label={<div className='flex-grow-1' dangerouslySetInnerHTML={{ __html: getDatumTransformationWKIDElement() }}></div>} className='mt-2'>
            <TextInput
              size='sm'
              className='search-placeholder w-100'
              ...
              suffix={<Tooltip title={<div className='plain-tooltip'>
                <div>Examples:</div>
                <ul className='mb-0 pl-3'>
                  <li>Single: 1170</li>
                  <li>Multiple: 1170,1196</li>
                  <li>Composite: 1315+1138</li>
                </ul>
              </div>} id={`datumTransformationWkidRemind${id}`} describeChild enterDelay={0} aria-live='off' placement='top'>
                <Button className='wkid-info-icon p-0' icon type='tertiary' aria-label={nls('additionalInformation')} aria-describedby={`datumTransformationWkidRemind${id}`}>
                  <InfoOutlined />
                </Button>
              </Tooltip>}
            ...
            />
            <div id='print-datum-transformation-wkid-describtion'
      ...
      {modeType === ModeType.Classic && <SettingRow className='mt-4 enable-setting-con' flow='wrap' role='group' aria-label={nls('selectEditableSettings')} label={nls('selectEditableSettings')}>
        <div className='w-100'>
          ...
          <div
            title={nls('printDatumTransformation')}
            aria-label={nls('printDatumTransformation')}
            className='d-flex w-100 align-items-center check-box-con'
            onClick={() => { handleCheckBoxChange('enableDatumTransformation') }}
          >
            <Checkbox
              title={nls('printDatumTransformation')}
              className='lock-item-ratio'
              data-field='enableDatumTransformation'
              checked={printTemplateProperties?.enableDatumTransformation || false}
            />
            <div className='lock-item-ratio-label text-left ml-2'>
              {nls('printDatumTransformation')}
            </div>
          </div>
          <div
