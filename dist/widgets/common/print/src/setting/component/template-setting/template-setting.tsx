...
import * as printUtils from '../../../utils/utils'
...
const TemplateSetting = (props: Props) => {
  ...
  const handelTemplateListChange = (newTemplate: PrintTemplateProperties[]) => {
    const isCustomTemplate = printUtils.checkIsCustomTemplate(config?.printServiceType, config?.printTemplateType)
    if (isCustomTemplate) {
  ...
  const newCustomTemplate = async () => {
    let newPrintCustomTemplate = config?.printCustomTemplate?.[0]?.asMutable({ deep: true }) || {}
    const newCustomTemplate = config?.printCustomTemplate?.asMutable({ deep: true }) || []
    const serviceUrl = await printUtils.getUrlOfUseUtility(config?.useUtility?.asMutable({ deep: true }))
    const isPremiumService = checkPremiumService(serviceUrl)

    if (isPremiumService) {
      newPrintCustomTemplate = {}
    }
  ...
  return (
    <SettingSection className={className} css={STYLE} role='group' aria-label={nls('configurePrintTemplate')}>
      ...
      >
        ...
        />
        {showLoading && <div className='loading-con position-relative'><Loading useAriaLive type={LoadingType.Secondary}/></div>}
