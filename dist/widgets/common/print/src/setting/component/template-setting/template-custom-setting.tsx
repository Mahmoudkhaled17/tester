...
import { getIndexByTemplateId, checkIsCustomTemplate, checkIsMapOnly, mergeTemplateSetting, checkIsReportsTemplateAvailable, getUrlOfUseUtility } from '../../../utils/utils'
...
const CustomSetting = (props: Props) => {
  ...
  const preAvailableReportRef = useRef<AvailableReportTemplateInfo | null>(null)
  ...
  useEffect(() => {
    if (!config?.useUtility) {
      setIsPremiumService(false)
      return
    }
    getUrlOfUseUtility(config.useUtility?.asMutable({ deep: true })).then(serviceUrl => {
      setIsPremiumService(checkPremiumService(serviceUrl))
    }).catch(() => {
      setIsPremiumService(false)
    })
  }, [config?.useUtility])
  ...
  const handelCustomSettingChange = React.useCallback((key: string[], value) => {
  ...
  const checkAndResetAvailableTemplate = (index?: number) => {
    ...
    if (!isOpenRef.current && isPremiumService && !preTemplate?.customLayoutItem?.id) {
      newTemplateList.splice(itemIndex, 1)
      handelTemplateListChange(newTemplateList)
      return
    }
