/** @jsx jsx */
import { React, jsx, hooks, type AppInfo, css, DataSourceTypes, polished, type UseDataSource, Immutable } from 'jimu-core'
...
import { getNewConfigWidthNewTemplateItem, getReportTemplateTypes, checkDsIsOutputDs, checkIsReportItemAvailable, getUrlOfUseUtility } from '../../../utils/utils'
...
const ReportTemplateSetting = (props: Props) => {
  ...
  const [printServiceUrl, setPrintServiceUrl] = useState(null)
  ...
  const getReportSectionOverridesSetting = () => {
    const reportSectionOverrides = (template?.reportOptions as any)?.reportSectionOverrides || Immutable({})
    ...
    return <div className='mt-3'>
      {Object.keys(reportSectionOverrides).map((key, index) => {
        ...
        const reportItemAvailable = checkIsReportItemAvailable(reportItem)
        return (
          <SettingRow
            label={<div className='d-flex align-items-center'>
              <div className='mr-1'>{key}</div>
              {!reportItemAvailable && <Alert
