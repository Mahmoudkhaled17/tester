...
export const getNewLayoutTemplateByLayoutName = (preTemplate: PrintTemplateProperties, layoutTemplate: string, layoutChoiceList: LayoutInfo[]): PrintTemplateProperties => {
  ...
  if (!layout) return preTemplate
  delete (layout as any).label
  let newTemplate = {
    ...preTemplate,
    ...layout,
  ...
  if (isMapOnly) {
    ...
    if (newTemplate?.layoutOptions?.customTextElements) {
      delete newTemplate.layoutOptions.customTextElements
    }
    if (newTemplate?.layoutOptions?.elementOverrides) {
      delete newTemplate.layoutOptions.elementOverrides
    }
  }
