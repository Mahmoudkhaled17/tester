...
export default class AppConfigOperation implements extensionSpec.AppConfigOperationsExtension {
  ...
  utilityWillRemove (appConfig: IMAppConfig, utilityId: string): IMAppConfig {
    appConfig.widgets && Object.keys(appConfig.widgets).forEach(widgetId => {
      ...
      if (widgetJson.uri === 'widgets/common/print/') {
        if (widgetJson.config?.useUtility?.utilityId === utilityId) {
          appConfig = appConfig.setIn(['widgets', widgetId, 'config', 'useUtility'], null).setIn(['widgets', widgetId, 'useUtilities'], [])
        }
      }
    })
    return appConfig
