...
const Classic = (props: Props) => {
  ...
  const { config, jimuMapView, templateList, id, locale, useMapWidgetIds, outputDataSourceWarning, showUtilityErrorRemind, previewOverlayItem, printButtonLabel } = props
  ...
  useEffect(() => {
    if (!props.disablePrintByCreditsPermission) {
      setShowPrintPermissionRemind(false)
    }
  }, [props.disablePrintByCreditsPermission])
  ...
  */
  ...
  const getNewTemplateWithCommonSetting = (template: IMPrintTemplateProperties): IMPrintTemplateProperties => {
    ...
    if (template?.overrideCommonSetting) {
      ...
      template = template.set('datumTransformationWkid', null).set('datumTransformationWkidLabel', null)
    ...
    return(
      <div>
        ...
        {showLoading && <div className='loading-con position-relative ml-1 d-inline-block align-middle'>
          <Loading useAriaLive width={16} height={16} type={LoadingType.Donut}/>
        </div>}
      </div>
    )
  }
  ...
  return (
    <div className='w-100 h-100 d-flex flex-column' css={STYLE}>
      <div className='flex-grow-1 w-100 classic-setting-con overflow-hidden position-relative'>
        <Tabs
          value={views}
          keepMount
          onChange={toggleNav}
          ...
          <Tab innerRef={settingTabRef} id={Views.PrintTemplate} title={nls('printTemplate')} className='flex-grow-1 h-100'>
            <div className='w-100 h-100' onKeyUp={e => { handleKeyUp(e, true) }}>
              <TemplateSetting
                ...
                disablePrintByCreditsPermission={props.disablePrintByCreditsPermission}
                onPermissionDenied={() => {
