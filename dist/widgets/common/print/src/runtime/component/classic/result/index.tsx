...
const Result = (props: Props) => {
  ...
  const renderLoading = () => {
    return (<div className='print-loading-con position-relative'><Loading useAriaLive width={16} height={16} type={LoadingType.Donut}/></div>)
  }
  ...
  return (
    <div className='w-100 h-100' css={STYLE}>
      ...
      {printResultList.map((item: PrintResultListItemType, index: number) => {
        return (
          <div className='d-flex align-items-center mb-2 w-100' key={item?.resultId}>
            ...
            <ResultItemAction item={item} index={index} deleteResultItem={deleteResultItem} />
          </div>
        )
      })}
    </div>
