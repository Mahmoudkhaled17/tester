/** @jsx jsx */
import { React, jsx, css, polished, ReactRedux, type IMState, type ImmutableArray, classNames, esri, Immutable, moduleLoader, defaultMessages as jimuCoreDefaultMessage, loadArcGISJSAPIModule, hooks } from 'jimu-core'
...
import { type IMConfig, type IMPrintTemplateProperties, type PrintTemplateProperties, AuthorTextType, PrintExtentType, WKID_LINK, DATUM_TRANSFORMATION_WKID_LINK, Views, type OutputDataSourceWarningOption } from '../../../config'
import { getIndexByTemplateId, getScaleBarList, checkIsMapOnly, getKeyOfNorthArrow } from '../../../utils/utils'
import { checkIsValidDatumTransformationWkids, formatDatumTransformationLabelForDisplay, getDatumTransformationWkidLabel, validateDatumTransformationWkids } from '../../../utils/datum-transformation'
...
import ScaleBarSetting from './scale-bar-setting'
...
interface Props {
  ...
  disablePrintByCreditsPermission: boolean
...
const TemplateSetting = (props: Props) => {
  ...
  const fullNameOfUser = ReactRedux.useSelector((state: IMState) => state.portalSelf?.user?.fullName)
  const oldWkid = useRef(null)
  const oldDatumTransformationWkid = useRef(null)
  ...
  const authorTextTypeRef = useRef(null as AuthorTextType)
  const selectedTemplateId = useRef(null as string)
  ...
  `
  const { templateList, jimuMapView, selectedTemplate, id, config, outputDataSourceWarning, confirmPrint, views, updatePreviewOverlayItem, handleSelectedTemplateChange, setSelectedTemplateByIndex, printButtonLabel, disablePrintByCreditsPermission, onPermissionDenied } = props

  const [title, setTitle] = useState(selectedTemplate?.layoutOptions?.titleText || '')
  const [wkid, setWkid] = useState(selectedTemplate?.wkid || '')
  const [datumTransformationWkid, setDatumTransformationWkid] = useState(selectedTemplate?.datumTransformationWkid || '')
  ...
  const [descriptionOfDatumTransformationWkid, setDescriptionOfDatumTransformationWkid] = React.useState(null)
  const [northArrowKey, setNorthArrowKey] = useState(null)
  const formattedDescriptionOfDatumTransformationWkid = React.useMemo(() => {
    return formatDatumTransformationLabelForDisplay(descriptionOfDatumTransformationWkid)
  }, [descriptionOfDatumTransformationWkid])
  ...
  useEffect(() => {
    ...
    setDatumTransformationWkid(selectedTemplate?.datumTransformationWkid)
    ...
    oldDatumTransformationWkid.current = selectedTemplate?.datumTransformationWkid
    ...
    if (selectedTemplate?.datumTransformationWkidLabel) {
      setDescriptionOfDatumTransformationWkid(selectedTemplate?.datumTransformationWkidLabel)
    } else {
      if (wkidUtilsRef.current && selectedTemplate?.datumTransformationWkid) {
        getDatumTransformationWkidLabel(selectedTemplate?.datumTransformationWkid as string, getDatumSRLabelDynamic).then(label => {
          setDescriptionOfDatumTransformationWkid(label)
        })
      }
    }
  ...
  const initDescriptionOfDatumTransformationWkid = async () => {
    const wkidLabel = await getDatumTransformationWkidLabel(selectedTemplate?.datumTransformationWkid as string, getDatumSRLabelDynamic)
    setDescriptionOfDatumTransformationWkid(wkidLabel)
  }
  ...
  const getDatumSRLabelDynamic = React.useCallback(async (wkid: number) => {
    if (!modulesLoadedRef.current) {
      return moduleLoader.loadModule<typeof jimuCoreWkid>('jimu-core/wkid').then(module => {
        wkidUtilsRef.current = module
        modulesLoadedRef.current = true
        const { getDatumSRLabel } = wkidUtilsRef.current
        return Promise.resolve(getDatumSRLabel(wkid))
      })
    } else {
      const { getDatumSRLabel } = wkidUtilsRef.current
      return Promise.resolve(getDatumSRLabel(wkid))
  ...
  const isValidDatumWkidDynamic = async (wkid: number) => {
    if (!modulesLoadedRef.current) {
      return moduleLoader.loadModule<typeof jimuCoreWkid>('jimu-core/wkid').then(module => {
        wkidUtilsRef.current = module
        modulesLoadedRef.current = true
        const { isValidDatumWkid } = wkidUtilsRef.current
        return Promise.resolve(isValidDatumWkid(String(wkid)))
      })
    } else {
      const { isValidDatumWkid } = wkidUtilsRef.current
      return Promise.resolve(isValidDatumWkid(String(wkid)))
  ...
  const handleDatumTransformationWkidAccept = async (value) => {
    const isValid = value ? await validateDatumTransformationWkids(value, isValidDatumWkidDynamic) : true
    if (!isValid) {
      setDatumTransformationWkid(oldDatumTransformationWkid.current)
      const oldWkidLabel = oldDatumTransformationWkid.current
        ? await getDatumTransformationWkidLabel(oldDatumTransformationWkid.current, getDatumSRLabelDynamic)
        : ''
      setDescriptionOfDatumTransformationWkid(oldWkidLabel)
      return false
    }
    oldDatumTransformationWkid.current = value
    const wkidLabel = value ? await getDatumTransformationWkidLabel(value, getDatumSRLabelDynamic) : ''
    setDescriptionOfDatumTransformationWkid(wkidLabel)
    setDatumTransformationWkid(value)
    const newTemplate = selectedTemplate.set('datumTransformationWkid', value || '').set('datumTransformationWkidLabel', wkidLabel || '')
    handleSelectedTemplateChange(newTemplate)
  }

  const handleDatumTransformationWkidChange = async (event) => {
    const value = event?.target?.value
    const isValid = value ? await validateDatumTransformationWkids(value, isValidDatumWkidDynamic) : true
    if (isValid) {
      const wkidLabel = value ? await getDatumTransformationWkidLabel(value, getDatumSRLabelDynamic) : ''
      ...
      setDescriptionOfDatumTransformationWkid(nls('invalidWKID'))
    }
    setDatumTransformationWkid(value)
  }
  ...
  const handleAuthorAccept = React.useCallback((value) => {
  ...
  const checkAndInitDescriptionOfWkid = (isOpenAdvanceCollapse: boolean) => {
    ...
    const { showOutputSpatialReference, showDatumTransformation } = showList
    ...
    if (isOpenAdvanceCollapse && showDatumTransformation && !selectedTemplate?.datumTransformationWkidLabel && selectedTemplate?.datumTransformationWkid) {
      initDescriptionOfDatumTransformationWkid()
    }
  }
  ...
  const handelConfirmPrint = () => {
    if (disablePrintByCreditsPermission) {
      onPermissionDenied()
      return
    }
  ...
  const checkIsValidDatumWkid = (wkid) => {
    if (wkidUtilsRef.current) {
      const { isValidDatumWkid } = wkidUtilsRef.current
      return wkid
        ? checkIsValidDatumTransformationWkids(wkid, wkidNumber => isValidDatumWkid(String(wkidNumber)))
        : true
    } else {
      return true
    }
  ...
  useEffect(() => {
    if ((authorTextTypeRef.current !== selectedTemplate?.authorTextType || selectedTemplateId.current !== selectedTemplate?.templateId)&& selectedTemplate) {
      if (selectedTemplate?.authorTextType === AuthorTextType.UserName) {
        handleAuthorAccept(fullNameOfUser || '')
      } else {
        handleAuthorAccept(selectedTemplate?.layoutOptions?.authorText)
      }
    }
    authorTextTypeRef.current = selectedTemplate?.authorTextType
    selectedTemplateId.current = selectedTemplate?.templateId
  }, [fullNameOfUser, handleAuthorAccept, selectedTemplate])
  ...
  const renderAdvancedSetting = () => {
    ...
    const showList = getShowList()
    const { showLayoutOption, showFeatureAttribution, showMapAttribution, showQuality, showMapSize, showMapPrintExtents, showScalebarUnit, showOutputSpatialReference, showDatumTransformation } = showList
    const showAdvance = showLayoutOption || showFeatureAttribution || showMapAttribution || showQuality || showMapSize || showMapPrintExtents || showScalebarUnit || showOutputSpatialReference || showDatumTransformation
    return (
      <div>
        ...
        {isOpenCollapse && <div>
          ...
          {showDatumTransformation && <SettingRow flowWrap label={<div dangerouslySetInnerHTML={{ __html: getDatumTransformationWKIDElement() }}></div>}>
            <TextInput
              size='sm'
              className='w-100 dpi-input no-right-padding'
              value={datumTransformationWkid || ''}
              onAcceptValue={handleDatumTransformationWkidAccept}
              onChange={handleDatumTransformationWkidChange}
              placeholder={nls('wellKnownId', { wkid: 'WKID' })}
              aria-label={nls('printDatumTransformation')}
              aria-describedby='print-datum-transformation-wkid-describtion'
            />
            <div
              id='print-datum-transformation-wkid-describtion'
              title={descriptionOfDatumTransformationWkid}
              aria-label={descriptionOfDatumTransformationWkid}
              className={classNames('mt-1 wkid-describtion datum-wkid-description-wrap', { 'wkid-describtion-invalid': !checkIsValidDatumWkid(datumTransformationWkid) })}
            >
                {formattedDescriptionOfDatumTransformationWkid}
            </div>
          </SettingRow>}
  ...
  const getShowList = () => {
    ...
    return {
      ...
      showDatumTransformation: selectedTemplate?.enableDatumTransformation,
  ...
  const renderLayoutOptionSetting = () => {
    const showList = getShowList()
    const { showAuthor, showCopyright, showLegend, showFeatureAttribution, showQuality, showScalebarUnit, showNorthArrow } = showList
    return (
      <div role='group' aria-label={nls('LayoutOptions')}>
        ...
        <ScaleBarSetting
          ...
          handleSelectedTemplateSettingChange={handleSelectedTemplateSettingChange}
        />
  ...
  const getDatumTransformationWKIDElement = () => {
    return sanitizer.sanitize(
      `${nls('printDatumTransformation')} <a target="_blank" href="${DATUM_TRANSFORMATION_WKID_LINK}">WKID</a>`
    )
  }
    ...
    <div className={classNames('w-100 h-100', { invisible: (views !== Views.PrintTemplate) })} css={STYLE}>
      <div className='w-100 h-100 d-flex flex-column'>
        ...
        <div className='print-button-con'>
          ...
          />
          <Button className='w-100 mt-2' type='primary' onClick={handelConfirmPrint} title={printButtonLabel} aria-label={printButtonLabel}>{printButtonLabel}</Button>
        </div>
      </div>
    </div>
  )
