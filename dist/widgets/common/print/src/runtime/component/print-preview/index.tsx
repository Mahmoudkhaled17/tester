...
const PrintPreview = (props: Props) => {
  ...
  const loadEsriModules = useCallback(async () => {
    if (!unitUtilsRef.current) {
      unitUtilsRef.current = await loadArcGISJSAPIModule('esri/core/units')
    }
  ...
  */
  ...
  const updateOverlayItem = useCallback((updateOverlayItem?: boolean): void => {
    ...
    const width = overlayItemSize?.width
    const height = overlayItemSize?.height
  ...
  useEffect(() => {
    if (!config?.enablePreview && showPrintArea) {
      setShowPrintArea(false)
      showPrintAreaRef.current = false
      removePrintPreviewArea()
    }
  }, [config?.enablePreview, removePrintPreviewArea, showPrintArea])
