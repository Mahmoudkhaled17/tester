...
const CompactPrint = (props: Props) => {
  const { config, templateList, jimuMapView, errorTip, id, locale, useMapWidgetIds, outputDataSourceWarning, previewOverlayItem, showPlaceholder, showUtilityErrorRemind, controllerWidgetId, printButtonLabel, disablePrintByCreditsPermission } = props
  ...
  const openPopperRef = useRef(false)
  const canClosePopperRef = useRef(true)
  ...
  const [showPrintPermissionRemind, setShowPrintPermissionRemind] = useState(false)
  ...
  const togglePopper = useCallback((e, type?: any) => {
    const newOpenPopper = type === 'clickOutside' ? false : !openPopperRef.current
    if (!newOpenPopper && !canClosePopperRef.current) {
      return
    }
    openPopperRef.current = newOpenPopper
  ...
  const handleCanClosePopperChange = useCallback((canClose: boolean) => {
    canClosePopperRef.current = canClose
  }, [])
  ...
  const handleConfirmPrint = () => {
    if (disablePrintByCreditsPermission) {
      setShowPrintPermissionRemind(true)
      toggleUtilityErrorRemind(true)
      return
    }

    setShowPrintPermissionRemind(false)
    void confirmPrint()
  }
  ...
  }, [disablePrintByCreditsPermission])
    ...
    return (
      <div className='d-flex flex-column'>
        ...
        <div className='print-button-con d-flex align-items-center mt-2'>
          ...
          />
          <Button className='print-button text-truncate ml-1' type='primary' onClick={handleConfirmPrint} title={printButtonLabel} aria-label={printButtonLabel}>{printButtonLabel}</Button>
        </div>
      </div>
    )
  }
      ...
      <div className={classNames('w-100 h-100', { 'operate-con-width-controller-widget': controllerWidgetId })}>
        {!showPlaceholder && <div className='d-flex flex-column w-100 h-100'>
          ...
          <div className='flex-grow-1'>
            ...
            {showResult && <PrintResult printResult={printResult} restPrint={restPrint} useUtility={config?.useUtility} onCanClosePopperChange={handleCanClosePopperChange}/>}
          </div>
        </div>}
        {showPlaceholder && <div>{errorTip}</div>}
        <UtilityErrorRemind
          utilityId={config?.useUtility?.utilityId}
          show={showUtilityErrorRemind}
          showPrintPermissionRemind={showPrintPermissionRemind}
          toggleUtilityErrorRemind={toggleUtilityErrorRemind}
          onClose={() => {
            setShowPrintPermissionRemind(false)
          }}
  ...
  return (
    <div className='w-100 h-100' css={STYLE}>
      ...
      {!controllerWidgetId && <div className='w-100 h-100'>
        <Button
          className={classNames('w-100 h-100 compact-con', { 'off-panel-con': widgetJson?.inControllerUx === 'offPanel' })}
          type='tertiary'
          ref={printButtonRef}
