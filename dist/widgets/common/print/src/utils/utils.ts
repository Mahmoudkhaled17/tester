...
async function getLayoutTempInfoTaskUrlOfPremiumService (premiumServiceUrl: string) {
  ...
  const layoutTemplatesInfoTaskUrl = `${templateTaskServiceUrl}/${layoutTemplateTaskName}`
  const layoutTemplatesInfoRequestUrl = `${layoutTemplatesInfoTaskUrl}/${supportedOperations}`
  return {
    layoutTemplatesInfoTaskUrl: layoutTemplatesInfoTaskUrl,
    layoutTemplatesInfoRequestUrl: layoutTemplatesInfoRequestUrl,
    executionType: executionType
...
export function getElementOverridesOptions (elementOverrides: ElementOverrides): ElementOverrideOptions {
  ...
  for (const key in elementOverrides) {
    const info = {
      ...elementOverrides[key],
      name: key,
      translatedName: elementOverrides[key].name
    }
    ...
    switch (info.type) {
      ...
      case 'CIMGraphicElement': {
        if (info.dynamicTextElements?.some((element) => element.type === 'table')) {
...
export function getTranslationNameForElementOverridesOption (elementOverridesOption, nls, isUseInclude = false): string {
  switch (elementOverridesOption.type) {
    case 'CIMMarkerNorthArrow': {
      return isUseInclude ? nls('includeNorthArrow') : nls('northArrow')
    }
    case 'CIMScaleLine': {
      if (elementOverridesOption?.translatedName?.toLowerCase() === 'scale bar') {
        return isUseInclude ? nls('includeScaleBar') : nls('ScaleBarLabel')
      } else {
        return isUseInclude ? nls('includeElementOverride', { name: elementOverridesOption.translatedName }) : elementOverridesOption.translatedName
      }
    }
    case 'CIMGroupElement': {
      if (elementOverridesOption.elements?.some((element) => element.type === 'CIMScaleLine') && elementOverridesOption?.translatedName?.toLowerCase() === 'scale bar') {
        return isUseInclude ? nls('includeScaleBar') : nls('ScaleBarLabel')
      } else {
        return isUseInclude ? nls('includeElementOverride', { name: elementOverridesOption.translatedName }) : elementOverridesOption.translatedName
      }
...
export function checkIsTemplateAvailable (template: PrintTemplateProperties, config: IMConfig): boolean {
  ...
  const customLayoutAvailable = template?.layoutTypes !== LayoutTypes.CustomLayout || !!(template?.layoutTypes === LayoutTypes.CustomLayout && !!template?.customLayoutItem?.id)
  return reportsAvailable && elementOverridesAvailable && customLayoutAvailable
}
...
function checkReportOption (reportOptions): boolean {
  ...
  Object.keys(reportSectionOverrides).forEach((key, index) => {
    const reportItem = reportSectionOverrides[key]
    available = checkIsReportItemAvailable(reportItem)
  })
  return available
}

export function checkIsReportItemAvailable (reportItem): boolean {
  let available = true
...
export const initDefaultTemplates = (orgTemplate: PrintTemplateProperties[], printServiceType: PrintServiceType, printTemplateType: PrintTemplateType) => {
  const newTemplates = orgTemplate.map(template => {
    ...
    newTemplate = newTemplate.setIn(['layoutOptions', 'scalebarUnit'], null).setIn(['layoutOptions', 'legendLayers'], null)
