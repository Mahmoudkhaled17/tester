import { UtilityManager, ServiceManager, Immutable, SessionManager, getAppStore, esri } from 'jimu-core'
import type { IMUseUtility, UseUtility } from 'jimu-core'
import { PrintServiceType, CIMMarkerNorthArrow, DEFAULT_TEMPLATE_INFO, PrintTemplateType, ItemInfoType, ReportTemplateTypes, ReportTypes, LayoutTypes, DEFAULT_COMMON_SETTING, AuthorTextType } from '../config'
...
import { searchItemByPortalUrl } from './search-item-util'
import { checkPremiumService } from './premium-service-util'
...
export const getNewTemplateInfo = async (utility: IMUseUtility, config: IMConfig) => {
  return getUrlOfUseUtility(utility).then(url => {
    if (checkPremiumService(url)) {
      return getPrintTemplateByLayoutGroup(url, utility, config)
    }
    return getPrintTemplate(url, utility, config)
  })
}

interface PortalSearchResultItem {
  id: string
  title: string
}

const parseLayoutGroupId = (layoutGroupQuery: string): string => {
  if (!layoutGroupQuery) return ''
  const match = layoutGroupQuery.match(/id\s*:\s*([^\s]+)/i)
  return match?.[1] || ''
}

const getLayoutTemplateItemsByGroup = async (portalUrl: string): Promise<PortalSearchResultItem[]> => {
  const layoutGroupQuery = getAppStore().getState().portalSelf?.layoutGroupQuery as string
  const groupId = parseLayoutGroupId(layoutGroupQuery)
  if (!portalUrl || !groupId) return []

  const session = SessionManager.getInstance().getSessionByUrl(portalUrl)
  const query = new esri.restPortal.SearchQueryBuilder()
    .match(groupId).in('group')
    .and()
    .match('Layout').in('type')
    .toParam()
  const res = await searchItemByPortalUrl({
    portalUrl,
    start: 1,
    num: 20,
    query,
    ignoreCurrentPortalCheck: true,
    authentication: session
  })

  return (res?.results || []) as PortalSearchResultItem[]
}

const initTemplateByLayoutGroupItems = async (
  serviceUrl: string,
  utility: IMUseUtility,
  config: IMConfig,
  printTask: any
): Promise<IMConfig> => {
  const getConfigWithoutTemplate = (baseConfig: IMConfig) => {
    return baseConfig
      .set('printCustomTemplate', [])
      .set('layoutChoiceList', [])
      .set('reportTemplateChoiceList', [])
  ...
  const portalUrl = await getPortalUrlByUtility(utility)
  const layoutItems = await getLayoutTemplateItemsByGroup(portalUrl)
  if (!layoutItems || layoutItems.length === 0) {
    return getConfigWithoutTemplate(config)
  }

  const layoutTemplateInfoListPromise = layoutItems.map(item => {
    return getTemplateOrReportInfo(serviceUrl, ItemInfoType.LayoutTemplate, item.id).then(res => {
      const templateInfo = res?.[0]
      if (!templateInfo) return null
      const layoutName = item.title || templateInfo?.layoutTemplate
      return {
        templateInfo,
        taskTemplate: {
          label: layoutName,
          layout: layoutName,
          format: printTask?.defaultFormat || 'pdf'
        }
      }
    }).catch(() => null)
  })

  const layoutTemplateInfos = (await Promise.all(layoutTemplateInfoListPromise)).filter(item => !!item)
  if (layoutTemplateInfos.length === 0) {
    return getConfigWithoutTemplate(config)
  }

  const templateTaskInfo = layoutTemplateInfos.map(item => item.templateInfo)
  const templatesInTask = layoutTemplateInfos.map(item => item.taskTemplate as PrintTemplateProperties)
  const defaultLayoutOptionInfo = {
  ...
  const options = {
    ...
    templatesInTask,
    reportOption: null,
    defaultLayoutOptionInfo,
    printTemplateType: config?.printTemplateType,
    printServiceType: getPrintServiceType(utility?.utilityId)
  }

  const template = initPrintTemplateList(options)?.map((templateItem, index) => {
    const layoutItem = layoutItems?.[index]
    return {
      ...templateItem,
      layoutTypes: LayoutTypes.CustomLayout,
      customLayoutItem: layoutItem ? { id: layoutItem.id, title: layoutItem.title } : templateItem.customLayoutItem
    }
  })
  const layoutChoiceList = getLayoutChoiceList(templatesInTask, template)

  const newConfig = config.set('printCustomTemplate', template)
    .set('useUtility', utility)
    .set('formatList', printTask?.formatList)
    .set('defaultFormat', printTask?.defaultFormat)
...
const getPrintTemplateByLayoutGroup = async (serviceUrl: string, utility: IMUseUtility, config: IMConfig): Promise<IMConfig> => {
  const printTask = await getPrintTaskInfo(serviceUrl)
  return initTemplateByLayoutGroupItems(serviceUrl, utility, config, printTask)
}
...
export async function initNewTemplateItem (config: IMConfig, template: PrintTemplateProperties, newTemplateId: string, defaultLabel: string): Promise<PrintTemplateProperties> {
  ...
  let isPremiumService = false
  if (config?.useUtility) {
    const serviceUrl = await getUrlOfUseUtility(config.useUtility?.asMutable({ deep: true }))
    isPremiumService = checkPremiumService(serviceUrl)
  }

  if (isPremiumService) {
    newTemplate.layoutTypes = LayoutTypes.CustomLayout
  } else {
  ...
  newTemplate.authorTextType = AuthorTextType.Custom
...
const initTemplateByLayoutOptionsInfo = (temp, layoutOptions) => {
  ...
  if (!temp.layoutOptions?.scalebarUnit) {
    temp.layoutOptions.scalebarUnit = null
  }
  return temp
}
