...
import {
  ...
  type IconResult, type IMLinkParam, uuidv1, OpenTypes, urlUtils, getAppStore, type LinkParam, type ImmutableObject
} from 'jimu-core'
...
import { TextInput, UrlInput, Button, Select, Tabs, Tab, Switch, defaultMessages as jimuUIMessages, defaultMessages as jimuUiDefaultMessages, Radio, Label, CollapsablePanel, Checkbox } from 'jimu-ui'
...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<IMConfig>, State> {
  ...
  onLogoutAllResourcesChange = (logoutAllResources) => {
    this.props.onSettingChange({
      id: this.props.id,
      config: this.props.savedConfig.setIn(['functionConfig', 'loginOptions', 'logoutAllResources'], logoutAllResources)
    })
  }
  ...
  getBasicContent = (): React.ReactElement => {
    ...
    return (
      <SettingSection title={this.i18nMessage('loginType')}>
        <SettingRow>
          <Radio id="usePopupLogin" style={{ cursor: 'pointer' }} name="login-type"
        ...
        <SettingRow>
          <Radio id="redirectLogin" style={{ cursor: 'pointer' }} name="login-type"
      ...
      <SettingSection className='px-14' aria-label={this.i18nMessage('appearance')}>
        <CollapsablePanel className='' level={1} type='default' label={this.i18nMessage('appearance')} role='group' defaultIsOpen={false} >
          <Tabs className='mt-4' fill type='pills' onChange={this.onAdvanceTabSelect} defaultValue={SettingTabs.regular}>
            <Tab className='tab-label' id={SettingTabs.regular} title={this.i18nMessage('default')}>
              ...
              />
              <SettingRow className='dropdown-menu-label' label={this.i18nMessage('dropdownMenu')} />
            ...
            <Tab className='tab-label' id={SettingTabs.hover} title={this.i18nMessage('hover')}>
              <AdvanceStyleSetting intl={this.props.intl} appTheme={this.props.theme2}
  ...
  getRedirectUrl = (linkParam: ImmutableObject<LinkParam>): string => {
    let redirectUrl = ''
    if (!linkParam) {
      return redirectUrl
    }
    const queryObject = getAppStore().getState().appStateInBuilder?.queryObject
    const href = urlUtils.getHrefFromLinkTo(linkParam, queryObject.without('draft'), window._appWindow.location)
    if (href && linkParam.linkType === 'WEB_ADDRESS') {
      redirectUrl = href
    } else if (href) {
      redirectUrl = `${window.location.origin}${href}`
    }
    return redirectUrl
  }

  getOptionsContent = (): React.ReactElement => {
    const loginOptions = this.props.config.functionConfig.loginOptions
    const useAdvanceLogin = loginOptions.useAdvanceLogin
    const logoutAllResources = loginOptions.logoutAllResources
    const afterLoginLinkParam = this.props.config.functionConfig.afterLoginLinkParam
    const redirectUrlAfterLogin = this.getRedirectUrl(afterLoginLinkParam)
    const afterLogoutLinkParam = this.props.config.functionConfig.afterLogoutLinkParam
    const redirectUrlAfterLogout = this.getRedirectUrl(afterLogoutLinkParam)
    return (
      <SettingSection title={this.i18nMessage('postSignInSetting')} aria-label={this.i18nMessage('postSignInSetting')} role='group'>
        <label id='link-label-1' className='second-header mt-4'>{this.i18nMessage('redirectAfterLogin')}</label>
        <SettingRow role='group' aria-label={this.i18nMessage('redirectAfterLogin') + this.i18nMessage('postSignInSetting')}>
          <LinkSelector
        ...
        {redirectUrlAfterLogin && <SettingRow className='redirect-url mt-1' label={redirectUrlAfterLogin} truncateLabel />}

        <label className='second-header mt-4'>{this.i18nMessage('redirectAfterLogout')}</label>
        <SettingRow role='group' aria-label={this.i18nMessage('redirectAfterLogout') + this.i18nMessage('postSignInSetting')} >
          <LinkSelector
        ...
        {redirectUrlAfterLogout && <SettingRow className='redirect-url mt-1' label={redirectUrlAfterLogout} truncateLabel />}

        <SettingRow className='mt-6' label={this.i18nMessage('onSignInButtonClick')} truncateLabel />
          <SettingRow>
            <Radio id="simpleLogin" style={{ cursor: 'pointer' }} name="login-mode"
          ...
          <SettingRow>
            <Radio id="advanceLogin" style={{ cursor: 'pointer' }} name="login-mode"
          ...
          <SettingRow>
            <Checkbox
              id="logoutAllResources"
              checked={logoutAllResources}
              onChange={(evt) => {
                this.onLogoutAllResourcesChange(evt.target.checked)
              }}
            />
            <Label className='ml-2' for='logoutAllResources' style={{cursor: 'pointer'}}>
              {this.i18nMessage('signOutAllResources')}
            </Label>
          </SettingRow>
