...
const HeaderItem = ({config}): React.ReactElement => {
  ...
  return ( (loginOptions.userAvatar || loginOptions.username) &&
    <div>
      <DropdownItem tabIndex={0} className='login-dropdown-item header-item p-4' tag='div'>
        <div className='d-flex user-info-content'>
          {loginOptions.userAvatar && <div className='header-portrait'>
            {getIconContent}
          </div>}
          <div className='user-title-content flex-grow-1'>
            {loginOptions.username && session && <div className='user-title main-title' title={profileName}>{profileName}</div>}
            {loginOptions.username && session && <div className='user-title sub-title' title={username}>{username}</div>}
          </div>
        </div>
      </DropdownItem>
...
const FooterItem = ({ intl, config, appMode, closeCallback }): React.ReactElement => {
  ...
  const onSignOutClick = useCallback(() => {
    signOut(config.functionConfig.afterLogoutLinkParam, config.functionConfig?.loginOptions.logoutAllResources)
  }, [config.functionConfig.afterLogoutLinkParam, config.functionConfig?.loginOptions.logoutAllResources])
...
const LoginPopper = styled(Popper)<StyleState<any>>(({ theme, styleState }) => {
  ...
  const regularFontSize = styleState?.dropdownRegularStyle?.fontSize
  const defaultRegularMainTitle = {fontSize: regularFontSize || 'var(--sys-typography-title1-font-size)', fontWeight: '600'}
  const defaultRegularUserName = {fontSize: regularFontSize || 'var(--sys-typography-label3-font-size)'}
  const defaultRegularStyle = {fontSize: 'var(--sys-typography-label1-font-size)', '.main-title': defaultRegularMainTitle, '.user-name': defaultRegularUserName}
  ...
  //}
  const regularStyle = {...defaultRegularStyle, ...styleState?.dropdownRegularStyle, ...{'button, a': dropdownRegularStyle}}
  const hoverStyle = {...(styleState?.dropdownHoverStyle || {}), ...{'button, a, div': dropdownHoverStyle}}
  return {
    ...regularStyle,
    ...{'&:hover': hoverStyle}
  }
...
const LoginDropDown = (props: LoginDropDownProps ): React.ReactElement => {
  ...
  const [denyOpenDropdown, setDenyOpenDropdown] = useState(true)
  ...
  useEffect(() => {
    if (useAdvanceLogin && !denyOpenDropdown) {
      setOpen(true)
    } else {
      setOpen(false)
      if (!useAdvanceLogin && denyOpenDropdown) {
        setDenyOpenDropdown(false)
      }
    }
  }, [useAdvanceLogin, denyOpenDropdown])
  ...
  return (
    <LoginPopper
      ...
      <Paper className='login-dropdown-box'>
        ...
        <FooterItem intl={intl} config={config} appMode={appMode} closeCallback={closeCallback} />
      </Paper>
