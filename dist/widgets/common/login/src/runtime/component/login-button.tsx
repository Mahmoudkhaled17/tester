...
const LoginButton = (props: LoginButtonProps): React.ReactElement => {
  ...
  const onLoginButtonClick = useCallback(() => {
    if (useAdvanceLogin) {
      ...
      isSigned ? signOut(functionConfig.afterLogoutLinkParam, functionConfig?.loginOptions.logoutAllResources) : signIn(usePopupLogin, functionConfig.afterLoginLinkParam)
    }
  ...
  const buttonLabel = useMemo(() => {
    ...
    if (useAdvanceLogin) {
      const profileName = (user?.firstName || user?.lastName) ? `${user?.firstName || ''} ${user?.lastName || ''}` : ''
      const username = profileName || user?.username || ''
      return (isSigned && username) ? username : signInStr
    } else {
  ...
  const ariaLabel = useMemo(() => {
    const usernameStr = intl.formatMessage({ id: 'userName', defaultMessage: 'Username'})
    if (useAdvanceLogin) {
      return `${usernameStr} ${buttonLabel}`
    } else {
      return ''
    }
  }, [useAdvanceLogin, intl, buttonLabel])
  ...
  return (
    <div className='login-button-box w-100 h-100'>
      ...
      type={functionConfig.quickStyleMode === QuickStyleMode.linkLabelOnly ? 'link' : 'default'}
      size='sm'
      icon={false}
