...
export const supportedItemTypeCategories = Immutable([
  ItemTypeCategory.KnowledgeGraph,
    ...
    <div className='widget-setting-add-data jimu-widget-setting' css={style}>
      <SettingSection className='border-0 way-of-add-data-section' role='group' title={translate('wayOfAddingData')} aria-label={translate('wayOfAddingData')}>
        ...
        {!config.disableAddBySearch && itemCategoriesInfo && <SettingRow className='account-config-row' label={translate('tabs')} aria-label={translate('tabs')} role='group' flow='wrap'>
          {/*
            Keep hidden categories (e.g. Organization content) in config order, but omit them from the UI list.
            Index-based updates should map back to the full itemCategoriesInfo list.
          */}
          <List
            className='search-config-list w-100'
            size='sm'
            // The organization content should not be shown and sorted in setting.
            itemsJson={itemCategoriesInfo.asMutable().filter((item) => !needHideItemCategories.includes(item.type) && item.type !== ItemCategory.OrganizationContent).map((i, x) => ({ itemStateDetailContent: i, itemKey: i.id }))}
            dndEnabled
            onUpdateItem={(actionData, refComponent) => {
              ...
              const isVisibleItem = (item: ItemCategoryInfo) => !needHideItemCategories.includes(item.type) && item.type !== ItemCategory.OrganizationContent
              const visibleItems = itemCategoriesInfo.filter(isVisibleItem)
              if (newSortData.length !== visibleItems.length) {
                return
              }
              const reorderedAll: ItemCategoryInfo[] = []
              let visibleIndex = 0
              itemCategoriesInfo.forEach((item) => {
                if (isVisibleItem(item)) {
                  reorderedAll.push(newSortData[visibleIndex++])
                } else {
                  reorderedAll.push(item)
                }
              })
              if (reorderedAll.map((item) => item.id).join(',') !== itemCategoriesInfo.map((item) => item.id).join(',')) {
                onSettingChange({
                  id,
                  config: {
                    ...config,
                    itemCategoriesInfo: reorderedAll.map((item) => Immutable(item))
                  }
                })
              }
            }}
            ...
            renderOverrideItemMainLine={(actionData, refComponent) => {
              ...
              const currentItem = currentItemJson.itemStateDetailContent as ItemCategoryInfo
              const realIndex = itemCategoriesInfo.findIndex((item) => item.id === currentItem.id)
              return createListItemElement(currentItem, realIndex)
            }}
          ></List>
        </SettingRow>}
