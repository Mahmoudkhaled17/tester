...
export const addDataSupportedItemTypes = Immutable([
  JimuSupportedItemTypes.FeatureService,
  JimuSupportedItemTypes.MapService,
  JimuSupportedItemTypes.SceneService,
  JimuSupportedItemTypes.KnowledgeGraphLayer,
  JimuSupportedItemTypes.FeatureCollection,
  JimuSupportedItemTypes.ImageService,
  JimuSupportedItemTypes.VectorTileService,
  JimuSupportedItemTypes.CSV,
  JimuSupportedItemTypes.GeoJSON,
  JimuSupportedItemTypes.KML,
  JimuSupportedItemTypes.WFS,
  JimuSupportedItemTypes.WMS,
  JimuSupportedItemTypes.WMTS,
  JimuSupportedItemTypes.GroupLayer
])
...
export const DataItemSearch = (props: DataItemSearchProps) => {
  ...
  return <div className={`data-item-search w-100 h-100 ${className}`} css={style}>
    <ItemSelector
      mode={ItemSelectorMode.Simple} itemType={addDataSupportedItemTypes} itemTypeCategory={displayedItemTypeCategories} portalUrl={portalUrl} isMultiple={true}
...
export async function getDsJsonFromItem (dsId: string, item: IItemWithPortalUrl): Promise<DataSourceJson> {
  try {
    if (!item) {
