...
import { applyWmtsSelectionToDataSourceJson, type WmtsSelectionOptions } from '../../wmts-utils'
...
import { WmtsLayerPopper } from './wmts-layer-popper'
...
export const DataUrlInput = (props: UrlInputProps) => {
  ...
  const [isWmtsPopperOpen, setIsWmtsPopperOpen] = useState<boolean>(false)
  ...
  useEffect(() => {
    ...
    if (selectedUrlType !== SupportedUrlTypes.WMTS && isWmtsPopperOpen) {
      setIsWmtsPopperOpen(false)
    }
  }, [selectedUrlType, isWfsPopperOpen, isWmsPopperOpen, isWmtsPopperOpen])

  useEffect(() => {
    const currentUrl = urlResult?.value
    if ((isWfsPopperOpen || isWmsPopperOpen || isWmtsPopperOpen) && urlValueRef.current && currentUrl !== urlValueRef.current) {
      setIsWfsPopperOpen(false)
      setIsWmsPopperOpen(false)
      setIsWmtsPopperOpen(false)
    }
    urlValueRef.current = currentUrl
  }, [urlResult?.value, isWfsPopperOpen, isWmsPopperOpen, isWmtsPopperOpen])
  ...
  const openWmtsPopper = useCallback(() => {
    setIsWmtsPopperOpen(true)
  }, [])

  const closeWmtsPopper = useCallback(() => {
    setIsWmtsPopperOpen(false)
  }, [])
  ...
  const onAdd = async () => {
    ...
    if (selectedUrlType === SupportedUrlTypes.WMTS) {
      openWmtsPopper()
      return
    }
  ...
  const onConfirmWmts = async (options: WmtsSelectionOptions) => {
    const normalizedUrl = normalizeUrlInput(urlResult?.value)
    if (!normalizedUrl || !options?.wmtsLayerId || !options?.wmtsTileMatrixSetId) {
      return
    }
    try {
      setIsLoading(true)
  ...
  return <div className={`data-url-input w-100 h-100 p-4 ${className}`} css={style}>
    ...
    <div className='mt-4'>
      <Button onClick={onAdd} type='primary' disabled={!urlResult.value || !urlResult.valid || isWfsPopperOpen || isWmsPopperOpen || isWmtsPopperOpen} className='px-4 w-100' title={translate('add')} aria-label={translate('add')} ref={addButtonRef}>
        {translate('add')}
      </Button>
    </div>
    ...
    />
    <WmtsLayerPopper
      open={isWmtsPopperOpen}
      url={normalizedUrlValue}
      reference={popperAnchorRef}
      translate={translate}
      onConfirm={onConfirmWmts}
      onClose={closeWmtsPopper}
      onError={setErrorMsg}
      failedToFetchMessage={translate(UrlError.FailedToFetch)}
    />
...
export async function getDsJsonFromUrl (
  dsId: string,
  url: string,
  urlType: SupportedUrlTypes,
  options?: { layerName?: string, maxRecordCount?: number, query?: { id: string, label: string, maximum?: number }, layerId?: string } & Partial<WmtsSelectionOptions>
  ...
  if (Object.keys(NonArcGISServiceUrlTypeToDsType).some(nonArcGISServiceUrlType => nonArcGISServiceUrlType === urlType)) {
    ...
    return urlType === SupportedUrlTypes.WMTS && options?.wmtsLayerId && options?.wmtsTileMatrixSetId
      ? applyWmtsSelectionToDataSourceJson(dsJson, options as WmtsSelectionOptions)
      : dsJson
