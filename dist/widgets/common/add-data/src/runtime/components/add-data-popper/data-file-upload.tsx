...
import { shouldStopUploadForCorruptedShapefile } from './shapefile-analyze-utils'
...
enum UploadFileError {
  ...
  CorruptedShapefile = 'corruptedShapefileError',
...
const UploadFileErrorToMultiFilesError = new Map([
  ...
  [UploadFileError.CorruptedShapefile, 'multiFilesCorruptedShapefileError']
])
...
export const DataFileUpload = (props: DataFileUploadProps) => {
  ...
  const handleUploadError = (err: any) => {
    ...
    } else if (err.message === UploadFileError.CorruptedShapefile) {
      return UploadFileError.CorruptedShapefile
    } else {
...
async function generateFeatureCollection (fileInfo: FileInfo, portalUrl: string): Promise<FeatureCollection | KmlServiceResponse> {
  ...
  if (fileInfo.type !== SupportedFileTypes.GPX) {
    ...
    if (isPortal && fileInfo.type === SupportedFileTypes.GeoJson) {
      ...
      let analyzeResponse: any
      try {
        analyzeResponse = await esriRequest(analyzeUrl, {
          body: fileInfo.data,
          method: 'post'
        })
      } finally {
      ...
      if (shouldStopUploadForCorruptedShapefile(fileInfo.type === SupportedFileTypes.Shapefile, analyzeResponse?.data)) {
        throw new Error(UploadFileError.CorruptedShapefile)
      }
