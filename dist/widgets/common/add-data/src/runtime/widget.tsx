/** @jsx jsx */
import { React, jsx, css, type AllWidgetProps, MutableStoreManager, hooks, ReactRedux, type IMState, WidgetState, indexedDBUtils, DataSourcesChangeType, type Size } from 'jimu-core'
...
const Widget = (props: AllWidgetProps<IMConfig>) => {
  const { portalUrl, id, enableDataAction = true, config, mutableStateProps, builderSupportModules } = props
  ...
  const handlePanelResizeStop = useCallback((size: Size) => {
    if (!window.jimuConfig.isInBuilder || !getAppConfigAction || !size) return
    const currentSize = config?.panelSize
    if (currentSize?.width === size.width && currentSize?.height === size.height) return
    const newConfig = config.set('panelSize', size)
    getAppConfigAction().editWidgetConfig(id, newConfig).exec()
  }, [config, getAppConfigAction, id])
      ...
      }
      {
        ...
        <div className='w-100 h-100 p-4'>
          ...
          <div className='w-100 d-flex justify-content-end add-by-search-samll'>
            <AddDataPopper buttonSize='sm' portalUrl={portalUrl} widgetId={id} onFinish={onAddData} hiddenTabs={hiddenAddDataTabs} popperReference={rootDomRef} nextOrder={nextOrder} config={config} itemCategoriesInfo={itemCategoriesInfo} hidePopper={hideAddDataPopper} displayedItemTypeCategories={config.displayedItemTypeCategories} panelSize={config?.panelSize} onResizeStop={handlePanelResizeStop} />
          </div>
        </div>
      }
    </Paper>
