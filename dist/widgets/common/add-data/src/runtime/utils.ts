...
export async function createDataSourcesByDataOptions (multiDataOptions: DataOptions[], widgetId: string, config: IMConfig, publishMessage = true): Promise<DataSource[]> {
  ...
  let WMTSLayer: typeof __esri.WMTSLayer
  ...
  if (dataOptions.some(o => hasWmtsSelection(o.dataSourceJson))) {
    WMTSLayer = await loadArcGISJSAPIModule('esri/layers/WMTSLayer') as typeof __esri.WMTSLayer
  }
  ...
  const dataSourceConstructorOptions: DataSourceConstructorOptions[] = dataOptions.map(o => {
    ...
    const configuredWmtsLayer = WMTSLayer && hasWmtsSelection(strippedJson)
      ? createConfiguredWmtsLayer(WMTSLayer, strippedJson)
      : null
    return ({
      id: strippedJson.id,
      dataSourceJson: Immutable(strippedJson),
      itemData: extractedItemData,
      layer: configuredWmtsLayer
