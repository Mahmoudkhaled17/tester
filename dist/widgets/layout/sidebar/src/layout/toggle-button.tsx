...
export class SidebarController extends React.PureComponent<SidebarControllerProps> {
  ...
  render (): React.JSX.Element {
    const { icon, iconSize, iconSource, style, expanded, shouldFlip, color, showAsExpressTip, collapseTooltip, expandTooltip, onClick } = this.props
    const rotate = shouldFlip ? 180 : 0
    let title
    if (expanded) {
      title = collapseTooltip ||i18n.getIntl().formatMessage({ id: 'collapse' })
    } else {
      title = expandTooltip ||i18n.getIntl().formatMessage({ id: 'expand' })
    }
