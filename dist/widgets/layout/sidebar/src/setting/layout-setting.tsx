...
import { Switch, Select, DistanceUnits, type LinearUnit, Icon, Button, NumericInput, TextInput, Collapse, utils as uiUtils, Label } from 'jimu-ui'
...
class Setting extends React.PureComponent<Props & StateToProps> {
  ...
  updateCollapseTooltip = (value: string) => {
    this.props.onSettingChange({
      id: this.props.widgetId,
      config: this.props.config.set('collapseTooltip', value)
    })
  }

  updateExpandTooltip = (value: string) => {
    this.props.onSettingChange({
      id: this.props.widgetId,
      config: this.props.config.set('expandTooltip', value)
    })
  }
      ...
      <div className='hsidebar-layout-setting' css={this.getStyle()}>
        ...
        >
          <Collapse isOpen={config.toggleBtn.visible}>
            ...
            <SettingRow flow='wrap' label={formatMessage('collapseTooltip')}>
              <TextInput
                size='sm'
                aria-label={formatMessage('collapseTooltip')}
                className='w-100'
                placeholder={formatMessage('collapse')}
                defaultValue={config.collapseTooltip || ''}
                onAcceptValue={this.updateCollapseTooltip}
              />
            </SettingRow>
            <SettingRow flow='wrap' label={formatMessage('expandTooltip')}>
              <TextInput
                size='sm'
                aria-label={formatMessage('expandTooltip')}
                className='w-100'
                placeholder={formatMessage('expand')}
                defaultValue={config.expandTooltip || ''}
                onAcceptValue={this.updateExpandTooltip}
              />
            </SettingRow>
          </Collapse>
        </SettingSection>
