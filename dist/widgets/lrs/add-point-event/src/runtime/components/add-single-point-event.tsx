...
import { Button, FOCUSABLE_CONTAINER_CLASS, Label, Option, Select, Tooltip } from 'jimu-ui'
...
export function AddSinglePointEvent (props: AddSinglePointEventProps) {
  ...
  const handleMethodChanged = React.useCallback((value: SearchMethod) => {
    ...
    handleRouteInfoUpdate(updatedRouteInfo)
  ...
  return (
      ...
      <div className={classNames('add-single-point-event__content', {
        ...
        {!hideEvent && (
          <div>
            <Label
              size='sm'
              className='mb-0 pt-3 px-3 w-100 title3'
              for={eventSelectId}
            >
        ...
        {!hideMethod && (
          <InlineEditableDropdown
            ...
            listItems={[SearchMethod.Measure, SearchMethod.Coordinate, SearchMethod.LocationOffset]}
            altItemDescriptions={[getI18nMessage('routeAndMeasure'), getI18nMessage('coordinate'), getI18nMessage('locationOffset')]}
          />
        )}
    ...
    <div className={classNames('add-point-footer w-100', {
      ...
      <Tooltip
        title={tooltipMsg || getI18nMessage('nextLabel')}
        placement="top"
        showArrow={true}
      >
      <div className='mt-auto ml-auto'>
        <Button
          ...
          size='sm'
          disabled={!isValidInput || tooltipMsg?.length > 0}
          onClick={submitForm}
        >
