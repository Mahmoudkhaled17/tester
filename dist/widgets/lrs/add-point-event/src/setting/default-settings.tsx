...
import { type DefaultInfo, type LrsLayer, SearchMethod, getConfigValue, getDefaultEvent, getDefaultNetwork, isDefined, isPointEvent, lrsDefaultMessages, updateConfig } from 'widgets/shared-code/lrs'
...
export function DefaultSettings (props: Props) {
  ...
  const getEventConfig = (): string => {
    return getDefaultEvent(lrsLayers, defaultEvent, true)?.name || ''
  }
  ...
  const handleDefaultEventChanged = (value: string) => {
    const lrsLayer = lrsLayers.find((layer) => layer.name === value && isPointEvent(Immutable(layer)))
  ...
  const handleDefaultMethodChanged = (value: SearchMethod) => {
    updateConfig(widgetId, config, 'defaultMethod', value, activeMapViewId, onSettingChange)
  }
