...
import { ReferentSettings } from './referent-settings'
...
const Setting = (props: AllWidgetSettingProps<IMConfig>) => {
  ...
  const handleLrsLayersUpdated = React.useCallback(async (lrsLayers: ImmutableArray<LrsLayer>, allDataSources: any) => {
    ...
    const defaultReferent = getDefaultReferentConfig(updatedConfig.lrsLayers, defaultNetwork, updatedConfig.defaultReferentConfig)

    updatedConfig = updatedConfig.set('defaultNetwork', defaultNetwork).set('defaultEvent', defaultEvent).set('defaultReferentConfig', defaultReferent)
    ...
    try {
      for (const mapId in mapViewsConfig) {
        if (!update) {
          ...
          const defaultReferent = getDefaultReferentConfig(mapViewConfig.lrsLayers, settingForView.defaultNetwork, settingForView.defaultReferentConfig)
          if (defaultReferent) {
            settingForView = settingForView.set('defaultReferentConfig', defaultReferent)
          }
      ...
      <div className='jimu-widget-setting setting-add-point-event__setting-content h-100'>
        ...
        {hasMap && hasEventConfig && (
          <React.Fragment>
            ...
            <CoordinateSettings intl={intl} widgetId={widgetId} config={config} activeMapViewId={mapViewIdMapSettings} onSettingChange={onSettingChange} />
            <ReferentSettings intl={intl} widgetId={widgetId} config={config} activeMapViewId={mapViewIdMapSettings} onSettingChange={onSettingChange} />
