...
import { advancedActionMap, type AttributeFieldSettings, type DefaultInfo, getConfigValue, isPointEvent, lrsDefaultMessages, type LrsLayer, ModeType, getDefaultEvent, getDefaultNetwork, getLayersByType } from 'widgets/shared-code/lrs'
...
export function LayerConfig (props: Props) {
  ...
  const mapViewConfig = React.useMemo(() => {
    return config.mapViewsConfig?.[activeMapViewId]
  }, [config, activeMapViewId])

  const lrsLayers = useConfigValue('lrsLayers', [])
  const lrsLayer = lrsLayers[index] as ImmutableObject<LrsLayer>
  const defaultNetwork = useConfigValue('defaultNetwork', null) as DefaultInfo
  const defaultEvent = useConfigValue('defaultEvent', null) as DefaultInfo
  ...
  const onLayerChanged = React.useCallback(
    (index: number, updatedLayerItems, dsUpdateRequired = false) => {
      ...
      // Get updated networks and event names after change.
      const { networks, pointEvents, intersections } = getLayersByType(layers, mapViewConfig?.disabledLayerIds ?? Immutable([]))
      const updatedDefaultNetwork = getDefaultNetwork(layers, defaultNetwork)
      const updatedDefaultEvent = getDefaultEvent(layers, defaultEvent, true)

      // Update widget json
      if (config.mode === ModeType.Map) {
        const updatedSettingsPerView = settingsPerView
          .set('networkLayers', networks)
          .set('eventLayers', pointEvents)
          .set('intersectionLayers', intersections)
          .set('defaultNetwork', updatedDefaultNetwork)
          .set('defaultEvent', updatedDefaultEvent)
    ...
    [lrsLayers, mapViewConfig?.disabledLayerIds, defaultNetwork, defaultEvent, config, settingsPerView, activeMapViewId, onSettingChange, widgetId]
  )
          ...
          <React.Fragment>
            <SettingSection role='group' aria-label={getI18nMessage('label')} title={getI18nMessage('label')}>
              <SettingRow>
                ...
                {isPointEvent(lrsLayer) && (
                <SettingSection role='group' aria-label={getI18nMessage('configureFields')} title={getI18nMessage('configureFields')}>
