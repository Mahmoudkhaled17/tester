...
export function ReferentSettings (props: Props) {
  ...
  }, [config, activeMapViewId, referentLayers, getUnit, widgetId, onSettingChange, getFallbackReferentLayer, referentConfig])

  const setDefaultOffsetUnitChange = (event) => {
    const offsetUnit = event?.target?.value
    const newReferentConfig = {...referentConfig, defaultOffsetUnit: offsetUnit}
    updateConfig(widgetId, config, 'defaultReferentConfig', newReferentConfig, activeMapViewId, onSettingChange)
  }
  ...
  const setDefaultReferentChange = (event) => {
    ...
    const newReferentConfig = {...referentConfig, defaultReferentLayer: referentItem}
    updateConfig(widgetId, config, 'defaultReferentConfig', newReferentConfig, activeMapViewId, onSettingChange)
  }
