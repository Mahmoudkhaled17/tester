...
import { type LrsLayer, type AttributeFieldSettings, advancedActionMap, lrsDefaultMessages, ModeType, getConfigValue, isLineEvent, type DefaultInfo, getLayersByType, getDefaultNetwork, getDefaultEvent } from 'widgets/shared-code/lrs'
...
export function LayerConfig (props: Props) {
  ...
  const onLayerChanged = React.useCallback(
    (index: number, updatedLayerItems, dsUpdateRequired = false) => {
      ...
      // Get updated networks and line event names after change.
      const { networks, lineEvents, intersections } = getLayersByType(layers, mapViewConfig?.disabledLayerIds ?? Immutable([]))
      const updatedDefaultNetwork = getDefaultNetwork(layers, defaultNetwork)
      const updatedDefaultEvent = getDefaultEvent(layers, defaultEvent, false)
