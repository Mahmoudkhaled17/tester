...
import { type FeatureLayerDataSource, AbstractDataAction, type DataRecordSet, MutableStoreManager, DataLevel, getAppStore, DataSourceStatus, type ImmutableObject, loadArcGISJSAPIModules, DataSourceTypes } from 'jimu-core'
import { getNetworkInfoFromLrsFeatureUrl, type RouteInfo, type EventInfo, isDefined, getRouteFromEndMeasures, QueryRouteMeasures, type NetworkInfo, queryRouteIdOrName, findNetworkInfoFromMapViewsConfig, findEventInfoFromMapViewsConfig, getInitialRouteInfoState } from 'widgets/shared-code/lrs'
  ...
  async onExecute (dataSets: DataRecordSet[], dataLevel: DataLevel): Promise<boolean> {
    ...
    if (dataLevel === DataLevel.DataSource) {
      ...
      if (!hideMeasures) {
        ...
        rteInfo.fromX = rteInfo.selectedPoint?.x
        rteInfo.fromY = rteInfo.selectedPoint?.y
        rteInfo.fromZ = rteInfo.selectedPoint?.z
      }
              ...
              if (isDefined(routePolyline)) {
                ...
                if (isDefined(routeEndPoints)) {
                  ...
                  if (hideMeasures) {
                    ...
                    rteInfo.fromX = firstPoint?.x
                    rteInfo.fromY = firstPoint?.y
                    rteInfo.fromZ = firstPoint?.z
                    rteInfo.toX = lastPoint?.x
                    rteInfo.toY = lastPoint?.y
                    rteInfo.toZ = lastPoint?.z
                  }
                }
              }
            }
