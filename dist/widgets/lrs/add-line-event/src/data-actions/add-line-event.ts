...
import { getNetworkInfoFromLrsFeatureUrl, type RouteInfo, type EventInfo, isDefined, getRouteFromEndMeasures, QueryRouteMeasures, type NetworkInfo, queryRouteIdOrName, getDateWithTZOffset, findNetworkInfoFromMapViewsConfig, findEventInfoFromMapViewsConfig, getInitialRouteInfoState } from 'widgets/shared-code/lrs'
...
export default class ExportJson extends AbstractDataAction {
  async isSupported (dataSets: DataRecordSet[], dataLevel: DataLevel): Promise<boolean> {
    ...
    if (!isDefined(networkInfo)) {
      const datasources = dataSet.dataSource?.dataSourceManager?.getDataSources()
      if (isDefined(datasources)) {
        const networkDSLayerId = Number((networkDS as FeatureLayerDataSource).layerId)
        if (datasources[networkDS.id]) {
          const ds = datasources[networkDS.id]
          if (Number(ds.getDataSourceJson().layerId) === networkDSLayerId) {
            if (isDefined(ds.getDataSourceJson().url)) {
              networkInfo = await getNetworkInfoFromLrsFeatureUrl(ds, this.widgetId)
            }
          }
        }
      }
    ...
    if (widgetJson.config.lrsLayers.length > 0) {
  ...
  async onExecute (dataSets: DataRecordSet[], dataLevel: DataLevel): Promise<boolean> {
    ...
    const rteInfo: RouteInfo = getInitialRouteInfoState()
    ...
    const fromDate = isDefined(dataAttributes[fromDateField]) ? dataAttributes[fromDateField] : null
    const fromDate2 = isDefined(dataAttributes2) ? dataAttributes2[fromDateField] : null
    const toDate = isDefined(dataAttributes[toDateField]) ? dataAttributes[toDateField] : null
    const toDate2 = isDefined(dataAttributes2) ? dataAttributes2[toDateField] : null
    ...
    if (!hideMeasures) {
      if (isDefined(dataAttributes.Measure)) {
        ...
        await loadArcGISJSAPIModules(['esri/geometry/Polyline']).then(modules => {
          ...
          const geometry = record.getRawGeometry()
          const poly = new Poly(geometry)
          if (poly.paths.length > 0) {
        ...
        .then(async (results) => {
          if (isDefined(results)) {
            ...
            await Promise.all(results.features.map((feature) => {
              ...
              const fromDateWithTZOffset = getDateWithTZOffset(feature.attributes[networkInfo.fromDateFieldSchema.name], networkDS)
              const toDateWithTZOffset = getDateWithTZOffset(feature.attributes[networkInfo.toDateFieldSchema.name], networkDS)
              ...
              if (((rteInfo.fromDate instanceof Date && fromDateWithTZOffset.getTime() === rteInfo.fromDate.getTime()) || fromDateWithTZOffset === rteInfo.fromDate) &&
                  ((rteInfo.toDate instanceof Date && toDateWithTZOffset.getTime() === rteInfo.toDate.getTime()) || toDateWithTZOffset === rteInfo.toDate)) {
                routePolyline = feature.geometry as Polyline
              }
