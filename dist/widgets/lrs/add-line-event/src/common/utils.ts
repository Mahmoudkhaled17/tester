...
import { LrsLayerType, type LrsLayer, type ModeType, SearchMethod, getDefaultNetwork, getDefaultAttributeSet, getDefaultEvent, isDefined, getAttributeSets, isLineEvent, getDefaultReferentConfig } from 'widgets/shared-code/lrs'
...
export function constructSettingsPerView () {
  const settingsPerView: SettingsPerView = {
    ...
    defaultAttributeSet: '',
    defaultEvent: { name: '', serviceId: -1 },
    defaultNetwork: { name: '', serviceId: -1 },
    hideMethod: false,
    hideEvent: false,
    hideNetwork: false,
    hideType: false,
...
export async function setValuesForView (
  ...
  const defaultReferentConfig = getDefaultReferentConfig(lrsLayers, defaultNetwork, settingsPerView.defaultReferentConfig)
  ...
  return settingsPerView
    ...
    .set('defaultReferentConfig', defaultReferentConfig)
}
...
export function resetConfig (config: IMConfig, mode: ModeType): IMConfig {
  return config
    ...
    .set('defaultReferentConfig', ({ defaultReferentLayer: null, defaultOffsetUnit: 'esriMiles' }) )
}
