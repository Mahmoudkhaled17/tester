...
export function AddLineEventConcurrencies (props: AddLineEventConcurrenciesProps) {
  ...
  const dateRangeSelectId = React.useId()
  const {
    intl,
    networkDS,
    network,
  ...
  const onSubmitClicked = () => {
    graphicsManager?.clearPickedConcurrencyGraphic()
      ...
      <div className='add-line-event-edit-attributes__content'>
        ...
        <div className='h-100'>
          <div className="time-range px-3">
            <div>
              <Label size="sm" centric className="text-truncate mb-0 pt-2 w-100" style={{ textOverflow: 'ellipsis', fontWeight: 500 }} for={dateRangeSelectId}>
                {getI18nMessage('dateRange')}
              </Label>
            </div>
            <div>
              {/*// @ts-expect-error */}
              <Select id={dateRangeSelectId} size='sm' value={selectedDateRange} onChange={(event) => { handleDateRangeChange(event) }} disabled={dateRanges?.length < 2}>
