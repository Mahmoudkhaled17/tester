...
import {
  ...
  extractFromRouteInfoFromRecord,
  extractToRouteInfoFromRecord
  ...
  */
  ...
  React.useEffect(() => {
    if (isDefined(network)) {
      ...
      clearPickedGraphic()
      graphicsManager?.clearHoverGraphic()
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [network])
  ...
  const setFromRouteInfoByRouteIdOrName = async (record: __esri.Graphic, routeIdOrName: string): Promise<RouteInfo> => {
    const baseRouteInfo = await extractFromRouteInfoFromRecord({
      record,
      routeIdOrName,
      network,
      networkDS,
    ...
    const minMeasure = Math.min(...routeEndPoints.map(p => (p as any).m))
    const maxMeasure = Math.max(...routeEndPoints.map(p => (p as any).m))

    if (useStartMeasure) {
      baseRouteInfo.selectedMeasure = round(minMeasure, network.measurePrecision)
      baseRouteInfo.selectedPoint = routeEndPoints[0]
    ...
    if ((!isFrom && useEndMeasure) || ((routeInfo.lineId !== fromLineIdValue) && useEndMeasure) || ((!routeInfo.lineId && !fromLineIdValue) && useEndMeasure)) {
      baseRouteInfo.selectedToMeasure = round(maxMeasure, network.measurePrecision)
      baseRouteInfo.selectedToPoint = routeEndPoints[1]
  ...
  const setToRouteInfoByRouteIdOrName = async (record: __esri.Graphic, routeIdOrName: string): Promise<RouteInfo> => {
    const baseRouteInfo = await extractToRouteInfoFromRecord({
      record,
      routeIdOrName,
      network,
      networkDS,
  ...
  const updateSelectedFromMeasure = (routeInfo: RouteInfo, fromDataAction: boolean): Promise<RouteInfo> => {
    return QueryMeasureOnRoute(networkDS, network, routeInfo.routeId, routeInfo.fromDate, fromDataAction || useStartMeasure ? routeInfo.selectedMeasure.toString() : fromMeasureInput)
      .then((point) => {
        if (point) {
  ...
  const updateSelectedToMeasure = (routeInfo: RouteInfo, fromDataAction: boolean): Promise<RouteInfo> => {
    if (fromDataAction && isDefined(routeInfo.selectedMeasure) && isDefined(routeInfo.selectedToMeasure)) {
      return QueryMeasureOnRoute(networkDS, network, routeInfo.routeId, routeInfo.fromDate, fromDataAction || useStartMeasure ? routeInfo.selectedMeasure.toString() : fromMeasureInput)
        .then((point) => {
          let updateRouteInfo
          if (point) {
            updateRouteInfo = { ...routeInfo, selectedPoint: point, selectedMeasure: parseFloat(fromDataAction || useStartMeasure ? routeInfo.selectedMeasure.toString() : fromMeasureInput) }
          } else {
            updateRouteInfo = { ...routeInfo, selectedPoint: null, selectedMeasure: NaN }
          }
          return QueryMeasureOnRoute(networkDS, network, routeInfo.toRouteId, routeInfo.toRouteFromDate, fromDataAction || useEndMeasure ? routeInfo.selectedToMeasure.toString() : toMeasureInput)
            .then((point) => {
              if (point) {
                const updateRouteInfo2 = { ...updateRouteInfo, selectedToPoint: point, selectedToMeasure: parseFloat(fromDataAction || useEndMeasure ? routeInfo.selectedToMeasure.toString() : toMeasureInput) }
                return updateRouteInfo2
              } else {
                const updateRouteInfo2 = { ...updateRouteInfo, selectedToMeasure: NaN }
                return updateRouteInfo2
              }
            })
        })
      ...
      {network && (
        <div className='w-100'>
          ...
          <div ref={searchConRef} className='d-flex w-100' style={{ paddingBottom: 3 }} >
            <TextInput
              aria-label={isFrom ? !network.useRouteName ? getI18nMessage('fromRouteIdLabel') : getI18nMessage('fromRouteNameLabel') : !network.useRouteName ? getI18nMessage('toRouteIdLabel') : getI18nMessage('toRouteNameLabel')}
              type='text'
              className='w-100'
              size='sm'
              allowClear
