/** @jsx jsx */
import { React, jsx, css, type ImmutableObject, hooks, type ImmutableArray, Immutable, DataSourceManager } from 'jimu-core'
...
export function LayerConfig (props: Props) {
  ...
  const isIntersection = React.useMemo(() => lrsLayer?.layerType === LrsLayerType.Intersection, [lrsLayer])
  const isUtilityNetwork = React.useMemo(() => lrsLayer?.layerType === LrsLayerType.UtilityNetwork, [lrsLayer])
  ...
  const getAttributeFields = (): string[] => {
    if (isIntersection) {
      return getIntersectionFields()
    }
    if (isUtilityNetwork) {
      return getUtilityNetworkFields()
    }
  ...
  const getIntersectionFields = (): string[] => {
    const referentProperties = lrsLayer.intersectionInfo?.referentProperties
    if (referentProperties) {
      return [...referentProperties.layerFields.map(f => f.name)]
    }
    return []
  }

  const getUtilityNetworkFields = (): string[] => {
    const referentProperties = lrsLayer.utilityNetworkInfo?.referentProperties
    if (referentProperties) {
      const ds = DataSourceManager.getInstance().getDataSource(lrsLayer.useDataSource?.dataSourceId)
      const oidField = ds?.getIdField()
      // exclude OID field from attribute list since it is not useful to show and can cause issues if used as display field
      return Array.from(referentProperties.layerFields.map(f => f.name).filter(name => name !== oidField))
    }
    return []
  }
  ...
  return (
    <div className='h-100'>
      <div css={css`height: 100%; overflow:auto;`}>
        {lrsLayer && (dsExist || config.mode === ModeType.Map) && (isEvent || isIntersection || isUtilityNetwork) && (
          <SettingSection role='group' aria-label={getI18nMessage('layerLabel')} title={getI18nMessage('layerLabel')}>
            {lrsLayer.name}
