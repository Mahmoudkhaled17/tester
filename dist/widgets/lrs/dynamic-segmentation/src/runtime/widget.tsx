...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & ExtraProps, State> {
  ...
  private getConfigValues (config: IMConfig, settingPerView: any, activeMapViewId: string): { [key: string]: any } {
    ...
    orientedImageryWidgetId: settingPerView.orientedImageryWidgetId,
    searchTolerance: settingPerView.searchTolerance,
    searchUnit: settingPerView.searchUnit
    }
  ...
  render () {
    ...
    const { defaultDisplayType, allowEditing, attributeInputType, defaultPointAttributeSet, defaultLineAttributeSet, attributeSets, mapHighlightColor, defaultDiagramScale, showEventStatistics, allowMerge, defaultNetwork, orientedImageryWidgetId, searchTolerance, searchUnit } = configValues
    ...
    return (
      ...
      <DynSegRuntimeStateProvider>
        <Paper variant='flat' shape="none" className='jimu-widget runtime-dynamic-segmentation surface-1 border-0 d-flex' css={getStyle(theme, config.tableHighlightColor)}>
          <div id={this.widgetOuterDivId} className='table-indent' role='region' aria-label={this.getI18nMessage('_widgetLabel')}>
            ...
            {hasConfig && (
              <DynamicSegmentation
                ...
                orientedImageryWidgetId={orientedImageryWidgetId}
