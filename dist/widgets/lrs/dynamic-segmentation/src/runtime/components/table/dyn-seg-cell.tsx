...
export function DynSegCell (props: CellProps) {
  ...
  const codedValueDomains: __esri.CodedValueDomain = React.useMemo(() => {
    if (isDefined(field) && isDefined(field.domain) && field.domain.type === 'coded-value') {
      ...
      if (field.nullable && !containsNull) {
        (field.domain.codedValues as any).unshift({ code: null, name: 'null' })
      }
      return field.domain
    }
    return null
  ...
  const handleInputSelectKeyDown = (event: React.KeyboardEvent) => {
    if (isCellEditable && event.key === 'Enter') {
      event.preventDefault()
      setIsSelected(true)
    }
  }

  const handleInputEscapeKeyDown = (event: React.KeyboardEvent) => {
  ...
  return (
    <calcite-table-cell
      onKeyDown={!isSelected ? handleInputSelectKeyDown : undefined}
      className={classNames('dyn-seg-cell', isCellEdited ? 'edited' : '', isCellEditable ? '' : 'non-editable', hasError ? 'error' : '')}>
      <div ref={focusRef} style={{ minWidth: getWidth(), maxWidth: '224px' }}>
        {isVisible && (
          <div className='dyn-seg-cell-content h-100 d-flex'>
            {!isSelected && (
              <div
                onDoubleClick={handleDoubleClick}
                aria-label={isCellEditable ? getI18nMessage('tableCellAriaLabel', { fieldName: fieldInfo.featureFieldName }) : fieldInfo.featureFieldName}
                className='d-flex'>
            ...
            {isSelected && fieldType === 'text' && (
              <TextInput
                ...
                value={currentValue}
                type={'text'}
                aria-label={fieldInfo.featureFieldName}
            ...
            {isSelected && fieldType === 'date' && (
              <div onKeyDown={handleInputEscapeKeyDown} style={{ width: '100%' }}>
