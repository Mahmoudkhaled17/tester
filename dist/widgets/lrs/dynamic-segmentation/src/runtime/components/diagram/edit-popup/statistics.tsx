...
import * as intersectOperator from "esri/geometry/operators/intersectsOperator"
import * as touchOperator from "esri/geometry/operators/touchesOperator"
...
export const Statistics = React.forwardRef((props: StatisticsProps, ref) => {
  ...
  const getDerivedFields = (): __esri.Field => {
    ...
    const lrsFieldNames = isDefined(eventInfo) ? eventInfo.lrsFields.map((lrsField) => lrsField.alias) : []
  ...
  const getDerivedFieldsValues = (field: __esri.Field) => {
    ...
    if (isDefined(allRecords) && isDefined(currentRecord) && isDefined(networkInfo) && isDefined(field) && allRecords.length) {
      const filteredRecords = allRecords.filter((record) => {
        // For records to be considered for statistics, they must intersect spatially with the current record.
        // Geometries that only touch the current record are not included.
        const isDisjoint = disjointOperator.execute(currentRecord.geometry, record.geometry)
        if (!isDisjoint) {
          const intersects = intersectOperator.execute(currentRecord.geometry, record.geometry)
          const touches = touchOperator.execute(currentRecord.geometry, record.geometry)
      ...
      css={getCalciteBasicTheme()}>
          ...
          <calcite-table
            ...
            <calcite-table-row slot='table-header'>
              <calcite-table-header heading={getI18nMessage('attribute')}/>
              <calcite-table-header heading={getI18nMessage('value')}/>
            </calcite-table-row>
            {currentStats.map((value, index) => {
              return (
                <calcite-table-row key={index}>
