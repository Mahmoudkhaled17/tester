...
import { OrientedImageryRow } from './oriented-imagry/oriented-imagery-row'
...
export function Body (props: BodyProps) {
  const { allLayersDS, trackMap, contentWidth, measureRange, featureLayer, subtypeLayers, scrollPos, onItemClick } = props
  ...
  const { highlightColor, highlightLayer, selectedSldId, jimuMapView, orientedImageryInfo } = useDynSegRuntimeState()
  ...
  const renderOrientedImageryRows = (isActive: boolean) => {
    // Always rendered at top of active/inactive rows. Not included in trackMap.
    return (
      <OrientedImageryRow
        ...
        scrollPos={scrollPos}
        isActive={isActive}
        onPanToMeasure={props.onPanToMeasure}
      />
    )
  }

  const renderRows = (tracks: string[], isActive: boolean) => {
  ...
  <div className="sld-body h-100" style={{ width: contentWidth }}>
    {/* Active Rows */}
    {orientedImageryInfo?.enabled && !orientedImageryInfo?.collapsed && renderOrientedImageryRows(true)}
    { renderRows(getActiveTracks(), true) }

    {/* Inactive Rows */}
    {orientedImageryInfo?.enabled && orientedImageryInfo?.collapsed && renderOrientedImageryRows(false)}
    { renderRows(getInactiveTracks(), false) }
