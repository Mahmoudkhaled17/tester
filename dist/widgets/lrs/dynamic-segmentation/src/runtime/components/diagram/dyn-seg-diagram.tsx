...
import { isDefined, type MeasureToGeometryResponse, queryRouteId, type LrsLayer, type NetworkInfo, isDisjoint } from 'widgets/shared-code/lrs'
...
export function DynSegDiagram (props: DynSegDiagramProps) {
  ...
  const { syncToMap, orientedImageryInfo } = useDynSegRuntimeState()
  ...
  const handlePanToMeasure = React.useCallback((measure: number, newZoom?: number) => {
    if (!sldRef.current || isNaN(measure)) return
    if (!triggerSource.current) triggerSource.current = 'range'

    const zoomLevel = newZoom || zoom
    const sldWidth = refWidthWatch.current - sidebarWidth - getScrollbarWidth
    const sldContentWidth = sldWidth * zoomLevel

    // Calculate the pixel position of the measure
    const measureX = getXFromM(measure, measureRange, sldContentWidth)

    // Center the measure in the view
    const centerScrollPos = measureX - (sldWidth / 2)

    // Clamp to valid scroll range
    const maxScroll = sldContentWidth - sldWidth
    const newScrollPos = Math.max(0, Math.min(centerScrollPos, maxScroll))

    sldRef.current.scrollLeft = newScrollPos
    setScrollPos(newScrollPos)

    // Set new zoom if provided
    if (newZoom !== undefined) {
      setZoom(zoomLevel)
    }
  }, [measureRange, refWidthWatch, sidebarWidth, zoom])
  //#endregion
  ...
  const handleOnRulerDoubleClick = async (e) => {
    ...
    .then((geometryInSR) => {
      ...
      jimuMapView?.view.goTo({ target: graphicInSR.geometry })
    })
    .catch((error) => {
  ...
  const getSidebarDisplayHeight = React.useMemo(() => {
    let height = 0
    if (orientedImageryInfo.enabled) {
      height += orientedImageryInfo.collapsed ? SLD_INACTIVE_HEIGHT : SLD_TRACK_HEIGHT
    }
  ...
  const handleExtentChange = async (extent) => {
    try {
      if (props.jimuMapView.view.stationary) {
        if (routeGeometryRef.current) {
          const isDisjointed = await isDisjoint(routeGeometryRef.current, extent)
          if (!isDisjointed) {
    ...
    <div className='diagram-container h-100 w-100 d-flex' >
      ...
      <div ref={sldRef}
        ...
        <Sld
          ...
          onPanToMeasure={handlePanToMeasure}/>
      </div>
    </div>
  </div>
  )
