...
export function FieldCalcPopup (props: FieldCalculatorProps) {
  ...
  const getDomainOptions = (domain) => {
    return domain?.codedValues?.map((element, i) => {
      ...
      return (
        <calcite-option key={String(i)} value={String(code)}>
    ...
    if (fieldType === 'text' || fieldType === 'number') {
      ...
      return (
        <calcite-select
          ...
          scale='s'
          label={getI18nMessage('fieldValue')}
          oncalciteSelectChange={onSelectChange}
        >
          {options}
        </calcite-select>
      )
      ...
      return (
        ...
        >
          {subtypeCodedValue.map((element, i) => {
            return (
              <calcite-option key={String(i)} value={String(element.value)}>
                {getDropDownLabel(element.label, element.value)}
              </calcite-option>
            )
          })
          }
  ...
  const renderEventLayers = () => {
    ...
    eventIds.forEach((eventId) => {
      ...
      options.push(<calcite-option value={eventLayerId}> {eventName || eventLayerId} </calcite-option>)
    })
    return options
  }
  ...
  const renderEventLayerFields = () => {
    ...
    fields.forEach((field, index) => {
      options.push(<calcite-option value={field.originalFieldName}> {field.originalFieldName} </calcite-option>)
    })
    return options
  }
  ...
  return (
    <div style={{ margin: '1rem', display: 'flex', flexDirection: 'column', width: '300px' }} >
        <Label className='title3'>
            ...
            <calcite-select scale='s' label={getI18nMessage('eventLayer')} value={eventLayerId} oncalciteSelectChange={handleEventLayerChange}>
                {renderEventLayers()}
            </calcite-select>
        </Label>
        <Label className='title3' style={{ marginTop: '0.3rem' }}>
            {getI18nMessage('updateField')}
            <calcite-select scale='s' label={getI18nMessage('updateField')} value={field} oncalciteSelectChange={handleFieldChange}>
