...
export function getNameOrId (routeInfo: RouteInfo): string {
  const lineId = isDefined(routeInfo?.lineId) && routeInfo.lineId !== '' ? routeInfo.lineId : ''
  const lineName = isDefined(routeInfo?.lineName) && routeInfo.lineName !== '' ? routeInfo.lineName : ''
  const routeName = isDefined(routeInfo?.routeName) && routeInfo.routeName !== '' ? routeInfo.routeName : ''
  if (lineId !== '' && lineName === '') { return lineId }
  if (lineId !== '' && lineName !== '') { return lineName }
  if (routeName !== '') { return routeName }
  return routeInfo.routeId || ''
}

export function getNameOrIdFromUnavailableLock (lock: DetailedLockInfo): string {
...
export function getErrorMessage (response: AcquireLockResponse, lockInfo: LrsLocksInfo, featureDS, intl) {
  let message
  let msgType
  // eslint-disable-next-line no-useless-assignment
  let nameOrId = ''
  const isLine = isDefined(lockInfo?.routeInfo?.lineId) && lockInfo.routeInfo.lineId !== ''
...
export async function createLockInfoFromParams (
  ...
  eventServiceLayerId: string
    ...
    if (!isDefined(routeInfoFromDataAction) || !isDefined(routeInfoFromDataAction.networkInfo) || !isDefined(lrsLayers) || lrsLayers.length === 0) {
      return {} as LrsLocksInfo
    }
    const routeOrLineId = []
    const isLine = []
