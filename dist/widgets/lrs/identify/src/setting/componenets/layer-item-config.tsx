...
import { advancedActionMap, type AttributeFieldSettings, type DefaultInfo, getConfigValue, type LrsLayer, LrsLayerType, ModeType, getDefaultNetwork, getLayersByType } from 'widgets/shared-code/lrs'
...
export function LayerConfig (props: Props) {
  ...
  const onLayerChanged = React.useCallback(
    (index: number, updatedLayerItems, dsUpdateRequired = false) => {
      ...
      const { networks, lineEvents, pointEvents } = getLayersByType(layers, mapViewConfig?.disabledLayerIds ?? Immutable([]))
    ...
    [lrsLayers, mapViewConfig?.disabledLayerIds, defaultNetwork, config, settingsPerView, activeMapViewId, onSettingChange, widgetId]
  )
