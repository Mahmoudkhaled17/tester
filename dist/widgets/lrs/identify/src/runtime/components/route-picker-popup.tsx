...
import { calculateAlignmentPosition, getCodedValueLabel, getDataRecordFromObjId, getPointFromPolyline } from '../utils/utils'
  ...
  React.useEffect(() => {
    ...
    if (selectedLocationInfo) {
      ...
      if (mapPoint && !isNetworkChange) {
        ...
        const info = calculateAlignmentPosition(mapPoint.x, mapPoint.y, jimuMapView?.view, size, window)
        if (info) {
    ...
    const fetchDataRecord = () => {
      ...
      try {
        ...
        outputDS && defaultRoutes?.forEach((ele) => {
          ...
          const ft = getPointFromPolyline(ele?.feature?.attributes, measure, measureFields, selectedPoint)
        ...
        const ft = getPointFromPolyline(selectedRecord?.feature?.attributes, measure, measureFields, selectedPoint)
  ...
  const handleNetworkChange = React.useCallback((event) => {
    ...
    networkLayers?.forEach((routeInfo, index) => {
      if (routeInfo?.id === id) {
        ...
        const ft = getPointFromPolyline(data?.feature?.attributes, measure, measureFields, selectedPoint)
        const record = outputDS?.buildRecord(ft)
    ...
    >
      ...
      >
        ...
        {attributeVals?.map((val, index) => {
          return (<calcite-table-row key={index}>
            <calcite-table-cell>
            {val.name}
            </calcite-table-cell>
            <calcite-table-cell>
