...
export function SplitEvent (props: SplitEventProps) {
  ...
  React.useEffect(() => {
    if (isDefined(selectedEventObjectId)) {
      ...
      } else if (isDefined(selectedEventDataSource)) {
        const eventLayer = lrsLayers.find(item => item.id === selectedEventDataSource.id)
  ...
  React.useEffect(() => {
    if (isDefined(selectedEventDataSource)) {
      if (selectedEventDataSource.id !== selectedEvent?.id) {
        const eventLayer = lrsLayers.find(item => item.id === selectedEventDataSource.id)
        if (isDefined(eventLayer)) {
  ...
  const handleRouteInfoUpdate = (newRouteInfo: RouteInfo) => {
    ...
    if (!isDefined(newRouteInfo.routeId)) {
      resetEventOid()
    }
      ...
      <div className="split-event__content">
        ...
        {networkDS && eventDS && (
          <SplitEventAttributes
            ...
            networkInfo={selectedNetwork?.networkInfo}
            jimuMapView={JimuMapView}
            graphicsManager={graphicsManager}
