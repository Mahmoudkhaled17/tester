...
export const SplitEventAttributes = React.forwardRef((props: SplitEventAttributesProps, ref) => {
  ...
  const [isEventPopupOpen, setIsEventPopupOpen] = React.useState<boolean>(false)
  const [eventInfos, setEventInfos] = React.useState<EventInfo[]>([])
  ...
  React.useEffect(() => {
    if (revalidateRouteFromDataAction) {
      setFieldInfosWithEventAttributeValues(selectedEventObjectId, eventLayer, eventDS)
    }
  ...
  const handleMultiEventSelect = React.useCallback((eventInfos: EventInfo[]) => {
  ...
  const handleEventPopupSelect = React.useCallback((eventInfo: EventInfo, eventLyr: ImmutableObject<LrsLayer>, eventDataSource: DataSource) => {
    if (isDefined(eventInfo)) {
      const newEventOid = eventInfo.objectId
      setFieldInfosWithEventAttributeValues(Number(newEventOid), eventLyr, eventDataSource)
    }
    setIsEventPopupOpen(false)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  const handleEventPopupCancel = React.useCallback(() => {
    setIsEventPopupOpen(false)
  }, [])

  const setSingleEventRecord = (eventRecord: any, objectIdFieldName: string, featureLayerDS: FeatureLayerDataSource, eventLyr: ImmutableObject<LrsLayer>, fieldInfos: FieldInfo[], rteInfo: RouteInfo): boolean => {
    let unableToSplitAtStartOrEnd = false
    const newEventOid = eventRecord.attributes[objectIdFieldName]
    ...
    eventLyr.eventInfo.attributeFields.forEach((fieldInfo) => {
      if (fieldInfo.enabled) {
        const field = fields.find(l => l.name === fieldInfo.field.name)
        if (field && field.editable) {
          let defaultValue
          let value = eventRecord.attributes[field.name]
          ...
          const fromMeasure = eventRecord.attributes[eventLyr.eventInfo.fromMeasureFieldName]
          const toMeasure = eventRecord.attributes[eventLyr.eventInfo.toMeasureFieldName]
          if (eventLyr.eventInfo.canSpanRoutes) {
            const fromRouteID = eventRecord.attributes[eventLyr.eventInfo.routeIdFieldName]
            const toRouteID = eventRecord.attributes[eventLyr.eventInfo.toRouteIdFieldName]
          ...
          setEventFromDate(eventRecord.attributes[eventLyr.eventInfo.fromDateFieldName])
          setEventToDate(eventRecord.attributes[eventLyr.eventInfo.toDateFieldName])
        }
      }
    })
    setNoEventFound(false)
    return unableToSplitAtStartOrEnd
  }

  const setFieldInfosWithEventAttributeValues = async (selectedEventOid: number, eventLyr: ImmutableObject<LrsLayer>, eventDataSource: DataSource) => {
    let fieldInfos: FieldInfo[] = []
    if (isDefined(eventLyr) && isDefined(eventDataSource) && isDefined(eventLyr.eventInfo) && isDefined(eventLyr.eventInfo.attributeFields) &&
      (isDefined(routeInfo) || isDefined(routeInfoFromDataAction))) {
      // Get basic field info for non lrs fields and set default values.
      const featureLayerDS = eventDataSource as FeatureLayerDataSource
      ...
      queryEventsByRouteIdOrEventObjectId(networkLayer.networkInfo.xyTolerance, featureLayerDS, eventLyr, rteInfo, routeIds, selectedEventOid).then((eventRecords) => {
        const featureLayerDS = eventDataSource as FeatureLayerDataSource
        ...
        if (eventRecords.length === 1) {
          unableToSplitAtStartOrEnd = setSingleEventRecord(eventRecords[0], objectIdFieldName, featureLayerDS, eventLyr, fieldInfos, rteInfo)
          ...
          if (!matchCurrentEventOid) {
            ...
            const eventObjectIds: number[] = []
            let singleEventIndex = -1
            for (let i = 0; i < eventRecords.length; i++) {
              const splitDate = routeInfo.selectedFromDate.getTime()
              const eventFromDate = eventRecords[i].attributes[eventLyr.eventInfo.fromDateFieldName]
              const eventToDate = eventRecords[i].attributes[eventLyr.eventInfo.toDateFieldName]
              const routeId = eventRecords[i].attributes[eventLyr.eventInfo.routeIdFieldName]
              if (eventLyr.eventInfo.canSpanRoutes) {
                const toRouteId = eventRecords[i].attributes[eventLyr.eventInfo.toRouteIdFieldName]
                if (routeId !== rteInfo.routeId && toRouteId !== rteInfo.routeId) {
                  continue
                }
              } else {
                if (routeId !== rteInfo.routeId) {
                  continue
                }
              }
              ...
              singleEventIndex = i
              eventObjectIds.push(eventRecords[i].attributes[objectIdFieldName])
            }

            if (eventObjectIds.length === 1) {
              unableToSplitAtStartOrEnd = setSingleEventRecord(eventRecords[singleEventIndex], objectIdFieldName, featureLayerDS, eventLyr, fieldInfos, rteInfo)
            } else if (!isDefined(eventOid) || !eventObjectIds.includes(Number(eventOid))) {
              queryEventsByEventObjectIds(featureLayerDS, eventObjectIds).then((features) => {
                const eventInfos: EventInfo[] = []
                features.forEach((feature, index) => {
                  const fromDateValue = feature.attributes[eventLyr.eventInfo.fromDateFieldName]
                  const toDateValue = feature.attributes[eventLyr.eventInfo.toDateFieldName]
                  const eventInfo: EventInfo = {
                    objectId: feature.attributes[objectIdFieldName],
                    fromMeasure: feature.attributes[eventLyr.eventInfo.fromMeasureFieldName],
                    toMeasure: feature.attributes[eventLyr.eventInfo.toMeasureFieldName],
                    fromDate: isDefined(fromDateValue) ? getDateWithTZOffset(fromDateValue, networkDS) : null,
                    toDate: isDefined(toDateValue) ? getDateWithTZOffset(toDateValue, networkDS) : null,
                    selectedPolyline: feature.geometry as Polyline
                  }
                  eventInfos.push(eventInfo)
                })
                handleMultiEventSelect(eventInfos)
              })
        ...
        if (unableToSplitAtStartOrEnd) {
          const toastMsg = getI18nMessage('unableToSplitAtStartOrEnd')
  ...
  const areEventDatesValid = (): boolean => {
    ...
    eventFromDate > getDateToUTC(new Date(Date.now()), true)) {
  ...
  const submitForm = async (locks?: LrsLocksInfo) => {
    ...
    if (noEventFound || unableToSplitAtStartOrEnd || unableToSplitOnDate) {
      const toastMsg = (noEventFound || unableToSplitOnDate) ? getI18nMessage('noEventFound') : getI18nMessage('unableToSplitAtStartOrEnd')
      onUpdateToastMsgType('error')
      onUpdateToastMsg(toastMsg)
      onUpdateToastOpen(true)
      setTimeout(() => {
    ...
    if (isDefined(eventLayer)) {
      ...
      fieldInfos1.forEach((item) => {
        ...
        if (isDefined(item.value) && isDate(item.type)) {
          ...
          indexedLrsAttrs[key] = getDateToUTC(date, true)
      ...
      fieldInfos2.forEach((item) => {
        ...
        if (isDefined(item.value) && isDate(item.type)) {
          // Remove timezone offset from date values, then get time in UTC.
          const date = getDateWithoutTZOffset(item.value.valueOf(), eventDS)
          indexedLrsAttrs2[key] = getDateToUTC(date, true)
        } else if (isNumber(item.type)) {
          // Convert any string values to number.
          if (!isDefined(item.value) || isNaN(Number(item.value))) {
    ...
    await LrsApplyEdits(eventFeatureLayer, eventLayer, null, request, null)
            ...
            queryEventsByEventObjectIds(featureLayerDS, result.editResults[0].splitEventResult.objectIds).then((features) => {
              features.forEach(async (feature, index) => {
                ...
                if (isDefined(polyline)) {
                  if (index === 0) {
                    graphicsManager?.flashGraphic(await getGeometryGraphic(await getSimpleLineGraphic(polyline), colorCyan))
                  } else {
                    setTimeout(async () => {
                      graphicsManager?.flashGraphic(await getGeometryGraphic(await getSimpleLineGraphic(polyline), colorCyan))
                    }, 5000)
    ...
    <div className='split-event-edit-attributes__content' css={getFormStyle()}>
      <div className='h-100'>
        ...
        <EventPickerPopup
          ...
          eventLayer={eventLayer}
          eventDS={eventDS}
          onEventSelect={handleEventPopupSelect}
          onEventSelectCancel={handleEventPopupCancel}
          measurePrecision={networkInfo?.measurePrecision}
          jimuMapView={jimuMapView}
