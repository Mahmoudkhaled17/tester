...
export default class ExportJson extends AbstractDataAction {
  async isSupported (dataSets: DataRecordSet[], dataLevel: DataLevel): Promise<boolean> {
    ...
    if (!isDefined(eventInfo)) {
      ...
      if (isDefined(datasources)) {
        const eventDSLayerId = Number((dataSource as FeatureLayerDataSource).layerId)
        if (datasources[dataSource.id]) {
          const ds = datasources[dataSource.id]
          if (Number(ds.getDataSourceJson().layerId) === eventDSLayerId) {
            if (isDefined(ds.getDataSourceJson().url)) {
              eventInfo = await getEventInfoFromLrsFeatureUrl(ds, this.widgetId)
            }
          }
        }
      }
    ...
    if (isDefined(selectedLrsLayer) && selectedLrsLayer.layerType === LrsLayerType.Network) {
      return Promise.resolve(false)
    }
    if (!eventInfo) {
      return Promise.resolve(false)
  ...
  async onExecute (dataSets: DataRecordSet[], dataLevel: DataLevel): Promise<boolean> {
    ...
    if (!isDefined(lrsLayerFound)) {
      const datasources = dataSet.dataSource?.dataSourceManager?.getDataSources()
      if (isDefined(datasources)) {
        const eventDSLayerId = Number((dataSet.dataSource as FeatureLayerDataSource).layerId)
        if (datasources[dataSet.dataSource.id]) {
          const ds = datasources[dataSet.dataSource.id]
          if (Number(ds.getDataSourceJson().layerId) === eventDSLayerId) {
            if (isDefined(ds.getDataSourceJson().url)) {
              eventInfo = await getEventInfoFromLrsFeatureUrl(ds, this.widgetId)
            }
    ...
    setTimeout(() => {
      if (isDefined(lrsLayerFound)) {
        ...
        MutableStoreManager.getInstance().updateStateValue(this.widgetId, 'selectedEventDataSource', dataSet.dataSource)
      }
      ...
      } else if (isDefined(eventInfo)) {
        MutableStoreManager.getInstance().updateStateValue(this.widgetId, 'selectedEventParentNetworkId', eventInfo.parentNetworkId)
        MutableStoreManager.getInstance().updateStateValue(this.widgetId, 'selectedEventRouteId', data[eventInfo.routeIdFieldName])
        MutableStoreManager.getInstance().updateStateValue(this.widgetId, 'selectedEventFromDate', new Date(data[eventInfo.fromDateFieldName]))
      }
    }, 1000)
    return Promise.resolve(true)
  }
