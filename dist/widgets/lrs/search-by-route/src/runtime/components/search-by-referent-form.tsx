...
import { convertStationToNumber, getDisplayField } from '../utils/utils'
import { debounce } from 'lodash-es'
import { maxLocationCount, Intellisense, getSuggestions, type Suggestion, GetUnits, isDefined, type LrsLayer, getOperationDate } from 'widgets/shared-code/lrs'
...
export const SearchReferentForm = React.forwardRef((props: SearchReferentProps, ref) => {
  ...
  */
  const querySuggestion = hooks.useEventCallback((starchText: string) => {
    const fieldName = getDisplayField(referentItem, originDS)
  ...
  */
  ...
  const validateReferentTextInput = (value: string | number): Promise<ValidityResult> => {
    ...
    return new Promise(resolve => {
      setTimeout(() => {
        ...
        const displayField = getDisplayField(referentItem, originDS)
        const displayFieldType = getDisplayFieldType(displayField)
        const code = getCode(value)
        if (code) value = code

        if (displayFieldType === 'NUMBER') {
          featureQuery.where = `${displayField} = ${value}`
        } else if (displayFieldType === 'STRING') {
          featureQuery.where = `${displayField} = '${value}'`
        } else if (displayFieldType === 'DATE') {
          const date = new Date(value)
          const opDate = getOperationDate(originDS, date)
          const isoDate = opDate.toISOString().slice(0, 10)
          const dateUTC = `TIMESTAMP '${isoDate}'`
          const where = `${displayField.toUpperCase()} = ${dateUTC}`
          featureQuery.where = where
        }
  ...
  const referentFieldLabel = getI18nMessage('referentDisplayName', { displayName: getDisplayField(referentItem, originDS) })
