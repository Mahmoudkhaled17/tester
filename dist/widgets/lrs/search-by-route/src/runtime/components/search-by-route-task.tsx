...
import {
  ...
  queryRoutesByGeometry,
  executeGeometryToMeasure,
  queryRoutesByClosestResults,
  isWithinTolerance,
  queryRoutesByGeometryWithTolerance,
  executeGeometryToMeasureWithTolerance,
...
import { executeMeasureToGeometry, queryRoutes, executeReferentToGeometry, executeMeasureToGeometryLine, getAliasRecord } from '../utils/service-utils'
...
export function SearchByRouteTask (props: SearchByRouteTaskProps) {
  ...
  const { widgetId, jimuMapView, lrsLayers, highlightStyle, labelStyle, resultConfig, defaultNetwork, hideMethod, hideNetwork, hideRoute , intl, hideTitle, graphicsManager } = props
  ...
  const clearGraphics = (): void => {
    graphicsManager?.clearCoordinateGraphic()
  }
  ...
  const handleCoordinatesFormSubmit = React.useCallback(async (query: CoordinateQuery) => {
    ...
    const coordinateConfig: CoordinateConfig = {
      spatialReferenceType: selectedNetwork.networkInfo.defaultSpatialReferenceFrom,
      searchUnits: selectedNetwork.networkInfo.unitsOfMeasure,
      searchRadius: selectedNetwork.networkInfo.searchRadius
    }

    // Perform spatial query based on coordinates without tolerance.
    queryRoutesByGeometry(originDS, selectedNetwork, query, Immutable(coordinateConfig))
      .then(async (routeRecords) => {
        if (routeRecords.length > 0) {
          // Find the measure and distance for the routes.
          executeGeometryToMeasure(widgetId, featureDS, selectedNetwork, routeRecords, query, labelLayer, lrsLayers, jimuMapView, Immutable(coordinateConfig))
            .then((newRecords) => {
              featureDS.setRecords(newRecords as FeatureDataRecord[])
              setResultCount(newRecords?.length)
            })
          ...
          await queryRoutesByGeometryWithTolerance(originDS, selectedNetwork, query, Immutable(coordinateConfig))
            .then(async (rteRecords) => {
              if (rteRecords.length > 0) {
                ...
                await Promise.all(rteRecords.map(async (rteRecord) => {
                  const dataSource: any = featureDS.getOriginDataSources()[0]
                  const originDs: FeatureLayerDataSource = dataSource as FeatureLayerDataSource
                  await executeGeometryToMeasureWithTolerance(rteRecord, dataSource, originDs, selectedNetwork, query, Immutable(coordinateConfig))
      ...
      <div className={classNames('search-by-route-task__content', {
        ...
        {selectedMethod === SearchMethod.Coordinate && (
          <SearchCoordinatesForm
            ...
            widgetId={widgetId}
            lrsLayer={selectedNetwork}
            network={selectedNetwork?.networkInfo}
