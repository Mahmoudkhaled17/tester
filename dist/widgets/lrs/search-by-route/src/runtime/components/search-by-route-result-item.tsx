...
export const SearchByRouteResultItem = React.memo((props: ResultItemProps) => {
      ...
      .then((geometryInSR) => {
        ...
        if (graphicInSR.geometry.type === 'point') {
          jimuMapView?.view.goTo({
            target: graphicInSR.geometry, zoom: 100
          })
        } else {
          jimuMapView?.view.goTo({ target: graphicInSR.geometry.extent.expand(1.25) })
        }
      })
  }, [jimuMapView.view])
  ...
  const handleClickResultItem = React.useCallback((e?: React.MouseEvent<HTMLDivElement>) => {
    e?.stopPropagation()
