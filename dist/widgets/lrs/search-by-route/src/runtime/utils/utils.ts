import type { IntlShape, FeatureLayerDataSource, ImmutableArray, DataSource } from 'jimu-core'
import { derivedFields, distanceField, GetUnits, Identifiers, isDefined, type LrsLayer, measureFields, SearchMeasuresType, SearchMethod, stationField, toRouteField, equalsIgnoreCase } from 'widgets/shared-code/lrs'
import type { ImmutableObject } from 'seamless-immutable'
import { getReferentProperties } from '../../common/utils'

export function getDisplayField (referentItem: ImmutableObject<LrsLayer>, ds: DataSource): string {
  let displayField = referentItem?.displayField
  const featureLayerDS = ds as FeatureLayerDataSource
  const editorTrackingInfo = featureLayerDS?.getLayerDefinition()?.editFieldsInfo
  if (equalsIgnoreCase(displayField, editorTrackingInfo?.creationDateField) ||
      equalsIgnoreCase(displayField, editorTrackingInfo?.creatorField) ||
      equalsIgnoreCase(displayField, editorTrackingInfo?.editDateField) ||
      equalsIgnoreCase(displayField, editorTrackingInfo?.editorField)) {
        displayField = getReferentProperties(referentItem)?.layerFields[0]?.name
  }
  return displayField
}
...
function getNetworkFieldNamesForDisplay (lrsLayer: ImmutableObject<LrsLayer>, record: __esri.Graphic, selectedMethod: string, derivedNetwork): string[] | string {
  ...
  allFields.forEach((field) => {
    ...
    if (lrsDisplayFields.includes(jimuFieldName) &&
...
export function getLabelFields (fields: __esri.Field[]): __esri.Field[] {
  ...
  if (!isDefined(hasMeasureField)) {
    const measureField = (fields.at(0) as any).clone()
  ...
  if (!isDefined(hasToMeasureField)) {
    const toMeasureField = (fields.at(0) as any).clone()
