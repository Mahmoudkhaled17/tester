...
import { maxRequestCount, getPolylineMidPoint, ConvertUnits, isDefined, requestService, getDateWithTZOffset, type LrsLayer, derivedFields, measureFields, distanceField, toRouteField, stationField, networkObjectIdField, LrsLayerType, SpatialReferenceFrom, type DerivedInfo, SearchMeasuresType } from 'widgets/shared-code/lrs'
...
export async function queryRoutes (
  ...
  if (isDefined(originDS && isDefined(originDS.layer))) {
    return await loadArcGISJSAPIModules(['esri/time/TimeExtent']).then(modules => {
      const TimeExtent: typeof __esri.TimeExtent = modules[0]
...
export async function executeReferentToGeometry (
  ...
  const createDataRecords = async (locationItem, record: DataRecord, uniqueLocations) => {
    ...
    if (locationItem?.status === 'esriLocatingMultipleLocation') {
      ...
      if (!includesObject && isDefined(data) && result?.routeId === data[lrsLayerItem.networkInfo.routeIdFieldName]) {
        uniqueLocations.push(result)
        if (result.geometryType === 'esriGeometryPoint') {
          geometry = new Point(result.geometry)
          geometry.spatialReference = spatialReference
  ...
  const queryRoutes = async () => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      // Get unique FromDt from objectIdFromDt
      const uniqueFromDts = [...new Set(Object.values(objectIdFromDt))]

      if (uniqueFromDts.length === 1) {
        let fromDate = uniqueFromDts[0]
        ...
        if (referentItem?.layerType === LrsLayerType.Event) {
          let count = 0
          results.records.some(record => {
            if (count < maxRequestCount) {
              records.push(record)
              uniqueFromDts?.forEach(uniqueFromDt => {
                count++
                const fromDate = uniqueFromDt
                promises.push(executeReferentToGeometryQuery(locations, lrsLayerItem, offsetUnit, gdbVersion, fromDate))
              })
              return false
            } else {
