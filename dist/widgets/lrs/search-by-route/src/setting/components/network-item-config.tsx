...
export function NetworkItemConfig (props: Props) {
  ...
  const defaultNetwork = useConfigValue('defaultNetwork', '') as string
  ...
  const onLayerChanged = React.useCallback((index: number, updatedLayerItems, dsUpdateRequired = false) => {
    ...
    // If the renamed layer is the default network, update defaultNetwork too
    const oldName = (lrsLayers[index])?.name
    const newName = updatedLayerItems?.name
    const updatedDefaultNetwork = (defaultNetwork === oldName && newName !== oldName)
      ? newName
      : defaultNetwork
  ...
  }, [lrsLayers, defaultNetwork, settingsPerView, config, activeMapViewId, onSettingChange, widgetId])
  ...
  const handleLabelAccept = React.useCallback((value) => {
    if (value.trim().length > 0) {
      onLayerChanged(index, (lrsLayer).set('name', value), true)
    } else {
      setItemLabel(lrsLayer?.name)
    }
  }, [lrsLayer, index, onLayerChanged])
