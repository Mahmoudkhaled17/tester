...
export default class BAMapActions {
  ...
  public initialize ( view ) {
    ...
    self.view.on( 'click', evt => {
      ...
      function highlightPolygon ( response: any, data: any ) {
        ...
        const attributes = graphic?.attributes ? graphic.attributes : {}
        const displayField = graphic?.layer?.displayField
        const displayName = displayField && attributes && attributes[displayField] != null
          ? attributes[displayField].toString()
          : ''

        let symbol
        if ( graphic?.symbol ) {
          symbol = typeof graphic.symbol.toJSON === 'function' ? graphic.symbol.toJSON() : graphic.symbol
        }
        ...
        if ( self.context && self.context.onSiteObjectChanged ) {
          const obj: any = {
            ...
            data: {
              ...
              spatial: graphic?.geometry?.spatialReference,
              attributes,
              displayName,
              symbol
            }
  ...
  public onDrawnPolygon ( e: any ) {
    if ( e.geometry?.type === 'polygon' ) {
      const geometry = e.geometry as __esri.Polygon
  ...
  public onDrawnPoint ( e: any ) {
    if ( e.geometry?.type === 'point' ) {
      const geometry = e.geometry as __esri.Point
