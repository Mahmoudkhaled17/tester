...
import {
  ...
  type AllWidgetProps, jsx, moduleLoader, semver, DataSourceComponent, AllDataSourceTypes, LayoutItemType, type BrowserSizeMode, ContainerType
} from 'jimu-core'
...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & ExtraProps, any> {
  ...
  componentDidMount () {
    this.updateNeedTabIndex()
    survey123Service.setQueryObject(this.props.queryObject)
    this.prepare() //this.loadSurveyAPI();
  }

  componentDidUpdate (prevProps) {
    if (prevProps.browserSizeMode !== this.props.browserSizeMode || prevProps.controllerWidgetId !== this.props.controllerWidgetId) {
      this.updateNeedTabIndex()
    }
  }

  updateNeedTabIndex = () => {
    const { id, browserSizeMode, controllerWidgetId } = this.props
    let needTabIndex: boolean

    if (controllerWidgetId) {
      needTabIndex = true
    } else {
      const appState = getAppStore().getState()
      const appConfig = appState?.appConfig
      const rootContainerInfo = searchUtils.getContentRootContainerInfo(appConfig, id, LayoutItemType.Widget, browserSizeMode)
      needTabIndex = rootContainerInfo?.type === ContainerType.Dialog
    }

    if (this.state.needTabIndex !== needTabIndex) {
      this.setState({ needTabIndex }, () => { this.updateIframeTabIndex() })
    } else {
      this.updateIframeTabIndex()
    }
  }
    ...
    return Promise.resolve(true)
      ...
      .then((feature) => {
        ...
        if (mode === 'new' && this.props.config.activeLinkData && this.props.config.fieldQuestionMapping) {
          const hasGeometryMapping = this.props.config.fieldQuestionMapping.find((item) => {
            return item.field === 'geometry' || item.field === 'geometry_shape'
          })

          const reservedGeometryFieldName = hasGeometryMapping ? this.getReservedGeometryName() : 'geometry'
          ...
          this.props.config.fieldQuestionMapping.forEach((item) => {
            ...
            if (field === reservedGeometryFieldName) {
  ...
  buildWebFormConfig (postProcess?: any) {
    ...
    option.onFormSubmitted = (data) => {
      ...
      survey123Service.updateRelatedDataSources(config.surveyItemId, data?.result)
    }
  ...
  /**
   * some layer has a geometry name field which the field type is string, we need to use the reserved geometry_shape field instead of the geometry field (which is defined in settings)
   * @returns
   */
  getReservedGeometryName (): string {
    ...
    const existStringGeoField = fieldsArray.find((item: any) => {
      return item.type !== 'esriFieldTypeGeometry' && item.name === 'geometry'
    })
    return existStringGeoField ? 'geometry_shape' : 'geometry'
  ...
  getSelectedDsRecord = (ds) => {
    ...
    const extraOption = uniqueIdFields.length ? {uniqueIdFields: uniqueIdFields} : null
    return query.executeQueryJSON(ds.url, queryOption, null, extraOption).then((results) => {
      if (results.features && results.features[0]) {
        return results.features[0]
      }
      return feature
    }).catch((e) => {
      console.log('Failed to query layer,', e)
      return feature
    })
  }
  ...
  updateIframeTabIndex () {
    const webappIframe = this.iframeContainer?.querySelector('iframe')
    if (webappIframe && this.state.needTabIndex) {
      webappIframe.tabIndex = 0
    }
  }
