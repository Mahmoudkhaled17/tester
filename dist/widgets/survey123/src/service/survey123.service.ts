...
interface Survey123ApplyEditsResult {
...
interface Survey123LayerApplyEditsResults {
  id?: string | number
  addResults?: Survey123ApplyEditsResult[]
  updateResults?: Survey123ApplyEditsResult[]
  deleteResults?: Survey123ApplyEditsResult[]
}

interface Survey123RelatedServiceInfo {
  type: string
  url?: string
  itemId?: string
}

type Survey123EditAction = 'add' | 'update' | 'delete'
...
export class Survey123Service {
  ...
  */
  public getSurvey123HostAPIUrl () {
    let url = this.getSurvey123HostUrl() + '/api/jsapi/3.25/' //  '/share/survey123webform-jsapi.js';
  ...
  */
  public updateRelatedDataSources (surveyItemId: string, layerResults?: Survey123LayerApplyEditsResults[]) {
    ...
    return Promise.resolve(true)
      ...
      .then((services: Survey123RelatedServiceInfo[]) => {
        if (!services?.length) {
          return
        }

        return Promise.all(layerResults.map(async (layerResult) => {
          const dataSources = this.getMatchedLayerDataSources(dataSourceList, services, layerResult.id)
          if (!dataSources.length) {
            return
          }

          await Promise.all(dataSources.map(async (dataSource: any) => {
            await Promise.all([
              this.afterLayerEdits(dataSource, 'add', layerResult.addResults),
              this.afterLayerEdits(dataSource, 'update', layerResult.updateResults),
              this.afterLayerEdits(dataSource, 'delete', layerResult.deleteResults)
            ])
          }))
        }))
      })
  ...
  /**
   * Collapse data views and local data sources to their main data source so one layer refresh only runs once.
   */
  private getMainDataSources (dataSourceList: any[]): any[] {
    const mainDataSourcesMap = {}

    dataSourceList.forEach((dataSource: any) => {
      // Data views/local data sources should only refresh once through their shared main data source.
      const mainDataSource = dataSource?.getMainDataSource?.() || dataSource
      if (mainDataSource?.id && !mainDataSourcesMap[mainDataSource.id]) {
        mainDataSourcesMap[mainDataSource.id] = mainDataSource
      }
    })

    return Object.values(mainDataSourcesMap)
  }

  /**
   * Find the survey-related main data sources whose layer id matches the current applyEdits layer result.
   */
  private getMatchedLayerDataSources (dataSourceList: any[], services: Survey123RelatedServiceInfo[], layerId?: string | number): any[] {
    return this.getMainDataSources(dataSourceList).filter((dataSource: any) => {
      return this.isSurveyRelatedDataSource(dataSource, services, layerId)
    })
  }

  /**
   * Match a data source to the survey's related services first, then narrow it to the target layer when a layer id is provided.
   */
  private isSurveyRelatedDataSource (dataSource: any, services: Survey123RelatedServiceInfo[], layerId?: string | number): boolean {
    const itemId = dataSource?.itemId || dataSource?.getDataSourceJson?.()?.itemId
    const dataSourceUrl = this.normalizeUrl(dataSource?.url || dataSource?.getDataSourceJson?.()?.url)
    const dataSourceLayerId = this.getDataSourceLayerId(dataSource)

    return services.some((serviceInfo) => {
      const serviceUrl = this.normalizeUrl(serviceInfo.url)
      // Survey related services may match by item id or by service url prefix.
      const isServiceMatched = (serviceInfo.itemId && serviceInfo.itemId === itemId) || !!(serviceUrl && dataSourceUrl && dataSourceUrl.startsWith(serviceUrl))
      ...
      // ApplyEdits results are layer-scoped, so only refresh the matching layer data source.
      return dataSourceLayerId === `${layerId}`
    })
  }

  /**
   * Read the effective layer id from runtime properties, ds json, or layer definition in that order.
   */
  private getDataSourceLayerId (dataSource: any): string {
    const layerId = dataSource?.layerId ?? dataSource?.getDataSourceJson?.()?.layerId ?? dataSource?.getLayerDefinition?.()?.id
    return layerId === undefined || layerId === null ? '' : `${layerId}`
  }
  ...
  /**
   * Apply one layer's add/update/delete results back to the matched data source so dependent widgets refresh correctly.
   */
  private async afterLayerEdits (dataSource: any, action: Survey123EditAction, results?: Survey123ApplyEditsResult[]): Promise<void> {
    ...
    if (action === 'delete') {
      // Delete refresh only needs objectIds; the records may no longer be queryable after applyEdits succeeds.
      const deletedIds = successfulResults
        .map((result) => result?.objectId)
        .filter((id) => id !== undefined && id !== null && Number(id) >= 0)

      if (deletedIds.length) {
        dataSource.afterDeleteRecordsByIds(deletedIds)
      } else {
    ...
    const records = (await Promise.all(successfulResults.map(async (result) => {
      ...
      try {
        // Re-query the saved feature so the data source refresh uses the server's final record state.
        return await dataSource.queryById(objectId)
      } catch (err) {
        return null
      }
    ...
    if (!records.length) {
      // Add can still invalidate caches even when the new record cannot be queried back immediately.
      if (action === 'add') {
    ...
    dataSource.afterUpdateRecords(records)
  }
