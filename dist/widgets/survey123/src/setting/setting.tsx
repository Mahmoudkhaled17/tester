...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<IMConfig>, any> {
  ...
  constructor (props: any) {
    ...
    this.prepare().then(() => {
      ...
      return this.loadExistingSurvey()
    }).then((survey) => {
      if (!survey) {
        this.querySurvey()
      }
  ...
  getLayerFields = (layer) => {
    ...
    // most of the layers hove no esriFieldTypeGeometry type field, but some have.
    // and some layers have geometry-name field with esriFieldTypeString type like: https://arundel4.esri.com:8443/rest/services/Hosted/globalearthquakes1milES8140OnDevDBB/FeatureServer/0
    const existGeoFieldIdx = (fields || []).findIndex((item) => {
      return item.type === 'esriFieldTypeGeometry'
    })
    // remove the esriFieldTypeGeometry field
    if (existGeoFieldIdx >= 0) {
      fields.splice(existGeoFieldIdx, 1)
    }

    const existStringGeoField = (fields || []).find((item) => {
      return item.type !== 'esriFieldTypeGeometry' && item.name === 'geometry'
    })

    // add geometry field with custom label
    ...
    fields.push({
      ...
      name: existStringGeoField ? 'geometry_shape' : 'geometry'
    })
    return fields
  }
  ...
  querySurvey = (options?: {
    ...
    return Promise.resolve(true)
      .then(() => {
         ...
         * even the current user is org admin, should still only show my owned surveys here
  ...
  /**
   * load existing survey when open the setting page
   */
  loadExistingSurvey = () => {
    ...
    return survey123Service.getItemInfo(usedSurveyItemId)
      .then((survey) => {
        this.setState({
  ...
  showSurvey123DesignerModal (surveyItemId?: string) {
    ...
    const checkTimer = setInterval(() => {
      ...
      if (target || index > 100) {
        ...
        this._setIframeAllowAttributes(target)
        target.src = url
      }
    }, 50)
  }
  ...
  /**
   * _setIframeAllowAttributes
   * @param iframe
   */
  _setIframeAllowAttributes (iframe: HTMLIFrameElement) {
    if (!iframe) {
      console.warn('Missing iframe.')
      return
    }

    const attributes = [ 'geolocation', 'camera', 'microphone', 'local-network-access']

    /**
     * only allow self domain for geolocation / camera / microphone
     * add local-network-access to allow access to local server when using survey123 desinger app in local environment
     */
    const originWhitelist = `self ${survey123Service.getSurvey123HostUrl()} ${window.location.protocol}//${window.location.host}`
    const allowAttributes = attributes.map((attr) => {
      if (['local-network-access'].includes(attr)) {
        return attr
      }
      return `${attr} ${originWhitelist}`
    }).join('; ')

    iframe.allow = allowAttributes
  }
                ...
                (this.state.currentActiveSurvey)
                  ? [
                    ...
                    <SettingRow className="items" key="surveyCreationDate">
                      <div className="w-100">{this.nls('updatedOn', { updatedDate: dateUtils.formatDateValueByIntlFormat(this.state.currentActiveSurvey.formItemInfo.modified, this.props.intl) })}</div>
                    </SettingRow>,
