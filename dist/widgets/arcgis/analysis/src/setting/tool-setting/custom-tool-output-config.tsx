...
import { React, jsx, Immutable, hooks, useIntl, dateUtils, EsriDateFormats, css, loadArcGISJSAPIModule, dataSourceUtils, EsriFieldType } from 'jimu-core'
...
const GPDateConfig = (props: GPTypeConfigProps) => {
  ...
  const dateFormatOptions = React.useMemo(() => {
    ...
    return dateFormats.map((format) => {
      const newFormat = dataSourceUtils.dateTimeFieldFormat({ dateFormat: format.format } as any)
      const label = dateUtils.formatDateValueByEsriFormat(date, newFormat, EsriFieldType.Date, intl)
...
const CustomToolOutputConfig = (props: Props) => {
  ...
  const handleParameterChange = (parameter: Immutable.ImmutableObject<AnalysisToolParam>) => {
  ...
  const outputParameters = [...parameters].filter(p => p.direction === 'esriGPParameterDirectionOutput' && !needHideOutputParams.includes(p.name))
  ...
  return (
    <div className={className}>
      ...
      <List
        size='default'
        className='input-parameter-list'
        itemsJson={outputParameters.map((parameter) => ({
          itemStateDetailContent: parameter,
          itemKey: parameter.name
        }))}
        dndEnabled
        onUpdateItem={(actionData, refComponent) => {
          ...
          const sortedOutputParameters = parentItemJson.map(item => {
            return item.itemStateDetailContent
          })

          // append the rest parameters which are not input parameters
          parameters.forEach(p => {
            const index = sortedOutputParameters.findIndex(sp => sp.direction === p.direction && sp.name === p.name)
            if (index < 0) {
              sortedOutputParameters.push(p)
            }
          })
          onParameterChange(sortedOutputParameters)
        }}
        ...
        renderOverrideItemMainLine={(actionData, refComponent) => {
          ...
          const parameter = currentItemJson.itemStateDetailContent
          ...
          return (<div>
            <CustomToolConfigCollapsablePanel
              key={parameter.name}
              parameter={parameter}
              onParameterChange={(newParameter) => { handleParameterChange(newParameter) }}
            >
