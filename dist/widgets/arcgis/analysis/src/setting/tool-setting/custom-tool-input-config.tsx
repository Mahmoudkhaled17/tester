...
const GPDateConfig = (props: GPTypeConfigProps) => {
  ...
  return (
    <React.Fragment>
      <SettingRow className='mt-2 label-dark-400 last-setting-row' label={translate('defaultValue')} flow='wrap' role='group' aria-label={translate('defaultValue')}>
        <DatePicker css={css`flex: 1;`} dateFormat={dateFormats} isLongTime
          showTimeInput strategy="absolute" showDoneButton
...
const GenericMultiValue = (props: GPTypeConfigProps) => {
  ...
  return (
    <SettingRow className='mt-2 label-dark-400 last-setting-row' label={translate('defaultValue')} flow='wrap' role='group' aria-label={translate('defaultValue')}>
      <analysis-value-table
        ref={(instance) => { setValueTableRef(instance as unknown as HTMLAnalysisValueTableElement) }}
        {...parameter.asMutable({ deep: true })}
      />
    </SettingRow>
  )
}
...
const style = css`
  ...
  --analysis-color-border-input: var(--ref-palette-neutral-300);
...
const CustomToolInputConfig = (props: Props) => {
  ...
  const handleParameterChange = (parameter: Immutable.ImmutableObject<CustomToolParam>) => {
  ...
  const inputParameters = [...parameters].filter(p => p.direction === 'esriGPParameterDirectionInput')
  ...
  return (
    <div className={className} css={style}>
      <List
        ...
        itemsJson={inputParameters.map((parameter) => ({
        ...
        onUpdateItem={(actionData, refComponent) => {
          ...
          const sortedInputParameters = parentItemJson.map(item => {
            return item.itemStateDetailContent
          })
          // append the rest parameters which are not input parameters
          parameters.forEach(p => {
            const index = sortedInputParameters.findIndex(sp => sp.direction === p.direction && sp.name === p.name)
            if (index < 0) {
              sortedInputParameters.push(p)
            }
          })
          onChange(sortedInputParameters)
        }}
            ...
            <CustomToolConfigCollapsablePanel
              ...
              {ConfigComponent && (
                <ConfigComponent
                  ...
                  translate={translate}
                  onChange={handleParameterChange}></ConfigComponent>
              )}
            </CustomToolConfigCollapsablePanel>
          </div>)
        }}
