...
import { React, urlUtils, type ImmutableObject, type UtilitiesJson } from 'jimu-core'
...
export function getAnalysisComponentsAssetPath () {
  return getAssetsPathByFolderName('analysis-components')
}

export function getAnalysisToolAppAssetPath () {
  return getAssetsPathByFolderName('analysis-tool-app')
}
...
export const getStrings = async (path: string) => {
  try {
    const strings = await StringsManager.getInstance().getStrings(path)
    return strings
  } catch (e) {
    return {}
  }
...
export const useCommonStringsByLocale = (locale: string) => {
  return useStrings(`${getAnalysisToolAppAssetPath()}assets/t9n/common.t9n.${locale ?? 'en-US'}.json`)
}

export const getCommonStringsByLocale = async (locale: string) => {
  return await getStrings(`${getAnalysisToolAppAssetPath()}assets/t9n/common.t9n.${locale ?? 'en-US'}.json`)
}
...
export const getToolInfoStringsByLocale = async (locale: string) => {
  const commonStrings = await getCommonStringsByLocale(locale)
  return commonStrings ? { ...(commonStrings.toolInfo as LocaleItem || {}), ...(commonStrings.toolInfoKeys as LocaleItem || {}) } : null
}
