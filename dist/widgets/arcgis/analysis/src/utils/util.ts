...
export const getCustomToolParamDisplayName = (toolJson: AnalysisToolInfo | ImmutableObject<AnalysisToolInfo>, paramName: string) => {
 ...
 * @param {GPMessage[]} messages the messages to be filtered
 * @returns {GPMessage[]} the filtered messages
 */
export function filterHistoryItemMessages (messages: GPMessage[], toolType: ToolType, messageLevel?: MessageLevel, filterByMessageLevel = true): GPMessage[] {
  if (!messages?.length) {
    return []
  }
  ...
  const convertedMessages = messages.map((msg) => {
    msg.type = convertEsriMessageType(msg.type) as GPMessage['type']
    return msg
  })
  return convertedMessages.filter((message: GPMessage) => {
...
export function getEnumerableObjectFromAccessor (accessor: Accessor): { [key: string]: any } {
  let obj = {}
...
export function depthTraversalProcessingValue<T> (value: ParameterValue['value'], rootId: string, callback: (...args: any) => T) {
...
export function setValueToResultById (id: string, result: ParameterValue, newValue: any) {
  const keys = id.split('-').map((n) => Number(n)).slice(1)
  // if no keys, change the value directly
  if (!keys.length) {
    result.value = newValue as ParameterValue['value']
    return
  }
...
export async function getActiveAnalysisEngines (portal: Portal): Promise<AnalysisEngine[]> {
...
export function useAnalysisEnginesAccess (portal: Portal) {
...
export function getValidToolsList (activeAnalysisEngines: AnalysisEngine[], userHasNaPrivilege: boolean, toolsArray: AnalysisToolItem[], portal: Portal, hasAdvancedLevelLicense: boolean): AnalysisToolItem[] {
...
export async function getAllValidToolsList (toolsArray: AnalysisToolItem[], portal: Portal) {
...
export function resultValueIsFeatureSet (value: ParameterValue['value'] | GPFeatureRecordSetLayer): value is __esri.FeatureSet {
  return value != null && typeof value === 'object' && 'features' in value && Array.isArray(value.features)
}

export function resultValueIsFeatureCollectionJson (value: ParameterValue['value'] | GPFeatureRecordSetLayer): value is __esri.FeatureSet {
  return value != null && typeof value === 'object' && 'featureSet' in value
}

export function resultValueIsFeatureLayer (value: ParameterValue['value'] | GPFeatureRecordSetLayer): value is __esri.FeatureLayer {
