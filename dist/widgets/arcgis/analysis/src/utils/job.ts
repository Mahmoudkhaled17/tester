...
import { getAnalysisToolAppAssetPath } from './strings'
...
export async function addProcessInfoToItems (results: ParameterValue[], processInfo?: string): Promise<void> {
  const itemUpdatePromises: Array<Promise<PortalItem>> = []
...
export async function waitForJobCompletionAndGetResults (
  jobInfo: JobInfo,
  toolUrl: string,
  statusCallback: (j: JobInfo) => void,
  gpMessages: LocaleItem,
  resultParams: string[],
  resultMapServerName: string,
  resultParamsNotInMapService: string[],
  formatResultFn?: (result: ParameterValue, resultParam: string, id: string) => Promise<ParameterValue>
) {
  const options = {
    interval: 1500,
  ...
  try {
    ...
    if (!isEmptyValue(resultMapServerName)) {
      const ParameterValueClz: typeof ParameterValue = await loadArcGISJSAPIModule('esri/rest/support/ParameterValue')
       ...
       */
      const mapServiceResult = ParameterValueClz.fromJSON({
...
export async function completeHistoryInfo (historyItem: HistoryItemWithDs, importedFromMap: boolean, toolList: ImmutableArray<ToolConfig>, gpMessages: LocaleItem) {
  ...
  if ((importedFromMap && historyItem.analysisType === 'tool') || toolInfo.type === ToolType.Standard) {
    ...
    const toolJsonPath = `${getAnalysisToolAppAssetPath()}assets/tool-json/${lowercaseToolName}.tool.json`
    try {
...
export async function executeJob (geoprocessor: typeof Geoprocessor, params: ExecuteProps, toolJson: AnalysisToolInfo, toolUrl: string, webToolServerUrl: string, ignoreResultMapServer?: boolean) {
  ...
  if (toolJson.executionType === 'esriExecutionTypeSynchronous') {
    ...
    const JobInfoClz: typeof JobInfo = await loadArcGISJSAPIModule('esri/rest/support/JobInfo')
    try {
      const res = await geoprocessor.execute(toolUrl, jobParamsPayload, gpOptions) as SynchronousJobExecuteResult
      const { messages, results } = res

      const jobInfo = JobInfoClz.fromJSON({
        jobId,
      ...
      notifyJobStatus(containerElement, {
        jobInfo: JobInfoClz.fromJSON({
          jobId,
          jobStatus: 'job-failed',
          messages: Array.isArray(error?.details?.messages) ? error.details.messages.map((msg) => ({ description: msg, type: 'error' } as GPMessage)) : [],
          sourceUrl: webToolServerUrl
        }),
        submissionData: {
          toolName: toolJson.name,
