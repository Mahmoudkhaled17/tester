...
export const loadAnalysisHistoryResourceItemsFromMap = async (jimuMapView: JimuMapView, portal: Portal) => {
  try {
    const loadedResources = await loadSavedResource<AnalysisHistory>(`${HISTORY_STORAGE_KEY}/${HISTORY_FILE_NAME}`, ((jimuMapView?.view?.map as unknown) as WebMap)?.portalItem)
...
function handleNestedValue (value: ParameterValue['value']) {
  if (Array.isArray(value)) {
    return value.map((v: ParameterValue['value']) => {
      return handleNestedValue(v)
    })
  } else {
    return getEnumerableObjectFromAccessor(value as Accessor)
  }
}
    ...
    .map((historyItem) => {
      ...
      const updatedHistoryItem: SerializedHistoryItem = {
        ...
        results: historyItem.results?.map((r, index) => {
          const storedValue: Partial<ParameterValue['value']> = handleNestedValue(r.value)
          ...
          if (isCustomTool) {
            depthTraversalProcessingValue(result.value, `${index}`, (id: string, value: ParameterValue['value']) => {
    ...
    const newList = historyList.map((h) => {
      ...
      if (matchedResult) {
        ...
        if (h.results?.length) {
          ...
          createDsPromises.push(createDsByResults(widgetId, tool, h.results, h, dsOrderRef))
        }
      }
      return h
    })
...
export const useHistoryList = (widgetId: string, currentJimuMapView: JimuMapView, config: IMConfig, dsOrderRef: React.MutableRefObject<number>) => {
  ...
  React.useEffect(() => {
    ...
    Promise.allSettled(updateJobInfoPromises).then((res) => {
      ...
      Promise.allSettled(values.map((v) => {
        ...
        return createDsByResults(widgetId, tool, v.results, v, dsOrderRef)
      })).then(() => {
  ...
  React.useEffect(() => {
    ...
    historyListFromMap.forEach((v) => {
      ...
      const tool = toolList.find((tool) => tool.id === v.toolId)
      if (!v.dsMap) {
        createDsByResults(widgetId, tool, v.results, v, dsOrderRef)
      }
    })
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [historyListFromMap])
