...
export const SelectParameterPopper = (props: Props) => {
  ...
  const getParameterFilterTypes = (parameter: AnalysisToolParam) => {
    // only use filter to check geometryType
  ...
  hooks.useUpdateEffect(() => {
    ...
    if (selectedTool.type === ToolType.Standard) {
      ...
      const toolJsonPath = `${widgetUrl}dist/assets/analysis-tool-app/assets/tool-json/${lowercaseToolName}.tool.json`
      const toolUIJsonPath = `${widgetUrl}dist/assets/analysis-tool-app/assets/tool-ui-json/${lowercaseToolName}.tool.ui.json`
      const t9nPath = `${widgetUrl}dist/assets/analysis-tool-app/assets/t9n/${lowercaseToolName}/${lowercaseToolName}.t9n.${locale}.json`
      Promise.all([
  ...
  const handleConfirm = async () => {
    ...
    if (isDsLevel && hasUrl && !isOutputFromWidget) {
      ...
      try {
        ...
        const featureSet: any = await featureUtils.convertDataRecordSetToFeatureSet({ dataSource, records, fields: layerDefinition.fields.map((f) => f.name), name })
        const hasZ = featureSet.features[0].geometry?.hasZ

        if (hasZ) {
          layerDefinition.hasZ = true
          featureSet.hasZ = true
        }
