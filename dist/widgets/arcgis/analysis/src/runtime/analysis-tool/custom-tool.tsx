...
import { type AnalysisToolDataItem, type AnalysisToolData, AnalysisEngine, AnalysisType, type LocaleItem, type AnalysisServiceInfo, type AnalysisToolValidationParams, AnalysisToolParamDataType, type AnalysisToolUI, type FeatureCollection, AnalysisToolParamDirection } from '@arcgis/analysis-ui-schema'
...
import type * as Geoprocessor from '@arcgis/core/rest/geoprocessor'
...
interface AnalysisToolDataChangeEventDetail {
...
const isPublicService = async (url: string): Promise<any> => {
  let endpoint = url
  if (endpoint.includes('?')) {
    const [endpointUrl] = endpoint.split('?')
    endpoint = endpointUrl
  }

  try {
    const response: any = await fetch(`${endpoint}?f=json`)
    const data = await response.json()
    if (data?.error && data.error.code && data.error.code > 400 && data.error.code < 500) {
      return false
    }
    return true
  } catch (error) {
...
const CustomTool = (props: ToolProps) => {
  ...
  const [geoprocessor, setGeoprocessor] = useState<typeof Geoprocessor>()
  const showHelp = useRef<(...args) => any>(null)
  useEffect(() => {
    loadArcGISJSAPIModules(['esri/rest/geoprocessor']).then((modules) => {
      const gp = modules[0] as typeof Geoprocessor
      setGeoprocessor(gp)
    })
  ...
  const setAnalysisToolRef = (ref: HTMLDivElement, recreate: boolean = false) => {
    ...
    container.toolJson = convertedToolJson
    container.skipApplyingDefaults = !!jobParams
    ref.replaceChildren(container)
    container.addEventListener('analysisToolLoaded', () => {
      setAnalysisToolAppLoaded(true)
    })
    container.addEventListener('analysisToolDataChange', setAnalysisToolData)
    setAnalysisToolContainer(container)

    isPublicService(webToolServerUrl).then((isPublic) => {
      if (isPublic) {
        container.options = {
          skipCheckUrlForInputLayers: true
        }
      }
    })
  }
  ...
  useEffect(() => {
    ...
    getWebToolUIJson(toolJsonCopy).then((json) => {
      setToolUIJson(json)
  ...
  const onToolDataChange = (e?: CustomEvent<AnalysisToolDataChangeEventDetail> | Record<'detail', AnalysisToolDataChangeEventDetail>) => {
    ...
    // add insibible parameters back to jobParams
    toolJson.parameters?.forEach((param: ImmutableObject<CustomToolParam>) => {
      if (param.direction === AnalysisToolParamDirection.Input && param.invisible) {
        jobParams[param.name] = param.defaultValue
      }
    })
  ...
  const handleExecutionFailure = (error: Error | ArcgisWebAnalysisError) => {
    ...
    if ('messages' in error && Array.isArray(error.messages)) {
      const hasExecuteError = error.messages.some((message: GPMessage) => message.type === 'error' && message.description.startsWith('Failed to execute'))
      if (hasExecuteError) {
  ...
  const convertParamsBySetting = async (jobParams: AnalysisToolData) => {
    ...
    for (const paramName of params) {
      ...
      if (param.useFeatureCollection && value.url) {
        try {
          const apiModules = await loadArcGISJSAPIModules(['esri/layers/FeatureLayer'])
          const FeatureLayer: typeof __esri.FeatureLayer = apiModules[0]
          const layer = new FeatureLayer({ url: value.url })
  ...
  useEffect(() => {
    if (!option.showHelpLink && toolHelpPopover.current) {
      ...
      toolHelpPopover.current = undefined
    }
  }, [option.showHelpLink])
        ...
        {option.showHelpLink && <CalciteAction
          ...
          onClick={() => {
            if (option.link !== undefined && showHelp.current) {
              toolHelpPopover.current = showHelp.current({
                helpUrl: option.link,
                helpId: toolId,
                heading: displayedToolName,
                referenceElement: headerElementRef.current,
                isHelpFileExternalAsset: false,
