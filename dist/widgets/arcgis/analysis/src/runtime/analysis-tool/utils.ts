...
import type { AnalysisToolData, AnalysisToolInfo, FeatureCollection } from '@arcgis/analysis-ui-schema'
...
export const changeDisplayOfAnalysisToolButtons = (disableBack: boolean, container: HTMLAnalysisToolAppContainerElement | HTMLAnalysisRfxAppContainerElement) => {
  ...
  const flowItem = container.shadowRoot.querySelector<HTMLElement>('calcite-panel')
  if (flowItem) {

    const headerBackButton: HTMLButtonElement = flowItem.querySelector<HTMLButtonElement>('calcite-action.analysis-panel-back-button')
    if (headerBackButton) {
      headerBackButton.style.display = disableBack ? 'none' : 'flex'
    }

    const buttons: NodeListOf<HTMLButtonElement> = flowItem.querySelectorAll('.tool-footer calcite-button')
    const runButton = buttons[0] as unknown as HTMLCalciteButtonElement
...
export type AnalysisToolElement = HTMLElement & {
  toolJson?: AnalysisToolInfo
  toolData?: AnalysisToolData
}

const getAnalysisToolElement = (analysisToolContainer: HTMLAnalysisToolAppContainerElement) => {
  return analysisToolContainer.shadowRoot?.querySelector<HTMLElement>('calcite-panel')?.querySelector('analysis-tool') as AnalysisToolElement
}

export const watchAnalysisToolDataChange = (
  analysisToolContainer: HTMLAnalysisToolAppContainerElement,
  onDataChange: (analysisToolElement: AnalysisToolElement, toolData: AnalysisToolData) => void
) => {
  let analysisToolElement: AnalysisToolElement
  let observer: MutationObserver

  const onAnalysisToolDataChange = (e: Event) => {
    const nextToolData = (e as CustomEvent<{ toolData: AnalysisToolData }>)?.detail?.toolData
    if (!nextToolData) {
      return
    }
    onDataChange(analysisToolElement, nextToolData)
  }

  const registerAnalysisToolDataChange = () => {
    analysisToolElement = getAnalysisToolElement(analysisToolContainer)
    if (!analysisToolElement) {
      return false
    }

    analysisToolElement.addEventListener('analysisToolDataChange', onAnalysisToolDataChange)
    observer?.disconnect()
    return true
  }

  const isRegistered = registerAnalysisToolDataChange()
  if (!isRegistered && analysisToolContainer.shadowRoot) {
    observer = new MutationObserver(registerAnalysisToolDataChange)
    observer.observe(analysisToolContainer.shadowRoot, {
      childList: true,
      subtree: true
    })
  }
  ...
  return () => {
    observer?.disconnect()
    analysisToolElement?.removeEventListener('analysisToolDataChange', onAnalysisToolDataChange)
  }
}
...
// eslint-disable-next-line max-params
  ...
  const updateRealJobParamsAndTempLayers = async () => {
    ...
    if (paramNames.length) {
      ...
      lodash.defer(() => {
        getAnalysisLayers([...(mapLayers || []), ...((layers as unknown) as SupportedLayer[])], {}).then(() => {
