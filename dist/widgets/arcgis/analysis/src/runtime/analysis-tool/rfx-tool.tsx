...
const { useState, useMemo, useEffect, useEffectEvent, useRef } = React
...
const RFxTool = (props: ToolProps) => {
  ...
  const setAnalysisRFxRef = (ref: HTMLDivElement, recreate = false) => {
    if ((analysisRFxContainer && !recreate) || !ref) {
      return
    }
    ...
    container.rfxName = toolName
    container.rfxTitle = rfxTitle
    container.mapView = jimuMapView?.view as unknown as MapView
    container.serializedJobParams = jobParams
    container.mapLayers = mapLayers || []
  ...
  const setAnalysisRFxRefEvent = useEffectEvent(setAnalysisRFxRef)

  useEffect(() => {
    setAnalysisRFxRefEvent(analysisRFxDivRef.current, true)
  }, [jobParams])
