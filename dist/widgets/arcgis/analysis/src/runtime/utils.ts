...
import {
  type CustomToolOutput,
  type StandardToolOutput,
  type ToolConfig,
  ToolType,
  type HistoryItemWithDs,
  type FailedLayer,
...
import type DataFile from '@arcgis/core/rest/support/DataFile'
...
import {
  depthTraversalProcessingValue,
  isLayerInputType,
  resultValueIsFeatureCollectionJson,
  resultValueIsFeatureLayer,
  resultValueIsFeatureSet,
  getCustomToolParamDisplayName
} from '../utils/util'
...
import { useCommonStrings, getAnalysisToolAppAssetPath } from '../utils/strings'
...
const getDataSourceJson = async (result: GPFeatureRecordSetLayer, widgetId: string, dsOrderRef: React.MutableRefObject<number>) => {
  ...
  if (resultValueIsFeatureSet(result) || resultValueIsFeatureLayer(result) || resultValueIsFeatureCollectionJson(result)) {
    ...
    const item: IItem = await esri.restPortal.getItem(result.itemId, {
      ...
      return SessionManager.getInstance().handleAuthError({error: err}) as any
    })
...
export function removeEmptyResults (results: ParameterValue[]) {
  return results.filter((result) => (result?.value instanceof Date) || !isEmptyValue(result?.value as AnalysisToolDataItem))
}

export const parseFailedLayerError = (value: ParameterValue['value'], error: ArcGISRequestError, defaultName?: string): FailedLayer => {
...
export async function createDsByResults (widgetId: string, toolConfig: ToolConfig, results: ParameterValue[], history: HistoryItemWithDs, dsOrderRef: React.MutableRefObject<number>) {
  const { type: toolType, config: { output } } = toolConfig
  const createDsAndStoreInMap = async (id: string, value: ParameterValue['value'], paramName: string) => {
    ...
    if (toolType === ToolType.Custom) {
      displayName = getCustomToolParamDisplayName((toolConfig.config as CustomToolConfig).toolInfo, paramName)
    }
    try {
      ...
      if (dsJson) {
        ...
        if (resultValueIsFeatureSet(value)) {
          ...
          const source = (value.features || []).map(f => {
            return convertGraphic(f, Graphic)
          })
          ...
          constructorOptions = {
            id: dsJson.id,
            dataSourceJson: Immutable({ ...dsJson, sourceLabel: displayName }),
            layer
          ...
          if (!dsJson.sourceLabel) {
            dsJson.sourceLabel = displayName
          }
          constructorOptions = {
            id: dsJson.id,
  ...
  // sort results according to the config
  const sortedResults = [...results]
  if (toolConfig.type === ToolType.Custom) {
    const params = (toolConfig.config as CustomToolConfig).toolInfo.parameters || []
    const outputParams = params
      .filter((p) => p.direction === AnalysisToolParamDirection.Output)
      .map((p) => p.name)
    sortedResults.sort((a, b) => {
      const indexA = outputParams.indexOf(a.paramName)
      const indexB = outputParams.indexOf(b.paramName)
  ...
  sortedResults?.forEach((result, index) => {
    ...
    depthTraversalProcessingValue(value, `${index}`, (id: string, v: ParameterValue['value']) => {
...
export async function getWebToolUIJson (toolJson: AnalysisToolInfo) {
  const toolUIJson = await analysisGetWebToolUIJson(
    toolJson,
    `${getAnalysisToolAppAssetPath()}assets`
  )
...
export const getCustomToolOutputParamNameByDsId = (dsMap: Map<string, DataSource>, targetDsId: string, results: ParameterValue[]) => {
...
function convertGraphic (f, Graphic: typeof __esri.Graphic) {
  if (f.declaredClass === 'esri.Graphic') {
    return f
  }
  try {
    return Graphic.fromJSON(f.toJSON ? f.toJSON() : f)
  } catch (error) {
    return f.toJSON ? f.toJSON() : f
  }
}

function convertField (f, Field: typeof __esri.Field) {
  if (f.declaredClass === 'esri.layers.support.Field') {
    return f
  }
  try {
    return Field.fromJSON(f.toJSON ? f.toJSON() : f)
  } catch (error) {
    return f.toJSON ? f.toJSON() : f
  }
}
...
export const canDisplayAsLink = (value: ParameterValue['value']): value is DataFile => {
  return value != null && typeof value === 'object' && 'url' in value && typeof value.url === 'string'
}

export const resultHasItemId = (value: ParameterValue['value']): value is DataFile => {
        ...
        res.forEach((r, index) => {
          if (r.status === 'fulfilled') {
            ...
            newParameterInfos.forEach((i) => {
              const pName = i.name
              const selectedLayersKey = generateSelectedLayersCreationModeKey(pName)
