...
import { type IMConfig, type SubmissionData, type ToolConfig, ToolType, type StandardToolConfig, type HistoryItemWithDs, type StandardToolOption, type AnalysisGPJobSubmitted, type CustomToolOutput, type CustomToolConfig } from '../config'
...
import { type GPFeatureRecordSetLayer, type AnalysisToolData, AnalysisToolParamDirection } from '@arcgis/analysis-ui-schema'
...
import { getAnalysisComponentsAssetPath, getAnalysisToolAppAssetPath, getAssetsPathByFolderName, useToolInfoStrings } from '../utils/strings'
...
import type JobInfo from '@arcgis/core/rest/support/JobInfo'
import type ParameterValue from '@arcgis/core/rest/support/ParameterValue'
import type GPMessage from '@arcgis/core/rest/support/GPMessage'
...
const Widget = (props: Props) => {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      analysisToolDefineCustomElements(window, { resourcesUrl: getAnalysisComponentsAssetPath() })
      analysisComponentsDefineCustomElements(window, { resourcesUrl: getAnalysisToolAppAssetPath() })
    })
      ...
      documentClickHandler = (event: MouseEvent) => {
        ...
        for (const el of path) {
          ...
          if (clickPosition.onPencilBtn && el.tagName === 'ANALYSIS-LAYER-INPUT') {
            ...
            break
          } else if (el.tagName === 'CALCITE-BUTTON' && (el as unknown as HTMLCalciteButtonElement).iconStart === 'pencil-mark') {
  ...
  const updatePortal = useCallback(async (loadPortal = true) => {
    const [PortalClz, PortalUserClz] = await loadArcGISJSAPIModules([
      'esri/portal/Portal', 'esri/portal/PortalUser'
    ]) as [typeof Portal, typeof PortalUser]
    const user = getAppStore().getState().user
    const newPortal = new PortalClz({ url: portalUrl })
    ...
    if (!loadPortal && !user) {
      if (!newPortal?.user) {
        ...
        const fakeUser = new PortalUserClz({ portal: newPortal, sourceJSON: {} })
  ...
  const showJobStatusAlert = useCallback((historyItem: HistoryItemWithDs) => {
    ...
    const hasWarnings = messages.some((message: GPMessage) => message?.type === 'warning' && !isWarningCreditMessage(message))
  ...
  const handleJobStart = useCallback((toolId: string, jobInfo: JobInfo, submissionData: SubmissionData) => {
  ...
  const handleJobStatusChange = useCallback(async (jobInfo: JobInfo, results?: ParameterValue[]) => {
    ...
    if (results) {
      const { dsMap, dsCreateError } = await createDsByResults(widgetId, toolInfo, historyProps.results, originHistory, dsOrderRef)
      ...
      if (toolInfo.type === ToolType.Custom && dsMap) {
        ...
        dsMap.forEach((ds, key) => {
          ...
          if (allowAddToMapAuto) {
            addToMapParams.push({ ds, paramName })
          }
        })
        if ((toolInfo.config as CustomToolConfig).toolInfo?.parameters) {
          const toolParams = [...(toolInfo.config as CustomToolConfig).toolInfo.parameters]
          const outputParams = toolParams
            .filter((p) => p.direction === AnalysisToolParamDirection.Output)
            .map((p) => p.name)
          addToMapParams.sort((a, b) => {
            const indexA = outputParams.indexOf(a.paramName)
            const indexB = outputParams.indexOf(b.paramName)
            return indexA - indexB
          })
        }
        addToMapParams.forEach(({ ds, paramName }) => {
  ...
  const [jobStartInfo, setJobStartInfo] = useState<{ toolId: string, jobInfo: JobInfo, submissionData: SubmissionData }>()
  const [jobStatusChangeInfo, setJobStatusChangeInfo] = useState<{ jobInfo: JobInfo, results?: ParameterValue[] }>()
