...
const HistoryResultForStandardTool = (props: Props) => {
  ...
  const [modules, setModules] = useState<[typeof PortalItem]>()
  useEffect(() => {
  ...
  const getNormalItem = (item: ResultInfo, useLabel: boolean) => {
    ...
    const displayLink = canDisplayAsLink(value as ParameterValue['value'])

    const itemId = resultHasItemId(value as ParameterValue['value']) ? (value as any).itemId : ''
