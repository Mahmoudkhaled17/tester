...
const HistoryMessages = (props: Props) => {
  ...
  const getMessageLabel = useCallback((message: GPMessage): string => {
    let label = ''
    switch (message.type) {
      case 'informative':
        break
  ...
  const getMessageColor = (type: GPMessage['type']): string => {
    switch (type) {
      case 'informative':
  ...
  const getFormattedMessage = useCallback((message: GPMessage): string => {
    try {
  ...
  const needDisplayMessages = useMemo(() => {
    const messages: Array<{ label: string, value: string, type: GPMessage['type'] }> = [
      {
