/** @jsx jsx */
import { React, jsx, css, hooks, dataSourceUtils, useIntl, dateUtils, type EsriDateFormats, type ImmutableObject, EsriFieldType } from 'jimu-core'
...
import type RasterData from '@arcgis/core/rest/support/RasterData'
...
const HistoryResultForCustomTool = (props: Props) => {
  ...
  const { results: historyResults, dsMap, dsCreateError } = history
  ...
  const outputParams = toolJson.parameters
  ...
  const getResultItem = (value: ParameterValue['value'], type: ParameterValue['dataType'], output: OutputConfig) => {
    ...
    // for raster data
    if (typeof value === 'object' && (value as RasterData).url && (value as RasterData).format) {
      return (value as RasterData).url
    }
    // for data file
    if (typeof value === 'object' && (value as DataFile).url) {
      return (value as DataFile).url
    }
    // for field
    if (typeof value === 'object' && (value as Field).name) {
    ...
    if ((value as any) instanceof Date || (type === 'date' && (typeof value === 'string' || typeof value === 'number'))) {
      ...
      const newEsriDateFormat = dataSourceUtils.dateTimeFieldFormat({ dateFormat: esriDateFormat } as any)
      return dateUtils.formatDateValueByEsriFormat(date, newEsriDateFormat, EsriFieldType.Date, intl)
    }
    // for other cases
    if (typeof value === 'object' && value !== null) {
      ...
      return value as DataFile | FeatureSet | Array<DataFile | FeatureSet>
    }
  ...
  const resultIsIgnored = (result: ParameterValue) => {
    return output.ignored[result.paramName]
  }

  // use analysis-json-table to display json
  const getDisplayResult = (id: string, value: ParameterValue['value'], type: ParameterValue['dataType'], outputOfCurrentResultItem: OutputConfig) => {
    ...
    if (ds) {
      ...
      const resultUrl = (value as DataFile)?.url
    ...
    const setJsonTableRef = (ref: HTMLAnalysisJsonTableElement) => {
      if (ref) {
        ref.json = getEnumerableObjectFromAccessor(displayValue as DataFile | FeatureSet) as AnalysisToolData
      }
    }
  ...
  const getValueTableDisplayedResult = (id: string, result: ParameterValue, outputOfCurrentResultItem: OutputConfig) => {
    ...
    return <div className='table-container'>
      <CalciteTable bordered striped caption={getCustomToolParamDisplayName(config.toolInfo, paramName)}>
        {(value as Array<ParameterValue['value']>).map((v, index1) => {
          return (v as Array<ParameterValue['value']>).map((item, index2) => {
            ...
            return <CalciteTableRow key={currentId}>
              ...
              <CalciteTableCell>{getDisplayResult(currentId, item, parameterInfo.dataType as ParameterValue['dataType'], outputOfCurrentResultItem)}</CalciteTableCell>
            </CalciteTableRow>
          })
        })}
      </CalciteTable>
    ...
    <CollapsablePanel label={translate('result')} aria-label={translate('result')} type="default" defaultIsOpen>
      <List
        ...
        renderOverrideItemContent={(actionData, refComponent) => {
          ...
          const result = currentItemJson.itemStateDetailContent.result as ParameterValue
          ...
          const isValueTable = (dataType as string) === 'value-table'
              ...
              {Array.isArray(value) && !isValueTable
                ? <List
                    ...
                    renderOverrideItemContent={(actionData, refComponent) => {
                      ...
                      const currentItemJson = itemJsons[0]
                      const id = currentItemJson.itemStateDetailContent.id as string
                      const value = currentItemJson.itemStateDetailContent.value as ParameterValue['value']
                      return <div className='content'>
                        {getDisplayResult(id, value, dataType?.replace('multi-value-', '') as ParameterValue['dataType'], outputOfCurrentResultItem)}
                      </div>
                    }}
                  />
                : isValueTable
