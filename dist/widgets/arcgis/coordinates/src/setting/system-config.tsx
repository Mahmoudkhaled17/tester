import { React, type IMThemeVariables, hooks, polished, css, type ImmutableObject } from 'jimu-core'
import {
  Link, Select, Tooltip, defaultMessages as jimuDefaultMessages, Label, Radio, Button, NumericInput, Loading,
  LoadingType, Checkbox, TextInput, Switch, AlertPopup
} from 'jimu-ui'
import defaultMessages from './translations/default'
import { SettingRow, SettingSection } from 'jimu-ui/advanced/setting-components'
import { InfoOutlined } from 'jimu-icons/outlined/suggested/info'
import { type CoordinateConfig, ElevationUnitType, type IMConfig, type MapInfo } from '../config'
...
export interface SystemConfigProps {
  config: IMConfig
  systemConfig: ImmutableObject<CoordinateConfig>
...
export const SystemConfig = (props: SystemConfigProps): React.ReactElement => {
  const { config, systemConfig, mapInfo, mapInfo2, mapView, theme, wkidUtils, allDataLoaded, multiOptionsChange, onWkidChangeSave } = props
  const {
    id, name, wkid, crs, displayUnit, elevationUnit, datumWkid, datumName, transformForward,
    datumWkid2, datumName2, transformForward2, overrideGeneralSettings = false
  } = systemConfig
  ...
  const onOverrideGeneralSettings = React.useCallback((checked: boolean) => {
    if (!checked) {
      ...
      const newConfig = systemConfig.merge(generalSettings).set('overrideGeneralSettings', checked)
      multiOptionsChange(newConfig)
    }
  }, [handleToggleRemindModel, config, systemConfig, multiOptionsChange])
  ...
  const renderRemindModel = () => {
    return (
      <AlertPopup
        ...
        hideHeader={true}
        onClickOk={() => { multiOptionsChange({ overrideGeneralSettings: false }) }}
    ...
    <div className='w-100'>
      <div className='p-1 d-flex justify-content-between'>
        ...
        <Tooltip
          ...
          enterDelay={0}
          aria-live='off'
          title={outputCoordinateHint}
          aria-label={outputCoordinateHint}
        >
          <Button
            icon type='tertiary' className='d-inline jimu-outline-inside'
            disableHoverEffect={true}
            disableRipple={true}
            aria-label={translate('additionalInformation')}
          >
            <InfoOutlined />
          </Button>
        </Tooltip>
    ...
    <div className='w-100'>
      <div className='p-1 d-flex justify-content-between'>
        ...
        <Tooltip
          ...
          enterDelay={0}
          aria-live='off'
          title={datumTransformationHint}
          aria-label={datumTransformationHint}
        >
          <Button
            icon type='tertiary' className='d-inline jimu-outline-inside'
            disableHoverEffect={true}
          ...
          ? <Fragment>
            ...
            <SettingSection>
              ...
              <SettingRow
                ...
                truncateLabel={false}
              >
                <div role='radiogroup' aria-label={elevationUnitLabel}>
            ...
            >
              <SettingRow tag='label' label={translate('overrideGeneralSettings')}>
                <Switch
                  ...
                  checked={overrideGeneralSettings}
                  onChange={evt => { onOverrideGeneralSettings(evt.target.checked) }}
                  aria-label={translate('overrideGeneralSettings')}
                />
              </SettingRow>
              {renderRemindModel()}
              {overrideGeneralSettings &&
                <GeneralSettings
                  level={'layer'}
                  config={systemConfig}
                  onPropertyChange={(name, value) => {
                    multiOptionsChange(systemConfig.set(name, value))
                  }}
                />
              }
            </SettingSection>
          </Fragment>
          : <Loading type={LoadingType.Secondary} useAriaLive/>
        }
      </div>
    </div>
  )
