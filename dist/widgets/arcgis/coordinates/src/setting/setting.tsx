import { React, Immutable, defaultMessages as jimuCoreMessages, urlUtils, moduleLoader, hooks, type ImmutableObject } from 'jimu-core'
import { type AllWidgetSettingProps, builderAppSync, getAppConfigAction } from 'jimu-for-builder'
import { Button, defaultMessages as jimuMessages, Icon, Tooltip } from 'jimu-ui'
import { MapWidgetSelector, SettingRow, SettingSection, SidePopper } from 'jimu-ui/advanced/setting-components'
import { type CoordinateConfig, ElevationUnitType, type IMConfig, WidgetStyleType } from '../config'
...
const Setting = (props: SettingProps): React.ReactElement => {
  ...
  const { coordinateSystem, widgetStyle, mapInfo, mapInfo2 } = propConfig
    ...
    <div className='widget-setting-coordinates jimu-widget-setting' css={getSettingStyle(theme)}>
      ...
      {useMap &&
        <Fragment>
          ...
          <GeneralSettings level={'widget'} config={propConfig} onPropertyChange={onPropertyChange} />
        </Fragment>
      }
      ...
      >
        <SystemConfig
          config={propConfig}
          systemConfig={coordinateSystem[panelIndex.current] ?? coordinateSystemDefault as ImmutableObject<CoordinateConfig>}
