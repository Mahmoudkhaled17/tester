...
const Widget = (props: AllWidgetProps<IMConfig>): React.ReactElement => {
  ...
  const {
    coordinateSystem, coordinateDecimal: originalCoordinateDecimal, altitudeDecimal: originalAltitudeDecimal, showSeparators: originalShowSeparators,
    displayOrder: originalDisplayOrder, widgetStyle, mapInfo, mapInfo2
  } = config
  ...
  const curSystem = coordinateSystem.find(item => item.id === selectedSystemId)
  const isOverride = curSystem?.overrideGeneralSettings
  const coordinateDecimal = isOverride ? curSystem?.coordinateDecimal : originalCoordinateDecimal
  const altitudeDecimal = isOverride ? curSystem?.altitudeDecimal : originalAltitudeDecimal
  const showSeparators = isOverride ? curSystem?.showSeparators : originalShowSeparators
  const displayOrder = isOverride ? curSystem?.displayOrder : originalDisplayOrder
  ...
  const displayOnClient = async (mapPoint, threeDPoint?) => {
    ...
    const normalizedPoint = mapPoint.normalize()
    // get default units
    let outputUnit = showBasemapChangeTips ? undefined : displayUnit
  ...
  return (
    <div
      className='jimu-widget-coordinates jimu-widget h-100'
      ref={coordinatesWidgetConRef}
      css={getStyle(theme, isClassic, widgetRect, widgetSizeAuto, showBasemapChangeTips)}
      role='group'
      aria-label={classicInfo}
    >
      {isClassic
