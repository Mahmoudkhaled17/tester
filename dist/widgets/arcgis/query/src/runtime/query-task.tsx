...
const style = css`
  ...
  .query-form__header {
    display: flex;
    &.has-content:has(> div:not(.d-none)) {
...
export function QueryTask (props: QueryTaskProps) {
  ...
  const previousDataSourceId = hooks.usePrevious(queryItem.useDataSource?.dataSourceId)
  ...
  React.useEffect(() => {
    if (previousDataSourceId && queryItem.useDataSource?.dataSourceId !== previousDataSourceId) {
      DataSourceManager.getInstance().destroyDataSource(queryItem.outputDataSourceId)
    }
  }, [queryItem.useDataSource?.dataSourceId, queryItem.outputDataSourceId, previousDataSourceId])
  ...
  const handleFormSubmit = async (sqlExpr: IMSqlExpression, spatialFilter: SpatialFilterObj) => {
    ...
    <div className={classNames('query-task h-100 d-flex', className, { wrapped: wrappedInPopper })} css={style}>
      <div className={classNames('query-task__content', {
        ...
        <div className={classNames('query-form__header mx-4 align-items-center', { 'in-popper': isInPopper, 'has-content': showAttributeFilter || showSpatialFilter })}>
          <div className={classNames('nav-action align-items-center', { 'd-none': wrappedInPopper || (total === 1 && !displayLabel && !icon) })}>
            <Button
