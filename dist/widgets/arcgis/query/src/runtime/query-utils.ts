...
export function combineFields (resultDisplayFields: ImmutableArray<string>, resultTitleExpression: string, idFields?: string[] | ImmutableArray<string> | string): string[] {
  ...
  if (idFields) {
    if (Array.isArray(idFields)) {
      ;(idFields as string[] | ImmutableArray<string>).forEach(idField => fields.add(idField))
    } else {
      fields.add(idFields as string)
    }
  }
...
export function generateQueryParams (
  ...
  if (sortOptions?.length > 0) {
    const nonEmptyOptions = sortOptions.filter(item => item.jimuFieldName != null && item.jimuFieldName !== "")
  ...
  if (resultFieldsType === FieldsType.SelectAttributes) {
    const fields = combineFields(outputFields as any, resultTitleExpression, outputDS.getUniqueIdFields() || outputDS.getIdField())
    Object.assign(queryParams, {
      outFields: fields
    })
  } else {
