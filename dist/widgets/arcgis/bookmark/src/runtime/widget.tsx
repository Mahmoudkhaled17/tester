...
interface States {
  ...
  // Track highlighted bookmarks by stable id so reordering does not change selection.
  highlightedBookmarkId: number | string | null
  showInView: boolean
  showGalleryArrow: boolean
}
...
> {
  ...
  componentDidUpdate (prevProps, preStates) {
    ...
    const { autoPlayStart, playTimer, jimuMapView, isSuspendMode, showInView, widgetRect, runtimeBmArray } = this.state
    const activeBookmarkId = this.props?.stateProps?.activeBookmarkId || 0
    const mapBookmarks = this.getMapBookmarks(jimuMapView) || []
    // bookmark arrow display logic
    const gallerySimilarType = [TemplateType.Slide1, TemplateType.Slide2, TemplateType.Slide3, TemplateType.Custom1, TemplateType.Custom2]
    const isGalleryType = config.templateType === TemplateType.Gallery
    const isGallerySimilarType = gallerySimilarType.includes(config.templateType)
    const isScrollType = config.pageStyle === PageStyle.Scroll
    const isVertical = config.direction === DirectionType.Vertical
    if (
      prevProps?.config !== config ||
      preStates?.runtimeBmArray !== runtimeBmArray ||
      preStates?.widgetRect !== widgetRect ||
      preStates?.jimuMapView !== jimuMapView
    ) {
      // we also need to consider the magnification effect of the bookmark
      if (isGalleryType) {
        const bookmarkGallerySpace = config.galleryItemSpace ?? 24
        const getTotalOfChildren = (element: HTMLDivElement, type: 'width' | 'height') => {
          if (!element?.children) return 0
          return Array.from(element.children).filter(child =>
            child.classList.contains('gallery-slide-card') || child.classList.contains('gallery-card') || child.classList.contains('gallery-card-add')
          ).reduce((total, child) => total + (type === 'width' ? (child as any).offsetWidth : (child as any).offsetHeight) + bookmarkGallerySpace, 0)
        }
        // width
        if (!isVertical) {
          if (!this.containerRef.current) return
          const totalWidth = getTotalOfChildren(this.containerRef.current, 'width')
          this.setState({ showGalleryArrow: this.containerRef.current.clientWidth < totalWidth + 10 })
        // height
        } else {
          if (!this.containerRef.current) return
          const totalHeight = getTotalOfChildren(this.containerRef.current, 'height')
          this.setState({ showGalleryArrow: this.containerRef.current.clientHeight < totalHeight + 10 })
        }
      }
      // Templates similar to gallery can also be switch to scroll mode
      if (isGallerySimilarType && isScrollType) {
        const bookmarkSpace = config.space ?? 10
        const getTotalOfChildren = (element: HTMLDivElement, type: 'width' | 'height') => {
          if (!element?.children) return 0
          return Array.from(element.children).filter(child =>
            child.classList.contains('gallery-slide-card')
          ).reduce((total, child) => total + (type === 'width' ? (child as any).offsetWidth : (child as any).offsetHeight) + bookmarkSpace, 0)
        }
        // width
        if (!isVertical) {
          if (!this.containerRef.current) return
  ...
  handleRuntimeAdd = async () => {
    ...
    const baseMapLayersArray = view.map.basemap?.baseLayers?.toArray() || []
    const excludeBookmarkLayersArray = layersArray.filter(layer => !layer.id?.startsWith(`bookmark-layer-${id}`))
    const layersConfig = getLayersConfig(layersArray)
    let newBookmarkId = this.rtBookmarkId.toString()
    // 148x120 The picture will be a bit blurry
    const usedScreenshotLayers = baseMapLayersArray.concat(excludeBookmarkLayersArray)
  ...
  onViewBookmark = (item: ImmutableObject<Bookmark>, isRuntime?: boolean, index?: number) => {
    ...
    const { id, useMapWidgetIds, config } = this.props
    const activeBookmarkId = this.props?.stateProps?.activeBookmarkId || 0

    this.setState({ highlightedBookmarkId: item.id })
    ...
    const highLightIndex = config.bookmarks.findIndex(x => x.id === item.id)
    if (!isRuntime && index === highLightIndex) {
      BookmarkNodeList.forEach(node => {
        if (node.classList.contains(currentBookmarkId)) {
    ...
    const runtimeHighLightIndex = this.state.runtimeBmArray.findIndex(id => id === item.id)
    if (isRuntime && index === runtimeHighLightIndex) {
      BookmarkNodeList.forEach(node => {
        if (node.classList.contains(currentBookmarkId)) {
    ...
    if (
      ...
      if (bookmarks.length > 0) {
        ...
        jimuMapView.view.when(() => {
          this.setState({
            bookmarkOnViewId: bookmarks[0].id,
            highlightedBookmarkId: bookmarks[0].id
          })
          // execute after snapToZoom is set to false
          setTimeout(() => { this.onViewBookmark(bookmarks[0]) }, 300)
        })
      }
    }
  }
    ...
    return (
      ...
      >
        <div className={classNames('bookmark-slide', { 'd-none': !displayName && !item.description })}>
          <div className={classNames('bookmark-slide-title text-truncate', { 'd-none': !displayName })} title={item.name}>{item.name}</div>
          <div className='bookmark-slide-description' title={item.description}>{item.description}</div>
        </div>
        {imageSrc
          ? <Image
              src={imageSrc}
    ...
    return (
      ...
      >
        ...
        <div className={classNames('bookmark-slide2', { 'd-none': !displayName && !item.description })}>
          <div className={classNames('bookmark-slide2-title text-truncate', { 'd-none': !displayName })} title={item.name}>{item.name}</div>
          <div className='bookmark-slide2-description' title={item.description}>{item.description}</div>
        </div>
      </div>
    )
  }
  ...
  renderSlideScroll = (bookmarks: ImmutableArray<Bookmark>) => {
    const bookmarkGalleryList = bookmarks.map((item, index) => {
      const typeSnap = item.imgSourceType === ImgSourceType.Snapshot
      const imageSrc = typeSnap ? item.snapParam?.url : item.imgParam?.url
      const isBookmarkItemActive = item.id === this.state.highlightedBookmarkId
      const { displayName } = this.props.config
      return (
  ...
  getMapBookmarks = (jimuMapView: JimuMapView) => {
    if (jimuMapView && jimuMapView?.dataSourceId) {
      ...
      let extraBookmarks = []
      if (view.type === '3d') {
        const slidesJson = mapSource.presentation?.slides?.toJSON?.()
        extraBookmarks = slidesJson || []
  ...
  renderBookmarkList = (bookmarks: ImmutableArray<Bookmark>) => {
    ...
    const { bookmarkOnViewId, autoPlayStart, jimuMapView, showGalleryArrow } = this.state
    ...
    const highLightIndex = bookmarks.findIndex(item => item.id === this.state.highlightedBookmarkId)
    const runtimeHighLightIndex = this.state.runtimeBmArray.findIndex(itemId => itemId === this.state.highlightedBookmarkId)
    const currentIndex =
      bookmarks.findIndex(x => x.id === bookmarkOnViewId) > -1
        ? bookmarks.findIndex(x => x.id === bookmarkOnViewId)
        : 0
    ...
    const typedBookmarkList = (tempType: TemplateType) => {
      ...
      switch (tempType) {
        case TemplateType.Card:
          ...
          runtimeHighLightIndex = {runtimeHighLightIndex}
        ...
        case TemplateType.Slide1:
          ...
          return (
            <Fragment>
              ...
              {config.pageStyle === PageStyle.Scroll && showGalleryArrow &&
        ...
        case TemplateType.Gallery:
          bookmarkList = <GalleryList
            ...
            showGalleryArrow={this.state.showGalleryArrow}
          />
          break
      ...
      <div className='gallery-slide-card'>
        <div className='w-100 h-100 bookmark-pointer border-0'>
          <div className='bookmark-slide-gallery'>
            <div className='bookmark-slide-title text-truncate' title={bookmark.title}>{bookmark.title}</div>
            <div className='bookmark-slide-description' title={bookmark.description}>{bookmark.description}</div>
          </div>
          <div className='default-img'>
            <div className='default-img-svg'></div>
          </div>
  ...
  renderBookmarkExample = (bookmark: Bookmark) => {
    const { appMode, config } = this.props
    const { jimuMapView, showGalleryArrow } = this.state
    const directionIsHorizon = config.direction === DirectionType.Horizon
    const isWebMap = jimuMapView?.view?.type !== '3d'
                  ...
                  >
                    <div className='w-100 h-100 bookmark-pointer border-0'>
                      ...
                      <div className='bookmark-slide2'>
                        <div className='bookmark-slide2-title text-truncate' title={bookmark.title}>{bookmark.title}</div>
                        <div className='bookmark-slide2-description' title={bookmark.description}>{bookmark.description}</div>
                      </div>
                    </div>
                  </TransitionContainer>
                  )
