...
const Setting = (props: AllWidgetSettingProps<IMConfig>) => {
  ...
  const [mapUseDataSources, setMapUseDataSources] = useState(initDsIds || Immutable([]))
  ...
  const handleResetSwipeStyleClick = evt => {
    ...
    // When switching between templates of the same type，keep configured leader/trailing layers
    const newConfig = config.set('swipeStyle', undefined)
    onSettingChange({
      id: id,
      config: newConfig
    })
  ...
  // show setting type depends on swipeStyle and swipeMode
  const isSimpleHorizontal = swipeStyle === SwipeStyle.SimpleHorizontal
  const isSimpleVertical = swipeStyle === SwipeStyle.SimpleVertical
  const isAdvancedHorizontal = swipeStyle === SwipeStyle.AdvancedHorizontal
  const isAdvancedVertical = swipeStyle === SwipeStyle.AdvancedVertical
  const showCustomizeSwipeLayersSetting = (isSimpleHorizontal || isSimpleVertical) && swipeMode === SwipeMode.SwipeBetweenLayers
  const showCustomizeSwipeMapsSetting = (isSimpleHorizontal || isSimpleVertical) && swipeMode === SwipeMode.SwipeBetweenMaps
  const showCustomizeScrollLayersSetting = isAdvancedHorizontal || isAdvancedVertical
  const showAllowDeactivateLayersSetting = ((isSimpleHorizontal || isSimpleVertical) && swipeMode === SwipeMode.SwipeBetweenLayers) || (isAdvancedHorizontal || isAdvancedVertical)
        ...
        <SettingSection className='reset-template-and-select-map-section'>
          ...
          >
            <MapWidgetSelector
              ...
              warningMessage={!isMapUnoccupied() ? translate('mapOccupied') : ''}
              aria-describedby={'swipe-blank-msg'}
            />
          </SettingRow>
        </SettingSection>
        {
          hasMap
            ? <div>
                {(isSimpleHorizontal || isSimpleVertical) &&
                  <SettingSection
                ...
                {showCustomizeSwipeLayersSetting &&
                  <CustomizeSwipeLayers
                    ...
                    leadingLayersAlias={config.leadingLayersAlias}
                    trailingLayersAlias={config.trailingLayersAlias}
                    swipeStyle={swipeStyle}
            ...
            : <div className='empty-placeholder w-100'>
                <div className='empty-placeholder-inner'>
                  ...
                  <div className='empty-placeholder-text' id='swipe-blank-msg'>
