...
export function SwipeBetweenLayers (props: SwipeBetweenLayersProps) {
  const { widgetId, activeMapView, leadingLayersId, trailingLayersId, swipeStyle, sliderPosition, dividerColor, handleColor, isDesignMode } = props
    ...
    function updateSwipeWidget () {
      ...
      if (leadingLayersPromiseArray && trailingLayersPromiseArray) {
        Promise.all(leadingLayersPromiseArray.concat(trailingLayersPromiseArray)).then(async jimuLayerViews => {
          ...
          // Keep compatibility across ArcGIS map-components versions by setting both legacy and non-internal CSS vars.
          swipeRef.current.style.setProperty('--arcgis-internal-swipe-handle-color-background', handleColor)
          swipeRef.current.style.setProperty('--arcgis-internal-swipe-divider-color-background', dividerColor)
          swipeRef.current.style.setProperty('--arcgis-swipe-handle-color-background', handleColor)
          swipeRef.current.style.setProperty('--arcgis-swipe-divider-color-background', dividerColor)
          swipeRef.current.style.setProperty('pointer-events', 'none', 'important')
          swipeRef.current.startLayers = leadingArray as unknown as Collection<Layer>
          swipeRef.current.endLayers = trailingArray as unknown as Collection<Layer>
          swipeRef.current.direction = swipeDirection
          swipeRef.current.position = positionRef.current
  ...
  }, [widgetId, activeMapView, sliderPosition, leadingLayersId, trailingLayersId, swipeStyle, rightStart, dividerColor, handleColor])
