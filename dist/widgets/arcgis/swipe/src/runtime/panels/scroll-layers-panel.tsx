import { React, css, hooks, Immutable, type ImmutableObject, type ImmutableArray, classNames} from 'jimu-core'
...
export function ScrollLayersPanel (props: ScrollLayersPanelProps) {
  const {
    scrollMapViewList, activeMapView, inactiveMapView, handlePanelSelectedFirstScrollLayer, handlePanelSelectedSecondScrollLayer,
    isAllowDeactivateLayers, toggleLayerVisibility, scrollingLayerOfFirstMapView, scrollingLayerOfSecondMapView, scrollFirstMapView,
    showSwipeOnMap, runtimeActiveMapViewAddedLayers, runtimeInactiveMapViewAddedLayers, handleRuntimePanelSelectedFirstScrollLayer,
  ...
  const renderLegend = hooks.useEventCallback(() => {
    ...
    if (showSwipeOnMap && (selectedFirstMapViewLayerList?.length > 0 || selectedSecondMapViewLayerList?.length > 0)) {
      ...
      const usedLayer = scrollFirstMapView ? scrollingLayerOfFirstMapView?.layer : scrollingLayerOfSecondMapView?.layer
      const legend = document.createElement('arcgis-legend')
      legend.view = scrollFirstMapView ? activeMapView?.view : inactiveMapView?.view
      legend.style = 'classic'
      legend.layerInfos = [{
        title: usedLayer?.title,
        layer: usedLayer
      }]
  ...
  const handleSelectedMapViewLayers = (
    ...
    setScrollLayerList: React.Dispatch<React.SetStateAction<LayerInfo[]>>,
    setSelectedLayerList: React.Dispatch<React.SetStateAction<Immutable.ImmutableArray<string>>>,
    scrollMapViewLayerListRef: React.RefObject<LayerInfo[]>,
    layerIds: string[]
  ) => {
  ...
  const handleSelectedFirstMapViewLayers = (layerIds: string[]) => {
  ...
  const handleSelectedSecondMapViewLayers = (layerIds: string[]) => {
    handleSelectedMapViewLayers(
  ...
  const handleRuntimeSelectedFirstMapViewLayers = (layerIds: string[]) => {
    handleSelectedMapViewLayers(
      runtimeActiveMapViewLayerList,
      setRuntimeActiveMapViewLayerList,
  ...
  const handleRuntimeSelectedSecondMapViewLayers = (layerIds: string[]) => {
    handleSelectedMapViewLayers(
  ...
  const updateLayerVisible = React.useCallback((
    ...
    setSelectedLayerList: React.Dispatch<React.SetStateAction<Immutable.ImmutableArray<string>>>,
    handleSelectedLayers: (list: ImmutableArray<string>) => void
  ) => {
    const newList = JSON.parse(JSON.stringify(layerListRef.current || [])) as LayerInfo[]
    let hasLayer = false
    newList.forEach(layerObj => {
    ...
    const selectedLayerList = (Immutable as unknown as (value: string[]) => Immutable.ImmutableArray<string>)(selectedLayerIds)
    setSelectedLayerList(selectedLayerList)
    handleSelectedLayers(selectedLayerList)
  }, [])

  const onToggleLayerVisibility = React.useCallback(async (
    layerId: string,
    mapView: JimuMapView,
    layerListRef: React.RefObject<LayerInfo[]>,
    setLayerList: React.Dispatch<React.SetStateAction<LayerInfo[]>>,
    ...
    const jimuLayerView = await getJimuLayerViewById(mapView, layerId)
    const visible = !jimuLayerView.layer?.visible
    if (jimuLayerView.layer) {
      jimuLayerView.layer.visible = visible
    }
    updateLayerVisible(layerId, visible, layerListRef, setLayerList, setSelectedLayerList, handleSelectedLayers)
  }, [updateLayerVisible])
  ...
  return (
    <div ref={scrollLegendRef} css={style}>
      ...
      >
        {hasActiveMapViewLayers
          ? <TreeLayerList
            ...
            onToggleLayerVisibility={(layerId: string) => onToggleLayerVisibility(
              layerId,
              activeMapView,
              scrollFirstMapViewLayerListRef,
              setScrollFirstMapViewLayerList,
      ...
      {inactiveMapView && isWebMap(inactiveMapView.dataSourceId) &&
        <CollapsablePanel
...
const style = css`
  ...
  .currentLegend {
    ...
    arcgis-legend {
      ...
      --arcgis-internal-padding: 0.75rem 0.75rem 0.75rem 0;
    }
