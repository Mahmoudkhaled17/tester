...
export default class Measure extends BaseTool<BaseToolProps, States> {
  ...
  destroy = () => {
    ...
    if (measureInstance) {
      ...
      if (!measureInstance.destroyed) {
        ...
        const stateHandle = anyMeasureInstance._exbStateHandle as __esri.Handle

        if (stateHandle) {
          stateHandle.remove()
          anyMeasureInstance._exbStateHandle = null
        }
  ...
  handleMeasureInstanceCreated = (measureInstance: __esri.Measurement) => {
    ...
    return (
      ...
      >
        ...
        <MeasureInner
          ...
          onMeasureInstanceCreated={this.handleMeasureInstanceCreated}
...
class MeasureInner extends React.PureComponent<MeasureInnerProps, MeasureInnerState> {
  ...
  componentDidUpdate (prevProps: MeasureInnerProps) {
    if (this.state.apiLoaded && this.parentContainer && this.container) {
      if (!this.props.measureInstance) {
        ...
        const tempInstance: __esri.Measurement = new this.MeasureClass({
          container: this.container,
          view: jimuMapView.view
        })
        tempInstance.activeTool = this.props.measureModule.activeTool as any
        const stateHandle = this.reactiveUtils.watch(() => tempInstance.viewModel?.state, () => {
          const state = tempInstance.viewModel?.state

          if (state === 'ready' || state === 'measuring') {
            this.disablePopupEnabled()
          } else {
            this.restorePopupEnabled()
          }
        })
        ...
        ;(tempInstance as any)._exbStateHandle = stateHandle
  ...
  disablePopupEnabled () {
    const jimuMapView = this.popupEnabledInfo?.jimuMapView
