...
import { getLayerObject, getViewByDataSourceId } from '../../utils'
...
export class DataLoader extends React.PureComponent<Props, State> {
  ...
  getWherePhraseForField = (fieldName: string, fieldValue: any): string => {
    let wherePhrase
    ...
    if (fieldType === 'esriFieldTypeDate') {
      const clause = dataSourceUtils.createSQLClause(fieldName, ClauseOperator.DateOperatorIsBefore, [{ value: fieldValue, label: '' }])
      ...
      wherePhrase = fieldType === 'esriFieldTypeString' ? `'${fieldValue}'` : `${fieldValue}`
    }
    return wherePhrase
  }
  ...
  async getDataIndexByObjectId (objectId): Promise<number> {
    ...
    if (index < 0 && this.dataSource) {
      ...
      const uniqueIdFields = this.dataSource.getUniqueIdFields()?.asMutable() || []
      let orderByFieldInfos = this.getQueryParamsFromDataSource()?.orderByFields || []
      orderByFieldInfos = orderByFieldInfos.concat(uniqueIdFields)

      // 1, data source 'order by' fields are configured
      // 2, data source supports 'unique id', deal with it as 'order by'
      if (orderByFieldInfos?.length > 0) {
        ...
        if (record?.feature) {
          for (let i = 0; i < orderByFieldInfos.length; i++) {
            ...
            const orderByFieldWherePhrase = this.getWherePhraseForField(orderByField, record.feature.attributes[orderByField])
            ...
            const nextOrderByFieldWherePhrase = this.getWherePhraseForField(nextOrderByField, record.feature.attributes[nextOrderByField])
            if (i === 0) {
        ...
        const objectIdWherePhrase = this.getWherePhraseForField(idField, objectId)
        index = await this.dataSource.queryCount({ where: `${idField}<=${objectIdWherePhrase}` } as QueryParams).then(result => {
  ...
  queryGraphicByIndex (indexParam) {
    ...
    this.onDataSourceStatusChanged(DataSourceStatus.Loading, this.dataSource?.getLabel())
    let layerObject
    return this.getLayerObject().then(async layer => {
      layerObject = layer
      ...
      const view = getViewByDataSourceId(this.dataSource.id)
      const query = {
        // where: where,
        outFields: ['*'],
        notAddFieldsToClient: true,
        returnGeometry: true,
        outSR: view?.spatialReference?.wkid || null,
      ...
      if (this.dataSource.isDataView && this.dataSource.dataViewId === CONSTANTS.SELECTION_DATA_VIEW_ID && records.length > 0) {
        // unique id
        const objectIds = records.map(record => record.getId())
        const view = getViewByDataSourceId(this.dataSource.id)
  ...
  serializeDSQueryCondition = (): string => {
    ...
    const dsQueryCondition = `${where}_${geometry?.xmax?.toString() || ''}_${geometry?.xmin?.toString() || ''}_${geometry?.ymax?.toString() || ''}_${geometry?.ymin?.toString() || ''}_${queryParams?.time?.toString() || ''}`
  ...
  onDataSourceInfoChange = (info: IMDataSourceInfo) => {
    ...
    if (this.dataSource && this.dataSource.isDataView && this.dataSource.dataViewId === CONSTANTS.SELECTION_DATA_VIEW_ID) {
      ...
      const beSelected = selectedId === 0 ? true : !!selectedId
      if (beSelected) {
