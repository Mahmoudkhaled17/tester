...
import { getStyle } from './lib/style-features-info'
//import { getStyle } from './lib/style-features-info-component'
import defaultMessages from './translations/default'
//import FeatureInfo from './components/feature-info'
import FeatureInfos from './components/features-info'
//import FeatureInfos from './components/features-info-component'
...
export default class Widget extends React.PureComponent<WidgetProps, WidgetState> {
  ...
  private jimuTableCreatedListener
  ...
  constructor (props) {
    ...
    this.jimuTableCreatedListener = null
  ...
  getDSConfigs = () => {
    ...
    if (this.props.config.useMapWidget) {
      ...
      const loadedJimuTables = jimuMapView?.getJimuTables() || []
      loadedJimuLayerViews.forEach(loadedJimuLayerView => {
        // sort layer ds config
        const dsConfig = this.dsConfigsOfMapView.find(dsConfig => dsConfig.useDataSourceId === loadedJimuLayerView.layerDataSourceId)
        if (dsConfig && loadedJimuLayerView.isLayerVisible()) {
          jimuLayerDsConfig.push(dsConfig)
        }
      })

      loadedJimuTables.forEach(loadedJimuTable => {
        // sort table ds config
        const dsConfig = this.dsConfigsOfMapView.find(dsConfig => {
          const dataSourceId = jimuMapView.getDataSourceIdByAPILayer(loadedJimuTable.table)
          return dsConfig.useDataSourceId === dataSourceId
        })
        if (dsConfig) {
      ...
      //dsConfigs = dsConfigs.filter(dsConfig => this.isValidData(dsConfig.useDataSourceId))
  ...
  onJimuMapViewLoaded = (jimuMapView) => {
    this.addTableDatasFromJimuTables(jimuMapView)
  }

  addTableDatasFromJimuTables = (jimuMapView) => {
    ...
    if (mapDataSource) {
      jimuMapView.getJimuTables().forEach(async jimuTable => {
        ...
        if (isSupportedDataType(dataSource) && !this.getDSConfigs().find(dsConfig => dsConfig.useDataSourceId === dataSource.id)) {
  ...
  onActiveViewChange = (jimuMapView: JimuMapView) => {
    ...
    if (this.prevJimuMapView) {
      ...
      this.jimuTableCreatedListener && this.prevJimuMapView.removeJimuTableCreatedListener(this.jimuTableCreatedListener)
    }
    ...
    this.jimuTableCreatedListener = (jimuTable) => {
      this.addTableDatasFromJimuTables(jimuMapView)
    }
    jimuMapView.addJimuTableCreatedListener(this.jimuTableCreatedListener)
    ...
    if (this.props.config.clearSelection) {
      ...
      clearSelectionContent = (
        <div className='clear-selection d-flex justify-content-center align-items-center'>
          <div style={{width: 0, height: 0, overflow: 'hidden'}} className='aria-live-label' aria-live='polite'>{selectedRecordIds.length <= 0 ? title : ''}</div>
  ...
  getDsErrorMessageContent = () => {
    return (
      <div className='data-inaccessible'>
        <WidgetPlaceholder icon={featureInfoIcon} name={this.props.intl.formatMessage({ id: '_widgetLabel', defaultMessage: defaultMessages._widgetLabel })} widgetId={this.props.id} />
        <Alert
    ...
    return (
      <div style={{ cursor: cursor }} onClick={this.onCurrentFeatureClick} >
        <FeatureInfos
          ...
          //featureInfoConRef = {this.featureInfoConRef}
          //styleConfig={this.props.config.styleType === 'custom' && styleConfig.textColor ? styleConfig : null}
        />
      </div>
    )
  }
  ...
  getDataSourceContent = () => {
    const dataLoaderContent = this.getDSConfigs().map((dsConfig, index) => {
