...
import {
  ...
  type UseDataSource, type DataSource, observeStore, type IMDataSourceInfo, DataSourceStatus
} from 'jimu-core'
...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<IMConfig> & ExtraProps, WidgetSettingState> {
  ...
  constructor (props) {
    ...
    dsConfigs.forEach(dsConfig => {
      observeStore(this.onDataSourceInfoChange, ['appStateInBuilder', 'dataSourcesInfo', dsConfig.useDataSourceId])
    })
  }

  onDataSourceInfoChange = (preInfo: IMDataSourceInfo, info: IMDataSourceInfo) => {
    if (info?.instanceStatus === DataSourceStatus.Created) {
      this.setState({dsFailedFlag: uuidv1()})
    }
  }
  ...
  onRadioChange = (useMapWidget) => {
    ...
    this.props.onSettingChange({
      ...
      useMapWidgetIds: useMapWidget ? this.props.useMapWidgetIds : [] as any
    })
  }
  ...
  isDataSourceAccessible = (dataSourceId: string) => {
    const dataSource = DataSourceManager.getInstance().getDataSource(dataSourceId)
    return !!dataSource && this.props.useDataSources?.some(useDs => dataSourceId === useDs.dataSourceId)
  }
        ...
        {!!itemsLength &&
          <List
            ...
            onClickItemBody={(actionData, refComponent) => {
              ...
              const accessible = this.isDataSourceAccessible(dsId)
              if (accessible) {
                if (this.state.showSidePanel && this.state.currentDSConfigId === currentDSConfigId) {
  ...
  getDSConfigContentTrigger = () => {

    const dsConfigs = this.getDSConfigs()
    let node: HTMLElement
    if (this.state.showSidePanelForNew) {
      node = this.dsListContentRef.current?.getElementsByClassName('add-edit-btn')[0] as HTMLElement
    } else {
      const index = dsConfigs.findIndex(dsConfig => dsConfig.id === this.state.currentDSConfigId)
      node = this.dsListContentRef.current?.getElementsByClassName('jimu-tree-item__body')[index] as HTMLElement
    }
    return node
  }
  ...
  getDSConfigContent = () => {
    return (
      <SidePopper
        ...
        trigger={this.getDSConfigContentTrigger()}
