...
import {
  ...
  QueryScope,
  React,
  getAppStore, jsx, utils, defaultMessages as jimuCoreMsgs
} from 'jimu-core'
...
export default class Widget extends React.PureComponent<WidgetProps, WidgetState> {
  ...
  componentDidUpdate (prevProps: WidgetProps, prevState: WidgetState) {
    ...
    // Announce loading completion when loading ends
    if (prevState.loading && !this.state.loading) {
      this.setState({ a11yLoadingStatus: this.props.intl.formatMessage({ id: 'loaded', defaultMessage: jimuCoreMsgs.loaded }) })
    }
 ...
 */
  ...
  handleSelection = (ids: number[], type: string) => {
    if (type === 'point') {
  ...
  render () {
        ...
        content = (
          <div className='widget-track-panel p-2' >
            ...
            <div className='header-section'>
              {this.state.loading && <Loading useAriaLive type={LoadingType.Secondary} />}
              <div className='left'>
    ...
    const highlightLocationContent = (
      <HighLightLocation
        jimuMapView={this.state.jimuMapView}
    ...
    return (
      <Paper shape='none' css={getStyle(this.props.theme, getAppStore().getState()?.appContext?.isRTL, this.props.autoWidth, this.props.autoHeight)} className="jimu-widget">
        ...
        <div className='sr-only' aria-live='assertive' >{this.state.a11yLoadingStatus}</div>
        {
