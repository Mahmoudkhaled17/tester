...
`
  ...
  handleSelect = (track: TrackPoint | TrackLinePoint, handle: boolean) => {
    let selIds
    if (handle) {
      selIds = this.props.selectedIds.concat([track.OBJECTID])
      const graphic = getPointGraphic(track)
      if (graphic) {
      ...
      selIds = this.props.selectedIds.filter(m => m !== track.OBJECTID)
    }

    if (this.props.dataSource) {
      this.props.dataSource.selectRecordsByIds(selIds.map(m => m))
    }
  }
  ...
  render () {
    ...
    return (
      <div className={'track-content'} css={this.widgetStyle} >
        ...
        {this.props.tracks.length > 0 && <TrackList theme={this.props.theme} dataSource={this.props.dataSource} tracks={this.props.tracks} isLine={false} selectedFields={this.props.selectedFields} onHandleSelect={this.handleSelect} onHandleDelete={this.props.onHandleDelete} selectedIds={this.props.selectedIds} notFilterPointIds={this.props.notFilterPointIds} />}
