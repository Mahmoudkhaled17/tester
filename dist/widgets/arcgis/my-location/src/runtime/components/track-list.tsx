/** @jsx jsx */
import { css, type DataSource, injectIntl, jsx, type IMThemeVariables, type IntlShape } from 'jimu-core'
...
const _TrackList = (props: Props & ExtraProps) => {
  ...
  const childTracks = (tracks: TrackLinePoint[]) => {
    return tracks.map(t => {
      ...
      return (<TrackListItem key={t.OBJECTID} dataSource={props.dataSource} theme={theme} track={t} selectedFields={selectedFields} active={selectedIds?.includes(t.OBJECTID)} onHandleSelect={onHandleSelect} onHandleDelete={onHandleDelete} ></TrackListItem>)
    })
  }
