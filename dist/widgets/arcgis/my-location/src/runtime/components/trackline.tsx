...
class TrackLineView extends React.PureComponent<Props & ExtraProps, State> {
  ...
  handleSelect = (model: TrackLinePoint | TrackLine, handle: boolean, dataSource: DataSource, selectedIdsKey: string, type: string) => {
    let selIds
    if (handle) {
      selIds = this.props[selectedIdsKey].concat([model.OBJECTID])
      let graphic = null
      if (type === 'point') {
      ...
      selIds = this.props[selectedIdsKey].filter(m => m !== model.OBJECTID)
    }
    if (dataSource) {
      dataSource.selectRecordsByIds(selIds.map(m => m))
    }
  }
