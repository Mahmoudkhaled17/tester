...
useEffect(() => {
  const el = pageNumInputRef.current
  if (!el) return

  const handleInputBlur = () => {
    const input = Number(el.value)
    if (Number.isFinite(input) && (input < 0 || input > 1000)) {
      onSettingChange({
        id,
        config: config.set("maxImageItemCountPerPage", 60)
      })
    }
  }

  el.addEventListener("blur", handleInputBlur)
  return () => { el.removeEventListener("blur", handleInputBlur) }
// eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleMaxImageItemNumberInputChange = (e: Event): void => {
    const node = e.target as unknown as HTMLCalciteInputNumberElement
    const value = typeof node.value === "string" && node.value.trim() !== '' && Number.isInteger(Number(node.value)) ? parseInt(node.value, 10) : ''
    onSettingChange({
      id,
  ...
  return (
    <Fragment>
      ...
      <SettingSection title={translate("options")}>
       ...
       <CalciteInputNumber ref={pageNumInputRef} value={config.maxImageItemCountPerPage.toString()} status={config.maxImageItemCountPerPage > 0 && config.maxImageItemCountPerPage <= 1000 ? "valid" : "invalid"} onCalciteInputNumberChange={handleMaxImageItemNumberInputChange} onCalciteInputNumberInput={handleMaxImageItemNumberInputChange} min={1} max={1000} step={1} integer />
      </SettingSection>
    </Fragment>
  )
}
