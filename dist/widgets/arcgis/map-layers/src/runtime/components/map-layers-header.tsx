...
import type { LayerListItemType, LayerListItemCollection, TableListItemType } from '../types'

interface MapLayersHeaderProps {
  theme: IMThemeVariables
  jimuMapViewId: string
  layerListRef: React.RefObject<{
    filterPredicate?: ((item: LayerListItemType) => boolean) | null
    operationalItems?: LayerListItemCollection
  } | null>
  tableListRef: React.RefObject<{
...
export const onFilterListItem = (searchContent) => {
  return (item) => {
...
export default function MapLayersHeader (props: MapLayersHeaderProps) {
  const { theme, jimuMapViewId, layerListRef, tableListRef, headerKey, config } = props
  const {searchLayers: enableSearch = false, layerBatchOptions: enableBatchOption = false, useMapWidget: isMapWidgetMode, expandAllLayers} = config
  ...
  const onTurnAllLayersClickGenerator = useCallback((visible: boolean) => {
    return () => {
      const showJlvIds = config.customizeLayerOptions?.[jimuMapViewId]?.showJimuLayerViewIds
      ...
      function toggleVisible (jimuLayerViews, visible) {
        for (const layerView of jimuLayerViews) {
          ...
          const currentJlvId = jimuMapView.getJimuLayerViewIdByAPILayer(layer)
          if (!showJlvIds || showJlvIds.includes(currentJlvId)) {
