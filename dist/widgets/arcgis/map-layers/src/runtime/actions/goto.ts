...
export default class Goto extends Action {
  ...
  isValid = (layerItem, isTableList): boolean => {
    ...
    const parentDeclaredClass = layerItem.parent?.layer?.declaredClass

    if (parentDeclaredClass && parentDeclaredClass !== 'esri.layers.GroupLayer' && parentDeclaredClass !== 'esri.layers.KnowledgeGraphLayer') {
      return false
    }
