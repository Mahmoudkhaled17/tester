...
export function StopsSelector (props: Props) {
  ...
  hooks.useEffectWithPreviousValues((prev) => {
    ...
    async function updateSearchWidget () {
      ...
      startSearchWidgetRef.current.on('search-start', handleSearchStart())
      endSearchWidgetRef.current.on('search-start', handleSearchStart())
      startSearchWidgetRef.current.on('search-complete', handleSearchComplete())
      endSearchWidgetRef.current.on('search-complete', handleSearchComplete())
  ...
  }, [currentJimuMapView, props.directionsProps.config.searchConfig, handleSearchResultSelected, translate, isAllModuleLoaded, handleSearchStart, handleSearchComplete, handleSearchClear])
  ...
  return (
    <div css={getGlobalStyle(theme, isLocating)}>
      ...
      <AlertPopup
        ...
        disableOK={isLocating || isSearching}
      >
        {!isJmvReady && <Loading useAriaLive></Loading>}
        <div
