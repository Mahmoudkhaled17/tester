import type { UseDataSource, Expression, ImmutableObject, DataSource } from 'jimu-core'
...
interface CommonLayersInfo {
  featureCount: boolean
  highlightResultsOnMap: boolean
  highlightColor: string
  expandAnalysisResults: boolean
}

export interface ExpressModeAnalysisPayload {
  layersInfo: any[]
  commonLayersInfo: CommonLayersInfo
}

export interface ArcGISFeatureFormElement {
  submit: () => void
...
export interface CacheState {
  combinations: Array<{ analysisType: string; featureLayer: DataSource }>
  uniqueLayers: DataSource[]
  lastSelectedDs: string
  lastSearchByActiveMapArea: boolean
}
