...
import { ErrorOutlined } from 'jimu-icons/outlined/suggested/error'
...
export default class LayersInfos extends React.PureComponent<Props, State> {
  ...
  */
  createAnalysisItem = (analysis, index: number) => {
    // Normalize the ID comparison to handle data views/derived datasources:
    // Use mainDataSourceId when present (the main feature layer ID), otherwise fall back to dataSourceId
    const layerId = analysis.useDataSource.mainDataSourceId || analysis.useDataSource.dataSourceId
    const isLayerUnavailable = this.props.availableLayerDsIds !== undefined &&
      !this.props.availableLayerDsIds.includes(layerId)
    const showActiveMapAreaWarning = !isLayerUnavailable && this.props.isActiveMapAreaSelected && analysis.analysisInfo.analysisType === AnalysisTypeName.Closest
    const tooltipTitle = isLayerUnavailable ? this.nls('warningMsgIfLayerUnavailable') : this.nls('warningMsgIfActiveMapArea')
    const layerItem = (
      <div className={classNames('flex-grow-1 analysis-item cursor-pointer')} key={index}>
        <div className='d-flex w-100 align-items-center'>
          <div className='layer-analysis-name text-truncate' title={analysis.label}
            style={{ maxWidth: showActiveMapAreaWarning || isLayerUnavailable ? '150px' : '165px' }}>
            {analysis.label}
          </div>
          <div className='d-flex align-items-center' style={{ marginLeft: 'auto' }}>
            {(isLayerUnavailable || showActiveMapAreaWarning) &&
              <Tooltip title={tooltipTitle} showArrow placement='top'>
                <div className='title3 text-default d-flex align-items-center mr-1'>
                  {showActiveMapAreaWarning && <WarningOutlined color='var(--sys-color-warning-dark)' />}
                  {isLayerUnavailable && <ErrorOutlined color='var(--sys-color-error-main)' />}
                </div>
              </Tooltip>
            }
