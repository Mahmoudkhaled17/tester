...
import { type DataSourceOptions, type IMConfig, type LayersInfo, type LayerDsId, type FontStyleSettings, AnalysisTypeName, type SummaryFieldsInfo, type ConfigInfo, type SummaryAnalysis, type SketchTools, type SearchSettings, type SaveFeatureSetting, type AnalysisSettings } from '../config'
...
import { defaultConfigInfo, expressModeUIVisibility } from './constants'
...
export default class Setting extends BaseWidgetSetting<AllWidgetSettingProps<IMConfig>, State> {
  ...
  private Color: typeof Color = null
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      isGeneralSettingsOpen: !expressModeUIVisibility.generalSettingsCollapsedByDefault,
        ...
        if (updatedMapViewGroups.jimuMapViews[idx].dataSourceId) {
          if (updatedMapViewGroups.jimuMapViews[idx].status === JimuMapViewStatus.Loaded) {
            ...
            await this.waitForChildDataSourcesReady(jimuMapView).finally(async () => {
              const allDsLayers = await getAllAvailableLayers(updatedMapViewGroups.jimuMapViews[idx].id)
              this.setState({
 ...
 */
  ...
  */
  ...
  /**
   * Update analysis-related configuration for the active data source.
   *
   * Write paths:
   * - `isSaveFeatures === true`: updates `analysisSettings.saveFeatures[property]`.
   * - `isAllAnalysisSettingsUpdate === true`: updates `configInfo[activeDataSource][property]`
   *   (used for full object replacement like `analysisSettings`).
   * - default: updates `analysisSettings[property]`.
   *
   * If both flags are true, both corresponding writes are applied.
   *
   * @param property Analysis setting property key.
   * @param value Analysis setting value to persist.
   * @param isSaveFeatures Whether the update targets the `saveFeatures` subsection.
   * @param isAllAnalysisSettingsUpdate Whether the update writes a full top-level object for the active data source.
   */
  updateAnalysisSettings = (property: string, value: string | boolean | LayersInfo[] | SaveFeatureSetting | AnalysisSettings,
    isSaveFeatures?: boolean, isAllAnalysisSettingsUpdate?: boolean) => {
    let currentConfig = this.props.config
    if (isSaveFeatures) {
      currentConfig = currentConfig.setIn(['configInfo', this.state.activeDataSource, 'analysisSettings', 'saveFeatures', property], value)
    }
    if (isAllAnalysisSettingsUpdate) {
      currentConfig = currentConfig.setIn(['configInfo', this.state.activeDataSource, property], value)
    }
    if (!isSaveFeatures && !isAllAnalysisSettingsUpdate) {
      currentConfig = currentConfig.setIn(['configInfo', this.state.activeDataSource, 'analysisSettings', property], value)
    }
    this.updateOutputDS(currentConfig)
  }
    ...
    try {
      ...
      for (const dsId in currentConfigInfo) {
        ...
        currentConfigInfo?.[dsId]?.analysisSettings.layersInfo?.forEach((layerDetails, index) => {
          ...
          const uniqueIdFields = getDatasource?.getUniqueIdFields?.() ?? []
          ...
          if (layerDetails.analysisInfo.analysisType === AnalysisTypeName.Summary) {
            ...
            // Handle unique id fields: if present, set idField and ensure they are in the output schema
          ...
          if (schema) {
            const outputSchemaFields = filteredSchemaFields && Object.keys(filteredSchemaFields).length > 0
            ...
            const outputDsJson: DataSourceJson = {
              ...
              schema: fieldSchema,
              geometryType: getDatasource ? getDatasource.getGeometryType() : undefined
            }
            if (layerDetails.analysisInfo.analysisType !== AnalysisTypeName.Summary &&
  ...
  render () {
    ...
    const mapSectionAriaLabel = this.props.useMapWidgetIds && this.props.useMapWidgetIds.length > 0 && this.state.mapWidgetExists && (!(useDataSource?.length > 0)) ?
    this.nls('warningMsgIfNoLayersOnMap') : this.state.isNoMapSelected && !(this.props.useMapWidgetIds && this.props.useMapWidgetIds.length > 0) ?
    this.nls('selectMapHint') : ''

    return (
      <div css={getStyle(this.props.theme)}>
        <div className={'widget-setting-near-me'}>
          {/* Map Selector*/}
          <SettingSection role='group' aria-label={mapSectionAriaLabel} className={classNames('map-selector-section', { 'border-0': this.state.isNoMapSelected && !(this.props.useMapWidgetIds && this.props.useMapWidgetIds.length > 0) })}>
          ...
          {this.props.useMapWidgetIds && this.props.useMapWidgetIds.length > 0 && this.state.mapWidgetExists && this.state.isLayerAvailable && (useDataSource?.length > 0) &&
           configInfo?.searchSettings && configInfo?.analysisSettings &&
            <React.Fragment>
              <SettingSection role='group' aria-label={this.nls('analysisSettingsLabel') + ' ' + this.nls('mapSettingsHintMsg')}>
                <SettingRow>
                  <Label title={this.nls('analysisSettingsLabel')}
                    ...
                    {this.state.dataSources.length > 0 &&
                      ...
                      sidePopperContent={this.state.isAnalysisSettingsOpen &&
                        <React.Fragment>
                          ...
                          <SettingSection>
                            <SettingRow>
                              <Label title={this.nls('defineSearchAreaLabel')}
                                className='w-100 d-flex'>
                                <div className='text-truncate flex-grow-1 title2 text-paper'>
                                  {this.nls('defineSearchAreaLabel')}
                                </div>
                              </Label>
                              <Tooltip title={this.nls('defineSearchAreaTooltip')} showArrow placement='top'>
                                <div className='title3 text-default d-inline'>
                                  <InfoOutlined />
                                </div>
                              </Tooltip>
                            </SettingRow>
                            <SettingRow role='group' aria-label={this.nls('defineSearchAreaLabel') + ' ' + this.nls('defineSearchAreaTooltip')} flow='wrap'>
                              <SearchSetting
                            ...
                            <SettingRow className='border-top pt-3'>
                              <Label title={this.nls('analysisLabel')}
                                className='w-100 d-flex'>
                                <div className='text-truncate flex-grow-1 title2 text-paper'>
                                  {this.nls('analysisLabel')}
                                </div>
                              </Label>
                              <Tooltip title={this.nls('configureAnalysisTooltip')} showArrow placement='top'>
                                <div className='title3 text-default d-inline'>
                                  <InfoOutlined />
                                </div>
                              </Tooltip>
                            </SettingRow>
                            <SettingRow role='group' aria-label={this.nls('analysisSettingsLabel') + ' ' + this.nls('configureAnalysisTooltip')} flow='wrap'>
                              <AnalysisSetting
