...
/**
 * Get all the configured data source layers from the selected webmap/webscene
 * @param configuredUniqueUsedDsIds configured unique Ds ids
 * @param jimuMapViewDsId mapview ds id
 * @returns all Configured Ds Layers
 */
export const getConfiguredDsForRuntime = async (configuredUniqueUsedDsIds: string[], jimuMapViewDsId: string): Promise<DataSource[]> => {
  let layerInstance = null
  const allConfiguredDsLayers = []
  for (const currentLayerDsId of configuredUniqueUsedDsIds) {
    layerInstance = getSelectedLayerInstance(currentLayerDsId)
    //If layer datasource is not created yet, create it from the active map datasource.
    //Ensures all available layers are loaded even if their datasources weren't explicitly configured
     const mapDs = DataSourceManager.getInstance().getDataSource(jimuMapViewDsId)
    if (!layerInstance && mapDs?.isDataSourceSet()) {
      try {
        layerInstance = await mapDs?.createDataSourceById(currentLayerDsId)
      } catch {
        layerInstance = null
      }
    }
    if (layerInstance) {
      if (layerInstance?.type === DataSourceTypes.MapService || layerInstance?.type === DataSourceTypes.GroupLayer || layerInstance?.type === DataSourceTypes.SubtypeGroupLayer) {
        const recursiveCheckForGroupLayers = (grpLayer) => {
          ...
          grpChildlayers.forEach((subLayers) => {
            if (subLayers?.type === DataSourceTypes.MapService || subLayers?.type === DataSourceTypes.GroupLayer || subLayers?.type === DataSourceTypes.SubtypeGroupLayer) {
              ...
              allConfiguredDsLayers.push(subLayers)
            }
          })
        }
        ...
        // Avoid duplicates when a feature layer also appears via map/group child datasources.
        const matchedLayerWithMapService = allConfiguredDsLayers.find(item => item.id === layerInstance.id)
        if (!matchedLayerWithMapService) {
          allConfiguredDsLayers.push(layerInstance)
        }
      }
    }
  }
  return allConfiguredDsLayers
}
...
export const getAllAvailableLayers = async (mapViewId: string): Promise<DataSource[]> => {
  ...
  for (const layer of jimuLayerViews) {
    const currentLayerDsId = layer?.layerDataSourceId
    if (!currentLayerDsId) {
      continue
    }
    layerInstance = getSelectedLayerInstance(currentLayerDsId)
...
export const getDisplayField = (layerDs): string => {
  ...
  const uniqueIdFields = layerDs?.getUniqueIdFields?.() ?? []
  const schemaFields = layerDs?.getSchema?.()?.fields
  const hasField = (fieldName: string): boolean => {
    if (!fieldName) return false
    return !!(schemaFields?.[fieldName] || layerDefinition?.fields?.some((field) => field?.name === fieldName))
  }

  // Get all valid unique ID fields
  const validUniqueIdFields = uniqueIdFields.filter((fieldName) => hasField(fieldName))
  const objectIdFieldByLayerDs = layerDs?.getIdField()

  // If one or more unique ID fields exist, use the first one; otherwise fall back to objectId, displayField, or first field
  if (validUniqueIdFields.length >= 1) {
    displayField = validUniqueIdFields[0]
  } else if (objectIdFieldByLayerDs) {
 ...
 */
export const createSymbol = async (selectedRecord: any, symbolRef: React.RefObject<HTMLDivElement>, accessibleMapSymbolLabel: string) => {
  const symbolUtils = await loadArcGISJSAPIModule('esri/symbols/support/symbolUtils')
  symbolUtils.getDisplayedSymbol(selectedRecord.feature as Graphic).then(async (symbol) => {
    if (!symbol) {
      return
    }
    if (symbol.size > 23) {
    ...
    await symbolUtils.renderPreviewHTML(symbol as __esri.Symbol, {
      node: nodeHtml
    })
    ...
    if (nodeHtml.children?.length) {
      ...
      if (imgOrSvgElm) {
        ...
        const accessibleLabel = selectedRecord?.dataSource?.getLayerDefinition()?.name
         ? selectedRecord.dataSource.getLayerDefinition().name + ' ' + accessibleMapSymbolLabel : accessibleMapSymbolLabel

        // Ensure map symbol previews expose accessible text for screen readers.
        const tagName = imgOrSvgElm.tagName?.toLowerCase()
        if (tagName === 'svg') {
          imgOrSvgElm.setAttribute('role', 'img')
          imgOrSvgElm.setAttribute('aria-label', accessibleLabel)
          imgOrSvgElm.setAttribute('title', accessibleLabel)

          const existingTitle = imgOrSvgElm.querySelector('title')
          if (!existingTitle) {
            const titleElement = document.createElementNS('http://www.w3.org/2000/svg', 'title')
            titleElement.textContent = accessibleLabel
            imgOrSvgElm.insertBefore(titleElement, imgOrSvgElm.firstChild)
          }
        } else if (tagName === 'img') {
          imgOrSvgElm.setAttribute('alt', accessibleLabel)
          imgOrSvgElm.setAttribute('title', accessibleLabel)
        }
