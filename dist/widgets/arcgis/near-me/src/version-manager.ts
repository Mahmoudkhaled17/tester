...
class VersionManager extends WidgetVersionManager {
  versions: any[] = [{
    ...
    upgrader: (oldConfig: IMConfig) => {
      ...
      version: '1.20.0',
      description: 'Upgrade general settings config to add url parameters setting and upgrade Analysis settings results to Display all layer results regardless of visibility on',
      upgrader: (oldConfig: IMConfig) => {
        let newConfig = oldConfig
        // Case 1: Add url parameters setting in general settings
        newConfig = newConfig.setIn(['generalSettings', 'urlParametersEnabled'], true)
        // Case 2: Upgrade Analysis settings results to Display all layer results regardless of visibility on
        if (newConfig.configInfo) {
          for (const dsId in newConfig.configInfo) {
            const displayAllLayersResult = newConfig.configInfo[dsId].analysisSettings.displayAllLayersResult
            newConfig = newConfig.setIn(['configInfo', dsId, 'analysisSettings', 'displayAllLayersResult'], true)
            newConfig = newConfig.setIn(['configInfo', dsId, 'analysisSettings', 'displayAllLayersResultOnMap'], displayAllLayersResult)
          }
        }
        return newConfig
      }
