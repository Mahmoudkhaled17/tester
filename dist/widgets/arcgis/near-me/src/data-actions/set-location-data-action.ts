...
export default class SetLocationDataAction extends AbstractDataAction {
  ...
  async isSupported (dataSets: DataRecordSet[], dataLevel: DataLevel): Promise<boolean> {
    ...
    // Don't support knowledge graph data sources for set location action.
    if ([AllDataSourceTypes.KnowledgeGraphLayer, AllDataSourceTypes.KnowledgeGraphSublayer].includes(dataSourceType)) {
      return Promise.resolve(false)
    }
    ...
    if ((dataSet.records.length > 0 && dataLevel === DataLevel.Records && (dataSet.type === 'current' || dataSet.type === 'selected')) ||
     (dataLevel === DataLevel.DataSource && ([AllDataSourceTypes.FeatureLayer].includes(dataSourceType) ||
     [AllDataSourceTypes.SubtypeSublayer].includes(dataSourceType) ||
    [AllDataSourceTypes.GroupLayer].includes(dataSourceType)))) {
