 ...
 */
  ...
  render () {
    const unitSelectLabel = (this.props.bufferHeaderLabel && this.props.bufferHeaderLabel.trim().length > 0)
      ? this.props.bufferHeaderLabel
          ...
          <React.Fragment>
            <div data-testid={'distanceSettings'} className={'d-inline w-100 pt-1'} >
              <div className='column-section'>
                ...
                <label htmlFor='buffer-distance-unit-select' className='sr-only'>{unitSelectLabel}</label>
                <Select id='buffer-distance-unit-select' aria-label={unitSelectLabel} title={unitSelectLabel} placeholder={unitSelectLabel} useFirstOption className='w-50 pl-2' size={'sm'} value={this.state.distanceUnit} onChange={(evt) => { this.onUnitChange(evt.target.value) }}>
