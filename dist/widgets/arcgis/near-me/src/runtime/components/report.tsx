...
import { Label, Select, Option, Button, TextInput, Switch, defaultMessages as jimuUIDefaultMessages, Tooltip, Loading, LoadingType } from 'jimu-ui'
...
interface State {
  ...
  showLoadingOnExportButtonClick: boolean
...
export default class Report extends React.PureComponent<Props, State> {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      limitLegend: true,
      showLoadingOnExportButtonClick: false
    }
  }
  ...
  /**
   * Format and render tables using report data and table nodes (OPTIMIZED)
  ...
  formatAndRenderTables = (tableParentNode: HTMLElement, reportData: any) => {
    ...
    const colsLength = tableInfo.cols.length
    if (colsLength > chunk) {
      const remainingCols = colsLength - chunk
      if (remainingCols <= 2) {
        chunk = colsLength
      }
    }
    ...
    for (let i = 0; i < colsLength; i += chunk) {
      ...
      if (i === 0) {
        title = this.props.intl.formatMessage({
          id: 'totalCountLabel',
          defaultMessage: defaultMessages.totalCountLabel
        }, { tableTitle: reportData.title, total: reportData.totalCount })
      }
      const remainingCols1 = colsLength - sliceLength
      if (remainingCols1 <= 2 && remainingCols1 > 0) {
        sliceLength += remainingCols1
        breakLoop = true
      }
      const colsTempArray = tableInfo.cols.slice(i, sliceLength)
      const rowsLength = tableInfo.rows.length
      const rowsTempArray = new Array(rowsLength)
      // Optimized: Pre-allocate array and use simple loop
      for (let k = 0; k < rowsLength; k++) {
        rowsTempArray[k] = tableInfo.rows[k].slice(i, sliceLength)
      }
      const newTableInfo = {
        cols: colsTempArray,
        rows: rowsTempArray,
        title: title,
        totalCount: 0,
        groupFieldCol: undefined,
        subgroupFieldCol: undefined
      }
      const tableTitleDiv = document.createElement('div')
      tableTitleDiv.className = 'esriCTSectionTitle'
      tableTitleDiv.textContent = newTableInfo.title
      fragment.appendChild(tableTitleDiv)
      ...
      this.renderTable(fragment, newTableInfo, reportData.data.showRowIndex)
      if (breakLoop) {
        break
      }
    }
    // Single DOM append operation
    tableParentNode.appendChild(fragment)
    return tableParentNode
  }

  /**
   * Render each table data (OPTIMIZED)
   * @param parentNode parent node to append table
   * @param tableInfo table structure
   * @param showRowIndex each row index
   */
  renderTable = (parentNode: HTMLElement | DocumentFragment, tableInfo: any, showRowIndex: number) => {
    const noDataText = this.nls('noData')
    // Build HTML string for better performance with large datasets
    const htmlParts: string[] = ['<table class="esriCTTable" style="width: 100%"><tbody><tr>']
    // Header row
    if (showRowIndex) {
      htmlParts.push('<th style="width: 50px; word-break: break-word; padding: 4px">#</th>')
    }
    const colsLength = tableInfo.cols.length
    for (let i = 0; i < colsLength; i++) {
      const col = tableInfo.cols[i]
      const escapedCol = this.escapeHtml(col)
      htmlParts.push(`<th style="word-break: break-word; padding: 4px">${escapedCol}</th>`)
    }
    htmlParts.push('</tr>')
    // Data rows
    const rowsLength = tableInfo.rows.length
    for (let i = 0; i < rowsLength; i++) {
      htmlParts.push('<tr>')
      if (showRowIndex) {
        htmlParts.push(`<td style="padding: 4px; word-break: normal">${i + 1}</td>`)
      }
      const row = tableInfo.rows[i]
      const rowLength = row.length
      for (let j = 0; j < rowLength; j++) {
        const rowValue = row[j]
        const escapedValue = this.escapeHtml(rowValue)
        const italicStyle = rowValue === noDataText ? ' font-style: italic;' : ''
        htmlParts.push(`<td style="padding: 4px;${italicStyle}">${escapedValue}</td>`)
      }
      htmlParts.push('</tr>')
    }
    htmlParts.push('</tbody></table>')
    // Create table element and set innerHTML once
    const tempDiv = document.createElement('div')
    tempDiv.innerHTML = htmlParts.join('')
    const table = tempDiv.firstChild as HTMLTableElement

    // Apply rowspans if needed
   ...
   * in the specified columns. This function modifies the table in place. (OPTIMIZED)
   *
  ...
  addRowSpans (table: HTMLTableElement, columns: number[]) {
    ...
    columns.forEach((colIndex, groupOrSubGroup) => {
      ...
      let startIndex = 0
      // Cache the secondary column index for subgroups
      const secondaryColIndex = isSubGroup ? columns[1] : -1
      for (let i = 0; i < rowsLength; i++) {
        ...
        if (isSubGroup && secondaryColIndex >= 0) {
          const groupCell = rows[i].cells[secondaryColIndex]
          currentGroupValue = groupCell ? groupCell.textContent : null
        }
        ...
        if (currentCellValue === currentValue && (!isSubGroup || currentGroupValue === prevGroupValue)) {
          ...
          if (i > startIndex) {
            const startCell = rows[startIndex]?.cells[colIndex]
            if (startCell) {
              startCell.rowSpan = i - startIndex
            }
          }
          currentValue = currentCellValue
          prevGroupValue = currentGroupValue
          startIndex = i
          const currentCell = rows[startIndex]?.cells?.[colIndex]
          if (currentCell) {
            currentCell.rowSpan = rowsLength - startIndex
          }
        }
      }
    })
      ...
      <html lang="en" dir="ltr">
        <head id="reportHead">
            ...
            <style type="text/css">
                ...
                .scaleBarContainer .root {
                  box-shadow: none !important;
                }

                .scaleBarContainer .esri-widget {
                ...
                .scaleBarContainer .bar-container-ruler {
                  flex-direction: column;
                }

                .scaleBarContainer .bar-container-line:last-child {
                  align-items: flex-start;
                }

                .scaleBarContainer .bar-container {
                ...
                .scaleBarContainer .bar-container .line {
                ...
                .scaleBarContainer .bar-container .line-top {
                 border-bottom: 2px solid black;
                }

                .scaleBarContainer .bar-container .line-top:before {
                  bottom: calc(-1 * 2px));
                  left: 0;
                  content: "";
                  background-color: color-mix(in srgb, white 50%, transparent);
                ...
                .scaleBarContainer .bar-container .line-top:after {
                  bottom: calc(-1 * 2px));
                  right: 0;
                  content: "";
                  background-color: color-mix(in srgb, white 50%, transparent);
                ...
                // .scaleBarContainer .bar-container .ruler {
                //  --color-ruler-background: #363636;
                //  --color-ruler-foreground: white;
                //  height: 6px;
                //  box-shadow: 0 0 0 1px black, 0 1px 2px white;
                // }
                ...
                .scaleBarContainer .bar-container .ruler:before {
                  bottom: calc(-1 * 2px));
                  left: 0;
                  content: "";
                  background-color: color-mix(in srgb, white 50%, transparent);
                ...
                .scaleBarContainer .bar-container .ruler:after {
                  bottom: calc(-1 * 2px));
                  right: 0;
                  content: "";
                  background-color: color-mix(in srgb, white 50%, transparent);
                ...
                .scaleBarContainer .bar-container .label-container-line {
                  position: absolute;
                  left: 0;
                }

                .scaleBarContainer .bar-container .label-container-top {
                  bottom: 0;
                }

                .scaleBarContainer .bar-container .label-container-ruler {
                ...
                .scaleBarContainer .bar-container .label-container .label {
                  white-space: nowrap;
                  color: black;
                  font-size: inherit;
                  padding: 0 7px;
                ...
                .scaleBarContainer .bar-container .label-container-ruler .label {
   ...
   */
  aoiValueLabel = () => {
    const aoiValueLabel = this.props.intl.formatMessage({
      id: 'aoiValueLabel',
  ...
  dateValueLabel = () => {
    ...
    const dateValueLabel = this.props.intl.formatMessage({
      id: 'dateValueLabel',
  ...
  exportOnClick = () => {
    this.setState({
      showLoadingOnExportButtonClick: true
    }, async () => {
  ...
  handleAttribution = () => {
    if (this.props.mapView.view != null) {
      ...
      const attributionContainer = (attributionTool?.[0]?.element as __esri.Widget)?.container
  ...
  handleCompass = () => {
    if (this.props.mapView.view != null) {
      ...
      const compassContainer = (compassTool?.[0]?.element as HTMLArcgisCompassElement)?.shadowRoot?.querySelectorAll('.compass')
  ...
  handleScaleBar = () => {
    if (this.props.mapView.view != null) {
      ...
      if (scaleBarTool && scaleBarTool.length > 0) {
        const scaleBarContainer = (scaleBarTool?.[0]?.element as HTMLArcgisScaleBarElement).shadowRoot
        if (scaleBarContainer != null && typeof scaleBarContainer !== 'string') {
          return scaleBarContainer
        } else {
          return document.querySelector('.scalebar-map-tool')?.querySelector('.number-label')?.cloneNode(true)
        }
      }
    }
  }
  ...
  render () {
    return (
      <div style={{ width: 200 }}>
        ...
        {this.state.showLoadingOnExportButtonClick && <div className='pt-3' style={{ height: '40px', position: 'relative' }}> <Loading type={LoadingType.DotsPrimary} /></div>}
        {!this.state.showLoadingOnExportButtonClick && <div className='pt-3'>
