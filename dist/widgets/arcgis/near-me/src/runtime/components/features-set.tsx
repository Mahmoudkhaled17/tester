...
import { Row, Label, Dropdown, DropdownButton, DropdownMenu, DropdownItem, Icon, Collapse } from 'jimu-ui'
...
import { createSymbol, formatSmallNumberWithSignificantDigits, getDisplayLabel, getPortalUnit, getSelectedLayerInstance } from '../../common/utils'
...
export default class FeaturesSet extends React.PureComponent<Props, State> {
  ...
  public featureComponentTitle: React.RefObject<HTMLDivElement>
  ...
  private featuresElementWatchHandle: any = null

  constructor (props) {
    super(props)
    this.popUpContent = React.createRef()
    this.symbolRef = React.createRef()
    this.featureComponentTitle = React.createRef()
  ...
  componentDidMount = async () => {
    ...
    if (this.props.displayMapSymbol) {
      ...
      createSymbol(this.props.selectedRecord, this.symbolRef, this.nls('accessibleMapSymbolLabel'))
    }
  }
  ...
  componentWillUnmount = () => {
    // Clean up watch handle when component unmounts to prevent memory leaks
    this.removeFeatureElementWatcher()
  }
  ...
  /**
   * Removes the watch handle for the features element to prevent memory leaks
   */
  removeFeatureElementWatcher = () => {
    if (this.featuresElementWatchHandle) {
      this.featuresElementWatchHandle.remove()
      this.featuresElementWatchHandle = null
    }
  }
  ...
  createFeature = () => {
    ...
    if (featureRecord?.feature) {
      ...
      if (parent) {
        // Clean up previous watch handle before creating new features element
        this.removeFeatureElementWatcher()
        const existingFeaturesElement = parent.querySelector('arcgis-features')
        if (existingFeaturesElement) {
          existingFeaturesElement.remove()
        }
        // Create arcgis-feature web component
        const { view } = this.props.jimuMapView
        const featuresElement = Object.assign(document.createElement('arcgis-features'), {
          className: 'jimu-widget bg-transparent pointer',
        ...
        parent.insertBefore(featuresElement, parent.firstChild || null)
        //update the feature title div with the feature popup title if the feature popup title is present
        this.featuresElementWatchHandle = reactiveUtils.watch(() => featuresElement?.effectiveHeading, (title) => {
          if (title !== '' && this.featureComponentTitle?.current) {
            this.featureComponentTitle.current.innerHTML = title
          }
        })
      }
    }
  ...
  /**
   * Build clipped geometry and add it in features attributes
   * @param featureRecord Feature record
   */
  buildClippedGeometry = (featureRecord) => {
    const feature = featureRecord.getFeature()
    //clip the geometry if clip feature is enabled and AOI geometry is present
    const clippedGeometry = intersectionOperator.execute(feature.geometry, (this.props.aoiGeometries.bufferGeometry ?? this.props.aoiGeometries.incidentGeometry) as __esri.GeometryUnion)
    //calculate area or length based on the type of clipped geometry
    if (clippedGeometry?.type === 'polygon') {
      const area = this.getArea([featureRecord], clippedGeometry, this.props.aoiGeometries.distanceUnit || this.props.distanceUnit || getPortalUnit())
      feature.attributes = {
        ...feature.attributes,
        esriCTClippedInfo: Number(area).toString()
      }
    } else if (clippedGeometry?.type === 'polyline') {
      const length = this.getLength([featureRecord], clippedGeometry, this.props.aoiGeometries.distanceUnit || this.props.distanceUnit || getPortalUnit())
      feature.attributes = {
        ...feature.attributes,
  ...
  createFeatureItem = (callFeatureDetailsClick: boolean, displayMapSymbol?: boolean) => {
    const featureRecord = this.props.selectedRecord as any
    //Calculate the clipped geometry and add the clipped info in feature attributes before create the feature item when showClippedFeaturesInfo is true and AOI geometry is present
    if (this.props.showClippedFeaturesInfo && this.props.aoiGeometries?.incidentGeometry && featureRecord?.getFeature()?.geometry) {
      this.buildClippedGeometry(featureRecord)
    }
    ...
    const individualFeatureItem: React.JSX.Element = (
      <div>
        ...
        <div className='title2 feature-widget-title-container nm-border-bottom-color'>
          <div ref={this.featureComponentTitle} className='feature-widget-title'></div>
      ...
      <React.Fragment>
        <Dropdown
          ...
          <DropdownButton size='sm' arrow={false} css={actionButton} icon ref={this.dropdownRef}
            className='data-action-button' type='tertiary' title={this.nls('actions')} aria-label={this.nls('actions')}>
  ...
  getAriaLabelString = (approximateDistanceLabel: string, formattedDistance: string, distanceUnit: string): string => {
    const getAriaLabel = this.props.intl.formatMessage({
  ...
  getLabelForDistUnit = (formattedDistance: string, distanceUnit: string): string => {
    const getLabelForDistanceUnit = this.props.intl.formatMessage({
          ...
          <React.Fragment>
            <Row flow='wrap'>
              <div className={!this.props.approximateDistanceUI && this.state.isFeatureLayerOpen ? 'feature-title-container nm-border-bottom-color' : 'feature-title-container'} onClick={this.onToggleSelectedLayer.bind(this)}
                tabIndex={0} role={'button'} aria-label={featureTitleAriaLabel} aria-expanded={this.state.isFeatureLayerOpen} onKeyDown={(e) => {
                ...
                <div className='d-inline-flex'>
                  {(featureRecord.feature.distance !== null && this.props.showDistFromInputLocation) &&
                    <Label className='label2 approximateDist pr-1'>
                      {this.getLabelForDistUnit(formattedDistance, this.props.distanceUnit)}
                    </Label>
                  }
                  <span className={'mr-1 actionButton p-0'} style={{ color: 'inherit' }} aria-hidden='true'>
