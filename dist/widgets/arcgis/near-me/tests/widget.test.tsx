...
beforeAll(() => {
  ...
  jest.mock('jimu-arcgis', () => ({
    geometryUtils: {
      ...
      createBuffer: jest.fn((geometry, distance, unit) => {
...
describe('Validate Runtime State', () => {
  ...
  it('Should show info message indicating no analysis configured when no analysis is configured', async () => {
    const useMapWidgetId: ImmutableArray<string> = Immutable(['widget_1'])
    const ref: { current: HTMLElement } = { current: null }
    const Widget = wrapWidget(NearMeWidget as any, { theme: mockTheme, ref } as any)
  ...
  it('Show show retrieve all icon when results are generated', () => { //check
    const useMapWidgetId: ImmutableArray<string> = Immutable(['widget_1'])
    const ref: { current: HTMLElement } = { current: null }
    const Widget = wrapWidget(NearMeWidget as any, { theme: mockTheme, ref } as any)
    render(<Widget useMapWidgetIds={useMapWidgetId} theme={theme} config={commonConfig} />)
      ; (ref.current as any).queryLayers()
    waitFor(() => {
      ; (ref.current as any).state.showBatchRetriveAll.toBe(true)
    }, { timeout: 100 })
  })

  it('Save icon should be shown when it is enabled and valid layer has been selected', () => {
    const useMapWidgetId: ImmutableArray<string> = Immutable(['widget_1'])
    const ref: { current: HTMLElement } = { current: null }
    const Widget = wrapWidget(NearMeWidget as any, { theme: mockTheme, ref } as any)
    render(<Widget useMapWidgetIds={useMapWidgetId} theme={theme} config={commonConfig} />)
    waitFor(() => {
      expect((ref.current as any).state.showSaveIcon).toBe(true)
    }, { timeout: 100 })
  })

  it('Should show export icon if datasource supports', () => { //check
    const useMapWidgetId: ImmutableArray<string> = Immutable(['widget_1'])
    const ref: { current: HTMLElement } = { current: null }
    const Widget = wrapWidget(NearMeWidget as any, { theme: mockTheme, ref } as any)
    render(<Widget useMapWidgetIds={useMapWidgetId} theme={theme} config={commonConfig} />)
    waitFor(() => {
      expect((ref.current as any).state.showExportButton).toBe(true)
    }, { timeout: 100 })
  })
})
...
describe('Validate specify a location', () => {
  ...
  it('Should show address input when enabled from config', () => {
    const useMapWidgetId: ImmutableArray<string> = Immutable(['widget_1'])
    const ref: { current: HTMLElement } = { current: null }
    const Widget = wrapWidget(NearMeWidget as any, { theme: mockTheme, ref } as any)
    render(<Widget useMapWidgetIds={useMapWidgetId} theme={theme} config={commonConfig} />)
...
describe('Validate search settings', () => {
  it('Should display Update results button when result are generated', () => {
    ...
    modifiedSearchSettings.searchByActiveMapArea = true
    modifiedSearchSettings.searchByLocation = false
    const { queryByTestId } = render(<Widget useMapWidgetIds={useMapWidgetId} theme={theme} config={modifiedSearchSettings} />)
    expect(queryByTestId('searchByActiveMapAreaButton')).toBeInTheDocument()
  })
})
...
describe('Validate analysis output', () => {
  ...
  it('Should display no result found message when no features are found for the analysis', () => {
    const useMapWidgetId: ImmutableArray<string> = Immutable(['widget_1'])
    const ref: { current: HTMLElement } = { current: null }
    const Widget = wrapWidget(NearMeWidget as any, { theme: mockTheme, ref } as any)
    const { queryByTestId } = render(<Widget useMapWidgetIds={useMapWidgetId} theme={theme} config={commonConfig} />)
      ; (ref.current as any).queryLayers()
    waitFor(() => {
      expect(queryByTestId('noResultsFoundMsg')).toBeInTheDocument()
    }, { timeout: 100 })
  })
})
...
describe('Validate output analysis', () => { // when results are expanded
  ...
  it('Should highlight features with specified color when Highlight results on map is enabled from config', () => {
    const useMapWidgetId: ImmutableArray<string> = Immutable(['widget_1'])
    const ref: { current: HTMLElement } = { current: null }
    const Widget = wrapWidget(NearMeWidget as any, { theme: mockTheme, ref } as any)
    render(<Widget useMapWidgetIds={useMapWidgetId} theme={theme} config={commonConfig} />)
    waitFor(() => {
      expect((ref.current as any).state.analysisSettings?.layersInfo[0].analysisInfo.highlightColorOnMap).toBe('#f507f5')
    }, { timeout: 100 })
  })
})