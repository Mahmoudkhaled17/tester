...
beforeAll(() => {
    ...
    const dss = {
        ...
        getUniqueIdFields: jest.fn().mockReturnValue(['GlobalID']),
...
describe('validate Analysis settings', function () {
    ...
    it('By default for first Closest feature analysis should be selected', function () {
        const ref: { current: HTMLElement } = { current: null }
        const WidgetSettings = wrapWidget(AnalysisSetting as any, { theme: mockTheme, ref } as any)
        render(<WidgetSettings widgetId='analysisSettings' {...customAnalysisSettingsProps} />)
        const analysis = (ref.current as any).onAddAnalysisClick()
        expect(analysis[0].analysisInfo.analysisType).toBe('closest')
    })
    ...
    it('Should persist layer/type changes without marking edited-analysis key', async function () {
        const ref: { current: HTMLElement } = { current: null }
        const WidgetSettings = wrapWidget(AnalysisSetting as any, { theme: mockTheme, ref } as any)
        const WidgetSettingsAny: any = WidgetSettings
        const activeDsLayersConfigWithLayer = {
            ...analysisSettingProps.activeDsLayersConfig,
            layersInfo: mockLayersInfo
        }
        const analysisSettingPropsWithLayer: any = {
            ...analysisSettingProps,
            activeDsLayersConfig: activeDsLayersConfigWithLayer
        }
        render(<WidgetSettingsAny widgetId='analysisSettings' {...analysisSettingPropsWithLayer} />)

        const updatedAnalysis = [{
            ...mockLayersInfo[0],
            analysisInfo: {
                ...mockLayersInfo[0].analysisInfo,
                analysisType: 'proximity'
            }
        }]

        ;(ref.current as any).onAnalysisInfoUpdate(updatedAnalysis, 'dataSource_1-1234567890-layer-1', 'proximity', 0)

        await waitFor(() => {
            expect((ref.current as any).state.updatedAddedLayerAnalysis[0].analysisInfo.analysisType).toBe('proximity')
        })

        ;(ref.current as any).editOkButtonCLick()

        await waitFor(() => {
            expect((ref.current as any).state.newAddedLayerAnalysis[0].analysisInfo.analysisType).toBe('proximity')
        })

        expect((ref.current as any).editedAnalysisLocalKeys.has('dataSource_1-1234567890-layer-1|proximity')).toBe(false)
    })
...
describe('Validate Edit Analysis settings in case of Specify a location', function () {
    ...
    it('By default sort features by distance should be selected', function () {
        const ref: { current: HTMLElement } = { current: null }
        const WidgetSettings = wrapWidget(EditAnalysisPopper as any, { theme: mockTheme, ref } as any)
        const { getByTestId } = render(<WidgetSettings widgetId='editAnalysisSettingsPopper' {...proximityAnalysisPopperProps} />)
        expect((getByTestId('distanceLabel') as any).checked).toEqual(true)
    })
    ...
    it('Should show "Sum of intersected area/length" checkbox when polyline or polygon layer is configured', async function () {
        const ref: { current: HTMLElement } = { current: null }
        const WidgetSettings = wrapWidget(EditAnalysisPopper as any, { theme: mockTheme, ref } as any)
        const { getByTestId } = render(<WidgetSettings widgetId='editAnalysisSettingsPopper' {...summaryAnalysisPopperProps} />)
        await waitFor(() => {
...
describe('Validate Edit Analysis settings in case of Current map area', function () {
    ...
    it('By default for first analysis proxmity analysis should be selected', function () {
        const mockAnalysisSettingsProps = {
            ...analysisSettingProps,
            activeDsSearchConfig: {
                ...analysisSettingProps.activeDsSearchConfig,
                searchByActiveMapArea: true
            }
        }
        const ref: { current: HTMLElement } = { current: null }
        const WidgetSettings = wrapWidget(AnalysisSetting as any, { theme: mockTheme, ref } as any)
        render(<WidgetSettings widgetId='analysisSettings' {...mockAnalysisSettingsProps} />)
        const analysis = (ref.current as any).onAddAnalysisClick()
        expect(analysis[0].analysisInfo.analysisType).toBe('proximity')
    })
...
describe('validate Result Section', function () {
    ...
    it('Near me panel option should be checked by default', async function () {
        const ref: { current: HTMLElement } = { current: null }
        const WidgetSettings = wrapWidget(AnalysisSetting as any, { theme: mockTheme, ref } as any)
        const { getByTestId } = render(<WidgetSettings widgetId='analysisSettings' {...analysisSettingProps} />)
        await waitFor(() => {
        ...
        expect((getByTestId('nearmePanelLabel') as any).checked).toEqual(true)
    })

    it('Map option should be unchecked by default', async function () {
        const ref: { current: HTMLElement } = { current: null }
        const WidgetSettings = wrapWidget(AnalysisSetting as any, { theme: mockTheme, ref } as any)
        const { getByTestId } = render(<WidgetSettings widgetId='analysisSettings' {...analysisSettingProps} />)
        await waitFor(() => {
        ...
        expect((getByTestId('mapLabel') as any).checked).toEqual(false)
    })
