...
import { Button, Icon, Switch, Label, defaultMessages, CollapsablePanel, Tooltip } from 'jimu-ui'
...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<IMConfig>, States> {
  ...
  handleViewsCreate = (views: { [viewId: string]: JimuMapView }) => {
    const view3DIdx = Object.keys(views).findIndex(viewId => {
  ...
  // import & export
  handleEnableImportChange = (evt): void => {
    const checked = evt.target.checked
    this.props.onSettingChange({
      id: this.props.id,
      config: this.props.config.setIn(['drawOptions', 'enableImport'], checked)
    })
  }
  handleEnableExportChange = (evt): void => {
    const checked = evt.target.checked
    this.props.onSettingChange({
      id: this.props.id,
      config: this.props.config.setIn(['drawOptions', 'enableExport'], checked)
    })
  }
  ...
  render () {
    ...
    // import & export
    const importExportTips = this.props.intl.formatMessage({ id: 'importExportTips', defaultMessage: nls.importExportTips })
    const importExportInfoNls = this.props.intl.formatMessage({ id: 'importExportInfoNls', defaultMessage: nls.importExportInfoNls })
    const enableImportTips = this.props.intl.formatMessage({ id: 'enableImportTips', defaultMessage: nls.enableImportTips })
    const enableExportTips = this.props.intl.formatMessage({ id: 'enableExportTips', defaultMessage: nls.enableExportTips })

    const isSelectedMap = !!(this.props.useMapWidgetIds?.[0])
    return (
      <div css={getStyle(this.props.theme, polished)} className='widget-setting-menu jimu-widget-setting'>
        <SettingSection title={selectMapWidgetTips} className={classNames('map-selector-section', { 'border-0': !isSelectedMap })}>
          <SettingRow>
        ...
        {/* no map tips */}
        {!isSelectedMap && <div className='d-flex placeholder-container justify-content-center align-items-center'>
        ...
        {isSelectedMap && <React.Fragment>
          ...
          {/* 3 Options setting */}
          <SettingSection>
            <CollapsablePanel label={optionsTips} aria-label={optionsTips} defaultIsOpen>
              <React.Fragment>
                {/* 3.1 DrawTools */}
          ...
          {/* 4 Draw setting */}
          <SettingSection>
            <CollapsablePanel label={drawSettingsTips} aria-label={drawSettingsTips} defaultIsOpen={false}>
              <DrawOptions
          ...
          {/* 4 Advanced setting */}
          <SettingSection>
            <CollapsablePanel label={advancedTips} aria-label={advancedTips} defaultIsOpen={false}>
              <React.Fragment>
                {/* 4.1 layer list mode */}
                <SettingSection className='px-0'>
                  {/* switch */}
                  <SettingRow tag='label' label={displayDrawingsTips} className=''>
                  ...
                  {/* import & export */}
                  <Label className='d-flex align-items-center text-break import-export-label-title' aria-label={importExportTips}>
                    <div className='import-export-label'>{importExportTips}</div>
                    <Tooltip showArrow role="tooltip" title={importExportInfoNls} className='ml-1'>
                      <Button icon disableHoverEffect disableRipple variant="text"> <InfoOutlined /> </Button>
                    </Tooltip>
                  </Label>
                  <SettingRow tag='label' label={enableImportTips} className=''>
                    <Switch checked={this.props.config.drawOptions?.enableImport}
                      onChange={this.handleEnableImportChange} />
                  </SettingRow>
                  <SettingRow tag='label' label={enableExportTips} className=''>
                    <Switch checked={this.props.config.drawOptions?.enableExport}
                      onChange={this.handleEnableExportChange} />
                  </SettingRow>
                </SettingSection>
