...
import { settingSectionsInfo } from './constants'
...
function Setting (props: AllWidgetSettingProps<IMConfig>) {
  ...
  const renderConfigs = () => {
    return settingSectionsInfo.map((sectionInfo) => {
      return (
        <SettingSection title={sectionInfo.sectionNameLabelKey ? nls(sectionInfo.sectionNameLabelKey) : undefined}>
          {renderSettings(sectionInfo.toggles)}
        </SettingSection>
      )
    })
  }
