...
interface Props {
  ...
  getSliceWidget: () => HTMLArcgisSliceElement
  //
...
export const BottomBtns = React.memo((props: Props) => {
  ...
  // right btns
  const {
    jimuMapViewState, apiLoadedState, getSliceWidget,
  ...
  const handleSaveAnalysis = React.useCallback(() => {
    ...
    const sliceWidget = getSliceWidget()
    if (sliceWidget?.state === 'sliced' && sliceWidget.shape) {
      analysisJson = sliceWidget.shape.toJSON()
    }
    let analysisConfig = ''
    if (analysisJson) {
  ...
  }, [jimuMapViewState, getSliceWidget,
    setIsSavedFlagState,
    onPresetAnalysisChange, setPresetAnalysisInConfig, clearPresetAnalysisInConfig])
