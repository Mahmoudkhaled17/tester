...
export const useSliceAnalysis = (props: SliceAnalysisProps) => {
  ...
  const hasPresetAnalysisInConfig = React.useCallback(() => {
    const hasAnalysis = !!(props.sliceConfig.analyses && getPresetAnalysisInConfig())
    return hasAnalysis
  ...
  // view point
  const getPresetViewpointInConfig = React.useCallback((): Viewpoint => {
  ...
  const isSameMapViewForPresetAnalysis = React.useCallback((mapId: string) => {
    const isSameFlag = (mapId === getPresetMapViewIdInConfig())
    return isSameFlag
  ...
  //4
  const removeAnalysesFromView = React.useCallback((sliceAnalysisRef: SliceAnalysis) => {
    const view = props.jimuMapView?.view as SceneView
