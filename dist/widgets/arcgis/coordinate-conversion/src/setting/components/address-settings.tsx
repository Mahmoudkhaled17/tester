    ...
    return <div style={{ height: '100%', width: '100%', marginTop: '5px' }}>
      <div css={getAddressSettingsStyle(this.props.theme)}>
        <SettingRow role='group'
          aria-label={this.props.config?.useUtilitiesGeocodeService?.length < 1 && this.state.geocodeLocatorUrl + ' ' + this.nls('defaultGeocodeUrlTooltip')}
          flow='wrap'>
          <UtilitySelector
        ...
        {this.props.config?.useUtilitiesGeocodeService?.length < 1 &&
          <SettingRow className={'locator-url'}>
            <Label>{this.state.geocodeLocatorUrl}</Label>
            <Tooltip title={this.nls('defaultGeocodeUrlTooltip')} showArrow placement='top'>
        ...
        <SettingRow>
          <Label check centric style={{ cursor: 'pointer' }} className='title3 hint-default'>
            <Checkbox role={'checkbox'} aria-label={this.nls('showFullAddress') + ' ' + this.nls('showFullAddressHint')}
