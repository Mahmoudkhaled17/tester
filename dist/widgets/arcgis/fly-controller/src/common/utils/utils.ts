...
// 1. vec3.normalise(out, position) for globes
// 2. [0, 0, 1] for local viewing mode
export function worldUpAtPosition (position, outVec, vec3, view): void {
  if (view.viewingMode === "global") {
    vec3.normalize(outVec, position)
  } else {
    outVec[0] = 0
    outVec[1] = 0
    outVec[2] = 1
...
export function getRotatedAngle (position: {prePos, basePos, nextPos}, vec3, vec3d, sceneView: __esri.SceneView, esriMathUtils): number {
  ...
  worldUpAtPosition(basePos, auxUpVec, vec3, sceneView) // up-aux-line
