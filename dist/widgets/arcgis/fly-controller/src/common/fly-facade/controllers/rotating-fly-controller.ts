import { isDefined, degToRad, radToDeg, geoCoordToRenderCoord, worldUpAtPosition } from '../../utils/utils'
...
export default class RotatingFlyController extends BaseFlyController {
  ...
  _updateCameraGlByTilt = (tilt: number, opts?: { isUpdateCameraConfig: boolean }): void => {
    ...
    const wantedCamPos = this.rotateBySceneMode(basePoint, rotateAux, tilt, this._cache.lookAtTargetGL.pos)
  ...
  _updateInitCamera = (point: __esri.Point, opts: { isUpdateTilt: boolean }): __esri.Point => {
    ...
    worldUpAtPosition(this._cache.lookAtTargetGL.pos, this._cache.lookAtTargetGL.upAxis, this.vec3, this.sceneView) // up-aux-line
  ...
  private _rotateCam (rotateSpeed: number): void {
    ...
    const wantedCamPos = this.rotateBySceneMode(this._cache.cameraGL.pos, rotateAux, rotateSpeed, this._cache.lookAtTargetGL.pos)
    this._cache.cameraGL.pos = wantedCamPos
    // update UP dir
