...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<IMConfig> & ExtraProps & CustomProps, State> {
  ...
  private readonly handleControllerLayoutChanged = (layout: PanelLayout): void => {
    ...
    if (this.props.inControllerUx !== 'offPanel') {
      getAppConfigAction().editWidgetProperty(this.props.id, 'inControllerUx', 'offPanel').exec()
    }
    ...
    return (<React.Fragment>
      <SettingSection className={'d-2 item-detail-wrapper ' + cssClasses}>
        ...
        <SettingRow className='mt-2 radio-wrapper'>
          <Radio checked={isCW} id='CW' aria-label={cw}
            style={{ cursor: 'pointer' }} onChange={e => { this.handleRotateRadioChange(RotateDirection.CW, idx) }} />
          <Label style={{ cursor: 'pointer' }} for='CW' className='ml-1 text-break'>{cw}</Label>
        </SettingRow>
        <SettingRow className='mt-2 radio-wrapper'>
          <Radio checked={!isCW} id='CCW' aria-label={ccw}
            style={{ cursor: 'pointer' }} onChange={e => { this.handleRotateRadioChange(RotateDirection.CCW, idx) }} />
  ...
  renderPathSetting = (styleConfig: PathItemConfig, idx: number): React.ReactElement => {
    ...
    return (
      <SettingSection className={'d-2 item-detail-wrapper ' + cssClasses}>
        ...
        <SettingRow className='mt-2 radio-wrapper'>
          <Radio checked={isCurve} id='CURVED' aria-label={smoothedCurve}
            style={{ cursor: 'pointer' }} onChange={e => { this.handlePathRadioChange(PathStyle.Smoothed, idx) }} />
          <Label style={{ cursor: 'pointer' }} for='CURVED' className='ml-1 text-break'>{smoothedCurve}</Label>
        </SettingRow>
        <SettingRow className='mt-2 radio-wrapper'>
          <Radio checked={!isCurve} id='LINE' aria-label={realPath}
            style={{ cursor: 'pointer' }} onChange={e => { this.handlePathRadioChange(PathStyle.RealPath, idx) }} />
  ...
  render (): React.ReactElement {
    ...
    return (
      <div className='jimu-widget-setting h-100' css={getStyle(this.props.theme)}>
  ...
  renderRouteDetailPage = (): React.ReactElement => {
    ...
    if (!utils.isDefined(this.state.mapPopperJimuMapView)) {
      // loading
      content = <Loading type={LoadingType.Secondary} useAriaLive/>
    } else {
