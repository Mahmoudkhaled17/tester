...
export class RecordDetail extends React.PureComponent<Props, States> {
  ...
  render (): React.ReactElement {
    ...
    const nameLabel = this.props.intl.formatMessage({ id: 'recordLabel', defaultMessage: nls.recordLabel })
    ...
    return (
      <React.Fragment>
        ...
        <SettingRow className='mt-2 radio-wrapper'>
          <Radio checked={isCW} id='CW' style={{ cursor: 'pointer' }} aria-label={cw}
            onChange={e => { this.handlePointDirectionChange(RotateDirection.CW) }} />
          <Label style={{ cursor: 'pointer' }} for='CW' className='ml-1 text-break'>{cw}</Label>
        </SettingRow>
        <SettingRow className='mt-2 radio-wrapper'>
          <Radio checked={!isCW} id='CCW' style={{ cursor: 'pointer' }} aria-label={ccw}
            onChange={e => { this.handlePointDirectionChange(RotateDirection.CCW) }} />
  ...
  handlePathAltitudeChange = (altitude: number): void => {
    const _altitude = altNumFix(altitude)
  ...
  renderPathStyle = (): React.ReactElement => {
    ...
    return (
      <React.Fragment>
        ...
        <SettingRow className='mt-2 radio-wrapper'>
          <Radio checked={isCurve} id='CURVED' style={{ cursor: 'pointer' }} aria-label={smoothedCurve}
            onChange={e => { this.handlePathStyleChange(ControllerMode.Smoothed) }} />
          <Label style={{ cursor: 'pointer' }} for='CURVED' className='ml-1 text-break'>{smoothedCurve}</Label>
        </SettingRow>
        <SettingRow className='mt-2 radio-wrapper'>
          <Radio checked={!isCurve} id='LINE' style={{ cursor: 'pointer' }} aria-label={realPath}
            onChange={e => { this.handlePathStyleChange(ControllerMode.RealPath) }} />
