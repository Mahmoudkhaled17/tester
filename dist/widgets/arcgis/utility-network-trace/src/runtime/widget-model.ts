  ...
  public registerEvents () {
    ...
    this.unt.on('create-result-area', () => {
      ...
      setTimeout(() => {
        this.unt.viewModel.traceResults.forEach((traceResult) => {
          if ('resultAreaGraphic' in (traceResult as object)) {
            const graphic = (traceResult as { resultAreaGraphic?: __esri.Graphic }).resultAreaGraphic
            if (graphic) graphicsList.push(graphic)
          }
        })
    ...
    this.unt.on('clear-selection', (res) => {
      if (this.unt) {
        ...
        this.getActiveMap().mapComponent.closePopup()
        this.clearSelection(res)
      }
    })

    // eslint-disable-next-line no-unexpected-multiline
    (this.unt.viewModel).on('select-features', (res: any) => {
      this.clearSelection(res)
