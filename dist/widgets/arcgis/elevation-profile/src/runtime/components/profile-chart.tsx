...
import { convertSingle, niceScale } from '../../common/unit-conversion'
...
interface Props {
  ...
  visibleProfiles: any
  effectiveDisplayUnits: any
  analysisViewStatistics: any
  unitOptions: any
  isFlip: boolean
...
export default class Chart extends React.PureComponent<Props, IState> {
  ...
  createOrUpdateChartDefinition = () => {
    const { visibleProfiles, selectedElevationUnit } = this.props
    if (visibleProfiles?.length > 0 &&
      this.props.selectedLinearUnit && selectedElevationUnit && this.props.theme) {
      const profileOutput = visibleProfiles[0].samples
      this._viewOutput = null
      this._elevationLayersOutput = []
      let lastElevationProfileIndex
      let viewProfileIndex = -1
      if (this.props.mapView.view.type === '3d') {
        const inputProfileIndex = visibleProfiles.findIndex((line) => line.profile?.type === 'input')
        viewProfileIndex = visibleProfiles.findIndex((line) => line.profile?.type === 'scene')
        // In 3D the analysis may contain ground/query lines, an optional scene profile,
        // and an input profile. Derive the elevation-line cutoff from the actual profile types.
        lastElevationProfileIndex = inputProfileIndex >= 0 ? inputProfileIndex - 1 : visibleProfiles.length - 1
      }
      if (visibleProfiles.length > 0) {
        if (this.props.showVolumetricObj && this.props.mapView.view.type === '3d' && viewProfileIndex >= 0) {
          this._viewOutput = visibleProfiles?.[viewProfileIndex]?.samples
        }
        ...
        this.props.addedElelvationProfileLayers?.forEach((layersInfo) => {
          visibleProfiles.forEach((linesInfo, index) => {
            if ((lastElevationProfileIndex === undefined || index <= lastElevationProfileIndex) && linesInfo.profile?.type !== 'scene') {
              if (linesInfo.profile.id === layersInfo.id) {
                const style = {
                  style: layersInfo.style,
                  label: layersInfo.label
                }
                this._elevationLayersOutput.push({
                  id: linesInfo.profile.id,
      ...
      if (profileOutput?.length > 0) {
        ...
        if (profileOutput.length > 0) {
          profileOutput.forEach((chartData, pointIndex) => {
            ...
            if (referenceY !== null) {
              referenceY = convertSingle(chartData.elevation, this.props.effectiveDisplayUnits?.elevation, selectedElevationUnit)
            }
            const elevationInfo: any = {
              xCoordinate: chartData.x,
              yCoordinate: chartData.y,
              x: convertSingle(chartData.distance, this.props.effectiveDisplayUnits?.distance, this.props.selectedLinearUnit),
              y: referenceY,
              esriCTViewY: null,
              pathIdx: 0,
              pointIdx: pointIndex
            ...
            this._elevationLayersOutput?.forEach((profileOutput) => {
              ...
              if (profileOutput.id !== visibleProfiles[0].profile.id) {
                if (profileOutput.samples[pointIndex].elevation === null) {
                  elevationInfo[profileOutput.layerId] = null
                } else {
                  elevationInfo[profileOutput.layerId] = convertSingle(profileOutput.samples[pointIndex].elevation, this.props.effectiveDisplayUnits?.elevation, selectedElevationUnit)
                }
              }
            })

            if (this._viewOutput) {
              elevationInfo.esriCTViewY = convertSingle(this._viewOutput[pointIndex].elevation, this.props.effectiveDisplayUnits?.elevation, selectedElevationUnit)
            }
            ...
            this.createDataForIntersectingAssets(allElevations).then(() => {
              ...
              const newChartDefinition = {
                ...
                series: [{
                  ...
                  color: this.props.addedElelvationProfileLayers.filter((layer) => layer.id === visibleProfiles?.[0]?.profile?.id)[0]?.style?.lineColor
                }]
              }
              ...
              this.setState({
                ...
                if (this._chart) {
                  ...
                  this._chartLegend.labels.template.set('fontFamily', this.props.theme.sys.typography.body.fontFamily)
  ...
  getNearestElevationData = (chartObjectX: number) => {
    ...
    let needle = convertSingle(chartObjectX, this.props.selectedLinearUnit, this.props.effectiveDisplayUnits?.distance)
    if (this.props.isFlip) {
  ...
  initChart = async (params) => {
    ...
    this._elevationLayersOutput?.forEach((profileInfo) => {
      if (profileInfo.id === this.props.visibleProfiles[0].profile.id) {
  ...
  showChartTooltip = (chartInfo) => {
    ...
    if (nearestElevationData) {
      ...
      if (this.props.analysisViewStatistics != null) {
        let { maxDistance } = this.props.analysisViewStatistics
        if (maxDistance != null) {
          // convert the max distance from effectiveUnits to current selected linear units
          maxDistance = convertSingle(maxDistance, this.props.effectiveDisplayUnits?.distance, this.props.selectedLinearUnit)
      ...
      if (this.props.visibleProfiles?.length > 0 && this.props.showVolumetricObj && this.props.mapView.view.type === '3d') {
  ...
  */
  ...
  adjustAxisRanges = (ctx) => {
    ...
    if (this.props.analysisViewStatistics) {
  ...
  getAdjustedBounds = ({ pixelWidth, pixelHeight }) => {
    const statistics = this.props.analysisViewStatistics
    const minX = 0
    //get min max from ground statistics
    let maxX = statistics.maxDistance
    maxX = convertSingle(maxX, this.props.effectiveDisplayUnits?.distance, this.props.selectedLinearUnit)
    let minY = statistics.minElevation
    minY = convertSingle(minY, this.props.effectiveDisplayUnits?.elevation, this.props.selectedElevationUnit)
    let maxY = statistics.maxElevation
    maxY = convertSingle(maxY, this.props.effectiveDisplayUnits?.elevation, this.props.selectedElevationUnit)
    ...
    const dynamicElevationRange = this.props.visibleProfiles?.some((line) => line.profile.type === 'scene')
    if (dynamicElevationRange) {
  ...
  addGroundSeries = (ctx) => {
    // eslint-disable-next-line @typescript-eslint/prefer-find
    const groundLayer = this.props.addedElelvationProfileLayers.filter((layer) => layer.id === this.props.visibleProfiles?.[0].profile.id)[0]
    ...
    eachAssetLayerResult?.intersectionResult.forEach((eachIntersectingFeatureResult, featureIndex) => {
      ...
      eachIntersectingFeatureResult.connectedFeatureForProfiling.forEach((connectedFeature) => {
        ...
        connectedFeature.paths.forEach((eachPath, pathIndex) => {
          ...
          const seriesID = layerId + 'y_I_' + featureIndex + '_' + pathIndex
          const seriesIDPoint = layerId + 'y-point_' + featureIndex + '_' + pathIndex
            ...
            if (pointIndexOnPath !== 0) {
              ...
              if (nextDataToBeUpdated.length > 1) {
                ...
                if (ev1 === undefined || ev2 === undefined) {
                  ...
                  nextDataToBeUpdated.forEach((element) => {
                    const pointLengthFromStart = element.x - oldDistance
        ...
        dataToBeUpdatedForEachSegment.forEach((eachSegmentsData, pi) => {
          const seriesID = layerId + 'y_I_' + featureIndex + '_' + pi
          const seriesIDPoint = layerId + 'y-point_' + featureIndex + '_' + pi
          let sum = 0
      ...
      eachIntersectingFeatureResult.disconnectedFeatureForProfiling.forEach((disconnectedFeatureGeometry, pointIndex) => {
        ...
        let elementToBeUpdated
        const seriesId = layerId + 'y-point_' + featureIndex + '_' + pointIndex + '_d'
        //when point is in buffer which may intersect the last point of the line
        if (nextDataToBeUpdated.length < 1) {
          const chartDataLength = chartData.length
          //if the distance of the point is greater than the distance of the last point on the line
  ...
  addElevationForSelectedFeature = (config, selectedFeature, currentFeatureLength, nextDataToBeUpdated, currentTotalDistance): void => {
    ...
    if (config.type === 'one' && config.field1 && selectedFeature.attributes[config.field1] !== null) {
      ...
      const slope = (field2Value - field1Value) / currentFeatureLength
      nextDataToBeUpdated.forEach((element) => {
        const pointLengthFromStart = element.x - currentTotalDistance
        ...
        segments.forEach((eachSegment, segmentIndex) => {
          ...
          nextDataToBeUpdatedForSeg.forEach((element) => {
            if (!element[layerId + 'y']) {
              const pointLengthFromStart = element.x - totalSegLength
  ...
  createPointSeriesForIntersectingLines = (ctx, configuredAssetSetting, seriesName, featureIndex, pathIndex) => {
    ...
    const pointSeries = LineSeriesAm5.new(ctx.chart.root, {
      ...
      valueXField: 'x',
      valueYField: configuredAssetSetting.layerId + 'y-point_' + featureIndex + '_' + pathIndex,
      xAxis: ctx.xAxis,
      yAxis: ctx.yAxis
    })
  ...
  createSeriesForIntersectingLines = (ctx, configuredAssetSetting, seriesName, featureIndex, pathIndex) => {
    const lineSeries = LineSeriesAm5.new(ctx.chart.root, {
      ...
      stroke: colorAm5(configuredAssetSetting.style.intersectingAssetColor),
      valueXField: 'x',
      valueYField: configuredAssetSetting.layerId + 'y_I_' + featureIndex + '_' + pathIndex,
      xAxis: ctx.xAxis,
      yAxis: ctx.yAxis,
      legendLabelText: '{name}'
    })
  ...
  displayChartLegend = (ctx) => {
    ...
    legend.labels.template.setAll({
      ...
      fontFamily: this.props.theme.sys.typography.body.fontFamily
    })
