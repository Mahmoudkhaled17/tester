...
export default class ResultPane extends React.PureComponent<Props, IState> {
  ...
  componentDidUpdate = (prevProps: Props) => {
    ...
    if (prevProps.commonDsGeneralSettings.showLegend !== this.props.commonDsGeneralSettings?.showLegend ||
      ...
      this.setState({
        ...
        if (!this.state.displayStats || !this.props.visibleProfiles) {
          this.setState({
            statisticsOpen: false
          })
        }
  ...
  getMaxDistance = () => {
    return this.props.visibleProfiles?.[0]?.statistics?.maxDistance ? convertSingle(this.props.visibleProfiles[0].statistics.maxDistance, this.props.effectiveDisplayUnits?.distance, this.state.selectedLinearUnit) : 0
  }
    ...
    if (!chart?.series) {
        ...
        this.state.addedElevationLayers.forEach((layerInfo) => {
          if (layerInfo.id === series.get('id') && layerInfo.displayStatistics) {
            elevationLayersSeries.push({
              ...
              statistics: this.props.visibleProfiles?.filter((profile) => profile.profile.id === layerInfo.id)[0],
        ...
        chart?.series?.values.forEach((series, index) => {
          if (volumetricObjOptions?.id === series.get('id')) {
            elevationLayersSeries.push({
              ...
              statistics: this.props.visibleProfiles?.filter((profile) => profile.profile.id === volumetricObjOptions.id)[0],
      ...
      elevationLayersSeries.forEach((series, index) => {
        items.push(<ProfileStatistics
          ...
          visibleProfiles={this.props.visibleProfiles}
          effectiveDisplayUnits={this.props.effectiveDisplayUnits}
        ...
        <div className={classNames('w-100', this.props.chartRender ? '' : 'alignInfo align-items-center', this.props.noFeaturesFoundError ? 'alignInfo align-items-center' : '')}>
          ...
          {(this.props.chartRender || this.state.exportData) &&
            <ProfileChart
              ...
              visibleProfiles={this.props.visibleProfiles}
              effectiveDisplayUnits={this.props.effectiveDisplayUnits}
              analysisViewStatistics={this.props.analysisViewStatistics}
