...
import ElevationProfileAnalysis from 'esri/analysis/ElevationProfileAnalysis'
import type ElevationProfileAnalysisView2D from 'esri/views/2d/analysis/ElevationProfileAnalysisView2D'
import type ElevationProfileAnalysisView3D from 'esri/views/3d/analysis/ElevationProfileAnalysisView3D'
import ElevationProfileLineGround from 'esri/analysis/ElevationProfile/ElevationProfileLineGround'
import ElevationProfileLineQuery from 'esri/analysis/ElevationProfile/ElevationProfileLineQuery'
import ElevationProfileLineScene from 'esri/analysis/ElevationProfile/ElevationProfileLineScene'
import ElevationProfileLineInput from 'esri/analysis/ElevationProfile/ElevationProfileLineInput'
import * as reactiveUtils from 'esri/core/reactiveUtils'
import ElevationLayer from 'esri/layers/ElevationLayer'
import * as jsonUtils from 'esri/symbols/support/jsonUtils'
import Polyline from 'esri/geometry/Polyline'
import SpatialReference from 'esri/geometry/SpatialReference'
import * as unitUtils from 'esri/core/units'
...
export default class Widget extends BaseWidget<AllWidgetProps<IMConfig> & ExtraProps, IState> {
  private _sketchViewModelDrawingLayer: GraphicsLayer
  ...
  private _elevationProfileAnalysis: ElevationProfileAnalysis
  ...
  private _analysisView: ElevationProfileAnalysisView2D | ElevationProfileAnalysisView3D
  private _doneButtonClicked: boolean
  private _isSketchVMReshaping: boolean
  ...
  constructor (props) {
    ...
    this._elevationProfileAnalysis = null
    ...
    this._analysisView = null
    this._doneButtonClicked = false
    this._isSketchVMReshaping = false
    this.state = {
      initialStage: true,
      resultStage: false,
  ...
  createDrawingLayers = () => {
    //create new graphicsLayer to sketch
    this._sketchViewModelDrawingLayer = new GraphicsLayer({
      listMode: 'hide'
    })
  ...
  onDataSourceInfoChange = (info: IMDataSourceInfo) => {
    if (info?.selectedIds?.length) {
      ...
      const selectedFeatureRecord = dsLayer.getSelectedRecords()
      //Show profile on app load if feature is preselected
    ...
    if (jmv) {
      this.setState({
        ...
        setTimeout(() => {
          ...
          this.state.jimuMapView.view.map.addMany([this._sketchViewModelDrawingLayer, this._bufferLayer, this._nextPossibleSelectionLayer, this._drawingLayer, this._intersectionHighlightLayer])
          this.createApiWidget(jmv)
          this.createEpAnalysis(jmv)
  ...
  createDefaultConfigForDataSource = () => {
    ...
    const elevationUnit = getPortalSelfElevationUnits(this.props.portalSelf)
    const linearUnit = getPortalSelfLinearUnits(this.props.portalSelf)
  ...
  /**
   * Set the drawn polyline geometry for elevation profile to analyze
   * @param graphic drawn graphic
   * @param sketchState current state of the sketch view model
   */

  setDrawnLineGeometry = (graphic: Graphic, sketchState: 'active' | 'complete') => {
    if (graphic?.geometry?.type !== 'polyline') return

    if (this._elevationProfileAnalysis) {
      this._elevationProfileAnalysis.clear()
      this._elevationProfileAnalysis.setGeometryFromFeature(graphic)
    }
    let isGeometryChanged = true
    if (this._elevationProfileAnalysis?.geometry) {
      isGeometryChanged = !equalsOperator.execute(graphic.geometry, this._elevationProfileAnalysis.geometry)
    }
    // Set loading indicator and prepare for chart rendering
    this.setState({
      loadingIndicator: isGeometryChanged,
      sketchVMState: sketchState
    })
  }
  ...
  createApiWidget = (jmv: JimuMapView) => {
    const sketchVM = new SketchViewModel({
      view: jmv.view,
      layer: this._sketchViewModelDrawingLayer,
    ...
    //update the polyline geometry on reshape and set the drawn geometry to elevation profile analysis for result update on chart
    sketchVM.on('update', event => {
      const g = event?.graphics?.[0]
      if (!g) return
      if (event.state === 'active') {
        this._isSketchVMReshaping = true
        this.setDrawnLineGeometry(event.graphics[0], event.state)
      }
    })

    sketchVM.on('create', event => {
      this.setState({ sketchVMState: null })
      this._isSketchVMReshaping = false
      ...
      if (event.state === 'start') {
      ...
      } else if (event.state === 'complete' || event.state === 'active') {
        this.setState({
          noFeaturesError: false,
          sketchVMState: event.state
        })
        if (this.state.selectModeActive && event.state === 'complete') {
          this._sketchViewModelDrawingLayer?.graphics?.removeAll()
          this.setState({
        ...
        } else if (this.state.drawModeActive) {
          const pathLength = (event.graphic.geometry as any).paths?.[0]?.length
          //done button clicked functionality when there are less than 2 points in the geometry,
          //to clear the graphics and chart and reset the widget to initial stage
          if (this._doneButtonClicked && pathLength < 2) {
            this._sketchViewModelDrawingLayer?.graphics?.removeAll()
            this._analysisView.analysis.clear()
          } else if (!this._doneButtonClicked || pathLength > 1) {
            event.state === 'complete' && sketchVM.update([event.graphic], { tool: 'reshape' })
            // Handle draw mode completion by assigning the drawn polyline to elevation profile analysis
            this.setDrawnLineGeometry(event.graphic, event.state)
          }
          this._doneButtonClicked = false
        }
      }
    ...
    jmv?.view?.on('click', (event) => {
      ...
      else if (this.state.drawModeActive) { //update the drawn geometry on click in draw mode
        jmv.view.hitTest(event).then((response) => {
          if (response && response.results) {
            const res = response.results?.find((r: any) => r.graphic?.layer === sketchVM.layer) as any
            if (!res) return
            //stopPropagation so that info window is not shown
            event.stopPropagation()
            const g = res.graphic
            sketchVM.update([g], { tool: 'reshape' })
          }
        })
      }
    })
  ...
  getGroundSeriesLabel = (label: string): string => {
    const referenceLayerLabel = this.props.intl.formatMessage({
  ...
  createEpAnalysis = async (jmv: JimuMapView, layerInfoUpdated?: boolean) => {
    if (!jmv.view) {
      return
    }
    const profiles: any = []

    // Clear existing analysis if it exists
    if (this._elevationProfileAnalysis && !layerInfoUpdated) {
      this._elevationProfileAnalysis.destroy()
      this._elevationProfileAnalysis = null
    }
    ...
    // Create query profile lines for other elevation layers
    normalProfileLayers?.forEach((layerInfo) => {
      profiles.push(
        new ElevationProfileLineQuery({
          ...
          color: new Color(layerInfo.style.lineColor),
          title: layerInfo.label
        })
      )
    })

    // Add view profile for volumetric objects in 3D
    const viewProfile = this.checkForVolumetricObjects(jmv)
    if (viewProfile) {
    ...
    if (jmv.view.type === '3d') {
      profiles.push(
        new ElevationProfileLineInput({
          color: new Color(this.state.graphicsHighlightColor)
        })
      )
    }

    //Create new instance of ElevationProfileAnalysis
    //update the exitings instance ONLY when volumetric objects are changed in all other cases create new instance
    //when volumetricObjectsChanged and _elevationProfileAnalysis is null still create new instance
    if (!this._elevationProfileAnalysis || !layerInfoUpdated) {
      this._elevationProfileAnalysis = new ElevationProfileAnalysis({
        profiles: profiles
      })
      // Add analysis to the view
      jmv.view.analyses.add(this._elevationProfileAnalysis)
    } else if (layerInfoUpdated) {
      // Update existing analysis
      this._elevationProfileAnalysis.profiles = profiles
      if (this._elevationProfileAnalysis?.geometry) {
        ...
        this._analysisView = await jmv.view.whenAnalysisView(this._elevationProfileAnalysis) as ElevationProfileAnalysisView2D | ElevationProfileAnalysisView3D

        // Wait deterministically for analysis to complete instead of using arbitrary timeouts
        await reactiveUtils.whenOnce(() => this._analysisView?.progress === 1, abortController.signal)
        this.disableInputHighlight()
        const configuredGroundLayer = this.state.addedElevationLayers.filter(
          (layerInfo) => layerInfo.id === this.state.groundLayerId
        )

        const hasValidGroundProfile = this._analysisView?.results?.some(
          p => (p.profile.id === configuredGroundLayer?.[0]?.id && p.statistics !== null)
        )

        if (hasValidGroundProfile || this.onlyVolumetricConfigured()) {
          ...
          this.onAnalysisDataReady(abortController.signal)
        } else {
    ...
    this.watchAnalysisResults(jmv)
  }

  watchAnalysisResults = (jmv: JimuMapView) => {
    if (!this._elevationProfileAnalysis) return

    // Watch for input geometry changes
    reactiveUtils.watch(
      () => this._elevationProfileAnalysis?.geometry,
      async (geometry) => {
        if (!geometry) return

        if (this.checkIfCancelClick()) return
        ...
        try {
          ...
          // Wait deterministically for analysis to complete instead of using arbitrary timeouts
          await reactiveUtils.whenOnce(() => this._analysisView?.progress === 1, signal)
          const isSketchDrawComplete = this.state.drawModeActive && this.state.sketchVMState === 'complete'
          const isSketchReshapeActive = this._isSketchVMReshaping && this.state.sketchVMState === 'active'
          this.setState({
            onDrawingComplete: this.state.onDrawingComplete || isSketchDrawComplete || isSketchReshapeActive
          })
          this.disableInputHighlight()
          ...
          if (hasValidGroundProfile || this.onlyVolumetricConfigured()) {
            await new Promise((resolve) => { setTimeout(resolve, 500) })
            await this.onAnalysisDataReady(signal)
          } else {
            this.onErrorInChartData(true, this.nls('noProfileError'))
          }
        } catch (e) {
    ...
    // Watch for errors
    reactiveUtils.watch(
      () => this._analysisView?.error,
      (error) => {
        if (!error) return
        const errorState = this.getErrorMsgState(error.name)
        if (!errorState?.length) return
  ...
  // Selected line highlight should be displayed only on WebScene and if the selected line having the elevation value(zValue)
  disableInputHighlight = () => {
    if (this._mapView?.view?.type === '3d') {
      const inputProfile = this._analysisView.results.find(
        (r) => r.profile.type === 'input'
      )

      if (inputProfile) {
        inputProfile.profile.enabled = this._elevationProfileAnalysis?.geometry?.hasZ ?? false
      }
    }
  }
  ...
  onlyVolumetricConfigured = () => {
    ...
    const isGroundProfileAvailable = this._elevationProfileAnalysis?.profiles?.some(
      p => p.id === configuredGroundLayer?.[0]?.id
    )
    const elevationLayersSettings = this.props.config.configInfo[this.state.currentDatasource]?.elevationLayersSettings
    return !isGroundProfileAvailable && this.state.jimuMapView.view.type === '3d' && elevationLayersSettings?.showVolumetricObjLineInGraph
  }
  ...
  checkForVolumetricObjects = (jmv: JimuMapView) => {
    const elevationLayersSettings = this.props.config.configInfo[this.state.currentDatasource]?.elevationLayersSettings
    if (jmv.view.type === '3d' && elevationLayersSettings?.showVolumetricObjLineInGraph) {
      return new ElevationProfileLineScene({
        id: elevationLayersSettings?.volumetricObjSettingsOptions?.id,
        color: new Color(elevationLayersSettings?.volumetricObjSettingsOptions?.style?.lineColor),
  ...
  onAnalysisDataReady = async (signal) => {
    if (this.state.drawModeActive || this.state.selectModeActive) {
      ...
      this.setState({
        ...
        viewModelErrorState: false,
        profileResult: this._analysisView.results.filter((profileResult) => profileResult.statistics !== null) || null,
        chartDataUpdateTime: Date.now()
      }, async () => {
    ...
    return new Promise((resolve) => {
      ...
      if (this._elevationProfileAnalysis?.geometry) {
        ...
        if (this._selectedBufferValues.enabled && this._selectedBufferValues.bufferDistance > 0) {
          let inputGeometry = this._elevationProfileAnalysis.geometry.clone ? this._elevationProfileAnalysis.geometry.clone() as __esri.GeometryUnion : this._elevationProfileAnalysis.geometry
  ...
  getErrorMsgState = (errorMsg): any => {
    switch (errorMsg) {
      ...
      case ElevationProfileErrorState.RefinedButNoChartData:
        return [true, this.nls('noProfileError')]
      case ElevationProfileErrorState.ElevationQueryError:
        return [true, this.nls('elevationQueryError')]
  ...
  */
  ...
  getOutfieldsForDs = (dsLayer: FeatureLayerDataSource): string[] => {
    ...
    const uniqueIdFields = dsLayer.getUniqueIdFields()
    ...
    if (objectIdField && !outFields.includes(objectIdField)) {
      outFields.push(objectIdField)
    }
    if (uniqueIdFields) {
      uniqueIdFields.forEach((uidField) => {
        if (!outFields.includes(uidField)) {
          outFields.push(uidField)
        }
      })
    }
    return outFields
  ...
  getFeatureFromLayer = async (dsLayerId: string, oid: string | number) => {
  ...
  displayFeaturesResult = (layerDsId, selectedFeature) => {
    ...
    this.setState({
      ...
      //Clear the draw tool
      if (this._elevationProfileAnalysis) {
        this._elevationProfileAnalysis.geometry = null
      }
  ...
  componentDidUpdate = (prevProps) => {
    ...
    if (this.didElevationLayersColorLabelChanged(prevConfig?.elevationLayersSettings?.addedElevationLayers,
      ...
      this.setState({
        ...
        if (this._elevationProfileAnalysis) {
          this.createEpAnalysis(this._mapView, true)
        }
      })
    }
    ...
    prevConfig?.elevationLayersSettings?.showVolumetricObjLineInGraph !== currentConfig?.elevationLayersSettings?.showVolumetricObjLineInGraph) {
      if (this._elevationProfileAnalysis && this.state.jimuMapView.view?.type === '3d') {
        this.createEpAnalysis(this._mapView, true)
      }
    }
  }
  ...
  componentWillUnmount = () => {
    ...
    if (this._mapView?.view) {
      this._mapView.view.analyses.remove(this._elevationProfileAnalysis)
    }
  ...
  buildOutputStatistics = (selectedElevationUnit, selectedLinearUnit, isFlip) => {
    this.buildStatsValuesAsOutput(this.state.profileResult, this.props.outputDataSources?.[0], selectedElevationUnit, selectedLinearUnit, isFlip)
  }
  ...
  buildStatsValuesAsOutput = (visibleProfiles, outputDs, selectedElevationUnit, selectedLinearUnit, isFlip) => {
    ...
    const configuredGroundLayer = this.state.addedElevationLayers.filter((layerInfo) => layerInfo.id === this.state.groundLayerId)
    const isGroundProfileAvailable = this.state.profileResult?.some(
      p => p.profile.id === configuredGroundLayer?.[0]?.id
    )
    if (!this._elevationProfileAnalysis?.profiles?.length || !isGroundProfileAvailable) {
    ...
    this.state.addedElevationLayers?.forEach((layerInfo, index) => {
      ...
      const profileResult = visibleProfiles?.filter((p) => p.profile.id === layerInfo.id)
      if (profileResult?.length) {
        ...
        statsValues.name = profileResult?.[0] ? layerInfo.label : this.nls('noStatsValue')
        ...
        const statGraphic = new Graphic({
          ...
          geometry: this._elevationProfileAnalysis?.geometry ?? dummyGeometry
        })
    ...
    unitOptions.forEach((unit) => {
      if (name === ElevationProfileStatisticsName.MaxDistance) {
        if (unit.value === selectedLinearUnit) {
          convertedStats = convertSingle(statVal, this._analysisView?.effectiveDisplayUnits.distance, selectedLinearUnit)
        ...
        if (unit.value === selectedElevationUnit) {
          convertedStats = convertSingle(statVal, this._analysisView?.effectiveDisplayUnits.elevation, selectedElevationUnit)
          roundOffStat = this.props.intl.formatNumber(convertedStats, { maximumFractionDigits: 2 }) + ' ' + this.nls(unit.abbreviation)
        }
      }
    })
    ...
    if (Object.keys(featuresByLayer).length > 0) {
      for (const dsLayerId in featuresByLayer) {
        ...
        if (layerDataSource.type === DataSourceTypes.SubtypeGroupLayer &&
          ...
          selectedLayerDsId = subLayerDataSource.id
          feature = await this.getFeatureFromLayer(selectedLayerDsId, subLayerDataSource.getSelectedRecordIds()[0])
        }
    ...
    if (newSelectedFeature) {
      ...
      geometryUtils.projectToSpatialReference([newSelectedFeature.geometry],
        ...
        if (projectedGeometries?.length > 0) {
          newSelectedFeature.geometry = projectedGeometries[0] as __esri.GeometryUnion
        }
  ...
  checkForIntersectingLayer = (skipSettingResultState: boolean | undefined): Promise<any[]> => {
    return new Promise((resolve) => {
      ...
      if (this._intersectingLayersAtRuntime?.length > 0) {
        const selectableLineGeom = this._elevationProfileAnalysis?.geometry
  ...
  queryForIntersectingLayers = async (assetLayerSettings, selectableLineGeom?): Promise<LayerIntersectionInfo | null> => {
    ...
    return new Promise((resolve) => {
      if (ds?.layerDefinition?.geometryType === 'esriGeometryPolyline' || ds?.layerDefinition?.geometryType === 'esriGeometryPoint') {
        ...
        if (uniqueIdFields) {
          uniqueIdFields.forEach((uidField) => {
            if (!intersectingFeatureQuery.outFields.includes(uidField)) {
              intersectingFeatureQuery.outFields.push(uidField)
            }
          })
        }
            ...
            if (results?.records.length > 0) {
              results.records.forEach((record) => {
                ...
                if (intersectingFeatureGeom.type === 'polyline') {
                  ...
                  const intersectingLineToTheLine: __esri.GeometryUnion | __esri.GeometryUnion[] =
                  ...
                  if (multiPointFeature?.points?.length > 0) {
                    multiPointFeature.points.forEach((eachPoint, index) => {
                      const intersectingPointFeature = multiPointFeature.getPoint(index) as __esri.GeometryUnion
  ...
  */
  ...
  filterExistingFeatures = (layer: FeatureLayerDataSource) => {
    ...
    const uniqueIdFields = layer.getUniqueIdFields()
    const useUniqueIds = uniqueIdFields && uniqueIdFields.length > 0
    ...
    layerFeatureGraphics.forEach((graphic, index) => {
      if (graphic?.attributes?.hasOwnProperty('esriCTFeatureLayerId') &&
        graphic.attributes.esriCTFeatureLayerId === layer.getDataSourceJson().id) {
        /**
         * If unique id fields are present then build the query using those fields otherwise use object id field.
         * Query string using unique id fields will be like:
         * NOT((uidField1 = 'value1' AND uidField2 = 'value2') OR (uidField1 = 'value3' AND uidField2 = 'value4'))
         * Query string using object id field will be like:
         * oIdField <> value1 AND oIdField <> value2
         */
        // TODO: support uniqueIds, the code is updated, need review
        if (useUniqueIds) {
          const graphicConditions = uniqueIdFields
            .map(uidField => {
              const value = graphic.attributes[uidField]
              if (value === null) {
                return `${uidField} IS NULL`
              } else if (typeof value === 'string') {
                return `${uidField} = '${value}'`
              } else {
                return `${uidField} = ${value}`
              }
            })
            .join(' AND ')
          oIdQueryString += `(${graphicConditions})`
          if (index === layerFeatureGraphics.length - 1) {
            oIdQueryString = 'NOT (' + oIdQueryString + ')'
          } else {
            oIdQueryString += ' OR '
          }
        } else {
          // The object id can be either a number or string.
          const v = typeof graphic.attributes[oIdField] === 'string' ? `'${graphic.attributes[oIdField]}'` : graphic.attributes[oIdField]
          if (index === layerFeatureGraphics.length - 1) {
            oIdQueryString += oIdField + ' <> ' + v
          } else {
            oIdQueryString += oIdField + ' <> ' + v + ' AND '
          }
        }
      }
    })
  ...
  selectFeatureForProfiling = (feature) => {
    ...
    if (!this.state.addToSelectionTool && this._elevationProfileAnalysis) {
      this._elevationProfileAnalysis.geometry = polylineGraphic.geometry
  ...
  renderChartOnSelect = (graphicAddedToSelection: boolean) => {
    if (this.state.addToSelectionTool) {
      ...
      if (!graphicAddedToSelection && this._elevationProfileAnalysis) {
        this._elevationProfileAnalysis.geometry = graphic.geometry
      }
    }
  }
  ...
  _activateAddToSelectionTool = () => {
    ...
    if (this.state.jimuMapView && this.state.jimuMapView.view) {
      this.state.jimuMapView.mapComponent.popupDisabled = true
    }
    this._displayAddToSelectionCursor()
  ...
  _deActivateAddToSelectionTool = () => {
    ...
    if (this.state.jimuMapView && this.state.jimuMapView.view) {
      this.state.jimuMapView.mapComponent.popupDisabled = false
    }
    this._displayDefaultCursor()
  ...
  activateDrawOrSelectTool = () => {
    ...
    if (this.state.currentSketchVM) {
      ...
      this.setState({
        ...
        //Activate draw tool
        // Draw mode is now handled by sketchVM
        if (this.state.drawModeActive) {
  ...
  destroyDrawingLayers = () => {
    if (this._sketchViewModelDrawingLayer) {
      this._sketchViewModelDrawingLayer.removeAll()
      this._sketchViewModelDrawingLayer.destroy()
    }
  ...
  clearAll = (skipClearingSelectedFeatures?: boolean) => {
    ...
    if (this.state.drawModeActive || this.state.selectModeActive) {
      this.state.currentSketchVM?.cancel()
      this.state.currentSketchVM?.removeAllGraphics()
    }
    this._analysisView?.analysis?.clear()
    this._deActivateAddToSelectionTool()
    this.clearGraphics()
  ...
  onDoneButtonClicked = (): boolean => {
    this._doneButtonClicked = true
    const { sketchVMState, currentSketchVM } = this.state
    if (sketchVMState === 'active') {
      currentSketchVM?.complete()
    }

    const isSketchStateReady = sketchVMState === 'active' || sketchVMState === 'complete'
    const hasProfileGeometry = Boolean(this._analysisView?.analysis?.geometry?.paths?.length)
    const hasDrawnSketchGraphics = Boolean(this._sketchViewModelDrawingLayer?.graphics?.length)

    const enableNewProfileOption = isSketchStateReady && hasProfileGeometry || (this.state.drawModeActive && hasDrawnSketchGraphics)
    ...
    this.setState({ isCancelButtonClick: false })
  ...
  highlightChartPosition = (x) => {
    if (this._analysisView && this._elevationProfileAnalysis?.geometry && x != null &&
      this._analysisView.indicatorRelativePosition !== x) {
      this._analysisView.indicatorRelativePosition = x
    }
  }

  hideChartPosition = () => {
    if (this._analysisView && this._elevationProfileAnalysis &&
      this._analysisView.indicatorRelativePosition !== null) {
      this._analysisView.indicatorRelativePosition = null
    }
  }
        ...
        <div className='widget-elevation-profile'>
          ...
          {!this.state.onWidgetLoadShowLoadingIndicator && this.state.resultStage &&
            <ResultPane
              ...
              visibleProfiles={this.state.profileResult}
              effectiveDisplayUnits={this._analysisView?.effectiveDisplayUnits}
              analysisViewStatistics={this._analysisView?.statistics}
