...
export default class ViewProfileDataAction extends AbstractDataAction {
  isSupported (dataSets: DataRecordSet[], dataLevel): Promise<boolean> {
    ...
    const dataSourceType = dataSet?.dataSource?.type as any
    // Don't support knowledge graph data sources for view profile action.
    if (dataSourceType === AllDataSourceTypes.KnowledgeGraphLayer || dataSourceType === AllDataSourceTypes.KnowledgeGraphSublayer) {
      return Promise.resolve(false)
    }

    //check ds for map widget (webmap or webscene) or for other widgets like table, list for the configured layer (line layer)
    if (!(dataSourceType === 'WEB_MAP' || dataSourceType === 'WEB_SCENE' ||
      ((dataSourceType === AllDataSourceTypes.FeatureLayer || dataSourceType === AllDataSourceTypes.SubtypeSublayer) && (dataSource.getGeometryType() === 'esriGeometryPolyline')))) {
      return Promise.resolve(false)
    }
