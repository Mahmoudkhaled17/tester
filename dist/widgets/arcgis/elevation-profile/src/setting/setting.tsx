...
export default class Setting extends BaseWidgetSetting<AllWidgetSettingProps<IMConfig>, State> {
  ...
  */
  ...
  updateConfigAsPerNewWebMap = (newConfig) => {
    ...
    if (this.state.useMapWidget) {
      ...
      Object.keys(config).forEach((dsId) => {
      ...
      if (newConfig.activeDataSource && !dataSourcesToMatch.includes(newConfig.activeDataSource)) {
        newConfig = newConfig.set('activeDataSource', dataSourcesToMatch[0] ?? null)
      }
    }
  ...
  updateLayersFromConfig = (activeDataSource: string, config) => {
    const pointLayerIds = []
    const lineLayerIds = []
    const propsConfigInfo = config.configInfo ? config.configInfo.asMutable({ deep: true }) : config
    const configInfo = config.configInfo ? config.configInfo.asMutable({ deep: true }) : config
    if (!propsConfigInfo?.[activeDataSource]) {
      return configInfo
    }
  ...
  */
  ...
  render () {
    const isProfileSettingsEnabled = this.props.config.configInfo[this.state.activeDataSource]?.profileSettings.isProfileSettingsEnabled
    const selectableLayerAriaLabel = isProfileSettingsEnabled
      ? this.nls('profileSettingsLabel') + ' ' + (this.state.hasLineLayers
        ? this.nls('profileRendering') + ' ' + this.nls('profileRenderingTooltip')
        : this.nls('noLineLayersMessage'))
      : this.nls('profileSettingsLabel')
    const isAssetSettingsEnabled = this.props.config.configInfo[this.state.activeDataSource]?.assetSettings.isAssetSettingsEnabled
    const assetLayerAriaLabel = isAssetSettingsEnabled
      ? this.nls('assetSettingsLabel') + ' ' + (this.state.hasLineLayers &&this.state.hasPointLayers
        ? this.nls('intersectingLayersLabel') + ' ' + this.nls('intersectingLayersTooltip')
        : this.nls('noLinePointLayersMessage'))
    ...
    return (
      <div className='jimu-widget-setting widget-setting-elevation-profile h-100' css={getStyle(this.props.theme)}>
        <SettingSection role='group' aria-label={this.state.isNoMapSelected && this.nls('selectMapHint')} className={classNames('map-selector-section', { 'border-0': this.state.isNoMapSelected })}>
          <SettingRow role='group' aria-label={this.nls('selectMapWidget')}>
            <Label title={this.nls('selectMapWidget')}
        ...
        {this.props.useMapWidgetIds && this.props.useMapWidgetIds.length > 0 && this.state.mapWidgetExists &&
          <div>
            <SettingSection role='group' aria-label={this.nls('dataConfigLabel') + ' ' + this.nls('dataConfigTooltip')}>
              <SettingRow>
                <Label title={this.nls('dataConfigLabel')}
                        ...
                        <React.Fragment>
                          {this.state.activeDataSource && this.props.config.configInfo[this.state.activeDataSource] &&
                            <React.Fragment>
                              ...
                              <SettingSection>
                                <React.Fragment>
                                  <SettingRow role='group'
                                    aria-label={selectableLayerAriaLabel}
                                    tag='label' label={
                                    <div className='flex-grow-1 text-break title2 text-paper'>
                                      {this.nls('profileSettingsLabel')}
                                    </div>}>
                                    <Switch title={this.nls('profileSettingsLabel')}
                                  ...
                                  {this.props.config.configInfo[this.state.activeDataSource]?.profileSettings.isProfileSettingsEnabled &&
                                    <React.Fragment>
                                      {!this.state.hasLineLayers &&
                                        <SettingRow>
                                          <Alert className={'warningMsg py-1'}
                                            style={{ minWidth: 'auto' }}
                                            open={true}
                                      ...
                                      <SettingRow label={this.nls('profileRendering')}>
                                        <Tooltip title={this.nls('profileRenderingTooltip')} showArrow placement='top'>
                                      ...
                                      <SettingRow role='group' aria-label={this.nls('groundElevation') + ' ' + this.nls('groundElevationHint')} flow={'wrap'}>
                                      ...
                                      <SettingRow role='group' aria-label={this.nls('customizeSelectableLayersLabel') + ' ' + (this.props.config.configInfo[this.state.activeDataSource]?.profileSettings.isCustomizeOptionEnabled
                                        && this.nls('selectableLayersLabel') + ' ' + this.nls('selectableLayersTooltip'))} flow={'wrap'}>
                                        <Label className='m-0 color-label' centric>
                                          <Radio
                                            className={'cursor-pointer'}
                              ...
                              <SettingSection>
                                <React.Fragment>
                                  <SettingRow role='group' aria-label={assetLayerAriaLabel}
                                    tag='label' label={
                                    <div className='flex-grow-1 text-break title2 text-paper'>
                                      {this.nls('assetSettingsLabel')}
                                    </div>}>
                                    <Switch title={this.nls('assetSettingsLabel')}
            ...
            {this.state.activeDataSource &&
              <SettingSection>
                <SettingRow>
                  <Label title={this.nls('generalSettingsLabel')}
                    className='w-100 d-flex' style={{ maxWidth: '88%' }}>
                    <div className='text-truncate flex-grow-1 color-label title2 hint-paper'>
                ...
                <SettingRow role='group' aria-label={this.nls('generalSettingsLabel')} flow='wrap'>
                  <GeneralSettings
                    intl={this.props.intl}
                    theme={this.props.theme}
